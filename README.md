...
- **읽는 줄** — 아래 명령이 찾는 후보 줄(기사님 인용 · 날짜 · 지난 상태를 말하는 말 · «하지 않는다»·«말 것»). 줄마다 넷 중 하나로 판정한다
  ...
  - ✅ 그대로
- **절차 (파일마다)** — 판정 → 고친 사본 → 주석을 뺀 코드 글자가 그대로인지 확인 → 넣는다 →
  그 파일을 읽는 검사(`grep -rl <경로>`)와 `pnpm audit:docs` → 커밋. 묶음 안은 에이전트가 끝까지 한다 (기사님 결정)
- **묶음이 끝나면** — 전체 게이트를 한 번 돌리고 기사님께 보고한다. 보고에는 🔴 고친 곳과 **지운 기사님 인용**만 표로 담는다
  (기사님 눈이 필요한 곳은 그 둘이다 — 코드를 잘못 읽었거나, 규칙인 인용을 경위로 보고 지웠을 수 있다). 나머지는 파일 수 · 줄 수만.
  기사님이 짚은 줄은 그 파일의 커밋만 되돌리거나 고친다
