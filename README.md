...
앱이 아닌 자리: `ex_images/` 실물 캡처 · `.claude/skills/` 프로젝트 스킬
...
- 🔴 안 적는다: **끝난 사연 · 이미 없어진 함정 · 날짜별 경위**(«🔄 날짜 개정» · «예전 ~였다» · 사고 경위 · 날짜 붙은 인용) → 커밋 메시지로
...
- ✅ `pnpm audit:docs` ⑥ 이 문다 — 바꾼 파일의 경위 줄이 HEAD 보다 늘면 빨간불. 이미 쌓인 줄은 그 파일을 만질 때 줄인다
...
- 🔴 지우는 것은 **손으로, 의도적으로** 한다 — 서버가 켜질 때 조건을 보고 표를 지우는 코드를 새로 만들지 않는다
  (남아 있는 옛것은 [server/CLAUDE.md](onedal-web/server/CLAUDE.md) «조건부 DROP TABLE»).
