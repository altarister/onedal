...
1. **Turf.js 공간 연산 (스마트 회랑)**: 서버 메모리의 `merged_map.geojson`을 기반으로 카카오 경로 주변 10km 반경 내 동네 이름 50여 개를 자동 추출하여 합짐 필터에 주입.
2. **고속도로 톨게이트 우회 방지**: 카카오 가이드에서 `고속국도`/`도시고속도로` 판정 구간은 합짐 반경 연산을 0km로 제재.
3. **기기 페어링 (PIN 연동 시스템)**: 6자리 핀 번호로 기기를 계정에 안전하게 귀속시키는 SaaS 보안 강화 조치.
