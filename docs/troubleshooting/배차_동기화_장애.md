...
> ⚠️ **2026-08-09 작성 · 제목은 «계획서»지만 이 레포 규칙의 출처다.**
> 결함 **#1**(HTTP 물고 기다리기) · **#2**(orderId 미검증) · **#4**(좀비 타이머) 는
> [CLAUDE.md](../../CLAUDE.md) 의 **규칙 ②** 가 되었다.
> 🔴 **결함 #3 «이중 타이머 경쟁» 은 아직 안 고쳤다** — [todo.md](../../todo.md) 참조.
> 그러니 이 문서를 «다 해결된 옛 계획»으로 읽지 말 것.
