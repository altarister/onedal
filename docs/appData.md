...
3단계에서 서버로부터 받아 `SharedPreferences`에 저장해 둔 필터(`activeFilter`)를 꺼내어 5가지 조건(AND)을 모두 검사합니다.
- **조건 0 (차종):** 파싱된 `vehicleType`("오", "1t" 등)이 필터의 `allowedVehicleTypes` 배열에 포함되어 있는지? (**빈 배열이면 모든 차종 통과**)
- **조건 1 (도착지):** `dropoff`("서울 강남구")가 필터의 `destinationKeywords` 리스트에 있는지?
- **조건 2 (요금 하한):** 파싱된 76,000원이 필터 최소요금(`minFare`, 예: 4만원) 이상인지?
- **조건 3 (상차지 거리):** 콜의 접근거리 `9.0`km가 필터 반경(`pickupRadiusKm`, 예: 30km) 이하인지?
- **조건 4 (블랙리스트):** 텍스트 내에 `excludedKeywords`("혼적", "착불" 등) 기피 단어가 없는지?
...
[dispatchEngineArgs / activeFilter]
  ...
  shouldClick() 호출 시 꺼내어 5대 조건 검사에 사용한다.

  스키마 (AutoDispatchFilter):
  {
    "allowedVehicleTypes": [],              // 허용 차종 배열 (빈 배열 = 모든 차종 허용)
    ...
    "destinationKeywords": "광주시,경안동,송정동,...",      // 타겟 지역 키워드 (콤마 구분)
