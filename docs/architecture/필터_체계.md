...
## ⚠️ 먼저 읽을 것 — 이 문서의 어디가 옛 구조인가 (2026-08-13 대조)

**설계 원칙은 지금도 유효합니다.** 다만 **구현을 설명한 부분은 이름과 구조가 바뀌었습니다.**
모르고 읽으면 없는 함수·없는 필드를 찾게 되고, 더 나쁘게는 그것들이 있다고 전제하고 코드를 짭니다.

| 이 문서에 나오는 것 | 지금 실제 |
|---|---|
| `runtimeOverrides` (임시 덮어쓰기 바구니) | **없습니다.** `baseFilter` / `activeFilter` 둘로 **완전 격리**됐습니다 (§4 는 이 변화를 이미 반영) |
| `loadState: EMPTY / LOADING / DRIVING` | **없습니다.** `dispatchPhase: STANDBY / GATHERING / DELIVERING` 이고, **저장값이 아니라 활성 콜 수에서 파생**됩니다 |
| `applyFilter(userId, changes, io, persistToDB)` | **없습니다.** `saveBaseFilter()`(DB) / `updateActiveFilter()`(메모리) 로 갈라졌습니다 |
| `NativeScrapParser.kt` | **없습니다.** `InsungParser.kt` · `Hwamul24Parser.kt` (배차망별 플러그인) |
| `getSharedModeVehicleTypes()` | **없습니다.** `getRemainingCapacityTypesByPoints()` — 차종이 아니라 **실제 적재 점수**에서 구합니다 |

**§9 미구현 로드맵**의 세 항목은 그 뒤 전부 구현됐습니다 (출발 버튼 · 귀가콜 · Corridor Trim).

**398줄 이후 `## 나중에 문서에 추가할 내용들`** 은 `runtimeOverrides` 를 어떻게 고칠지 논의한
**옛 대화 기록**입니다. 그 구조 자체가 사라졌으므로 지금 코드와 대조하지 마세요.
다만 *"사장님의 멘탈 모델"* 로 적힌 흐름 — DB → baseFilter → activeFilter → 끝나면 비운다 —
은 지금 구조가 실제로 그렇게 되어 있습니다. **의도를 읽는 용도로만 보세요.**

### 여기서 뽑아 쓸 살아 있는 규칙

이 문서에서 **지금도 지켜야 하는** 것들입니다 (`CLAUDE.md` 의 규칙 절에도 올렸습니다):

- **서버는 콜을 자동으로 버리지 않는다** — 요율 미달이어도 사유만 표시하고 기사님이 판단 (§6)
- **데이터를 변조해서 동작을 바꾸지 않는다** — 합짐 상차반경을 `999km` 로 덮어쓰던 것을 없애고
  앱이 `isSharedMode` 룰로 건너뛰게 했다 (§5)
- **기본값의 단일 출처는 DB** — 서버·관제웹은 자체 폴백을 갖지 않는다. 앱만 오프라인 안전망 (§2)
- **어제 상태가 오늘 되살아나지 않는다** (§3 주의문 → 지금은 영업일 전환 `ensureBusinessDay`)
- **앱은 "일단 잡아와라", 서버는 "수익성을 판단한다", 기사님이 "최종 결재한다"** (§1)
...
## 8. 앱(Android) 필터 엔진 및 스마트 유추 시스템 (Task 8, 28)
...
| 1 | 도착지 매칭 | `destinationKeywords` 중 하나 포함 (2차 상세 필터 스마트 유추 적용) |
...
### 8.1 2차 상세 필터 스마트 유추 로직 (Task 8)

과거 방식은 단순히 하차지 주소 문자열이 `destinationKeywords`와 일치하는지만 검사했습니다. 하지만 "성남동", "신사동" 등 이름은 같으나 지역이 다른 동명이동 문제가 있었습니다. 이를 해결하기 위해 스마트 유추 시스템이 도입되었습니다.
- **키워드 계층화 분석**: 도착지 문자열에서 상위 행정구역(시/군/구)과 하위 행정구역(읍/면/동)을 분리 추출합니다.
- **인텔(Intel) 매핑 활용**: 서버에서 수집된 행정구역 지식 데이터베이스(`intel`)를 바탕으로, 단순 동 이름만 있을 경우 해당 동이 속할 확률이 가장 높은 상위 행정구역을 자동으로 유추하여 필터링합니다.

### 8.2 행정구역 지능화 (`intel.parentName` 우선 매핑) (Task 28)

카카오 지오코딩 L1/L2 캐시(`geocode_cache`) 테이블이 구축됨에 따라, 여기서 추출된 `parentName`(상위 행정구역명) 데이터를 지식 DB에 병합했습니다.
- 앱에서 올라온 파편화된 주소(예: "분당구 정자일로")를 서버 엔진(`OrderEvaluator`)이 평가할 때, 일차적으로 캐시의 `parentName`("성남시 분당구")을 조회하여 필터 조건식과 대조합니다.
- 이를 통해 신규 주소나 오타가 섞인 주소라도, 기존에 캐시된 지리 데이터를 바탕으로 스마트하게 매핑(필터 통과/차단)을 결정하여 광클 정확도를 대폭 상승시켰습니다.
