...
8. [구현 순서](#-최종-구현-순서)
9. [데스밸리 타이머 전체 아키텍처 (SSOT)](#-데스밸리-타이머-전체-아키텍처-ssot)
...
> **현행 코드 기준**: 서버에는 2개의 데스밸리 타이머가 존재합니다. 상세는 본 문서 하단의 [데스밸리 SSOT](#-데스밸리-타이머-전체-아키텍처-ssot) 참조.

| 항목 | 현행 동작 |
|------|---------|
| `/confirm` 수신 시 (orders.ts) | 30초 타이머 시작. 만료 시 `handleDecision(CANCEL)`. 단, `isActive`가 켜진 경우만 |
| `/detail` 수신 후 (detail.ts) | 30초 경고(`deathvalley-warning`) + 35초 타임아웃. **단, KEEP 결재 완료된 콜은 절대 취소 안 함 (Phase 1 방어)** |
| 앱 자체 (HijackService.kt) | 30초(SharedPref 조절 가능). `isAutoSessionActive == true`일 때만 작동 |

**서버가 직접 CANCEL을 수행하는 이유**:
앱의 데스밸리 타이머(`isAutoSessionActive`)는 **매크로가 자동으로 잡은 콜에만 작동**합니다.
기사님이 수동으로 잡은 콜이나, 앱-서버 간 통신이 끊긴 경우를 대비하여 서버에도 독립적인 타이머가 필요합니다.
다만, **KEEP 결재가 이미 완료된 콜까지 취소하는 것은 비즈니스 논리에 반하므로**, Phase 1 방어 로직으로 차단합니다.
...
## ⏳ 데스밸리 타이머 전체 아키텍처 (SSOT)

> [!IMPORTANT]
> **이 섹션이 데스밸리(Death Valley)의 유일한 정의 문서입니다.**
> 다른 문서(PRD, 시퀀스 다이어그램, 트러블슈팅 등)에서 데스밸리를 언급할 때는 이 섹션을 참조 링크로 연결합니다.

### 정의

**데스밸리(Death Valley)**란, 앱폰이 콜을 확정(인성앱에서 배차 성공)한 뒤 관제탑의 최종 판결(KEEP/CANCEL)이 전달되기까지의 **위험 구간**을 말합니다. 이 구간에서 통신 장애·관제 지연 등이 발생하면 기사님이 잡은 콜을 놓치거나(노쑈), 반대로 똥콜에 갇히는 참사가 일어날 수 있습니다.

### 3개 독립 타이머 상세

| # | 이름 | 코드 위치 | 트리거 | 타임아웃 | 동작 | 작동 조건 |
|---|------|----------|--------|---------|------|----------|
| ① | **서버 1차 (confirm)** | `orders.ts` L169-175 | `/confirm` 수신 시 | 30초 | `handleDecision(CANCEL)` | `isActive == true` |
| ② | **서버 2차 (detail)** | `detail.ts` L180-205 | `/detail` evaluate 완료 | 30초 경고 + 35초 취소 | 경고 emit → **KEEP 아닌 경우만** 콜 삭제 | `type !== "MANUAL"` |
| ③ | **앱 자체** | `HijackService.kt` L593-610 | 서핑 완료 후 `/detail` 전송 | 30초 (SharedPref) | 인성앱 `[취소]` 자동 터치 | `isAutoSessionActive == true` |

### 타이머 간 관계 (타임라인)

```
T+0s   앱: 콜 클릭 → /confirm 전송
       서버: ①번 타이머 30초 시작
T+7s   앱: 서핑 완료 → /detail 전송
       서버: ②번 타이머 30초(경고)+35초(취소) 시작
       앱(AUTO만): ③번 타이머 30초 시작
T+30s  ①번 발동 가능 (confirm 기준 30초)
       ②번 경고 발동 (detail 기준 30초) → deathvalley-warning emit
T+35s  ③번 발동 가능 (앱 자체 30초, /detail 전송 ~7초 후)
T+42s  ②번 타임아웃 발동 (detail 기준 35초) → KEEP 아닌 경우만 콜 삭제
```

### 핵심 방어 원칙

1. **KEEP된 콜은 절대 취소하지 않는다** — ②번 타이머(detail.ts)는 KEEP 결재가 이미 내려진 콜을 건너뜁니다 (Phase 1 방어). ✅ 구현됨
2. **matchType은 실제 클릭 주체를 반영한다** — 앱은 `isAutoSessionActive` 기반으로 AUTO/MANUAL을 정직하게 보고합니다 (Phase 2). ✅ 구현됨
3. ~~**서버는 클라이언트를 맹신하지 않는다** — /confirm→/detail 시간차 6초 이상이면 수동 클릭으로 재분류합니다 (Phase 3).~~
   > 🔴 **미구현입니다 (2026-08-13 코드 대조 확인).** 계획만 있고 코드에 없습니다.
   > 오히려 `detail.ts` 에는 정반대 주석(*"앱이 보내는 matchType 을 100% 신뢰"*)이 원칙처럼 박혀 있었습니다.
   >
   > 이 문서가 **현재형("재분류합니다")으로 적어 둔 탓에** 읽는 사람은 이 방어가 이미 도는 줄 알고
   > 다른 곳을 판단합니다. 2026-08-12 에 앱이 AUTO 를 MANUAL 로 잘못 보고했고,
   > 없는 방어를 믿은 채로 그대로 통과해 **관제웹에 유령 콜이 남았습니다.**
   >
   > 계획은 [todo.md](../../todo.md) 로 옮겼습니다. **구현된 것만 이 문서에 씁니다.**
   > 대신 다른 두 겹을 넣었습니다 — 못 읽은 값 경고(P3) · 평가 실패해도 확정 진행(C′).

### 설정값 참조

`dispatchConfig.ts`:
- `WAITING_WARNING_MS: 30000` (30초 경고)
- `WAITING_TIMEOUT_MS: 35000` (35초 취소)
