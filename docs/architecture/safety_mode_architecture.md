...
1. **KEEP된 콜은 절대 취소하지 않는다** — ②번 타이머(detail.ts)는 KEEP 결재가 이미 내려진 콜을 건너뜁니다 (Phase 1 방어).
2. **matchType은 실제 클릭 주체를 반영한다** — 앱은 `isAutoSessionActive` 기반으로 AUTO/MANUAL을 정직하게 보고합니다 (Phase 2).
