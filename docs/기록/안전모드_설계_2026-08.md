...
> 🔴 **옛 설계서다. 지금 코드와 다르다 — 이 문서를 근거로 코드를 짜지 말 것.**
>
> 여기 적힌 뼈대가 **코드에 없다** (2026-08-28 확인):
>   `mainCallState` · `subCalls[]` · `pendingDetailRequests` · `isAutoSessionActive` — **전부 0곳**
>
> 그리고 이 문서는 **Phase 3(서버가 앱의 `matchType` 을 안 믿는다)을 구현된 것처럼** 적어 뒀다.
> 그 «없는 방어»를 믿은 채로 2026-08-12 유령 콜 사고가 났다 — CLAUDE.md 가 «네 번째»라 부르는 그것이다.
>
> **지금 지키는 것은 규칙과 검사다:**
>   CLAUDE.md 규칙 ① (KEEP 된 콜은 절대 취소하지 않는다 · 직접콜은 심사하지 않는다)
>   CLAUDE.md 규칙 ② (즉시 응답 + 피기백 · orderId 대조 · ACK 까지 판결 보존 · 타이머 ID 저장)
>   `tests/rules/dispatchRules.test.ts` 가 그 규칙들을 코드에서 강제한다
>
> 남겨 둔 이유는 «무엇을 하려 했나»와 «어디서 어긋났나» 뿐이다.
