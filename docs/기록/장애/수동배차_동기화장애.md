...
> ⚠️ **2026-08 작성 · 제목은 «계획서»지만 아직 안 끝난 것이 있다.**
> 🔴 **Phase 3(서버가 앱의 `matchType` 을 안 믿는다) 은 코드에 없다.**
> `architecture/안전모드_구조.md` 가 이걸 **구현된 것처럼** 적어 뒀고, 그 «없는 방어»를
> 믿은 채로 2026-08-12 유령 콜 사고가 났다 (CLAUDE.md «네 번째»).
> 남은 일은 [todo.md](../../../todo.md) 에. 같은 계보의 v2 는 [배차_동기화_장애.md](배차_동기화_장애.md).
> ⏱️ **2026-09-14** — 아래에 나오는 서버 상수 `WAITING_TIMEOUT_MS`(35초)는 없앴다. 지금은 DB 의 배차망별 안전취소 시간 + 5초다 ([배차망별_대기_시간.md](../../지금/배차망별_대기_시간.md)).
...
#### 📄 1. [제품_요구사항.md](../../../CLAUDE.md) (글로벌 기획서)
...
> **진단**: 제품_요구사항.md는 초기 기획서로서 '데스밸리'라는 용어의 **원래 의미(기사가 판단할 30초)**를 정의합니다. 그러나 구현이 진화하면서 데스밸리가 3개로 분화되었는데, 이 문서는 갱신되지 않았습니다.

---

#### 📄 2. [ROADMAP_sequenceDiagram.md`ROADMAP_sequenceDiagram.md` (지금은 없음) (시퀀스 다이어그램)
...
#### 📄 3. [안전모드_구조.md](../안전모드_설계_2026-08.md) (안전 모드 설계서)
...
#### 📄 4. [관제탑_상태_분리.md](관제탑_상태_분리.md) (관제탑 연결 끊김 트러블슈팅)
...
#### 📄 5. [배차_동기화_장애.md](배차_동기화_장애.md) (배차 동기화 실패 트러블슈팅)
...
#### 📄 6. [how.md`how.md` (지금은 없음) (라이프사이클 리포트 작성 가이드)
...
#### 📄 7. [1DAL_Lifecycle_Report.md`1DAL_Lifecycle_Report.md` (지금은 없음) (실제 운행 리포트)
...
| **설계서 ↔ 코드 괴리** | 🔴 치명 | `안전모드_구조.md`는 "서버가 직접 CANCEL 안 함"이라고 적혀있지만, 코드에서는 서버가 직접 CANCEL함 |
...
1. **SSOT 후보**: [안전모드_구조.md](../안전모드_설계_2026-08.md)에 "데스밸리 타이머 전체 아키텍처" 섹션을 신설하여, 3개 타이머의 위치·시간·조건·동작을 한 곳에서 관리
2. **나머지 6개 문서**: 데스밸리 관련 상세 설명을 삭제하고, SSOT 문서로의 링크(`→ 상세는 안전모드_구조.md 참조`)로 대체
