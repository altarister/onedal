...
## 7. 멀티폰 기기 간 전역 동기화 로직으로 개편 (Task 15)

기존에는 `device.mode`가 각 기기별로 관리되어, 하나의 기기에서 AUTO로 켜도 다른 기기에서는 MANUAL로 남아있는 파편화 문제가 존재했습니다.

이를 해결하기 위해 `mode` 제어권을 디바이스 개별 상태에서 **유저 전역 세션(UserSession)**으로 격상시켰습니다.
- 관제탑에서 AUTO 스위치를 켜면, 해당 유저에 연동된 **모든 기기(멀티폰)에 일괄적으로 `isActive=true` 상태가 브로드캐스트**됩니다.
- 한 기기에서 매크로 사냥이 시작되어 `phase=GATHERING`이 되면, 나머지 기기들도 동시에 상태가 연동되어 동일한 페이즈로 전환되므로, 기기 간 상태 불일치에 따른 오작동(예: 한 폰은 첫짐 찾고 한 폰은 합짐 찾는 현상)이 원천 차단되었습니다.
