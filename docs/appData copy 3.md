...
*(위 콜들은 두 개의 행으로 인식 각각의 오더 객체로 탄생합니다.)*

## 1단계: 앱 첫 접속 및 필터 수신 (`POST /api/scrap` — 첫 호출)
앱이 켜지고 접근성 서비스가 활성화되면, 가장 먼저 `/api/scrap`을 호출하여 서버에 "나 살아있다"를 알립니다.
- 서버는 응답 꼬리(Piggyback)로 3가지를 한꺼번에 내려줍니다:
  ```json
  {
...
## 3단계: 자동 사냥(AUTO) 필터 통과 기원 (Smart Filter)
수동(MANUAL) 모드가 아닌 **AUTO 모드**일 때, 앱은 이 콜이 '꿀콜'인지 자체적으로 검증합니다.
1단계에서 서버로부터 받아 `SharedPreferences`에 저장해 둔 필터(`activeFilter`)를 꺼내어 4가지 조건(AND)을 모두 검사합니다.
...
✅ **결과:** 조건을 모두 통과했다면 터치(광클) 액션으로 넘어갑니다. 통과하지 못하면 6단계(스크랩)로 좌천됩니다.

## 4단계: 콜 1차 선점 및 낚아채기 보고 (`POST /api/orders/confirm`)
...
## 5단계: 상세 정보 수집 후 서버 최종 판결 대기 (`POST /api/orders/detail`)
...
## 6단계: 탈락자들 스크랩 수집 및 발송 (`POST /api/scrap`)
만약 필터(3단계)를 통과하지 못한 폐급 콜들이거나, 수동(MANUAL) 모드라서 그냥 관제만 해야 하는 일반 콜들의 경우입니다.
...
- **응답:** 즉시 반환되며, 응답에는 0단계와 동일하게 최신 필터(`dispatchEngineArgs`)가 피기백으로 포함됩니다. 앱은 매번 `SharedPreferences`를 덮어씌워 필터를 최신화합니다.

## 7단계: 서버 태세 역명령 하달 (Feedback Loop)
6단계에서 `/api/scrap` 응답을 받을 때, 서버가 `deviceControl.mode` 값으로 명령을 섞어 보냅니다.
- `"MANUAL"`: 수동 모드 유지 (관제만)
- `"AUTO"`: 자동 사냥 모드 돌입 (3단계 필터 가동)
