...
### 리팩토링 완료 후 의존 관계도 (목표: 계층 구조)

```mermaid
graph TD
    subgraph "계층 1: 상태 저장소 (State Layer)"
        DS["dispatchStore.ts<br/>(mainCallState, subCalls,<br/>pendingOrdersData 등 5개)"]
        FS["filterStore.ts<br/>(activeFilterConfig)"]
        LS["locationStore.ts<br/>(globalDriverLocation)"]
    end

    subgraph "계층 2: 설정 & 유틸 (Config/Util Layer)"
        DC["dispatchConfig.ts<br/>(DISPATCH_CONFIG 상수)"]
        RO["routeOptimizer.ts<br/>(getDistanceKm, optimizeWaypoints)"]
    end

    subgraph "계층 3: 비즈니스 로직 (Service Layer)"
        DE["dispatchEngine.ts<br/>(handleDecision,<br/>recalculateKakaoRoute 등 6개 함수)"]
        KU["kakaoUtil.ts<br/>(calculateSoloRoute, calculateDetourRoute)"]
        GS["geoService.ts<br/>(getCorridorRegions)"]
    end

    subgraph "계층 4: 라우터 (Route Layer) — HTTP 요청/응답만 담당"
        DT["detail.ts (350줄)<br/>POST /detail 핸들러만"]
        OR["orders.ts<br/>POST /confirm, /decision"]
        EM["emergency.ts<br/>POST /emergency"]
        SC2["scrap.ts<br/>POST /scrap"]
    end

    subgraph "진입점"
        IX["index.ts<br/>(Socket.io 이벤트 + Express 라우터 등록)"]
    end
...
**핵심 차이점:**
- **Before**: 5개 파일이 7개 전역 변수를 **거미줄처럼** 직접 참조 (어디서든 읽고 쓰기 가능)
- **After**: 명확한 **4계층 구조**. 라우터(계층 4)는 Service(계층 3)만 호출하고, Service만 State(계층 1)를 읽고 씁니다.
- 나중에 **다중 사용자(SaaS)로 전환할 때**: `dispatchStore.ts` 한 파일만 `Map<userId, UserState>` 형태로 바꾸면, 나머지 Service/Router 코드는 건드리지 않아도 됩니다.
