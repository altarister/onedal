...
### 🔴 궤적에 콜이 붙는지 — 오늘 작업의 유일한 실증

`2b408e1` 이 라이브에 올라갔다. 검사(단위 5 · 시나리오 3)는 통과했지만 **실폰 + 라이브에서는 아직 안 봤다.**

**집에서 10분이면 된다 — 움직일 필요 없다.** 궤적은 *"안 움직여도 15초 지나면 남긴다"* 가 규칙이라, 앉아서도 점이 쌓인다. 필요한 건 **활성 콜 하나**뿐.

```
1. 관제앱(S23) 라이브 로그인 · 위치 권한 «항상 허용»
2. 필터 도착지를 올릴 콜의 하차지와 맞춘다  (지금 «성남시» 로 걸려 있다)
3. 스캐너 폰에서 크롬으로 rehearsal.altari.com → 🎲 랜덤콜 → 콜 올림
   🔴 삼성 인터넷은 안 된다 (웹 내용을 접근성 트리에 안 올림 — 2개 vs 크롬 41개)
4. 콜 하나 KEEP → ORDER_CONFIRMED
5. 1분 기다린 뒤 라이브 DB 확인:
     SELECT count(*) FROM gps_tracks WHERE order_id IS NOT NULL;   → 0 이면 실패
     stop_type 이 'pickup' 으로 갈리는가
```
