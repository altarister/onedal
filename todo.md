...
### 🔴 그런데 `stop_type` 이 **전부 `pickup`** 이다 — `dropoff` 가 0건 (2026-08-28 발견)

세 콜 **모두** `pickup` 이다. 어제 주행의 558점이 12:58~16:37 **네 시간 가까이** 찍혔는데 그 사이 하차가 있었을 텐데도 안 넘어갔다.

`nextStopOf()` 는 *"아직 안 지난 첫 정거장"* 을 고른다 → **도착 감지(`arrivalFired`)가 안 찍히면 영영 `pickup` 에 머문다.**
08-26 44분 · 08-28 48분 GPS 공백(S23 배터리 최적화)과 이어져 보인다 — **점이 끊기면 도착 감지가 안 되고, 도착 감지가 안 되면 정거장이 안 넘어간다.**

🔴 **궤적에 콜은 붙는데 상차·하차 구분이 안 되면 *"어디서 무슨 일을 했나"* 의 절반만 남는다.**
고치기 전에 **먼저 진단**한다 — 배터리 최적화 제외 후 다음 주행에서 `dropoff` 가 찍히는지가 1차 관문이다.

### 🔴 궤적을 **읽는 코드가 없다** (2026-08-28 발견)

`gps_tracks` 는 지금 **쓰기 전용**이다. `INSERT`(`gpsTrackStore`)와 `DELETE`(7일 정리)뿐이고 **`SELECT` 가 서버 어디에도 없다** — 라우트도 없고 운행일지도 안 쓴다.

그래서 위 확인도 EC2 에 직접 들어가야 했다. **궤적은 쌓이기만 하고 아무도 안 본다.**
→ 최소한 «이 콜의 궤적» 하나는 운행일지에서 보여야 한다.
