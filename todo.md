...
## 🎨 색만 보고 잡으려면 — 판정이 아직 못 막는 것 (기사님 질문에서 나온 목록)

기사님: *"첫짐도 정차중합짐도 주행중합짐도 판단만 보고 잡으면 되려면 뭘 더해야 해?"*

전수 목록과 순서는 **[docs/기획/색만_보고_잡으려면.md](docs/기획/색만_보고_잡으려면.md) 한 곳**에 있다.
여기엔 어디를 볼지만 적는다.

- [ ] 🔴 **A1 제외 키워드를 서버 판정이 본다** — 판정식(`NATURE`)은 이미 있는데 재료(`excludedHits`)를
      한 번도 안 채운다. **픽커와 손으로 잡은 콜은 앱 필터도 안 지나 무방비다**
- [ ] 🔴 **A3 약속이 «몇 분» 늦는지 숫자로 넘긴다** — 지금은 1분과 60분이 똑같이 🔴
- [ ] 🔴 **A2 역주행을 판정이 본다** — 지금은 콜 필터만 본다
- [ ] ⚠️ B1 적재량 확정값이 판정까지 닿나 · B2 픽커 재료가 얼마나 차나
- [ ] 🟡 C1 정차 중 ↔ 주행 중을 판정이 가른다 (콜 필터는 이미 가른다) · C2 첫짐 거리 하한 ·
      C3 판의 시계 · C4 후속 콜 빈도
- [ ] 📊 D1 **실콜 검증 0건** — 색과 기사님 판단이 어긋난 콜을 모은다 (`pnpm db judge`)

🔴 **A 셋까지가 «막는 그물»이다** — 되면 색을 믿고 누를 수 있다. B·C 는 색을 더 좋게 하는 일이다.

## 🔧 앱이 «눌렀다»고 로그에 찍는데 안 눌렸을 수 있다 — 반환값을 버린다 (버그 대장 #157)

`DetailCollectMachine` 의 적요 · 출발지 · 도착지 **세 곳 모두** `findAndClickByText(rootNode, "닫기")` 의
**반환값을 버린다.** 못 찾아도 · 못 눌러도 상태는 다음 단계로 넘어가고 로그는 「닫기 버튼 누름」이라 찍힌다.
그래서 잠금이 「닫기」를 아홉 번 삼켰을 때 «눌렀는데 안 닫혔다»로 읽혀 진단이 한참 돌아갔다.

- [ ] 셋 다 반환값을 보고, 실패면 **상태를 안 넘긴다**(다음 스캔에 다시 시도) · 로그도 «못 눌렀다»로
- [ ] 닫기 버튼 이름이 배차망마다 다르다 — 화물24시는 «돌아가기» (아래 `HijackService.kt:620` 자리와 함께 본다)
- [ ] `AutoTouchManager` 는 접근성 서비스를 물고 있어 단위검사로 못 부른다 — 누름 성패를 어떻게 잠글지 정한다

## 📸 배차망 화면 읽기 공통 루틴 — 바뀌면 우리가 먼저 안다

규격·까닭·순서는 [docs/기획/스냅샷_검증_및_이상징후_어드민_관제_계획.md](docs/기획/스냅샷_검증_및_이상징후_어드민_관제_계획.md) 한 곳에 있다.
여기서는 **남은 일만 센다.**

**1걸음 — 보는 자리** (계획서 §4)
- [ ] 🔴 **먼저 정한다: 사진을 서버로 올리나** — 화주 상호·주소·전화번호가 찍힌다 (계획서 §4.1). 1걸음의 모양이 이 답에 달렸다
- [ ] 이상 징후를 관제웹에서 본다 — 지금은 `sqlite3` 로 표를 직접 열어야 안다
- [ ] `telemetry_anomalies.screenshot_path` 를 채운다 — 사진을 남기기 시작했는데 서버는 모른다
- [ ] 소켓 `anomaly-detected`

**2걸음 — 평소와 같았나** (계획서 §4)
- [ ] 잘 읽혔을 때도 문제지(기대치)와 대조해 어긋나면 신호 — 지금은 성공하면 아무 말도 안 한다
- [ ] **둘 다 읽혔는데 값이 다르면 무엇이 이기나** — 지금은 그림이 무조건 이긴다. 규칙이 없다

**3걸음 — 공통 루틴으로** (계획서 §2)
- [ ] 13걸음을 한 자리에 두고 ④⑨⑫ 만 배차망·화면에서 뽑아 쓴다
- [ ] ⑩⑬ 세 벌 복사를 한 벌로 (`handlePreConfirmSnapshot` 의 성공·판독실패·에러)
- [ ] 찍는 갈림길을 «픽커면»에서 «값이 비면»으로 (`if (plugin.ocrParser != null)`)
- [ ] 인식 흠집을 흡수하는 층 — 「픽업 16.7가km」 (계획서 §7.3-4)

**바로 고칠 것**
- [ ] **사진 저장이 흐름 끝에 있다** — 읽기·해석이 실패하면 거기까지 못 가 **정작 필요할 때 안 남는다**. 줄인 직후로 옮긴다
- [ ] **되돌아갈 목록 카드가 비어 있었다** — 실측 2026-09-20 07:56 (이상 기록 #70): `{"fare":8855,"pickup":"","dropoff":""}`

## 🟡 필터 영역 개정 — 코드를 새 정의에 맞춘다 (기사님 확정 2026-09-15)

규격: [docs/지금/필터.md](docs/지금/필터.md) «필터 영역» · «상차 영역» · «하차 영역». 다른 곳 표도 거기 («지금 코드와 다른 곳»).
...
## ✅ `pnpm scenario` 9건이 폐기된 규칙을 묻는다 (2026-09-13 · 배포 전 확인에서 잡힘 · 2026-09-15 고침)

`pnpm scenario` 가 **57건 중 9건 빨간불**이다. **코드가 깨진 게 아니라 검사가 낡았다.**

```
· 첫짐 저장이 지금 국면이라 바로 적용된다      ← 뿌리 5건
· 관내 탭에 저장해도 지금(첫짐) 필터는 그대로다
· 저장은 됐다 — 관내 국면 값이 서버에 남는다
· 관내로 바꾸면 관내 저장값이 펼쳐진다
· 첫짐으로 돌아오면 첫짐 저장값이 되살아난다
· 13·14·15·16번 KEEP                          ← 그 위에서 따라 깨진다
```

### 왜 깨졌나 — `9a2fd77` 에서 그릇을 걷었는데 검사가 안 따라갔다

```
9a2fd77  refactor(필터): 값 그릇도 한 벌로 — user_filter_phases 다섯 행을 걷는다 (C3-3b)
```
...
| 자리 | 적혀 있는 말 |
|---|---|
| `filterManager.ts:715` | *"소켓 `save-phase-settings` 도 함께 사라졌다"* |
| `socketHandlers.ts:322` | *"🥣 국면 저장 통로가 **여기 있었다**"* |
| `userSessionStore.ts:131` | *"🥣 국면별 필터 설정 셋이 **여기 있었다** … 걷어냄 2026-09-11 · 이식 C3-3b"* |

그런데 `scripts/scenario.mjs` 는 아직 그 옛 통로를 쏜다:

```js
s.emit('save-phase-settings', { phase, settings: { ...(st.phases?.[phase] ?? {}) } })
```

**서버가 안 받는 이벤트**라 아무 일도 안 일어나고, 「저장이 됐나」를 묻는 5건이 전부 실패한다.
KEEP 4건은 그 틀어진 필터 위에서 도는 것이라 따라 깨진다 — **뿌리는 하나다.**
`scenario.mjs` 자체는 C3-3b 뒤로 한 번도 안 고쳐졌다.

### 할 일

- [x] 🔴 **그 9건을 새 규칙으로 다시 쓴다** — 국면 저장 5건을 걷고 «필터 두 그릇 — 오늘만 · 💾 서버 저장» 5건으로 (`scenario.mjs` · `st.base` ← `filter-updated` 의 `baseFilter`) · KEEP 4건은 뿌리가 풀려 따라 초록 — 씨앗 `smoke-mode.db` 로 53/53 (2026-09-15)
- [x] 지울 것이면 **왜 지웠는지**를 그 자리에 남긴다 — `scenario.mjs` «국면별 저장 검사 다섯을 걷었다» 주석
- [ ] ⚠️ **빨간불이 늘 켜져 있으면 아무도 안 본다** (CLAUDE.md 가 `lint:gate` 를 두고 적은 그 이유).
      9건이 매번 떠 있어서 **다른 진짜 빨간불이 묻힐 자리**다 — 그게 이 항목의 시급함이다
- [ ] 담당: **콜 필터 국면 개편(C4) 쪽**. 그 판이 걷어낸 것이라 맥락이 그쪽에 있다

⚠️ 2026-09-13 배포(464 커밋)는 **이 9건이 빨간불인 채로** 나갔다. 기능은 멀쩡하고
   그날 다른 게이트는 전부 초록이었다 — `jest` 161스위트 1608건 · `test:web` 623건 ·
   `drive` 15건 · 감사 셋 · `lint:gate` 0 errors.

## 🖥️ 현황판을 어드민 프로젝트로 (2026-09-11 · 기사님 구상 · **지금은 안 한다**)

기사님: *"브라우져를 2개 열어서 프로젝트와 어드민, 어드민 접속하고 **기사 골라서 들어가면**
어짜피 **오른쪽에 있는것이 서버에서 하는 일들이라** 가능할꺼 같은데."*
→ *"**지금 당장 하지 말자는거지? 음.. 그래 그럼 todo에 넣어둬.**"*

### 왜 지금은 안 하나

분리 효과(충돌 0 · 한방에 지우기 · 영역 표시)는 **폴더로 이미 다 나왔다**
(`client-app/src/statusboard/` · 2026-09-11). 별도 프로젝트의 **추가** 이득 셋은 아직 없다:

| 이득 | 지금 |
|---|---|
| 운행용 번들이 가벼워진다 | 현황판은 **PC 넓은 화면에서만** 만들어진다 — 폰엔 애초에 안 들어간다 |
| 기사님 말고 다른 사람도 본다 | 기사님 혼자 보신다 |
| 현황판이 제 데이터를 갖는다 | 서버 것을 **비추기만** 한다 |

🔴 반면 비용은 실하다 — 현황판이 관제웹 훅 넷(소켓 구독 포함)에 기댄다. 옮기면
**두 벌**이 되거나 `shared` 로 또 올라간다 (이 레포가 «경유 4벌·상태목록 3벌»로 당한 모양).
그리고 **«한 화면»이 깨진다** — 기사님이 브랜치를 피하신 바로 그 이유다.

### 옮길 때가 오면 — 이삿짐은 이미 싸 뒀다

- [ ] **옮기는 신호**: 다른 사람이 볼 때 · 현황판이 제 데이터를 가질 때 · 번들에 부담이 될 때
- [ ] 갈아끼울 것은 **`statusboard/bridge.ts` 하나**다 (2026-09-11 신설).
      바깥 의존 **넷**이 거기 모여 있다 — `useFilterConfig` · `useDeviceStore` ·
      `summarizeTally`(순수 함수라 **그대로 쓴다**) · `apiBase`
- [ ] 소켓에 **기사 id 를 얹는 길**을 먼저 정한다 (어드민은 «남의 세션»을 본다) —
      규칙 ⑤-4 ⑤: 그 값을 **누가 읽고 무슨 질문의 답으로 쓰나**
- [ ] 🔴 **권한**: 어드민이 아무 기사나 볼 수 있으면 안 된다. 지금 서버는 «내 세션»만 안다

## 🔬 곁 패널 — 못 가져온 둘 (2026-09-11 · 실물에 재료가 없다)

목업 좌측 패널의 정보를 실물 곁 패널로 옮기며 **여덟 칸**을 세웠다. 남은 둘은
«안 만든 것»이 아니라 **실물에 그 값을 쌓는 자리가 없어서** 못 가져온 것이다.

### ⬜ 👀 미리보기 «요율 하한 미달 → 점수 0.6배 · 색을 똥으로 내리기» — 없앨 예정 (기사님 2026-09-14)

- 자리: `onedal-web/server/src/core/engine/OrderEvaluator.ts` 의 `previewRate` (첫짐 갈래) · `onedal-web/shared/src/criteria.ts` «평소 하한 미달»
- 기사님: 요율 판정은 자동콜에도 있다 — 미리보기(누가 열었든 확정 전 상세)만 점수·색이 달라질 까닭이 없다. **앞으로 사라질 예정** · 판정 기준 공통 일로 맡긴다
- 🔴 버그 후보: 판정 함수가 점수로 색을 정한 **뒤에** 색만 «똥»으로 덮어써, 점수와 색이 서로 다른 말을 할 수 있다
- 픽커 시뮬레이터에는 필요 없다 — 판정 색은 그대로 나온다 (한 단계 낮게 나올 뿐)

### ⬜ 📜 판정 기록 — 콜마다 «무엇을 보고 그렇게 판단했나»가 안 남는다

목업은 콜을 판정할 때마다 한 줄씩 쌓는다 (`{시각, 상차→하차, 거리 삼항, 통과, 관내, 확정/버림}`).
실물은 **판정 스냅샷(`JudgmentSnapshot`)을 콜에 붙여 두지만 «흘러간 콜»의 기록은 없다** —
취소·방출된 콜은 목록에서 빠지고 그 판단의 근거도 함께 사라진다.

- [ ] 먼저 정한다: **어디에 쌓나** (메모리 · DB · 로그). 실전은 하루 종일이라 메모리는 새로고침에 진다
- [ ] 규칙 ⑤-4 ⑤ — 이 기록을 **누가 읽나**. 「왜 안 잡았지」를 되짚는 용도라면
      기사님이 읽는 것이고, 그러면 화면 문법이 로그와 달라야 한다

### ⬜ 🧭 카카오 호출 수 — 세는 곳이 없다

목업은 `🧭 시스템 — 카카오 호출 N건` 을 적는다. 실물은 **서버 로그에만** 남고
(`🧭 [경로 순서] 카카오호출시점 …`) 세어 둔 값이 없다.

- [ ] 실전에서 값진 이유: 카카오는 **호출당 비용**이고, 합짐을 잡을 때마다 전 구간을 다시 잰다.
      *"오늘 몇 번 불렀나"* 가 보이면 **경로 재계산이 폭주하는 것**을 바로 안다
      (2026-09-11 에 매 틱 그물 재계산으로 느려진 일이 있었다 — `f7a5564`)
- [ ] 자리 후보: 세션에 카운터 하나 + `/api/health` 에 실어 곁 패널이 읽는다

## 🔵 합짐 지도에 **현위치 둘레 원이 안 보인다** (2026-09-11 · 기사님 지적 · 버그로 남겨 둠)

기사님: *"시작점 현위치 범위설정을 내가 못했는지 노출되지 않는다. 일단 버그로 남겨 두고 다음으로."*

갈마동(현위치 ①) → 성거읍(②) 을 잡은 **합짐** 화면에서, 그물이 **성거읍 위쪽(경기 남부~김포)**
에만 뜨고 대전 현위치 둘레에는 라인 띠만 얇게 있다.

### ⬜ 먼저 «설계인가»부터 가른다 — 코드에 기사님 확정이 있다

🔴 **콜을 쥐면 그물이 «라인 그물»로 바뀐다** (`buildLineNet`). 그 안의 판정은

```
dropIn = 라인 띠 ∪ 목적지 원 ∪ 마름모(마지막 하차지 → 목적지)
```

**현위치 원이 없다.** `callNet.ts:851-859` 에 그 이유가 적혀 있는데 **기사님 지적(2026-09-09)**
이다: *"중간 기착지인 평촌동도 점선 라인과 영역에 지역들을 가지고 있는데 이걸 빼야 해."*
— 출발 꼭짓점의 원은 «내가 서 있는 자리는 방위를 잴 수 없다»를 메우는 것이라 **첫짐(내 위치)
일 때만** 뜻이 있고, 노선에서 그 자리는 **마지막 하차지**라 이미 라인 반경이 담는다는 것.

> ✅ **2026-09-11 저녁 — ③(실물만 안 보인다)은 **아니다**. 코드로 확인했다.**
> 실물(`useCallNet:91`)과 실험실(`MapMockup:1550`)이 **둘 다 `netForGoal` 을 부른다.**
> 라인이 있으면 양쪽 다 `buildLineNet` 으로 가고, 거기 `circles` 는 **목적지 하나뿐**이다.
> 즉 **설계대로**이고 기사님이 9/9 에 빼라고 하신 그것이 맞다.
>
> 🔴 **다만 진짜 차이를 하나 찾았다** — 실험실은 `line: lineOn ? routeLine : null` 이라
> **「노선 ↔ 동선」 토글로 라인을 꺼 볼 수 있다.** 동선으로 돌리면 마름모가 되고
> **현위치 원이 돌아온다.** 실물에는 그 토글이 없어서, 콜을 쥐면 **되돌아볼 길이 없다.**
> 기사님이 실험실에서 보셨던 그 화면이 아마 «동선» 쪽이다.
>
> ✅ **2026-09-11 — 「노선/동선」 토글을 가져왔다.** 지도 위 레이어 버튼(🧅) 아래에 있다.
>    동선으로 돌리면 **현위치에서 목적지까지 마름모 하나**가 되고 **현위치 원이 돌아온다.**
>    실측으로 확인 — 노선은 경로 양옆 좁은 띠, 동선은 갈마동→김포 통째 마름모.
>    🔴 «고른 것»과 «실제»를 가른다 — 노선을 골라도 경로가 아직 없으면 마름모로 보고
>    화면이 *"⏳ 경로를 기다립니다"* 라고 말한다 (직선으로 지어내지 않는다 · 규칙 ④).

- [ ] ~~**그러니 셋 중 무엇인지부터 정한다**~~ (위에서 갈렸다):
      ① 설계대로다 — 기사님이 9/9 에 빼라고 하신 그것. 고칠 것이 없다
      ② 첫짐에서는 보이는데 합짐에서만 안 보이는 것이 **혼란스럽다** → 화면이 «왜 없는지»를
         말해야 한다 (규칙 ⑤-4 ④ — 없는 이유를 안 적으면 고장으로 보인다)
      ③ 실험실에서는 보이는데 실물만 안 보인다 → **진짜 갈라짐.** 재현이 먼저다
- [ ] 가르는 법: **첫짐 상태**(콜 0건)에서 같은 화면을 본다. 거기서 현위치 둘레에 원이
      뜨면 ①이나 ②, 안 뜨면 ③이다. 실험실도 같은 판으로 나란히 본다 (두 화면을 찍어 눈으로)
- [ ] ⚠️ **원이 그려지는 자리는 `circles` 다** — `buildNet` 은 둘(출발·목적), `buildLineNet` 은
      **목적지 하나만** 낸다(`callNet.ts:906`). 고친다면 여기지, 판정(`dropIn`)이 아니다.
      🔴 **판정과 그림을 함께 고치지 않으면 화면이 거짓말한다** — 원만 그리고 판정이 안 담으면
      «든다고 그려 놓고 탈락»이 된다 (이 레포가 이미 여러 번 당한 모양)

## 🕸️ ✅ 합짐·첫짐 목록은 아직 옛 계산이다 — 그물은 필터를 만질 때만 돈다 (2026-09-14 전수 조사 · 같은 날 끝냄 · 버그 대장 #121)

🔄 **이 일은 [목업_이식_전수표.md](docs/기획/목업_이식_전수표.md) 의 «1단계»다** — 체크는 거기서 한다 (기사님 2026-09-14).

처음엔 «경유 지역만 turf» 로 적었는데 **더 넓었다** («7지점 한 바퀴» 코드·서버 로그 대조). 표는 [필터.md §5 «코드는 아직 반만»](docs/지금/필터.md).

| 목록을 다시 만드는 때 | 지금 부르는 것 | 옮길 곳 |
|---|---|---|
| 서버 부팅 · 마지막 콜이 끝나 0건 | `rebuildDestinationKeywords` → `getCityRegionsWithRadius` (시 경계 버퍼) | 그물 (`line: null`) |
| KEEP · 경로 재계산 · 재탐색 · 귀가콜 | `syncDetourFilter` → `recalculateDetourFilter` (경로 버퍼 ∪ 시 경계 버퍼) | 그물 (`line: 지금 경로`) |

실제로 난 일: 06 콜(중리동 → **초월읍**)이 도착지 축을 통과했다 — 이천 시 경계 10km 버퍼가 초월읍을 담았다. 그물이면 빠진다(중심 18km).

- [x] 🔴 **먼저 `orderKm`·`progressKm` 의 원천을 정한다** — 그물 라인 진행도 (2026-09-14) — 지금은 옛 경로 버퍼(`getDetourRegions`)에서만 나온다.
      앱 역주행 검사(`orderKm`)와 지나온 곳 빼기(`progressKm`)가 거기 걸려 있어, 목록만 옮기면 **역주행 검사가 꺼진다**
- [ ] 옮기기 전후로 `pnpm net:compare` — ⚠️ 이 스크립트는 첫짐 조건만 비교해 합짐 옮기기를 못 잰다. 7지점 폰 시험으로 대신한다 — 통과 동 목록이 얼마나 달라지는지 먼저 잰다
- [ ] 🔴 **판정 색이 바뀔 수 있다** (규칙 ⑤-3) — 따로 한다
- [x] 옛 계산은 자동 반경(§10-1)을 안 쓴다 — 그물로 옮겨 자동 반경이 판정 목록에 걸린다 (2026-09-14) — 옮기면 자동 반경이 처음으로 판정 목록에 걸린다
- [ ] `GET /api/settings/preview-regions` · `preview-detour` 는 옛 계산이고 **관제웹이 안 부른다**(2026-09-14 grep) — 지우기 전에 묻는다

## 🕸️ 이식 C1-2 — 🔶 반만 끝났다 (2026-09-11 · `netKeywordsOf` 가 `netForGoal` 을 부른다 · 2026-09-14 정정: 옛 계산 길이 남았다 — 바로 위) — 아래는 그날의 실측 기록

기사님 Q1 확정은 **(나) 실험실 계산을 `shared` 로 올려 서버가 그걸 쓴다** 였다.
**올리기만 했고(B3-1) 서버는 아직 제 계산을 쓴다.** 계획서에 「C1 ✅」로 적었다가 되물렸다.

| | 관제웹 지도 | 서버 판정 |
|---|---|---|
| 계산 | `shared/callNet.ts` — 동 중심점 + 마름모 | `geoService.getCityRegionsWithRadius` — **turf 폴리곤 버퍼** |
| 부르는 곳 | `useCallNet` → `StageView` | `recalculateDerivedFields` → 앱 피기백 |

### ⬜ 실측 — 두 목록이 얼마나 갈리나 (2026-09-11)

```
▸ 인천   · 상차 15km · 하차지 주변 80km · 마름모 120°/140°/35km
   지도 1426개 동 · 판정 1208개 · 양쪽 다 1163      일치율 79.1%
   지도만 263개 (만년동·상대동·송강동 …) · 판정만 45개 (가남읍·가평읍 …)

▸ 파주시 · 상차 10km · 하차지 주변 15km · 마름모 110°/110°/25km
   지도 1146개 동 · 판정  190개 · 양쪽 다  132      일치율 11.0%
   지도만 1014개 · 판정만 58개
```

🔴 **파주 조건에서 956개 동이 「화면엔 드는데 판정은 모른다」.** 규칙 ⑤-3 이 말하는
«색이 곧 결정»인데, 화면이 담는다고 그린 곳을 판정이 모르면 화면이 거짓말하는 것이다.

⚠️ **이 숫자의 한계를 함께 적는다** — 두 계산은 **같은 질문에 답하지 않는다.**
실험실 그물은 «내 위치→목적지 길목 전체»(상차 반경 + 마름모 + 목적지 둘레)를 담고,
서버 목록은 «목적지 도시 둘레»만 만든다 (상차 반경은 앱이 따로 거른다).
그래서 이 표는 **«두 목록이 다르다»의 크기**이지 **«서버가 틀렸다»의 증거가 아니다.**
어느 쪽이 원천인지는 기사님이 (나)로 정하셨으니, 할 일은 **서버를 그쪽으로 옮기는 것**이다.
...
## 🎛️ 이식 C3-3 — ✅ 끝났다 (2026-09-11 저녁 기사님 *«그 기준은 바꿔»* · C3-3a·b) — 아래는 결정 전 기록 (실측표의 «첫짐 80»은 계획서가 22 로 정정한 값)

C3-2 로 **마름모 셋**(출발각·목적각·마름모반경)은 국면 밖 한 벌이 됐다. 남은 다섯 칸을
어떻게 할지는 **기사님이 «마름모 셋부터» 로 정하셔서 여기 멈춰 뒀다.**

### ⬜ 물어볼 것 — 이 칸들을 한 벌로 접나

기사님이 **실제로 국면마다 다른 값을 쓰고 계신다** (2026-09-11 실측 · 로컬 DB):
...
**하차지 주변이 첫짐 80km ↔ 합짐 1km**, **콜할인율은 관내만 20%** 다. 한 벌로 접으면
이 중 **하나를 골라야** 한다 — 그냥 합치면 기사님이 하루의 장면마다 쓰시던 조건이 사라진다.

🔴 **코드보다 먼저 만들 것: «한 벌이 되면 각 값이 언제 읽히는가» 표** (규칙 ⑤-4 ⑤).
목업 모델은 *"모두 꺼내 두고 노선이면 라인값을 사용하고 동선이면 사용 안 하면 되니까"*
(기사님 2026-09-09)인데, 위 칸들은 **국면마다 다른 값으로 읽히고** 있다. 둘 중 어느
쪽이 맞는지는 칸마다 답이 다를 수 있다.

> ⚠️ 계획서 §4 Q2 의 근거 문장(*"실험실에서 다섯 벌이 실제로 갈렸다 — 화면 15km ↔ 판정
> 다른 값"*)은 **틀렸다.** 그 사고의 원인은 국면이 아니라 «같은 값의 편집 화면이 두 곳»이었고
> 이미 고쳤다. 진짜 근거는 2026-09-09 확정이다. 계획서에 정정해 뒀다.

## ✅ 요율표를 바꿔도 앱에 **옛 단가**가 내려갔다 (2026-09-11 전수조사 · 2026-09-20 재현·수리 · 버그 대장 #163)

재현됐다 — 1t 을 1000 → 2000 으로 올려 저장하니 DB 는 2000, 앱이 받는 표는 770(옛 값) 그대로였다.
파생을 다시 만드는 방아쇠가 세 입력 중 `callDiscountPct` 하나뿐이었다. 조건을 없애 늘 만들게 했고,
`PUT /pricing` 이 파생 재계산 경로를 타게 했다. 검사 `pricingSaveReachesApp` 4건.

## 🪞 목업 ↔ 실물 대조가 잡은 것 둘 (2026-09-11 · 이식 A 단계)

기사님: *"같은 조건의 콜 리스트를 만들어서 대조해."* 볼트 오전 ①②③ 을 **양쪽에 똑같이**
세우고 격자 다섯 칸을 나란히 읽었다 (`docs/기획/이식_계획_지도필터콜.md` §11-3).

```
             순번 | 지명   | 약속  | ±     | 예상
실물  1) 상 1 갈마동 03:15 |  +3367 | 11:21   /  하 4 성거읍 13:45 | +1  | 13:46
목업  1) 상 1 갈마동 11:33 |      0 | 11:33   /  하 4 성거읍 13:30 | -3  | 13:27
실물  3) 상 3 오송읍 11:33 |      · | --:--   /  하 6 논현동    ·  |  ·  | --:--
목업  3) 상 3 오송읍 11:33 |    +68 | 12:41   /  하 6 논현동 14:23 | +78 | 15:41
```

🟢 순번(①②③→④⑤⑥)·지명은 **양쪽이 같은 답**을 냈다. 필터도 같았다 —
③ 오송은 자동콜로 올리자 서버가 CANCEL 했고, 목업 필터도 «탈락»으로 봤다.

### ⬜ ① `± +3367` — **하루 지난 약속을 그대로 견준다**

어제 잡힌 콜의 약속(03:15)이 남아 오늘 예상(11:21)과 견줘져 **56시간**이 찍혔다.
목업에서는 안 나온다 — 그 자리에서 잡으니까. **밤을 넘기면 실주행에서 매번 나올 자리다.**

- 무엇을 정할까: «하루 지난 약속»을 어떻게 적나 — 숫자를 안 적나, 날짜를 함께 적나,
  «지났음»으로 적나. 색이 곧 결정인 화면이라(규칙 ⑤-3) **뜻 없는 큰 수가 눈을 끌면 안 된다**
- 어디: `CallDeck.tsx` 격자의 `diff` · `docs/지금/시각_표시.md` 의 0911 개정 절

### ⬜ ② 같은 콜인데 **실물만 약속·예상이 비었다**

③ 오송읍→논현동: 목업은 `11:33 → 12:41` · `14:23 → 15:41` 을 다 적는데 실물은 `·`·`--:--`.
직접콜(MANUAL)로 올려서 서버가 약속을 안 박은 것인지, 경로가 아직인지 **갈라 봐야 한다.**

- 🔴 직접콜이라 그런 것이면 **규칙 ①과 부딪히지 않는지** 먼저 본다 — 서버는 직접콜에
  30초 자동 판결을 안 걸 뿐, 약속·경로까지 안 주는 것이 맞는지는 별개다
- 어디: `deriveRouteTimeline` 입력 · `routeStops` 에 직접콜이 드나

> ⚠️ **2026-09-11 오후 대조가 「빈 칸 0개」로 초록이었다 — 안심하면 안 된다.**
> 그날 선 콜은 **갈마동·문지동 둘뿐**이고, 이 구멍은 **세 번째 콜(오송 · 직접콜)**에서 났다.
> 즉 그 케이스를 **안 밟은 것**이지 메워진 것이 아니다. 재현하려면 **오송 직접콜까지 셋**을 세운다.
> ⚠️ 그때 쓰던 `pnpm lab:both` 는 2026-09-13 에 지웠다 (지어낸 콜로 대조하던 것 ·
> 이유는 `onedal-web/CLAUDE.md`). **지금은 셋을 세울 손쉬운 길이 없다** — 결재 카드까지 세우던
> `pnpm rehearsal` 도 2026-09-14 에 지웠다. 남은 길은 폰과 시뮬로 도는 `pnpm e2e:app` 뿐이다.
...
| | 콜① | 콜② | 주기 |
|---|---|---|---|
| 마커 (`callPalette.CALL_TONES`) | 토마토 | 초록 | 4 |
| **지도 선** (`useRouteDerivations.PALETTE`) | **파랑** `#4f8df9` | 주황 | 7 |
| 실험실 선 (`MapMockup.CALL_COLORS`) | 로즈 `#e11d48` | 보라 | 8 |

🔴 `callPalette.ts` 가 못박은 **「색 = 콜 번호」 규칙이 선 색에서 깨져 있다.**
기사님이 **팔레트 하나**를 고르면 그대로 통일한다 (자리표 D-1).
- 어디: `styles/callPalette.ts` (원천) · `hooks/useRouteDerivations.ts:327` · `MapMockup.CALL_COLORS`

### ⬜ ④ 경로를 다시 잰 뒤 **콜 행이 다시 저장되지 않는다**

구간 경계·주인을 DB 에 넣었는데(B1·B2), **경로를 든 콜의 행은 비어 있었다** — KEEP 때 한 번
저장하고 그 뒤 경로가 다시 잡혀도 행을 안 덮는다. 서버를 재시작하면 그 콜만 색을 잃는다.
- 어디: `OrderRepository` 저장 시점 · `applyRoute`/`applySoloRoute` 뒤에 저장을 걸지

### ⬜ ⑤ 하차지 지오코딩이 **카카오가 못 가는 자리**를 준다

«인천 남동구 논현동» 으로 올린 콜이 «도착지점 탐색불가 (도착 지점 주변의 도로를 탐색할 수 없음)»
로 **합짐 경로 전체를 막았다**. 동 중심점이 도로에서 떨어진 자리라 그렇다.
- 🔴 한 콜의 나쁜 좌표가 **다른 콜들의 경로까지 못 잡게 한다** — 그 콜만 빼고 잴 수 있어야 한다
- 어디: `composeMergedRoute` 의 실패 처리 · `geoService` 의 동 중심점

### 🔧 대조하는 법 — 🔴 **되돌렸다** (2026-09-13)

기사님 2026-09-11: *"너도 목업 화면과 실 프로젝트 화면을 **번갈아 봐야 해**."*
기사님 2026-09-13(뜻을 바로잡아 주심): *"내가 보는 화면을 너가 보지 못하니까 **브라우저 열어서
이미지 캡처해서 보라**는 의미였어. **너랑 나랑 똑같은 걸 봐야지** 너가 만든 똑같은 문제지를
본다고 같은 건 아니잖아."*

🔴 그런데 만든 것(`pnpm lab:both`)은 **콜을 지어내 양쪽에 세우고 DOM 글자를 맞춰 보는 도구**였다.
세우던 요금이 문제지의 **4배**(200,000 ↔ 50,050)라 「같은 조건」이라 적어 놓고
**기사님한테 일어날 수 없는 조건**을 보고 있었다. **2026-09-13 에 지웠다.**

**대조는 내가 화면을 보는 것이지 스크립트가 글자를 세는 것이 아니다.**

---

## ✅ 지도를 충청권까지 넓혔다 — **끝났다** (2026-09-06)

기사님 질문에서 시작했다: *"인천 남동공단에서 청주로 목표를 잡으면 몇 개를 잡을 수 있을까?"*
**돌려 봤더니 0개**였고, 뿌리는 필터가 아니라 **데이터**였다.

```
1,239동 (서울·인천·경기)  →  1,968동 (＋대전 177 · 세종 33 · 충북 234 · 충남 285)
27MB → 47MB · 부팅 372ms → 518ms · 그물 조회는 그대로 (오산 18ms · 청주 31ms)
```

`tests/rules/mapCoverage.test.ts` 가 🔴 → 🟢. **게이트 전부 초록** (jest 1,344 · test:web 464).

### 만드는 법은 [onedal-map/README.md](onedal-map/README.md) 에 있다 — 여기 또 안 적는다

### 🔴 1판(지금 셋으로 먼저 재생성)이 잡아낸 것 셋

**① VWorld 도메인이 키와 안 맞으면 `INCORRECT_KEY`**
스크립트가 `http://www.altari.com` 을 박아 보내는데 새 키는 **www 없이** 등록돼 있었다.
`.env` 의 `vworld_domain` 으로 넘길 수 있게 했다.

**② 인천 행정구역이 개편됐다 — 우리 지도가 낡았다**
```
낡음   중구 · 동구 · 서구
지금   제물포구 · 영종구 · 서해구 · 검단구
```
이름이 사라지거나 생긴 동은 **하나도 없고**, 코드만 64개(인천 58·서울 6) 바뀌었다.
⚠️ **배차망이 아직 옛 구 이름을 쓸 수 있다** — 리스트에 「서구」로 뜨는 인천 콜이 오면
`cityAliases('인천 서해구')` 는 「서구」를 안 만든다. 실콜로 확인할 자리다.

**③ 🔴 광역시는 구 이름에 광역명이 없다 — intel 이 그걸 담는 유일한 곳이다**
`SIG_KOR_NM` 이 「서해구」·「유성구」라 **「인천」·「대전」으로 조회하면 0개**가 나온다.
`intel.parentName`(=「인천 서해구」)만이 광역명을 갖는다.
그래서 코드가 바뀌면 intel 이 끊기고 **필터가 조용히 반토막 난다** (인천 127동 → 69동).
도(道)는 `SIG` 가 「청주시 흥덕구」라 이 문제가 없다.
`regenerate_seoul_incheon_intel.js` 를 **광역시 셋(11·28·30)으로 일반화**해 막았다.

🔴 **1판을 건너뛰었으면 셋 다 못 봤다.** 특히 ③은 화면이 멀쩡한 채로 콜만 줄어든다.

### ✅ 4판 — 시뮬 문제지 `볼첨지` 8문항 (2026-09-06)

🔴 **한 자리에서 여덟을 다 채점할 수 없다.** 대전에 서면 오송 32.6km · 성거읍 62.9km ·
서울 137.2km 라 **상차 반경에서 먼저** 떨어져, 도착지 축을 보려던 문제가 엉뚱한 축으로
채점된다. 볼첨지는 **움직이면서** 잡았으니 우리도 자리를 옮겨 가며 본다 — **판을 넷으로 쪼갰다.**
...
```
http://<PC IP>:5173/inseong/dispatch?preset=볼첨지대전
① 도착 목표 = 인천     ② 내 주소 = 위 표     ③ GPS 를 막고 PC 에서 돌린다
```

⚠️ **GPS 가 내 주소를 이긴다** (`socketHandlers.ts` — *"진짜 GPS 가 임시 출발지를 이긴다"*).
폰으로 켜면 폰 위치가 덮어쓴다. 서버 로그에 `📍 [출발지 대체] GPS 미수신 — 내 주소(…)`
가 떠야 맞다.

표 12줄 중 **동까지 확정되는 8줄**만 넣었다. 나머지 넷(1·2·8·10)은 상차지나 하차지가
시·구 단위까지만 적혀 있어 **지어내지 않았다.**

좌표는 **읍면동 중심점**이다(`turf.centroid`). 12개 전부 카카오 길찾기 성공을 확인했고,
오히려 주소 폴백보다 낫다 — 「인천 남동구 논현동」은 **폴백 좌표가 «도로없음»**이었다.

🔴 **정답지가 볼첨지와 다른 자리가 우리의 빈 곳이다**

| | 볼첨지 | 우리 | 왜 |
|---|---|---|---|
| 03·04·06 | **잡았다** | BLOCK 이 맞다 | 그는 「청주·음성·오산」처럼 목표를 여럿 쥔다. `destinationCity` 는 **도시 하나** — 묶음 평가가 없다 |
| 07 | 잡았다 **취소** | 첫짐이면 PASS 가 맞다 | 역주행은 경로가 생겨야 보인다. **합짐 국면에서 돌리면 BLOCK 이어야 한다** |
| 11 | 열어 보고 **포기** | BLOCK 이 맞는데 **지금 PASS 한다** | 🔴 **오탐** — 적요의 `11일(화) 09시픽업` 을 못 읽는다 (`cargoHints.ts` 에 날짜 축 없음) |
| 09 | 잡았다 | PASS | 🔴 경서동이 개편으로 **「인천 서해구」** — 배차망이 「서구」로 뿌리면 여기서 갈린다 |

⚠️ 표의 **「안산 송곡」은 지도에 없다.** 같은 날 화면에 `@성곡동`·`@안산성곡동` 이 찍혔으므로
**성곡동(안산 단원구)** 으로 읽었다 — 표를 고칠 자리다.

### ⬜ 곁가지

- `scripts/data/intel/28_incheon/` 에 옛 코드 파일(28110·28140·28260)이 남아 있다.
  새 코드와 안 겹쳐 무해하지만 죽은 파일이다 — **지울지는 기사님 확인 뒤**
- 게임(`map/map`)에는 **안 넣었다.** 넣으려면 파생본 셋(`build_vworld_levels.cjs`)도
  다시 만들어야 하고, `useGeoData.ts` 의 `ACTIVE_REGION_PREFIXES` 를 안 바꾸면 화면은 그대로다

## 🔬 적재 — 라면박스 환산이 화물 종류에 맞는가 (기사님 2026-09-09 · 실측 필요)

기사님: *"아 이건 유의미하네. **화물의 종류에 따라 부피가 확확 달라지는데**.. 이거 실측이
필요할 듯. 일단 별도로 마킹해 두고 나중에 논의하자."*

### 지금 이렇게 돈다

```
차종 정원        1t 100박스 · 1t짐 80 · 라보 40 · 다마스 30 · 승용차 5 · 오토바이 1
콜의 짐          신고(파레트·라면박스·마대·서류봉투) → 라면박스 점수로 환산
실물의 slotsUsed  서버가 **잡은 콜들의 점수를 더해 파생**한다 (입력이 아니다)
판정             공간 기준이 «용량 − 쓴 박스 − 이 콜»로 점수를 낸다 → **색이 바뀐다**
```

### 🔴 무엇을 모르나

**같은 «1박스»가 화물마다 부피가 다르다.** 지금은 라면박스 하나로 환산하는데, 스티로폼 상자와
공구 상자가 같은 1박스로 세어진다. 실제로 몇 개가 실리는지는 **실어 봐야 안다.**

- 실측할 것: 화물 종류별 «라면박스 몇 개어치인가» — 실주행에서 사진과 함께 센다
- 그때까지: 지도 실험실의 «쓴 박스 수»는 **손으로 넣는 시뮬 손잡이**다 (화면에 🔬 로 표시)
- ⚠️ 이 값이 **색을 바꾼다** — 환산이 틀리면 공간 점수가 틀리고 색이 틀린다 (규칙 ⑤-3)

### 💡 함께 볼 것 — 제외 단어가 «도착 위치 조건»일 수 있다 (기사님 2026-09-09)

기사님: *"제외 단어는 **목적지의 도착 위치 조건**이 될 수도 있어. 아파트 상가 같은?"*

지금 «제외 단어»는 **안 잡는다** 하나만 답한다. 그런데 «어떤 자리에 내리는가»(아파트·상가·
공장·엘리베이터 없음·층수)는 **다른 질문**이고, 정차 시간과 짐 나르는 품이 거기서 갈린다.
축을 가를지는 그때 정한다 — 지금은 고르는 자리를 지우고 값만 아웃풋에 남겼다.

## 🚗 차를 바꾸면 틀리는 값 둘 · 금액이 만원 단위 (기사님 2026-09-09)

기사님: *"내 차가 승용차에 오토바이도 있어. 나중에 트럭이나 다마스 같은 PV5를 살 수도 있기
때문에 이 부분은 아주 중요해."* · *"금액 부분이 만원 단위인 것도 문제야. 나중에 100원 단위가
음식 단위에는 많이 있더라구."*

⚠️ **먼저 정정** — 내가 *"1DAL 판정은 1톤 화물 전제"* 라고 했는데 **틀렸다.** 차종 12개가
단가표(`vehicle_rates` · 오토바이 700원/km …)·적재표(`VEHICLE_CAPACITY`)·카카오 차종 매핑까지
**전부 값으로 있고 DB에서 온다.** 진짜로 1톤이 박힌 곳은 아래 둘뿐이다.
...
🔴 **그냥 `VEHICLE_CAPACITY[내 차종]` 으로 바꾸면 틀린다** — 그 표의 `'1t': 80` 은
«1t 짐 **한 대 분량**»이지 «내 트럭에 실리는 양»이 아니다 (100 중 20이 안전 여유).
→ `users` 에 **«내 차 용량» 칸**이 필요하다. 차종에서 파생하되 여유를 포함한 값.

### ⬜ ② 정차 시간이 차종을 안 본다 — 상차 15분 · 하차 10분

`shared/src/timing.ts` 의 `DWELL_UNKNOWN_PICKUP_MINUTES` = 15, 이유는 주석에
*"찾기 + 상차 + **결박**"*. **오토바이엔 결박이 없다.** `dwellMinutes()` 에 차종 인자가 아예 없다.

### ⬜ ③ 금액이 만원 단위 — 음식은 100원 단위가 흔하다

⚠️ **어느 칸인지 기사님 확인 필요.** 화면을 훑었으나 만원 «단위»(스텝)로 입력받는 자리를
못 찾았다 — `pickerAlarmMinFare` 기본값이 10,000 인 것과 `min_fare` 기본 30,000 은 «기본값»이지
단위가 아니다. 기사님이 보신 화면을 알려 주시면 그 자리를 고친다.

### 🔴 이 셋은 «주말에 다른 일» 과 한 줄기다

기사님이 주말에 쿠팡·요기요를 하실 수 있다 — 그때 필요한 것은 차종만이 아니라
**요금 모델**(건당 + 거리 할증)과 **묶음 배차** 규칙이다. 그건 «일의 종류» 축이고
[지역 데이터 구조](docs/자료/노하우/일하는_법/노하우_추출.md) 의 `work.freight / delivery / parcel`
과 같은 자리에서 갈린다.

## 🐛 `tollFare`(탁송료 칸)에 「수수료 : 23%」가 들어가 있다 (2026-09-05 발견)

목업의 콜 내용을 실측과 맞추다가 DB 에서 나왔다.

```
sqlite> select distinct tollFare from orders where tollFare is not null and tollFare != '0';
수수료 : 23%
```

🔴 **탁송료 칸에 수수료 문자열이 들어갔다.** `parser.ts:213` 이
`extractField(lines, "탁송료") || extractField(lines, "경유비")` 로 긁는데,
09-03 콜 전부가 이 값이다.

⚠️ **경로.md §2-4 가 못박은 자리**다 — *"`orders.tollFare` 는 **다른 값이다** —
   배차망 화면에서 긁은 「탁송료/경유비」. 카카오 통행료와 **한 칸에 섞으면 안 된다**."*
   그런데 정작 그 칸에 **수수료**가 섞였다.

- [ ] `extractField` 가 «탁송료» 라벨을 못 찾았을 때 무엇을 돌려주는지 본다
- [ ] 버그를 잡았을 검사를 먼저 만들어 **빨간불 확인** (CLAUDE.md 수리 순서)
- [ ] 값이 숫자가 아니면 **안 넣는다** — «없는 숫자를 지어내지 않는다»의 반대편이다

## 🖥️ 폰 상태 바 정비 — 설계는 [docs/기획/폰_상태바.md](docs/기획/폰_상태바.md) (2026-09-02 기사님과 확정)

요소 12개를 전수로 꺼내 컬럼(앱·서버·관제 / 반영 시간 / 포함 관계)을 붙였다.
결론 셋 — ① **«관제가 바꾼다»는 12번 모드 하나뿐**이라 딤드도 거기만 필요하다
② **앱→관제는 즉시 가능**하고, **관제→앱→관제는 왕복**이라 «적용중»이 있어야 한다
③ 아직 없는 셋(**앱 버전 · 작업 단계 · 모드 대답**)은 전부 «앱 안엔 있는데 안 보내는 것»이다.

### 순서 — 앱 재배포를 **한 번**으로 모은다

- [x] **0단계 · 다섯을 정했다** (2026-09-02 · 원본은 [폰_상태바.md](docs/기획/폰_상태바.md) §2 뒤)
      · 작업 단계 **다섯 칸** — 대기 / 상세 / 팝업 N⧸3 / 판결 대기 / 안전취소 N초 (기사님 확정)
      · 모드 «대답»은 **값 그대로** — 버전은 본문이 클 때 아끼는 것이고 모드는 글자 하나다
      · 「적용중」은 **10초 뒤 버튼만** 풀고, 글자는 `적용중 · 마지막 통신 N초 전` 로 **사실만**

- [x] **1단계 · 관제웹만** (2026-09-02 · 앱은 한 줄도 안 고쳤다)
      · 6+9 배지 합치기 — 화면 꺼지면 배지가 통째로 `[💤 화면 꺼짐]`. 고르는 곳은
        `shared` 의 `deviceScreenBadge` 하나 (검사 6건). **끊김이 화면 꺼짐보다 위다**
      · 🔴 **3번 «조용한가» 는 절반만 됐다** (2026-09-05 발견 — ✅ 로 적어 뒀던 것을 정정).
        서버가 `prevSeen`(직전 보고)을 챙기는 것까지는 **맞다**(`devices.ts:182`).
        ~~그런데 **판정 함수 `isDeviceQuiet` 이 없다** — 주석만 그 이름을 가리킨다.~~
        ✅ **2026-09-05 만들었다** — `shared/src/index.ts` 의 `isDeviceQuiet` · 검사 6건 · 화면은 조용할 때만 `⏱️`.
        검사도, 화면도 없었다. 이 레포가 네 번 당한 «계획을 완료로 적는» 사고의 **다섯 번째**였다.
        - [x] `isDeviceQuiet(prevSeen, lastSeen)` — 기준 **30초** (생존신고 60초의 절반)
        - [x] 검사 먼저 → **빨간불 6건 확인** → 함수 → 초록불 (`shared/src/deviceQuiet.test.ts`)
        - [x] 화면은 **조용할 때만** `⏱️` (기사님 확정 2026-09-05 — 평소엔 안 그린다)
        - 🔴 **첫 보고를 «조용»으로 안 센다** — 모르는 것과 조용한 것은 다르다 (규칙 ④)
      · 🖥️ **화면이 바뀐 보고를 서버 로그에 한 줄** (기사님 지시) — 간격(초)이 함께 찍혀
        *"앱이 알아채는 데 얼마나 걸렸나"* 가 바로 읽힌다. **판정은 안 적고 사실만 적는다**

- [ ] **2단계 · 앱 (재배포 1회)** — 셋을 **한꺼번에**
      · 앱 버전 싣기 (`DeviceSession.version` 칸은 이미 있다)
      · 작업 단계 싣기 (0단계에서 정한 개수)
      · **모드 대답** 싣기 ← 딤드의 ④. 이것 없이는 3·4단계가 불가능하다

- [ ] **3단계 · 서버** — 받은 «대답»을 세션에 기록하고 소켓으로 흘린다
      🔴 **읽는 곳은 관제웹 버튼 하나로 못박는다** (규칙 ⑤-4 ⑤).
         서버 판정은 계속 **목표값**으로 돈다 — 나중에 «적용 안 된 폰은 콜을 안 준다» 같은 것이
         붙으면 한 값이 두 질문을 답하게 된다

- [ ] **4단계 · 관제웹** — 「적용중」 딤드 · 작업 단계 · 앱 버전 표시
      · 폰이 꺼져 있으면 딤드가 아니라 **«폰 꺼짐»** (이미 `status` 로 안다)

- [ ] **5단계 · 실측** — 폰에서 모드를 바꿔 «적용중»이 몇 초에 풀리는지.
      리스트에 있을 때와 홈에 있을 때를 **따로** 잰다 (오늘 실측대로 크게 다를 것이다)

> ⚠️ **각 단계마다 검사 먼저 → 빨간불 확인 → 구현** (CLAUDE.md 버그 수리 순서).
> 2·3·4 는 한 사슬이라 **2가 끝나기 전에 4를 그리지 않는다** — 그리면 또 «낙관적 업데이트»가 된다.

## ⏱️ 화면 값 반영이 정지 화면에서 늦다 — **해 봤고 되돌렸다** (2026-09-02)

기사님: *"이거 넘 오래 걸린다. 바뀌고 나서 1분 가까이 기다려야 하는 것 같아."*
→ *"먼발치에서 다시 생각해 봐. 이것이 가능한 건지부터."*

**시도 → 실패 → 되돌림.** 창 전환/«모름» 때 0.3·0.9·2초 뒤 화면을 다시 읽는 코드를 넣었으나
**18.3초 → 18.3초**, 전혀 안 나아졌다. 되돌리고 이유를 코드 주석에 남겼다
(`HijackService.onAccessibilityEvent` 의 «모름» 분기 · `TelemetryManager` 하트비트).

🔴 **왜 안 되나 — 원리다.** 안드로이드 접근성은 화면을 **캐시**하고 그 캐시는 **이벤트가 와야**
버려진다. 이벤트 없이 `rootInActiveWindow` 를 부르면 **아까 그 화면**을 준다.
세 번 다시 읽었는데 값이 한 번도 안 바뀌었다(`🔁 [다시 보기]` 로그 0줄).
**«내가 원할 때 다시 본다»는 것을 안드로이드가 보장하지 않는다.**

🟢 **그런데 피해가 작다.** 늦는 것은 **정지 화면뿐**이다 — 리스트·상세·팝업은 계속 움직여
이벤트가 쏟아져 **즉시** 반영된다(실측). 늦는 곳은 픽커 홈이고 거기는 일을 안 하는 시간이다.
서버가 화면 값을 실제로 쓰는 곳(`devices.ts` — «리스트로 돌아왔다» → 안 잡은 콜 정리)은
**안 늦는다.** 그래서 급하지 않다.

- [ ] **남은 길 셋** (하려면 이 중에서 고른다)
      · **A** 받는 이벤트를 더 늘린다 (`typeWindowsChanged` 등) — 될지는 **해 봐야 안다**.
        오늘 `typeWindowStateChanged` 로 한 번 빗나갔다(픽커 알럿은 진짜 창이 아니라 **덮개**였다)
      · **B** 접근성 캐시를 끈다 (`setCacheEnabled(false)` · Android 12+) —
        🔴 **인성 전체의 배터리·발열을 건다.** 얻는 것은 정지 화면 배지뿐이라 지금은 손해
      · **C** 생존신고 주기를 60초 → 15초 — 통신 4배. 단 **읽기도 함께 되게** 해야 의미가 있다
        (지금은 보내기만 하고 화면은 다시 못 읽는다 — B 없이는 C 도 반쪽이다)

## 🔴 «읽지 않고 단언한다» — 클래스로 잡았다 (2026-09-02)

하루에 **세 번** 같은 모양이 나왔다. 기사님이 세 번째를 보고 *"1, 2와 연관되어 그것도 같이
수정해야 하는 거 아님"* 이라고 물어 클래스로 묶었다.

| | 어디 | 무엇을 지어냈나 |
|---|---|---|
| ① | 붙는 순간 `updateScreenContext(LIST)` | 화면을 **읽지도 않고** «콜 리스트» |
| ② | `screenContextOf(HOME) = null` | 읽었는데 «모름»과 한 칸 |
| ③ | 부팅 배너 `"🎯 Keywords (인성콜)"` | 픽커로 돌아도 «인성콜» — **이 로그를 믿고 헛짚었다** |
...
## 🧩 배차망 통합 — **새 배차망을 붙이는 법**은 [docs/기획/배차망_통합.md](docs/기획/배차망_통합.md)

2026-09-02 전면 개정. 기사님 지시: *"다음에 또 어떤 앱을 추가하더라도 이것만 읽으면
추가할 수 있을 거니까."* 층 셋(페이지·치환·진리)·계약(`milestoneOf`)·함수 단위 실측·
체크리스트가 거기 있다. **⑤-4 의 ②(픽커 페이지 낱말)·③(치환 시점)이 비어 코드는 아직 시작 안 함.**

- [ ] **인성 전용 636줄을 인성 폴더로** — 옮기기만 하고 한 줄도 안 고친다.
      커밋을 «옮기기»와 «고치기»로 나눈다 (인성이 흔들리면 안 된다)
- [ ] **`ScreenDetector` 의 순서·`all`/`any` 를 배차망이 정하게** — 인성·24시가 함께
      흔들리니 **잡기를 시작하는 날**에
- [ ] **폴더 `plugins/` → `network/`** — import 33곳. 내용 이동이 끝난 뒤에.
      지금 `plugins/` 밑에 배차망이 아닌 것 둘(`RegionMatch`·`RouteOrderFilter`)이 있다

## 🔶 화물24시 파서 — 두 조각으로 그린 요금·거리를 읽는다 (2026-09-21 수리 · **폰 확인 남음**)

폰에서 `👁️ [리스트 스캔] 텍스트노드 51 · 콜그룹 0 · 통과 0` 이던 것.
시뮬레이터가 요금을 «200,000»+«원», 거리를 «14»+«Km» **두 조각**으로 그리는데
파서는 한 덩어리만 봤다. 배차망 셋 중 하나가 통째로 안 돌던 까닭이다.

- [x] 파서가 **둘 다** 받는다 — 실물이 어느 쪽인지 모르니 한쪽만 받으면 다른 쪽에서 같은 병이 난다
- [x] 카드 경계·요금·거리 규칙을 `Hwamul24CardGrouping` **한 곳**으로 (규칙 ③).
      따로 적으면 «카드는 묶었는데 요금이 0» 이 된다
- [x] «잔액 : 544,864 원» 을 카드로 오인하지 않는다 — 같은 줄의 낱말로 가른다 (높이 하드코딩 안 함)
- [x] 검사 11건 — `ScreenTextNode` 가 접근성 노드를 물어 못 부르던 것을 **순수 함수로 떼어** 잠갔다.
      빨간불 확인: 두 조각 규칙을 빼면 3건이 터진다
- [x] 🔴 **폰에서 확인했다** (앱 2.9.12-hwamul24 · 2026-09-21). 콜 6종이 요금·거리·차종까지
      제대로 올라간다. 확인하다 **두 가지가 더 나왔고 함께 고쳤다**:
      - 요금은 큰 글자, 단위는 작은 글자라 `top` 이 어긋난다 → «바로 다음 조각»이 아니라
        **«같은 줄에서 오른쪽»** 을 본다 («30,000» 뒤에 «계산서»·«독차»가 끼어든다)
      - **첫 카드가 머리 줄을 통째로 삼켰다** → 잔액이 요금으로 읽혀 «상차=최대, 요금=247947»
        이라는 가짜 콜이 올라갔다. 묶기 **전에** 머리 줄을 뺀다
- [ ] ❓ **실물 화물24시 앱이 한 조각인지 두 조각인지 아직 모른다** — 확인한 것은 시뮬레이터다.
      둘 다 받게 해 두었으니 어느 쪽이든 돌지만, 실물을 한 번 떠 두면 «무엇을 보는지»가 분명해진다

## ❓ 문제지 «7지점 한 바퀴»(`칠지점`) 5번의 정답이 문제지 안에서 어긋난다 (2026-09-14 · 기사님: 지금 안 정한다)

`onedal-sim/packages/core-simulator/src/presets.ts` — 5번 «이조갈비 → 이천터미널» 칸은 `expect: 'BLOCK'`
(집에 서 있으면 상차 19.1km > 반경 15km), 머리 주석은 *"정답: 알람(통과) 3번 — 01·03·05"*.
2026-09-14 세 판(04:26 · 13:04 · 13:21) 모두 **1·3번을 잡은 뒤 합짐으로 통과해 잡혔다** — 앱 동작은 세 판이 같다.

- [ ] 어느 쪽이 맞나 (기사님) — 통과가 맞으면 `expect` 를 `PASS` 로 · 차단이 맞으면 머리 주석을 고치고 원달앱 합짐 필터를 따로 본다

## 🔌 라이브 서버가 **4층**이다 — pm2 가 아는 pid ≠ 포트를 문 pid (2026-09-03 실측)

기사님: *"라이브 서버에는 고아가 된 포트가 없겠지?"* → **지금은 없다.** 리스너 하나뿐이다.
다만 구조가 고아를 부르는 모양이라 적어 둔다.

```
773(pm2)
 └ 150853  npm exec tsx src/index.ts      ← pm2 가 «이게 서버»라고 아는 것
    └ 150874  sh -c tsx src/index.ts
       └ 150875  tsx
          └ 150891  node … src/index.ts   ← 실제로 포트 4000 을 문 것
             └ 150904  esbuild
```

🔴 **pm2 는 맨 위만 안다.** 재시작은 150853 을 죽이는데 포트를 문 것은 **세 층 아래**다.
신호가 안 내려가면 손자가 살아남아 포트를 문 채 고아가 되고, 다음 기동이 `EADDRINUSE`
로 실패한다. 실측 당시 재시작 횟수가 **↺ 4** 였다 — 그때마다 이 위험을 지났다.

로컬에서 09-02 에 겪은 것과 **같은 계열**이다(그때는 `tsx watch` 2층 · 감시자가 죽어
자식만 남았다). 라이브는 `npm exec` → `sh` → `tsx` → `node` 라 더 깊다.

- [ ] `ecosystem.config.cjs` 가 **tsx 를 직접** 실행하게 바꾼다 (4층 → 2층).
      그러면 pm2 가 아는 pid 와 포트를 문 pid 가 같아져 재시작 신호가 곧장 닿는다
- [ ] 바꾸기 전에 **`EADDRINUSE` 로 죽었을 때 뭐가 남는지**를 한 번 재현해 본다 —
      지금은 «그럴 수 있다»는 추론이고 실측이 아니다
- [ ] 배포 스크립트에 «기동 전 4000 포트 주인 확인» 한 줄

> 🔴 **배포 경로를 건드리는 일이다.** 잘못되면 [#91](docs/기록/버그_대장.md)(라이브가 5시간 40분
> 죽어 있었다)이 재발한다. **주행 나가기 전에는 손대지 않는다** — 기사님이 책상에 계실 때 한다.

## 📸 폰이 읽은 화면 글자를 **남긴다** — 다음 페이지를 정의하려면 (기사님 지시 2026-09-03)

기사님: *"다음에는 잘 준비해서 그 폰에서 읽은 문서를 저장하도록 하자.. 그래야 다음 페이지를
정의하니까."*

**지금 남는 것은 상세 화면뿐이고, 그것도 서버로 보낼 때만이다.**

| 어디 | 지금 남는 것 |
|---|---|
| 서버 `intel.rawDetailText` | **상세 화면만** · 한 콜에 한 번 (09-02 픽커 68건) |
| 폰 `OneDalPrefs.xml` | API 마다 **마지막 한 벌**만 — 새 통신이 덮는다 |
| 폰 logcat | main 버퍼 5MiB = **약 3시간**. 앱은 파일로 안 남긴다 |

🔴 그래서 09-02 주행에서 **기사님이 실제로 수행한 콜의 원문이 없다** — 「픽업 완료 대기」·
「밀어서 픽업 완료」 같은 **새 페이지의 글자를 하나도 못 건졌다.** 화면 이름은 로그로 알지만
**그 화면에 무슨 글자가 있었는지는 모른다.** 다음 페이지를 정의할 재료가 없다는 뜻이다.

- [ ] **0단계 · 다섯을 먼저 정한다** (규칙 ⑤-4)
      · **무엇을 남기나** — 모든 화면인가, «모르는 화면»과 «새 단계»만인가
      · **어디에** — 폰 파일(`files/screens/YYYY-MM-DD.log`)인가 서버인가 **둘 다**인가
      · **언제** — 화면이 바뀔 때 1줄. 지문이 같으면 건너뛴다(지금 규칙 그대로)
      · **얼마나 오래** — 하루치? 사흘치? 폰 용량과 맞바꾼다
      · **누가 읽나** — 사람(파서 고칠 때) · 검사 문제지. **판정은 이걸 안 읽는다**
- [ ] 폰 파일로 남기기 — `AppLogger` 가 `Log.d` 만 부른다. 버퍼와 무관해지고 USB 로 뽑는다
- [ ] 서버로 «모르는 화면» 원문 올리기 — 자동 수집. 통신량을 재 보고 정한다
- [ ] 나가기 전 준비를 한 줄로 — `adb logcat -G 64M && adb logcat -c` ([필드테스트](docs/기록/필드테스트.md) 에)

> 🟢 손으로 뜨는 길은 이미 있다 — `log/카카오픽커/probe.sh` (`uiautomator dump`).
> 다음 주행 전에 **새 화면마다 한 번씩** 떠 두면 그 자체로 문제지가 된다.

## 🚗 픽커 실주행 — 1회차를 다녀왔다 (2026-09-02 · 기록은 [log/1dal-주행로그-20260902/](log/1dal-주행로그-20260902/읽어보세요.md))

**옛 계획(내일 판)** — 안내는 [docs/아카이브/2026-09-03_픽커_내일판.md](docs/아카이브/2026-09-03_픽커_내일판.md)

밤새 «상세를 열면 관제웹에 뜬다»를 넣었다(갈래 ⓑ · 앱 `2.7-picker-preview` 설치 완료).
**아침에 폰에서 셋**: 배차망을 «픽커»로 · 모드를 «알람»으로 · 바꾸고 1분 기다리기(최대 60초 지연).
🔴 수락 **뒤** 화면 낱말은 2023 자료로 만든 **추정**이다 — 캡처가 오면
`KakaoPickerKeywords.ACCEPTED_SCREEN_WORDS` 한 곳만 갈아끼운다.

- [ ] **`pnpm scenario` 가 고아 서버를 남긴다** (`scen.db` · PPID=1). 09-01 에 21시간짜리를
      치웠는데 09-02 에 또 생겼다 — **검사가 끝나며 자기 서버를 안 죽인다.** 되풀이되는 것이라
      인스턴스가 아니라 그 자리를 고쳐야 한다 (`scripts/scenario.mjs` 종료 처리)
- [x] **수락 후 화면 낱말 실물 대조** — 09-02 완주 캡처로 끝났다.
      「픽업 출발하기」는 **없어진 게 아니라 「수락하기」에 합쳐졌다**(기사님 확정) —
      그래서 **«0원 구간»이 없다.** 수락하기가 곧 계약이고 곧 출발이다
      (`ex_images/카카오픽커/README.md` · `docs/자료/노하우/카카오픽커/노하우_추출.md` §2 정정)
- [ ] 🔴 **수락했는데 «잡은 콜»로 승격이 안 된다** — 낱말은 맞았다(`픽업 완료 대기` 로 정확히
      찍혔다). 막은 것은 승격 자리의 조건 `previous == DETAIL_PRE_CONFIRM` 이다.
      18:18 실측: 상세 → **필수정보 팝업**(첫 계약 1회) → 픽업 완료 대기 순으로 갔고,
      팝업이 끼면서 «직전 화면이 상세»인 **한 번뿐인 기회**를 날렸다.
      → «미리보기가 있고(isPreview) 지금이 수락 후 화면인가»로 바꾼다. 안전장치는 그대로 둔다
- [ ] 🔴 **그 콜의 상세 원문이 안 남았다** — `sendPickerPreview` 첫 줄 `if (isDetailScrapSent) return`
      이 직전 콜의 표시를 물고 있었던 것으로 보인다. 정작 **수행한 콜만** 원문이 없다
- [ ] **`kotlin.Unit` 을 잡음 목록에** — 픽커 앱이 자기 화면에 그 글자를 내보낸다(그쪽 버그).
      원문 68건 중 **48건**에 상호 뒤에 붙어 파싱을 깨뜨린다.
      `server/config/keywords_picker.json` 의 `uiNoiseWords` 에 한 줄 — **앱 재설치 없이** 된다
- [ ] **리스트 카드 파싱** — 오더 68건 중 29건에서 상·하차지가 깨졌다
      (`34분 내 맘스터` · `픽업지 경기 광` · 상하차가 한 칸에 붙음). 원문이 있으니 폰 없이 고친다

### 📷 그림으로 읽기 — 폰 안에서 0.5초 안에 찍고 나누는 것까지 됐다 (2026-09-13 → 09-19)

기사님: *"주소를 추출할수 있느냐가 관건이야."* — **된다.** 그날 A24 실측으로 끝까지 뚫었다:

```
screencap → 가로540·JPEG60(29KB) → Vision OCR(428ms)
  경기 성남시 수정구 위례동 / 위례역푸르지오4단지아파트   픽업 16.9km  내일 15:00
  서울 강남구 삼성2동      / 더에스엠씨그룹             배송 10.1km  내일 16:40
→ 지오코딩 2/2 ✅ → 길찾기 도로 12.4km·19분 (직선 10.1 × 1.23 — 09-12 배수와 일치)
```

🔴 **접근성 트리로는 못 한다** — 같은 화면에서 **배송지가 한 글자도 안 온다**(같은 시각 앱 로그).
   거리도 화면에 없는 셋(14.4·16.9·19.3)이 섞이고, 건물명엔 `kotlin.Unit` 이 붙는다.

- [x] **줄 → 칸 함수와 검사** — `server/src/core/plugins/kakaopicker/pickerScreenOcr.ts`
      (`parsePickerDetailOcr`). 검사 10건이 **그날 OCR 출력 그대로**를 문다
      (`tests/core/pickerScreenOcr.test.ts` · 원자료 `ex_images/카카오픽커/실물_20260913_내가찍음/`).
      변이 5종으로 빨간불 확인함 (불릿·시각줄·예약·덩어리 경계·y 여유)
- [ ] 🔴 **부르는 곳이 없다 — `audit:dead` 가 못 보는 자리다** (`NaviQr` 와 같은 병).
      아래 둘이 이어지기 전까지 이 함수는 **검사에만** 산다. 이 줄을 지우지 말 것
- [x] **폰이 찍어 읽는다 — 서버로 안 보낸다** (2026-09-19 · 기사님 지시 «폰에서 0.5초 안에 사진 찍고 내용 파악까지»).
      접근성 설정 `canTakeScreenshot` + `takeScreenshot`(API 30↑) → 540폭 축소 → 앱에 번들한 한국어 인식(ML Kit)
      → `PickerScreenOcr.parseDetail`. `onedal-app/.../core/ScreenReader.kt` · `plugins/kakaopicker/PickerScreenOcr.kt`.
      **A24(SM-A245N · Helio G99 · 램 4GB · 안드로이드 16) 실측, 픽커 상세 화면:**

      | 단계 | ms |
      |---|---|
      | 찍기 | 30~73 |
      | 변환(하드웨어 버퍼 → 비트맵 → 540폭) | 54~69 |
      | 인식 · 전체 화면 (35줄) | 225~308 |
      | 인식 · 아래 60% 만 (14줄) | 167~214 |
      | 나누기 | 1~13 |
      | **합계** | **전체 337~448 · 잘라서 약 290** |

      예열은 서비스가 붙을 때 한 번 265ms. 지도 위 지명(송파·판교·도립공원)은 주소로 안 샜다.
      캡처 차단(FLAG_SECURE)은 픽커 홈·상세 둘 다 없었다. 시스템 제한 «0.33초에 한 장»은 상세 한 장에 한 번이라 안 걸린다.
      🔴 **서버 OCR 엔진 고르기는 없어졌다** — 읽는 자리가 폰이라 EC2 리눅스 엔진이 필요 없다.
- [x] **줄 → 칸이 두 벌이 됐다 — 일부러** (서버 `pickerScreenOcr.ts` ↔ 앱 `PickerScreenOcr.kt`).
      까닭은 코드 주석에: 폰이 0.5초 안에 나눠야 해서 서버 왕복을 못 한다.
      두 검사가 **같은 문제지 둘**(09-13 위례→삼성2동 · 09-19 광남1동→남한산성면)을 문다 — 한쪽만 고치면 갈린다.
      - [x] 루트 `CLAUDE.md` 「짝이 있는 것」 표에 한 줄 (2026-09-19 등록 완료)
- [x] **오늘 콜의 시각 줄 「10:00까지 픽업」** — 09-13 문제지는 예약 콜뿐이라 «내일 15:00» 꼴만 알았다.
      09-19 실측에서 이 줄이 **건물명 자리에 들어가고 시각은 null** 이었다 → 두 벌에 문제지 넣어 빨간불 → 같은 규칙(`DEADLINE_RE`)으로 초록.
- [x] 📸 **콜 흐름에 연동 완료 (1·3·7단계)** — `PreConfirmSequence.kt`에서 픽커 상세 진입 시 자동으로 찍어 기억 카드(`alarmTappedCard`)와 대조.
      일치 시 정상 진행, 불일치 시 뒤로가기 + 이상 징후 보고 (`anomalyExecutor` 전송줄 분리).
      수동으로 연 상세(`alarmTappedCard == null`)는 OCR 판독 결과를 유일한 원천으로 삼아 콜 살림(버그 대장 #119 수호).
- [x] **이상 징후 보고 라우트·저장 표 (1단계 완료)** — `POST/GET /api/telemetry/anomalies`, SQLite `telemetry_anomalies` 표 및 인덱스 신설 완료.
- [ ] 관제 화면 현황판 카드 (2단계 — 관제 UI 필요 시점 연동)
- [ ] 🔴 **«예약 콜»이라는 축을 그날 처음 봤다** — 화면의 「내일 15:00 픽업예약」.
      접근성 트리에 이 줄이 없어 **내일 콜을 지금 콜로 알고** 판정하고 있었다.
      시각을 모르면 시급도 상차버퍼도 전부 틀린다 — 새 축이라 규칙 ⑤-4 의 다섯을 먼저 정한다

## ⬜ 낱말 — «여유» 와 «닻» (2026-09-02 · 기사님이 «못 알아듣겠다»고 지적)

기사님: *"닻, 버팀목, 여유(무슨 여유인지 시간인지 공간인지 알 수 없음), 얼어붙은 경로 —
내가 이해할 수가 없어."*

- [x] **CLAUDE.md ⑤-5 절**을 용어집 말로 고침 — `상차지 여유→상차버퍼` · `하차지 여유→경유버퍼`,
      «둘 다 분이다»를 명시. 기사님 지시로 **문서만** 고쳤다
- [ ] **코드·로그의 «여유» 는 그대로 둔다** (기사님 결정 2026-09-02). 지금 상태:
      · 용어집은 «여유»를 **이미 폐기어**로 적어 뒀다(2026-08-17 · 상차버퍼·경유버퍼로 갈랐다)
      · 그런데 `glossary.test.ts` 의 `BANNED` 에 **«여유»가 없어** 코드 18곳이 초록불로 산다
      · 한 화면에 `[여유] -28분`(시간)과 `공간 여유 70%`(적재)가 나란히 찍힌다 — **세 번째 뜻**
      → 고치는 날 `BANNED` 에 넣고 상차버퍼·경유버퍼·적재공간 셋으로 가른다
- [ ] **곁가지: 금지 패턴이 대문자를 못 잡는다** — `[Dd]eath[Vv]alley` 라 앱의
      `1DAL_DEATHVALLEY` 태그·화면 이름이 통과한다. `/i` 한 글자면 메워진다
- [x] **«닻» 은 안 쓴다** (기사님 확정 2026-09-02: *"카카오한테 경로를 마지막으로 물어본
      시각이란 좋은 말이 있는데 왜 닻을 쓰는거야.. 그냥 쓰지마"*).
      → 설명할 때는 **«경로를 마지막으로 잰 시각»** 으로 풀어 쓴다
- [ ] **로그 두 줄도 고칠지 — 기사님 결정 대기.** `닻` 이 나가는 자리는
      `helpers.ts:260`(경로 순서)·`socketHandlers.ts:618`(약속 진단) 둘이고 나머지 6곳은 주석이다.
      같은 줄의 **«홀더»** 도 같은 문제 — 그 줄은 통째로 개발자용으로 쓰여 있다.
      제안: `닻 0시 47분 26초` → `0시 47분 26초에 잰 경로` · `홀더` → `기준 콜`
- **내가 만든 말은 안 쓴다** — «버팀목»(=순서가 뒤집히지 않게 둔 20% 폭 `STOP_ORDER_HYSTERESIS`)·
  «얼어붙은 경로»(=그때 저장해 둔 정거장 순서와 주행분)는 코드에 없는 **내 은유**였다.
  목록으로 막지 않는다 — 목록에 없는 새 은유를 또 만들 뿐이다. **풀어 쓴다**

## ⬜ 경로 순서와 누적 주행분이 어긋난 채 나간다 (2026-09-02 · 폰 한 바퀴에서 발견)

`🧭 [경로 순서]` 는 재료 둘을 합쳐 찍는데 **시계가 다르다** —
순서는 `planArrivalStops(현위치)` 로 **매 GPS 틱** 다시 계산하고, 누적 주행분은
경로 잰 시각에 **얼어붙은** `sectionDriveMin` 을 **이름으로** 붙인다.
그래서 순서가 바뀌어도 숫자는 안 따라가고, 배열이 자기모순이 된다:

```
00:48:39.874  ⑴ 23분 ⑵ 25분 ⑶ 30분 ⑷ 15분   ← 누적인데 단조 증가가 아니다
00:48:41.876  ⑴ 15분 ⑵ 23분 ⑶ 25분 ⑷ 30분   ← 2초 뒤 스스로 복구
```

증상: 관제웹의 여유(=경유버퍼)가 **같은 정거장에서 -28 ↔ -18 을 왕복**했다
(순서가 찍힌 뒤 50ms 안에 따라 움직였다 — 세 번 다).

🔴 **순서가 바뀌는 것 자체는 버그가 아니다** — `STOP_ORDER_HYSTERESIS`(20%)는 #90 을
고치며 기사님이 승인한 설계고, 얼렸다가 «도착이 아예 안 찍히는» 사고를 이미 겪었다.
막을 것은 **어긋난 배열이 밖으로 나가는 것**이다. 지금 검사는 [helpers.ts:255]
`길이 어긋남` 뿐이라 **개수만** 본다 — 이번엔 4개로 같아서 안 걸렸다.

- [ ] 검사 먼저: *"`routeStops` 의 `driveMinutes` 는 순서를 따라 단조 증가한다 — 아니면 전부 `null`"*
      문제지는 위 `23·25·30·15` 를 그대로 쓴다. **빨간불 확인 후** 수정 (버그 대장 규칙)

## 🚗 **2026-09-03 실주행에서 나온 기사님 관찰 13** — 화면·내비 (그날 밤 받아 적음)

> 기사님이 문제지 3콜(초월역DT → 성남 → 안양 → 가산 → 구로 → 강서)을 **실제로 몰고 돌아온 뒤**
> 그 자리에서 적어 주신 것이다. **주행 중에만 보이는 것들**이라 값이 크다.
> 기록 원본: [log/1dal-주행로그-20260903/](log/1dal-주행로그-20260903/읽어보세요.md)
>
> 🔴 **아래 인용은 기사님 말씀 그대로다. 다듬지 않는다.** 그 밑의 «— 내 주석»만 내 것이다.
> 정리하다 뜻이 바뀐 적이 이 레포에 여러 번 있었다.

### ⬜ 낱말 — «무대» 를 «지도» 로 (기사님 지적 2026-09-04)

기사님: *"지도란 말이 있었는데.. 왜 무대라고 비유를 했을까? 이전 «무대»란 말을
«지도»로 정확히 명시하면 **파생되는 단어를 줄일 수 있을** 것 같아."*

**맞다. 은유 하나가 낱말 네댓을 만들었다.**

```
지도  →  무대(v23 비유)  →  자막 · 무대 시트 · 무대 규칙 · 엿보기 줄 · 무대 지도 …
```

지도를 지도라고 불렀으면 «자막»이 나올 이유가 없었다. 영화가 연상되니 그 밑줄이
자연히 «자막»이 됐고, 나는 그것을 기사님께 **넷씩 다른 이름**으로 말하고 있었다
(2026-09-04 에 그래서 «버튼»을 «이름표»로 알아듣고 기능을 흘렸다 · 095d091).

**크기** — 지금 손대면 목업 작업과 섞여 위험하다. 별도 판으로 잡는다.
```
코드    26곳 · 파일 10개
식별자  36곳 (Stage · StageView · StageSheet · stageRules · stagePreview …)
폴더    components/stage/
문서    용어집 · wireframe-v23 · v24 · todo
```
⚠️ 식별자는 영문만 쓰므로(규칙) `Stage*` → `Map*` 같은 개명이다 — **컴파일이 잡아 주는**
   부류라 위험이 낮다. 다만 한 번에 해야 하고, 도중에 다른 작업과 겹치면 충돌이 크다.
👉 화면 개편이 한 매듭 지어진 뒤에 **하루 판으로** 잡는다. 그때 `glossary.test.ts` 에
   «무대» 를 금지어로 넣어 되돌아오는 것을 막는다.

### 00. 🔴🔴 **색이 틀렸다** — 기사님 기억이 가장 큰 것을 짚었다

**00-1** *"3콜이 끝나고 나서 랜덤 콜을 돌면서 갔는데.. 초월을 지난 시점에서도 초월에서
출발하는 콜이 자꾸 올라왔고, 정확히 기억없는데 좋은 점수를 받았던거 같아."*

— 내 주석 —
🔴 **기억이 정확했다.** 15:00~16:16 에 초월·곤지암 상차 콜이 **20건** 올라왔고
(그때 기사님은 성남·안양), 잡힌 셋의 판정이 이랬다.

```
초월읍 → 구로중앙로  67,000원 · 🟢 보통 53점
  돈 100점 · 약속 0점(최소 -81분) · 공간 10점(적재 공간 10%) · 성질 100점
```

**오늘 판정 25건 중 «약속 0점»이 10건. 그 10건이 전부 «가도 된다» 색이었다** (🟡 똥 0건).

| 시각 | 색 | 점수 | 돈 | 약속 | 공간 |
|---|---|---|---|---|---|
| 14:24·14:34·14:49 | 🟢 보통 | 60 | 100 | **0** | 40 |
| 15:01 | 🟢 보통 | 45 | 71 | **0** | 10 |
| 15:10·15:11·17:02 | 🟢 보통 | 53 | 100 | **0** | 10 |
| 19:01 | 🟢 보통 | 62 | 87 | **0** | 60 |
| 19:30·19:36 | 🔴 사고 | 69·75 | 78·100 | **0** | 99 |

🔴 **병은 «평균»이다.** 약속 0점은 «점수가 낮다»가 아니라 **«불가능하다»** 인데,
지금은 돈 100점이 그걸 덮는다. CLAUDE.md 규칙 ⑤-3 —
*"색을 틀리는 것이 이 시스템의 가장 큰 사고다."* **정확히 그 자리다.**

👉 **어느 축이 «불가능»을 말하면 평균에 넣지 말고 색을 잠근다.**
⚠️ **버리라는 뜻이 아니다** (규칙 ①: 콜의 주인은 기사님이다) —
   **색으로 정직하게 말하고** 결재는 기사님이 하신다.
⚠️ 「공간 10%」(거의 다 찬 차)도 같은 자리다. 이것도 함께 볼 것.
⚠️ 🔴 **다섯을 먼저 정한다** (규칙 ⑤-4) — «불가능»의 기준·색·화면 표시·읽는 곳.
   **코드부터 짜지 않는다.**

### 00-B. 🔴 **기사님 질문 — «복귀는 첫짐과 다른 판단을 해야 하나»** → **네. 그리고 그 축이 꺼져 있다**

**00-B-1** *"집으로 올 때 엄청 막히는 길의 첫짐을 고른 것이 문제일 수 있겠다.
복귀는 현위치 + 최종하차지를 포함한 경로란 걸 알고 있으니 첫짐과는 다른 판단을 해야 하는 걸까?"*

— 내 주석 —
🟢 **질문이 정확하다. 그리고 답할 축이 이미 코드에 있다.**

```
criteria.ts:229   GEOGRAPHY  key:'geography'  name:'지리'
                  asks: "가는 길 위에 있나"
                  measure: onDetourPath ? 100 : 0
```

🔴 **그런데 오늘 25건 전부 «— 안 봄 (가중치 0)» 이었다.**

```
user_judgment.weight_geography = 0.0
```

코드 주석(criteria.ts:227)에 이유가 적혀 있다 — *"아직 없다. 근거가 생기면 기사님이
판정 기준 탭에서 켜시면 된다."* **오늘이 그 근거다.**

**두 국면의 질문이 실제로 다르다.**

| | 첫짐 (STANDBY) | 복귀 (HOME) |
|---|---|---|
| 도착지 | **자유** — 콜이 나를 데려간다 | **정해져 있다** (집) |
| 물어야 할 것 | «이 시간 써서 얼마 버나» | «**집 가는 길에 얼마나 얹히나**» |
| 지리 축 | 잴 «길»이 없다 → 자연히 못 잼 | **정확히 이 축의 질문** |

이건 CLAUDE.md 규칙 ⑤-1 이 이미 말한 것이다 —
*"서버 판정의 일은 «이 콜이 돈이 되는가»가 아니라 «지금 경로에 붙일 때 얼마나 좋은가»."*

🟢 **«막힘»도 이미 자료에 있다.** 카카오 길찾기는 `apis-navi.kakaomobility.com` 이라
**실시간 교통이 반영된 소요시간**을 준다. 「집 도착이 몇 분 늦어지나」로 재면
막히는 길은 **자동으로 낮은 점수**가 된다 — 새 자료가 필요 없다.

👉 **그러니 할 일은 «새 판단을 만드는 것»이 아니라 «있는 축을 켜는 것»이다.**
⚠️ 켜기 전에 다섯을 정한다 (규칙 ⑤-4) — 특히
   ① **가중치를 얼마로** (지금 0.0 → ?) ② **국면마다 다르게 둘 것인가**
   ③ **`onDetourPath` 가 복귀에서 «집 가는 길»을 기준으로 계산되는지 확인** (미확인)
   ④ 화면에 어떻게 보일지 ⑤ 이 값을 누가 또 읽는지

### 0-G. 🔴🔴 **폰에서 못 읽겠다** — «진짜 중요한 것만» (기사님 2026-09-03 실주행 뒤)

**0-G-1** *"S23 에 관제화면을 넣으니까 보기가 너무 어렵더라.. 진짜 중요한 것만 보여야 하는데..
불필요한 것들이 많아 보이고.."*

— 내 주석 —
🔴 **이건 «다듬기»가 아니라 이 제품의 전제가 처음으로 실물에서 시험받은 것이다.**
관제웹은 그동안 **책상 브라우저**에서 만들어졌다. 오늘 처음으로 **6.4인치 · 운전석 ·
먼발치 1~2초**라는 진짜 조건에 들어갔고, **거기서 안 읽혔다.**

이미 아는 제약과 정확히 맞물린다 —
- 「운전 중에는 입력을 못 한다」 — 먼발치 1~2초에 읽혀야 하고, 무입력에도 일이 되어야 한다
- 규칙 ⑤-3 — *"기사님은 **색만 보고 1~2초 안에 누른다.** 설명 문구는 그다음이다."*

🔴 **덜어내려면 잣대가 필요하다. 「불필요」는 사람마다 다르다.**
   내가 보기에 잣대는 하나다 — **«지금 이 순간 기사님이 답해야 하는 질문이 무엇인가».**
   그 질문에 답하지 않는 것은 **화면에서 내린다**(지우는 게 아니라 한 단 아래로).

| 국면 | 그 순간의 **한 질문** | 화면에 남을 것 |
|---|---|---|
| 첫짐 탐색 | «잡을까 말까» | **색** · 요금 · 어디서 어디로 |
| 합짐 수집 | «이거 얹을까» | **색** · 몇 분 늘어나나 · 남은 상차버퍼 |
| 운행 중 | «어디로 가고 있나 / 순서가 맞나» | **다음 정거장** · 지도 · 남은 곳 수 |
| 도착 | «찍었나» | 그 정거장 · 단계 하나 |

⚠️ **덜기 전에 «지금 무엇이 올라가 있나»를 한 장으로 펼쳐야 한다.**
   코드 grep 으로는 못 센다(주석까지 잡힌다). **실물 화면 캡처**가 필요하다 —
   국면마다 한 장씩(첫짐·합짐·운행·도착). 그 위에 기사님이 **«이건 빼»** 를 찍으시면 된다.
   👉 다음에 폰을 켜실 때 그 넷을 찍어 주시면 그걸로 목록을 만든다.

⚠️ 이건 **§관제웹 전면 개편(v23 3단 시트)** 의 5단계 «기사님 비교 운행»이 답을 낸 것이기도
   하다. 비교 운행이 **처음으로 실주행에서 돌았고, 결과가 «안 읽힌다»** 였다.
   그러니 v23 명세를 **이 결과로 한 번 접고 가야 한다** — 없던 정보가 생겼다.

⚠️ 🔴 **여기서 코드부터 짜지 않는다** (규칙 ⑤-4 ④ «화면»).
   무엇을 남길지 기사님과 맞춘 뒤에 손댄다.

### 0-E. 🔴🔴 **배터리** — 마지막 하차지에서 20% 남았다

**0-E-1** *"마지막 하차지에 도착하니 배터리가 20%밖에 남지 않았어. 그건 문제야."*

— 내 주석 —
🔴 **이건 «불편»이 아니라 «오늘 일을 못 끝낼 수도 있다»는 문제다.**
관제폰이 죽으면 궤적·도착·지나침이 **전부 멈춘다.** 오늘 주행은 약 4시간 20분이었다
(14:30 출발 → 18:51 마지막 하차). 하루 7콜을 목표로 하면 **더 길어진다.**

원인이 §6-2 에 이미 잡혀 있다 —
```
관제앱이 화면이 켜진 동안 초당 101회 화면을 다시 그린다
(CapacitorWebView.onDraw · 로그 380,805줄 중 204,334줄이 그 한 줄)
관제웹에 끝없이 도는 CSS 애니메이션이 23곳 — 하나라도 화면에 있으면 안 멈춘다
```
그리고 **주행 중엔 KeepAwake 로 화면이 계속 켜져 있다.** 즉 **내내 이 상태다.**

👉 손댈 곳 셋, 큰 것부터 —
   ① **끝없는 애니메이션 23곳** — «연결됨» 표시등처럼 늘 뛰는 것부터. 상태가 바뀔 때만 잠깐 뛰게
   ② **지도 다시 그리기** — 좌표가 안 바뀌면 안 그린다 (GPS 는 3~5초에 한 번인데 100회 그린다)
   ③ **화면 밝기·KeepAwake** — 지도 화면에서만 켤 것인가

⚠️ **재는 법이 먼저다** (규칙 ⑤-4). *"고쳤다"* 를 말하려면 **같은 조건에서 시간당 몇 %**를
   비교해야 한다. `S23_관제앱_logcat_전체.log.gz` 의 `BatteryDump` 줄이 그 자료다 —
   오늘 것이 있으니 **비교 기준선이 이미 있다.**

### 0-F. ⚪ 「마포 출발」 랜덤 문제지 — **설정은 지켜졌다** (기사님 확인 요청 건)

**0-F-1** *"문제는 마포 출발로 하고 랜덤으로 문제 생성하는 걸 받아왔어.
내 위치에서 가까운 걸 찾은 거 같은데.. 거기는 문제가 있을 수 있겠어. 확인 필요."*

— 내 주석 —
🟢 **검산했다. 시뮬레이터는 마포를 제대로 썼다.**

```
후보 원점              15km 안 상차지     평균 거리
마포구                 66/91 (73%)        11.5km    ← 가장 잘 맞는다
18:51 실제 위치(강서)   49/91 (54%)        15.8km
집(초월)                8/91 ( 9%)        30.1km
```

⚠️ **다만 이름이 오해를 부른다.** 「마포 출발」은 **「마포에서 출발하는 콜」이 아니라
「마포 반경 15km 안에서 출발하는 콜」**이다 (`generateSimCall`: `maxPickupKm` 기본 15,
`Math.pow(random, 2)` 로 가까울수록 자주). 그래서 마포는 **8%** 뿐이고
가산·신도림·구로가 더 많았다. 화면 낱말을 고치는 게 맞다.

### 0-C. 🔴🔴 **클래스 하나** — «얼거나 튄 좌표»를 사실로 읽는 자리가 **넷**이다

기사님: *"이동 중이었는데.. 정차 중이라고 나온 부분이 있었어."*

증거가 궤적에 있다 —
```
17:41~17:47   126.8133,37.5735 에 좌표가 6분간 얼어 있다 (15~16초 간격)
              두 점이 같으니 속도 0 → isMoving = currentSpeed > 5 → «정차 중»
              (VehicleStatusPanel.tsx:117)
```

**오늘 하루에 같은 병이 네 자리에서 나왔다.**

| # | 어디 | 얼거나 튄 좌표를 무엇으로 읽었나 | 결과 |
|---|---|---|---|
| 1 | 정차 배지 | 속도 0 = 정차 | **이동 중인데 «정차 중»** |
| 2 | 도착 감지 | 얼어붙은 자리 = 도착 | 17:15 잘못된 도착 |
| 3 | 지나침 | 따라잡기 = 멀어짐 | 17:15 오발화 |
| 4 | 궤적 거리 | 튄 점 = 주행 | **68% 부풀림** |

🔴 **CLAUDE.md: «같은 클래스가 두 번 나오면 인스턴스가 아니라 그 클래스를 없애는 수정을 한다.»**
   **네 번 나왔다.** 그러므로 자리마다 따로 고치지 않는다.

👉 **한 곳에서 «이 좌표를 믿을 수 있는가»를 판정하고, 넷이 그걸 읽는다.**
   재료는 셋 — ① **정확도**(관제웹이 이미 보낸다·서버가 버린다) ② **직전 점과의 시간 간격**
   ③ **직전 점과의 거리/속도**. ②③은 **지금 자료로 당장 된다.**
   ⚠️ 규칙 ⑤-4 대로 다섯을 먼저 정한다 — 특히 **⑤ «읽는 곳»**: 넷이 각자 다른 질문을 답한다.
   «못 믿는 좌표»가 정차 배지엔 «모름», 도착 감지엔 «판정 보류»로 갈릴 수 있다.

### 0-D. ⬜ 마지막 구간의 헤맴 · 복귀는 카카오내비로 집만 찍고 왔다

**0-D-1** *"마지막즈음에 길을 잘못 들어서 헤멘 부분이 있었어. 그래서 빙 둘러가고 유턴하고."*
**0-D-2** *"돌아올 때는 그 위치에서 카카오내비를 켜고 그냥 집을 찍고 오느라 경로대로 오지 못했어."*
**0-D-3** *"카카오맵의 내비게이션이 너무 별로였어."*

— 내 주석 —
⚠️ **0-D-2 는 자료를 읽을 때 꼭 알아야 한다** — 복귀 구간(18:51~20:29) 궤적은
**우리 경로가 아니다.** 그 구간으로 경로 판정을 평가하면 안 된다.
🔴 **0-D-3 이 A-3 을 굳힌다** — 기사님이 실주행 중 **스스로 카카오내비로 갈아타셨다.**
   우리가 만든 링크가 실전에서 버려진 것이다. **카카오내비 조사를 앞으로 당긴다.**

### 0. 🔴 **터널** — 기사님 증언이 오늘 GPS 문제의 원인을 밝혔다

**0-1** *"오늘 긴 터널이 주행중에 많았어."*

— 내 주석 —
🟢 **이 한마디로 진단이 끝났다.** 서부간선지하도로(약 10km) 구간 궤적을 보면
좌표가 **9~10분 동안 얼어붙고**, 나오는 순간 **8.2km 를 한 번에 따라잡는다.**
그러니까 «위치 점프»라는 이름이 **두 가지를 한 이름으로 부르고 있었다.**

| | 무엇 | 어떻게 가르나 |
|---|---|---|
| **거짓 좌표** | 기지국 추정 (17:11:37 의 24.7km 왕복) | **정확도**가 수백~수천 m |
| **따라잡기** | 터널을 나오며 되찾은 **진짜** 좌표 | **시간 간격**이 크다 (9~10분) |

🔴 **지나침이 그 «따라잡기»에 발화했다** (17:15:58). 오늘은 결과가 맞았지만,
**터널이 상차지 «앞»에 있으면 반대가 된다** — 얼어붙은 좌표로 «도착»을 찍고
나오면서 «지나쳤다»고 **완료까지** 찍는다. **싣지도 않은 콜이 「실었다」가 된다.**

👉 그래서 §1(지나침이 점프를 거르게)은 **두 조건**이어야 한다 —
   ① 정확도가 나쁜 점을 안 믿는다 ② **시간이 크게 벌어진 뒤 첫 점**을 안 믿는다.
   ②는 **지금 자료로도 당장 된다**(`at_ms` 간격). ①은 정확도 저장이 먼저다.

🔴 **그런데 터널은 «양쪽»으로 틀린다** (기사님 확인 2026-09-03):
   *"구로 진일텍푸라… 거기가 지하차도 안이었던 것 같아."*

| | 어디 | 무슨 일 | 결과 |
|---|---|---|---|
| 오발화 | 17:15 가산 | 터널을 **나오며** 따라잡음 | 지나침이 찍혔다 |
| **불발** | 구로 진일텍푸라 | **지하차도로 지나쳐 버렸다** | 아무것도 안 걸림 — 손으로 찍으심 |

🔴 **둘째는 «정거장이 터널 안»이 아니다** — 기사님 정정:
*"카카오 경로가 거기를 잘 찍었으면 지하차도로 안내하지 않았을 거야..
중간 어딘가쯤에서 실시간 경로를 바꾸었을 가능성이 있어."*

궤적이 뒷받침한다 — **17:43 에 기사님은 구로가 아니라 강서(126.813, 37.5735)에 계셨다.**
구로는 이미 지나친 뒤였고 그 자리에서 손으로 정리하셨다.
👉 **이건 §A-1(재탐색)과 같은 사건이다.** 카카오가 길을 바꿔 **우리 정거장을 안 들르는 길**로
   몰았다. **터널이 문제가 아니라 «순서를 깨는 재탐색»을 아무도 안 막은 것이 문제다.**

⚠️ 그래도 «진짜 터널 안 정거장»은 따로 있을 수 있다 — 그때는 GPS 로 영영 못 잡는다.
   **손으로 찍는 길이 늘 열려 있어야 한다**(지금 열려 있고 오늘 실제로 쓰셨다).

🟡 **그리고 «하차지 앞 주차»가 새 축이다** (기사님 확인):
   *"도착하고 나서 찍지 못하고 거기 앞에 역 공영주차장으로 안내받아 거기에 주차하느라."*
   강서 개화 — 도착 17:43(자동) / 완료 18:51(직접), **1시간 8분**.
   지금 규칙(300m→400m)은 **주차하러 벗어나는 것**과 **지나쳐 가는 것**을 못 가른다.
   👉 **주차장이 400m 밖이면 내리기도 전에 「내렸다」가 찍힌다.** 기사님과 확정 필요.

⚪ 17:54~18:51 **57분 공백은 결함이 아니었다** — *"상가에 가는 겸이어서 빈소 방문했었어."*

⚠️ 그리고 **터널은 없어지지 않는다** — 기사님 노선에 늘 있다.
   «터널 안에서는 판정을 멈춘다»가 제품 규칙이 되어야 할 수도 있다. 기사님 확인 필요.

### A. 내비 — 카카오맵으로는 못 쓴다

**A-1** *"경로가 실시간으로 바뀌어 처음 경로가 실 내비게이션에서 바뀐다."*

**A-1 상세** (기사님이 그날 밤 풀어 주신 실제 사건) —
*"너가 준 경로의 내비 링크를 가지고 가다가 길을 잘못 들었어. 처음 찍었던 경로에 사고가
있었어. 그래서 중간에 경로가 바뀌어 안내했고, 난 바뀐 경로로 이동하다 정해진 경로가
아닌 것을 확인하고 성남시청 그쪽에서 두 바퀴를 돌면서 판단해야 했어 — 처음 길로 가야
하나, 새로운 길로 가야 하나. 그러다 2바퀴 돌고 나서 «오늘은 그냥 처음 정한 경로로 가자»
해서 다시 링크를 클릭해서 내비 세팅을 다시 하였는데, 그 전쯤에 사고가 있었는지 다시
돌아가는 길을 추천했고, 무시하고 처음 정해진 길로 갔어. 그래서 공사중에 걸려 오랜
차막힘을 겪었어."*
**A-2** *"전체 경로를 주어도 카카오맵이 처음 경로만 만들고 경유지를 표시해 주지 않는다."*
**A-3** *"카카오맵의 주행이 너무 불친절해서 못쓰겠다 카카오네비를 이용할수 있어야 하겠다."*
**A-4** *"출발할때 시작점을 찍지 않고 출발지에서 바로 두번째 상차지로 안내했어
그래서 내가 첫 상차지 들렸다 갔다."*

— 내 주석 —

🔴 **A-1 은 «불편»이 아니라 이 제품의 빈 자리다.** 궤적에 두 바퀴가 그대로 남아 있다.

```
15:32:14 → 15:38:34   같은 지점을 6분 뒤 다시 지남
15:33:41 → 15:39:55   127.114~127.128, 37.4217  (성남 여수동·시청 일대)
```

무슨 일이 있었나 —
**① 우리는 «정거장 순서»를 정하고, 카카오는 «어느 길로 가는가»를 정한다.**
   사고로 카카오가 재탐색하자 **두 결정이 갈라졌고**, 기사님은 «지금 이 안내가
   내 경로를 깨는 것인가»를 **판단할 근거가 화면에 없었다.**
**② 그래서 길에서 두 바퀴를 돌며 사람이 대신 판단했다.** 6분을 썼다.
**③ 결국 처음 경로를 고수했고 공사 구간에 걸려 더 오래 막혔다** — 판단의 대가까지 치렀다.

👉 **필요한 것은 «다른 길로 가고 있다»가 아니라 «그래도 순서는 지켜진다/깨진다»다.**
   관제웹은 현위치를 안다. 카카오가 어디로 몰든, **다음 정거장에 가까워지고 있는가**만
   말해 주면 기사님은 안 세워도 된다. 새 값이 아니라 **있는 값으로 답할 수 있는 질문**이다.

🔴 **그리고 그 헤맴 한가운데서 지나침이 발화했다 — 15:33:12.**
   성남 택시쉼터를 **싣지 않고 스쳐 지나가며** 「상차 도착·완료」가 자동으로 찍혔다.
   오늘은 문제지라 손해가 없었지만, **실콜이면 싣지도 않은 콜이 「실었다」가 된다.**
   👉 §0 의 «지나침을 못 믿는 자리» 목록에 **«길을 헤매는 중»**이 하나 더 붙는다.

🔴 **A-4 는 사고다.** 링크가 «첫 정거장»을 **경유지가 아니라 출발지로** 넣었을 가능성이 높다.
`buildKakaoRouteUrl` 은 `sp`=현위치, `vp…`=중간, `ep`=마지막으로 만드는데, 기사님은
**현위치가 이미 첫 상차지 근처**였다. 카카오가 «출발지=1번 정거장»으로 붙여 버리면
안내는 2번부터 시작한다 — 그러면 **첫 상차지를 안 들르는 경로**가 나온다.
👉 첫 정거장이 현위치와 가까울 때 그것을 `sp` 로 삼키지 않도록 **직접 확인해야 한다.**
   지금 `naviLink.test.ts` 는 «몇 개를 어디에 넣나»만 보고 **이 경우를 안 본다.**

⚠️ A-1·A-2 는 **카카오맵의 성질**이지 우리 버그가 아닐 수 있다 — 딥링크로 넘긴 경유지를
   카카오맵이 «한 번 계산하고 끝»으로 다루는 것으로 보인다. 그래도 **결과가 못 쓰겠다면
   방식을 바꾸는 게 맞다** (A-3).

👉 **A-3 이 방향 전환이다 — 🟢 실측으로 확정 (2026-09-04, 기사님 폰).**

🔴 **기획서로 옮겼다 — [docs/기획/경로.md](docs/기획/경로.md) §4** (⑤-4 다섯을 거기서 채운다).
   아래는 실측으로 알아낸 «값»만 남긴다. **코드는 아직 없다.**

**SDK 를 안 깔아도 된다. URL 하나면 된다.** SDK 도 속을 보면 URL 을 만들어 여는 것뿐이다.
...
🔴 **경유지 칸 이름은 `via_list` 다 — `viaPoints` 는 조용히 무시된다.**
   문서 어디에도 없어서 **두 벌을 만들어 폰에서 눌러 보고** 갈랐다 (2026-09-04):

| 보낸 이름 | 결과 |
|---|---|
| **`via_list`** | ✅ 「경유 1」 마커가 찍힌다 · **45km / 1시간 42분** |
| `viaPoints` | ❌ 마커 없음 · 44km / 1시간 44분 — **경로가 그냥 무시됐다** |

   `coord_type` 도 밑줄이니 앞뒤가 맞는다 — `viaPoints` 는 **JS SDK 가 자바스크립트
   쪽에서만 쓰는 이름**이고 SDK 가 안에서 `via_list` 로 바꿔 보내는 것이었다.
   👉 **틀린 이름을 보내도 에러가 안 난다.** 경유지만 조용히 사라진다 — 검사로 잠글 자리다.

| | 카카오맵 (지금) | 카카오내비 (갈아탈 곳) |
|---|---|---|
| 여는 법 | `kakaomap://route?…` | `kakaonavi-sdk://navigate?…` — **둘 다 URL 하나** |
| 경유지 | 5개 (`vp`~`vp5`) | **3개** (`via_list`) |
| 좌표 | 위도,경도 텍스트 | JSON `x`=경도 `y`=위도 — **우리 DB 차례 그대로** (뒤집지 않는다) |
| 이름 | 필요 없음 | 🔴 **`name` 필수** — 좌표만으로 안 된다 |
| 차종 | 없음 | `vehicleType` 1~7 — **1t 카고를 뭘로 할지 기사님과 확인** |
| 출발지 | `sp` | 생략하면 **현위치**에서 시작 (실측 확인) |
| 준비물 | 없음 | **JS 앱 키 + `1dal.altari.com` 도메인 등록** |

🔴 **`coord_type` 기본값이 `katec` 이다.** 안 넣으면 조용히 엉뚱한 데로 간다.
🔴 **도메인 등록은 선택이 아니다** — 콘솔이 *"등록하지 않으면 JavaScript 키를 사용할 수
   없습니다"* 라고 못박는다. 2026-09-04 에 `https://1dal.altari.com` 을 등록했다.
🟢 **아이폰에서 열렸다** — 데브톡의 «iOS 사파리에서 안 열린다»는 2017년(iOS 10 · 내비 3.6.4
   미만) 이야기였고 지금은 «앱을 여시겠습니까» 알림 뒤 정상으로 열린다.
🟢 **카카오맵에 없던 것이 보인다** — 실시간 교통 · **대안경로**(1시간 54분/54km) · 내비추천.

⚠️ **경유지가 5 → 3 으로 준다** — 한 번에 «경유 3 + 도착 1 = 4곳». 도착할 때마다
   `/navi` 가 남은 경로를 다시 주므로 치명적이진 않지만, 이어 보내는 횟수가 는다.
⚠️ 미설치 시 동작은 문서에 없다 — 기사님 폰엔 깔려 있으므로 실측으로 확인.
👉 **남은 것** — ①은 끝났다.
| | | |
|---|---|---|
| ~~① JS 키·도메인 등록~~ | ✅ | 2026-09-04 완료 |
| ~~② 경유지 칸 이름~~ | ✅ | **`via_list`** — 실측으로 확정 |
| **③ 주행 중에도 지키나** | ⬜ | 🔴 **이게 진짜 시험이다.** 지금까지 본 것은 «경로가 만들어진다»까지다. 09-03 의 불만은 주행 중이었다 — 경유지로 안내하나 · 지나면 넘어가나 · **재탐색 뒤에도 순서를 지키나**(카카오맵이 무너진 자리) |
| ④ `vehicleType` 확정 | ⬜ | 1t 카고가 1~7 중 무엇인가 |
| ⑤ `/navi` 버튼 교체 | ⬜ | ③이 좋게 나온 뒤에 |

⚠️ **③ 전에는 코드를 안 고친다.** 여기서 «별로다»가 나오면 ④⑤ 가 통째로 무의미해진다.

### B. 지도 — 줌과 «지금 어디»

**B-1** *"줌이 한계가 있어서 불편했어 여기가 어디인지 보고 싶은데 볼수가 없었어."*
**B-2** *"네비와 전체 경로가 같이 보이니까 그건 좋았는데. 지금 가고 있는곳만 볼수 있음 좋겠어.
상황판에 나온 경로만 줌으로 보여주는 거지."*
**B-3** *"지도에서 네비처럼 현위치가 가운데 있는 옵션도 있어야 할꺼 같아."*
**B-4** *"지도 줌 하려니 이상하게 작동해."*

**B-4 상세** (여쭤서 받은 답) —
*"손가락 중간을 기준점으로 줌인이 될 거라 생각했는데.. **한쪽 방향으로 치우쳐서 줌인**되었어."*

**B-5** *"줌인이 되면 지도의 색이 투명해도 될 거 같아. **지도를 보겠다는 의지가 있었던 거니까.**"*
**B-6** *"줌, 혹은 지도가 그리는 면적이 크지 않으면 지도가 그냥 **딤드 없는 그냥 지도**로
표현되어도 좋겠어. 거기에는 좋은 정보들이 표현되는 거니까."*

— 내 주석 —
🟢 **B 는 우리 지도(자작 줌·팬 + OSM 타일)의 일이라 우리가 다 고칠 수 있다.**
- **B-1** 줌 상한이 걸려 있다. «여기가 어디인가»를 보려면 **동네 글씨가 읽히는 배율**까지
  들어가야 한다 — 지금 상한이 그보다 얕다. 숫자를 정해 풀면 된다.
- **B-2** 지금은 **전체 경로에 맞춰** 화면을 잡는다. 필요한 것은 **«지금 구간»에 맞추는 모드**다.
  ⚠️ 둘 다 필요하다 — 전체를 보는 게 좋았다고 하셨다. **모드를 가르는 것**이지 바꾸는 게 아니다.
- **B-3** «현위치 중앙 고정»(내비 모드). B-2 와 **같은 축**이다 —
  «무엇에 화면을 맞추나: 전체 / 지금 구간 / 현위치» **셋 중 하나**로 묶는 게 맞아 보인다.
- ✅ **B-4 — 수리 완료** (2026-09-04 · 버그 대장 #96). 확대 갈래가 **둘**이었고
  09-01 수리가 `zoomAround` 만 지나 **핀치가 옛 식으로 남아 있었다.**
  `pinchStep()` 하나로 합쳐 휠·버튼과 같은 공식을 쓰게 했다. 검사 10건.
  곁가지로 배율을 «거리의 비»로 — 이제 두 배 벌리면 두 배다.

- (진단 기록) **B-4 — 답을 받았다. 재현 없이도 진단이 선다.**
  **핀치 줌의 기준점이 «두 손가락 중간»이 아니다.** 09-01 에 «확대·축소가 기준점을
  화면 원점으로 알던 것»을 고쳤는데, 그건 **버튼 줌**이었을 가능성이 높다.
  핀치는 **두 손가락 중점(midpoint)** 을 화면 좌표로 잡아 그 점이 제자리에 남도록 평행이동해야
  한다. 지금은 그 중점이 아니라 다른 점(화면 중앙 또는 원점)을 쓰고 있어 **한쪽으로 쏠린다.**
  👉 `mapProjection.test.ts` 에 «핀치 중점이 제자리에 남는가» 검사를 **먼저** 만들어
     빨간불을 본다 (CLAUDE.md 버그 수정 순서).

- **B-5·B-6 — 딤(어둡게)을 «의지»에 반응시킨다.** 기사님 말이 규칙을 준다:
  *"지도를 보겠다는 의지가 있었던 거니까."*
  즉 **딤은 «지금 지도를 안 본다»는 가정**이고, 줌인은 그 가정을 깬다.
  👉 축이 둘로 보인다 — ① **줌 배율이 올라가면** 딤을 걷는다
     ② **지도가 그리는 면적이 작으면**(요약 줄·작은 카드) 애초에 딤 없이 그냥 지도로 둔다.
  ⚠️ 딤은 그 위에 얹은 글씨·마커의 **가독성**을 위해 넣은 것이다. 걷으면 그 글씨가 묻힌다 —
     **딤을 걷는 대신 글씨 쪽에 그림자·테두리를 주는** 손질이 함께 가야 한다.
     👉 이건 «어떻게 보일지»가 먼저다 (규칙 ⑤-4 ④) — **화면부터 기사님과 맞춘다.**

### C. 시트 — 지금 무엇을 가리키는지 모르겠다

**C-1** *"현재 시트가 어디를 가리키고 있는지 모르겠어. 거기가 플리킹(스와이프) 되고 있는것
같은데.. 아코디언이 좋을것 같기도 하고. 그건 고민이 필요한것 같아."*

**C-2** (그날 밤 캡처 셋과 함께 주신 결론) —
*"올라오는 시트에 진행중, 완료됨.. 그 라인은 거의 필요 없는 것 같아. 그건 어디 따로 봐야
할 것 같아. 시트에 콜리스트 3개 아래로 관련된 스텝이 보이고 있는데.. 그러니까 뭘 보고 있는지
어려워. **아코디언으로 만들고, 아코디언 헤더는 무조건 화면에 노출하고, 컨텐츠 영역에
스크롤할 수 있게** 하는 것이 어떨까?"*

— 내 주석 —
🟢 **기사님이 «고민 필요»를 스스로 푸셨다 — 아코디언으로 확정. C-1 은 닫힌다.**
캡처(2026-09-03 22:20 · S23 실물)를 보면 지적이 그대로 보인다 —

| 캡처에서 보인 것 | 기사님 판정 |
|---|---|
| 탭 줄 «진행중(3) · 완료됨(3) · 취소(11) · 방출(9) · 전체(26)» | **주행 중엔 거의 안 쓴다 — 딴 데로 뺀다** |
| 콜 목록 3줄 밑에 26번 콜의 스텝(통화·단위·수량…)이 이어짐 | **지금 뭘 보고 있는지 모르겠다** |

아코디언 규칙(기사님 안 그대로) —
- **헤더(콜 한 줄)는 늘 화면에 있다** — 접혀도 «무엇이 있는지»는 보인다
- **펼친 콜 하나의 내용만 스크롤** — 스크롤이 콜 경계를 넘지 않으니 «주어»가 안 헷갈린다
- 탭 줄(완료됨·취소·방출)은 시트에서 **뺀다** — 어디로 옮길지는 기사님과 (설정? 별도 화면?)

👉 이것이 **0-G(진짜 중요한 것만)의 첫 구체안**이다. v23 시트 문법을 이걸로 갱신한다.
⚠️ 캡처 셋은 실물 증거다 — 22:21 캡처엔 «사고 75» 색·버퍼 줄·경로 최소 -42분까지 담겨 있어
   00(색)·화면 정리 논의 때 다시 쓴다.

### D. 「지금 출발」 버튼

**D-1** *"출발하고 한참되었는데.. 내가 지금 출발을 누르지 않았어서 계속 큰 영역에
지금 출발하면 늦는다는 그 영역이 나와 있었어."*
**D-2** *"그건 기획적으로 내가 누르는것이 맞는것 같은데. 일단 그 버튼이 지도 상단에 있어야 할꺼 같아."*

— 내 주석 —
🟢 **D-2 에서 기사님이 규칙을 확정하셨다 — «출발은 사람이 누른다».** 자동으로 넘기지 않는다.
🔴 문제는 **안 누르면 큰 경고가 화면을 계속 덮는 것**이다. 오늘 아침 배너 10초 자동 닫힘을
넣었는데 **이 영역은 그 배너가 아니다**(무대 안 큰 영역). 같은 병이 다른 자리에 있다.
👉 ① 버튼을 **지도 상단**으로 (손이 닿는 자리) · ② 경고가 화면을 덮지 않게.
   실측: 16:14 에 누르셨는데 **14:51 부터 1시간 23분** 그 화면이 떠 있었다.

### E. KEEP 할 때 «어떻게 바뀌는지»가 안 보인다

**E-1** *"킵 하려했는데.. 기존 경로가 보여지고 그것이 어떻게 바뀐다가 보여야 할꺼 같아..
지금은 새로운 경유지 2곳만 점선으로 표시되고 있어."*

— 내 주석 —
🔴 이게 **규칙 ⑤-3(색이 곧 결정)과 같은 자리**다. 기사님은 1~2초에 판단하시는데,
지금 화면은 «새 것 둘»만 보여 주고 **«그래서 전체가 어떻게 되나»** 를 안 보여 준다.
👉 필요한 것은 **전/후 비교** — 지금 경로(실선)와 붙였을 때의 경로(점선)를 **같이**,
   그리고 **얼마나 늘어나나**(+km/+분)를 숫자로. 그건 서버가 이미 계산한다(우회 비용).
   **새 값을 만드는 게 아니라 있는 값을 화면에 올리는 일**이다.

### F. 관제앱에 새로고침

**F-1** *"관제앱에 새로고침 버튼이 있어야 하겠어 실시간 반영되도록."*
**F-1 상세** (여쭤서 받은 답) — *"새로고침은 **지도의 경로** 이야기였어."*

— 내 주석 —
🔴 **내 첫 짐작(소켓 끊김)이 틀렸다.** 기사님이 말한 것은 **지도에 그려진 경로가
현실을 안 따라오는 것**이다. 화면 전체를 다시 부르자는 게 아니다.

그러면 §A-1(재탐색)과 **같은 자리**다 — 카카오가 길을 바꿨거나 기사님이 다른 길로 갔을 때
**지도의 경로선이 옛것 그대로** 남는다. 그래서 «다시 그려 달라»가 필요했던 것이다.
👉 손댈 곳은 **«경로를 언제 다시 계산하나»** 이지 새로고침 버튼이 아니다.
   ⚠️ 다만 **버튼도 있는 편이 낫다** — 자동이 놓쳤을 때 기사님이 쥘 것이 있어야 한다.
   그 버튼은 **지도만** 다시 그린다(화면 전체 새로고침이 아니다 — 그건 소켓·상태를 날린다).

⚠️ 소켓 재연결 10회(18:51~20:01)는 **별개 사실로 남겨 둔다.** 기사님이 겪은 것과는
   다른 일이지만 그 자체로 봐야 한다.

---

### 우선순위 — 내가 보는 순서 (기사님 확인 필요)

| | 무엇 | 왜 먼저인가 |
|---|---|---|
| 00-B | **지리 축을 켠다 (가중치 0.0)** | 🔴 «가는 길 위에 있나»를 **아무도 안 묻고 있었다.** 오늘이 켤 근거 |
| 00 | **«불가능» 축이 색을 잠그게** | 🔴🔴 오늘 **10건이 틀린 색**으로 나갔다. 규칙 ⑤-3 «가장 큰 사고» |
| 0-G | **폰에서 «진짜 중요한 것만»** | 🔴🔴 실물 6.4인치에서 **안 읽혔다.** 제품 전제의 첫 실전 시험 |
| 0-E | **배터리 — 4시간에 80% 썼다** | 🔴🔴 관제폰이 죽으면 **모든 판정이 멈춘다.** 하루 7콜이 목표다 |
| 0-C | **«못 믿을 좌표» 판정을 한 곳으로** | 🔴🔴 같은 병이 **네 자리**. 클래스로 없앤다 |
| 0 | **터널에서 지나침이 오발화** | 🔴 **싣지도 않은 콜에 「실었다」가 찍힐 수 있다.** 오늘은 운이 좋았다 |
| 1 | **A-4 첫 상차지 건너뜀** | 화면 쪽 유일한 «잘못 간다» 사고. 나머지는 불편이다 |
| 2 | **D 「지금 출발」 버튼 자리 + 경고 안 덮기** | 1시간 23분 화면을 덮었다 |
| 3 | **F-② 소켓 끊김** | 화면이 조용히 낡는다 — 관제의 근본 |
| 4 | **B 지도 3모드(전체·지금 구간·현위치)** | 한 축이라 같이 짠다 |
| 5 | **E KEEP 전/후 비교** | 있는 값을 올리는 일 |
| **2** | **A-3 카카오내비로 갈아타기** | 🔴 기사님 재확인: *«이게 크게 중요한 것 같아. 길을 잘못 들어 일을 망칠 수 있어»* — 실주행에서 이미 손으로 갈아타셨다. 조사부터, 코드는 그다음 |
| 7 | **F-① 새로고침 버튼** | 3 을 고치면 덜 급해진다 |
| 3 | **C 시트를 아코디언으로** | 🟢 기사님이 안까지 주셨다 — 헤더 상시 노출 · 내용만 스크롤 · 탭 줄 제거 |
| — | **B-4 줌 이상** | 🔴 **재현 방법을 여쭤야 시작한다** |

🔴 **묻어 둔 질문 둘** — 다음에 여쭐 것
- B-4 「지도 줌이 이상하다」는 **어떻게** 이상했나 (안 커진다 / 튄다 / 손 떼면 돌아온다)
- ~~A-1 「경로가 실시간으로 바뀐다」~~ → **답 나옴**: 카카오맵이 **사고 때문에 재탐색**한 것.
  우리 링크는 그대로였다. 문제는 재탐색 자체가 아니라 **«그래도 순서는 지켜지나»를 아무도 안 알려준 것**


### 0-I. 🛣️ **경로를 고른다** — 고속도로냐 국도냐는 전략의 선택 (기사님 2026-09-04)

기사님: *"첫콜을 잡을 때 경로가 계산될 거야. 고속도로면 **가는 길에 콜이 떠도 한참을
돌아서** 가야 하고 통행료도 내지만 **시간은 단축**할 수 있어 — 대기시간이 늘어나
**비싼 콜을 잡을 수 있는 장점**이 있는 거지. 국도는 반대고. 첫콜을 잡고 나면 경로가
확정되니 **이후 필터 방향이 바뀌어야 할지도 몰라.** 그럼 **콜을 수락하기 전에** 옵션을
선택할 수 있었으면 좋겠어."*

🔴 **기획서로 옮겼다 — [docs/기획/경로.md](docs/기획/경로.md) §3**
   («통행료를 안 읽는다»와 «죽은 설정 셋»도 **여기 흡수했다** — 통행료는 이 선택의
   한 축이지 따로 할 일이 아니다. 기사님: *"나중에 두 번 작업 안 하려면"*). **코드 없음.**

**실측 (09-03 구간 · car_type=1)** — 고속도로가 **30분을 1,900원에 판다**:

| | 거리 | 시간 | 통행료 |
|---|---|---|---|
| 추천 | 47.5km | **104분** | 1,900원 |
| 무료도로 | 51.9km | 132분 | **0원** |
| 국도(`avoid=motorway`) | 54.6km | **134분** | 0원 |

🟢 **대안경로도 API 로 온다** — `alternatives=true` 로 routes 2개.
   `kakaoService.ts` 가 **`routes[0]` 만 읽고 있었다.**
🟢 **통행료도 이미 온다** — `summary.fare.toll`. 그 값이 카카오내비 앱 화면과 같았다.
   ⚠️ `orders.tollFare`(배차망 「탁송료」)와 **다른 값**이다. 섞지 않는다.
🔴 **설정 셋이 죽어 있다** — `avoid_toll`·`car_hipass`·`car_fuel` 이 URL 에 안 실린다.
   **「유료도로 회피」를 켜도 아무 일이 안 일어난다.**
🔴 **진짜 손해는 「잡아 놓고 취소하는 것」** — 콜 필터의 경유 반경이 **직선**이라
   고속도로에서 「2km 옆」이 실제로는 22분이다. 그렇게 태우는 **취소 횟수는 하루 10회뿐**이다.

✅ **선택지 확정 (기사님 2026-09-04)** — **추천 · 무료도로 · 고속도로우선.**
   §1 의 전략 세 극점과 맞는다. 내비 `rpOption` 은 각각 `100` · `2` · `6`.
   🟡 「고속도로우선」만 우리 쪽 대응이 없어 `priority=TIME` 으로 근사한다 —
      09-03 여덟 구간 중 **일곱은 추천과 같았고**, 갈린 하나(방화동→집)에서
      **13.6km 더 멀고 통행료 두 배인데 6분 빠른** 길을 골랐다. 정확히 고속도로우선의 행동이다.
✅ ~~`rpOption` 조사~~ — 표를 찾았다(100 추천 · 1 빠른길 · 2 무료 · 3 최단 · 4 자동차전용제외 ·
   5 큰길 · 6 고속도로 · 8 일반도로). ⚠️ 내가 *"큰길우선은 대응이 없다"* 고 한 건 **틀렸다.**
⬜ **남은 것** — ②콜 필터의 경유 반경을 **분**으로 잴 것인가 (0-H 와 같은 재료를 쓰는 큰 판)

🔴 **값이 실시간으로 움직인다** — 같은 구간을 30분 뒤에 물으니 47.5km/104분/1,900원 →
   **49.0km/114분/3,800원.** ① 비교표는 **한 번에** 불러야 하고 ② 0-H 가 생각보다 심각하다
   (**거리와 통행료까지** 늙는다).

### 0-H. ⏱️ **남은 주행 분이 늙는다** (기사님 물음 2026-09-04)

`StopLead.driveMinutes` 는 «카카오 호출 시점부터의 누적»이라 30분 달린 뒤에도
같은 수를 말한다. 시트 상태바가 그 값을 그대로 적는다.

🔴 **기획서로 옮겼다 — [docs/기획/경로.md](docs/기획/경로.md) §5**
기사님(2026-09-04): *"계획이 아직 완료 상태가 아닌데 코드를 수정하면 레거시가
계속 생길 것 같아."* ⑤-4 다섯을 그 문서에서 채운다. **코드는 아직 없다.**

한 줄 요약 — **이탈은 «값을 지울 이유»가 아니라 «값을 새로 받을 신호»다.**
화면에 «경로 이탈» 상태가 아예 없고, 숫자 하나가 서서히 나빠졌다가 맞춰진다.

**기사님께 여쭐 것 셋** (기획서 §9)
- ① 재호출을 **자동**으로 할까, **눌러서**(F-① 새로고침) 할까 — 내 안은 자동 + 버튼은 «지금 당장»
- ② **1판**(관제웹 뺄셈만)을 먼저 태울까, 2판(서버 재호출)까지 같이 갈까
- ③ 카카오 쿼터 확인 — 하루 34→51회가 되는데 한도를 모른다 (내가 확인)

## 🔴 관제웹 전면 개편 — «지도 배경 + 3단 시트» (기사님 확정 2026-08-31 · 진행 중)

설계 원본: [docs/기획/화면개편/](docs/기획/화면개편/) — v21(시트 문법) · v22(경우별) ·
**v23(부품 정의서·상태 규칙 표 = 명세)** · **v24(목표 포함관계)**.
방식(기사님 확정): **토글 병행** — 새 무대를 «새 화면 미리보기» 토글 뒤에 다 만들고,
기사님 비교 운행(v23 Ⅲ표 체크리스트) 통과 시 기본값 전환 + 옛 UI 일괄 삭제.
파생은 1단계에서 한 벌로 통일 — 병행 기간에도 계산 두 벌 없음.

- [x] 0. 체크포인트 푸시 (377a736)
- [ ] 1. `useRouteDerivations` 파생 훅 추출 — 옛 화면도 이걸 쓰게 · 화면 변화 0
- [ ] 2. 토글 + `Stage`(지도 배경+정거장 오버레이) + `StageSheet`(3단 그릇, 기존 콜덱 담기)
- [ ] 3. `useStageRules` — v23 Ⅲ표 코드화
- [ ] 4. 연결 — S4 후보 미리보기 · S5 킵→통화 · S6 정거장 탭 · S7 도착 마중
- [ ] 5. 기사님 비교 운행 → 통과 시 기본값 전환
- [x] 5-a. ☰ **왼쪽 서랍** — 끝난 콜(완료됨·취소·방출)이 살 자리를 먼저 만들었다 (기사님 확정).
      새 화면에는 옛 탭 줄이 없고 현황판은 폭 1018px 이상에서만 떠서, 지우면 **폰에서 볼 곳이 없어진다**.
      거르는 규칙은 `lib/finishedCalls` 한 벌(옛 탭도 같은 것을 부른다) · 🌓 화면 밝기도 여기로 옮겼다
      (헤더 로고 오탭) · 목업 `/mockup/drawer` · 검사 `finishedCallsSingleSource` 10건 · 화면규칙 D1~D10
- [x] 6. 옛 UI 일괄 삭제 — **끝났다** (기사님 «거기 작업 끝났어 확인하고 진행해»).
      토글(`stagePreview`)이 코드에서 사라졌고 새 화면이 기본이다. `PinnedRoute` 는 437→173줄
      (겉껍데기·`sheetOnly` 갈래·머리 줄·미니맵·탭 줄·종료 콜 목록을 걷었다) · `Dashboard` 519→483줄.
      옛 화면을 조준하던 규칙 검사 **11건은 지우지 않고 새 화면으로 조준을 옮겼다** —
      지웠으면 그 규칙이 조용히 사라진다.
      - [x] 💰 «진행 중 운임»을 헤더로 옮겼다 (기사님 확정) — `confirmedCalls` 로만 더한다
      - [ ] 🚗 **`MovingBadge` 를 현황판에 잇는다** — «시뮬/실측 이동 중 42km/h». 옛 지도에만 있던
            표시라 갈 자리가 없어졌다. 모의 주행이 도는지 눈으로 가리는 유일한 표시다 (`audit:dead` KEEP)
      - [ ] 🪗 **`CallDeck` 의 스와이프 갈래가 쓰는 곳이 없다** — 옛 화면 전용이었다.
            부르는 곳은 시트 하나이고 늘 `accordion` 이다. 지우면 654줄 중 상당수가 준다.
            🔴 지우기 전에 «폰에서 스와이프로 넘기고 싶다»가 아직 유효한지 기사님께 여쭌다
      - [ ] 🔴 **관제에서 정산으로 가는 길이 없다** — 새 화면은 아래 네비 바를 안 그린다.
            정산은 운행일지(logbook)가 받기로 했으니 그때 정리한다. 정산 화면에서는 네비 바가 떠서 돌아올 수 있다
- [x] (후속) **지도 배경을 실제 지도로** — 2026-09-01. 카카오가 아니라 **OSM 타일**로 갔다.
      세 안(A 지금 · B 타일 그대로 · C 회색조·연하게)을 같은 데이터로 나란히 그려 보고
      기사님이 **C 채택**. 카카오 JS 키·도메인 등록·Capacitor 오리진이 전부 필요 없고,
      **지금의 자작 줌·팬을 그대로 쓴다** — 지도한테 빌리는 것은 «어느 동네 어느 도로인가»뿐이다.
      바꾼 것은 투영 하나(선형 → 메르카토르, 한국 위도에서 세로가 1.25배 어긋난다).
      마커·경로선·발자취·이름표·탭 판정은 **한 줄도 안 고쳤다** — 전부 `toScreenPoint` 를 지난다.
      곁가지로 확대·축소가 기준점을 화면 원점으로 알던 것도 고쳤다(여백만큼 흘렀다).
      `mapProjection.test.ts` 11건 · 변이 둘로 빨간불 확인.


## ⬜ 라이브 전환 전 — git 이력에서 DB 백업 뜯어내기 (2026-08-30 신설)

과거 add -A 로 백업 102MB(콜 기록 포함)가 이력·GitHub 에 들어갔다. 현재 목록에서는
뺐고(.gitignore 잠금) 이력에만 남았다. 라이브 데이터가 담기기 전에 filter-repo 로
과거 페이지에서도 제거 — 이력 재작성 + force push 라 배포·클론 조율이 필요해 별도 판.

## 🔶 픽커 수집 — 플랫폼 통합의 첫 실측 (기사님 확정 2026-08-30 · 설계 중)

본질은 **플랫폼 앱 통합**이다. 카카오T픽커를 셋째 배차망으로: 알람+수집만(잡지 않음),
스캐너 폰에서 픽커를 띄우고, 수집은 전부. 설계·인성 대조·남은 결정은
[docs/지금/픽커_수집.md](docs/지금/픽커_수집.md) — §5 의 ①③④(추가 스키마·시점·화면)와
알람 조건이 차야 코드 시작. 1단계는 타겟앱 매핑 네 곳 → 한 곳 통합 공사.
...
### 🔴 ⬜ 모드를 바꿔도 앱은 «다음 통신»까지 모른다 — 그 사이 자동으로 잡는다

**2026-09-02 알람 판 실측.** 기사님이 알람으로 바꾸셨는데 앱이 **14.5초 뒤 자동으로 콜을 잡았고**,
리스트로 돌아오자 서버가 강제 정리하며 **취소 카운트가 1 올랐다**(10회 제한에 들어가는 그것).

```
01:17:31.351  서버  ⚙️ [모드 전환] 앱폰 → ALARM
01:17:45.846  앱    💥 [AUTO] 꿀콜 조건 통과! 강제 터치 진행!        ← 14.5초 뒤, 아직 AUTO
01:17:46.282  앱    📤 확정 전송 — 모드: AUTO (매크로클릭: true)
01:17:46.989  서버  🔔 [알람] … 기사님이 직접 누르십니다             ← 이미 잡은 뒤에 울렸다
01:17:47      앱    수신된 모드: ALARM                              ← 15.6초 만에 알았다
01:18:01.024  서버  📈 취소 카운트 +1 (reason: FORCE_CANCEL)
```

**왜 늦나** — 모드는 앱이 서버에 말을 걸 때 **응답 꼬리(피기백)** 로만 간다. 그날 실측 공백은
**21초**였고(`01:17:25.807` → `01:17:46.986`), 화면이 안 바뀌면 **생존신고 주기인 60초**까지 벌어진다.

🔴 **한 콜에 «자동»과 «알람»이 겹쳤다** — 앱은 자기가 눌렀고 서버는 «기사님이 직접 누르십니다»를
울렸다. 한 값(기기 모드)을 둘이 **서로 다른 시점의 것으로** 읽은 것이다 (규칙 ⑤-4 ⑤).

⚠️ 1단계는 관제웹 알람만 만들었고 **앱은 한 줄도 안 고쳤다**. 그래서 «앱이 모드 전환을 언제
아느냐»가 통째로 비어 있다 — 새 결함이라기보다 **1단계에서 안 다룬 자리**다.

**기사님 방향 (2026-09-02): UI/UX 로 푼다** — 프로토콜을 바꾸는 게 아니라,
*"알람을 선택하면 **60초 후에 반영된다**"* 를 **화면이 말하게 한다.**
> 기사님: *"이건 ui / ux 로 풀어야 하겠다. 알람을 선택하면 60초후에 반영된다는걸 알려야 할듯
> 한데.. 이건 고민해보고"* — **아직 고민 중이다. 확정 전이므로 코드를 시작하지 않는다** (규칙 ⑤-4).

🔴 **세 모드(자동·알람·대기) 전부 같다** (기사님 지적 2026-09-02). 늦는 이유가 모드가 아니라
**전달 방식**이라, 어느 쪽으로 바꾸든 최대 60초 늦는다.

**다만 위험은 대칭이 아니다** — 화면이 어느 쪽을 더 세게 말해야 하는지가 여기서 갈린다:

| 바꾸는 방향 | 그 60초 동안 앱은 | 손해 |
|---|---|---|
| **자동 → 알람·대기** | **계속 자동으로 잡는다** | 🔴 **취소 카운트 +1** (10회 제한을 깎는다) · 실측된 사고 |
| 알람·대기 → 자동 | 계속 안 잡는다 | 🟡 그동안 뜬 꿀콜을 **놓친다** (되돌릴 수 없지만 페널티는 없다) |

- [ ] 화면 문구·모양 확정 (기사님) → 그 뒤에 코드
- [ ] 정할 것: 남은 시간을 **세어 보여줄지**(«알람 반영까지 42초») 아니면 문장 하나로 끝낼지 ·
      반영이 **끝난 순간**을 어떻게 알릴지 · 그 사이 앱이 잡아 버린 콜을 화면이 뭐라고 말할지 ·
      **자동에서 나가는 쪽만 더 세게 경고할지**(위 표 — 그쪽만 페널티가 붙는다)
...
**계획서: [docs/아카이브/2026-08-29_판정_한_함수.md](docs/아카이브/2026-08-29_판정_한_함수.md)** — 일곱 단계·되돌리는 법까지.
...
6. [ ] 라이브 로그인 → `user_filters` 한 행이 `FILTER_FIELDS` 기본값으로 시드되는지

라이브 후 해야 할 일 (기사님):
7. [ ] 설정 재입력 — 차종·집 주소·경로 옵션 / 요율표·수수료·블랙리스트 / 🔍 필터 한 벌 / 판정 기준
...
- [x] **A. 링크 형식 검증** — ⓑ **URL 로 생성 가능하다** (카카오 공식 문서 2026-09-03 확인)
      ```
      kakaomap://route?sp=출발위도,출발경도&vp=경유1&vp2=…&ep=도착위도,도착경도&by=car
      ```
      · 경유지 **최대 5개** (`vp`·`vp2`~`vp5`) · **좌표만으로 된다**(이름 불필요)
      · **iOS 도 같은 스킴**이고 사파리에서 눌러도 앱이 열린다 (미설치면 앱스토어로)
      · 🔴 차례가 «위도,경도» — 우리 DB 는 `x=경도·y=위도` 라 **뒤집어** 넣는다 (`naviLink.ts` 한 곳)
- [x] **B. 보내는 방법 — 관제웹의 `/navi` 한 장** (기사님 «가장 간단한걸로 하자»)
      🔴 **카톡은 못 쓴다** — 폰 한 대에 한 계정이라 «나에게 보내기»는 관제폰에서만 보인다.
         기획에 적힌 «카카오 로그인 연동» 보다 **앞에 이 벽이 있었다.**
      🔴 **관제앱을 개인 폰에 까는 것도 안 된다** — 두 폰이 좌표를 보내면 한 차량으로 섞여
         「위치 점프」가 찍히고 도착·지나침 판정이 흔들린다. 게다가 개인 폰이 **아이폰**이다.
      🟢 그래서 **아이폰 사파리로 `https://1dal.altari.com/navi`** 를 연다 — 설치 0.
         위치는 **읽되 보내지 않는다**(`useGpsTelemetry(!naviOnly)` · `naviGpsOff.test.ts` 가 잠금)
- [ ] ⚠️ **남은 것: 알림이 없다** — 사파리를 열어 둔 채여야 하고, 카카오맵에서 **돌아오는
      손짓**이 필요하다. 운전 중에 그게 번거로우면 **문자(SMS)** 가 낫다
      (알림이 카카오맵 위에 바로 뜬다). 발송 서비스가 붙어야 해서 로컬 검증이 안 되므로 미뤘다.
      링크 만드는 곳은 `buildKakaoRouteUrl` 하나라 문자로 바꿔도 그 자리만 쓰면 된다
- [ ] ⚠️ **위치는 `https` 에서만 읽힌다** — 폰에서 `http://<PC IP>:3000/navi` 로 열면
      브라우저가 위치를 막아 링크가 안 선다. 로컬 검증은 «목록이 뜨는가»까지다
...
**기획: [docs/아카이브/2026-08-28_콜_시한과_합짐_예산.md](docs/아카이브/2026-08-28_콜_시한과_합짐_예산.md)** — 예시 검산까지 끝나 있다.
...
⚠️ [기획서 §2](docs/아카이브/2026-08-28_콜_시한과_합짐_예산.md) 는 이 둘을 하나로 써 뒀다 — 그대로 구현하면
...
- [ ] **콜할인율 시간 하강 (기획 §7)** 🔴 — 기사님: *"처음에는 **시세로** 합짐을 잡다가
      **5분 지나면 10%씩 줄여나가서** 단가 좋은 합짐을 잡으려는 거다."*
      지금 `callDiscountPct` 는 **한 벌 고정값**이라(`shared/src/phases.ts` — `FILTER_FIELDS`) 시간이 지나도
...
## 🟡 취소 한도 알림 — 폰·배차망별 자리 (2026-09-15 · 기사님)

기사님: *"취소한도 토스트는 빼 그건 폰마다 배차망마다 다르기때문에 다른곳에서 해야 할것같다"*. 필터 줄에서 뺐다 (버그 대장 #146).
서버는 여전히 `cancel-budget-reached` 를 보내고 관제웹 `useOrderEngine` 이 듣는다 — **그리는 자리만 없다.**

- [ ] 알림에 «어느 폰»을 싣는다 — 지금 페이로드는 `{ app, used, limit, round }` 뿐이라 폰을 못 가린다
- [ ] 그 폰의 기기 줄(`DeviceControlPanel`) 또는 현황판 폰 카드에 «배차망 · N판째» 로 띄운다 (자리는 기사님과 정한다)

## 🟡 현위반경을 «N분 안에 닿는 거리»로 — «40km 기준 반경»이 어렵다 (2026-09-15 · 기사님)

기사님: *"40km 기준 이라는것이 넘 어렵다.. 나중에 이걸 수정 해야 할듯 하다.. 현 위치반경을 20분에 다녀올 거리반경 뭐 이런식으로
20분에 상차를 해야 한다고 하면 그것이 깔끔 할수도 있겠다."*

**지금** — 필터 판 «📐 얼마나 넓게»의 `[40km 기준 반경 | 수동]`. 목적지까지 거리(`radiusDistanceKm`)를 기준거리(`radiusBaseKm` · 기본 40)로 나눈 배율(`radiusScaleOf`)을
원래 반경에 곱한다(`effectiveRadii`). 기사님 눈에는 «40km 가 무엇의 기준인지» 안 읽힌다.

**바꿀 모양 (안)** — 현위반경을 **«상차까지 N분»** 으로 고른다. 예: «20분 안에 상차» → 그 분에 닿는 거리를 반경으로.
- 코드에 재료가 있다: 상차 약속 기본값 `DEFAULT_DEADLINE_RULES.pickupPromiseMinutes`(20) · 분 → km 잠정 계수 `reachRadiusKm` · 표본 `pnpm reach` (계수 확정은 기사님)
- 필터 판에서 걷은 옛 안내 «상차 반경은 곧 도달 시간에서 자동으로 정해집니다 (잠정 계수)» 가 같은 방향이었다

**하기 전에**
- [ ] 🔴 규칙 ⑤-4 의 다섯을 먼저 정한다 — ① 어느 칸(`user_filters` 에 «상차 분»?) ② 기본값과 근거(20분 = 상차 약속) ③ 언제 읽나 ④ 필터 판에 어떤 이름 ⑤ 누가 읽나(서버 상차 목록 · 지도 «상차» 레이어 · 판정의 상차 약속)
- [ ] 목적반경 · 라인반경 · 마름모반경도 분으로 갈지, 현위반경만 갈지 기사님과 정한다
- [ ] 분 → km 계수를 표본으로 확정한다 (`pnpm reach` · 도시·시간대마다 다를 수 있다)
- [ ] «40km 기준 반경» 토글·`radiusBaseKm`·`radiusScaleOf` 를 걷을지 남길지 — 두 벌로 두지 않는다

## 🔴 픽커 — 목록 파싱이 «글자 순서»를 믿는데 픽커가 순서를 안 지킨다 (기사님 라이브 확인)

**증상** — 같은 콜이 읽을 때마다 다른 주소가 된다. 서버 장부에서 **같은 요금인데 원문이 여러 가지인 콜이 59가지**:

```
12,265원 — 원문 3가지 (같은 카드다)
   중형 송파 퀵 12,265 하남 춘궁 잠실본 18.9km    → 하남 춘궁 → 송파 잠실본  (맞다)
   송파 중형 12,265 퀵 잠실본 춘궁 하남 18.9km    → 잠실본 춘궁 → 송파 하남  (뒤섞임)
   중형 송파 퀵 12,265 잠실본 하남 춘궁 18.9km    → 또 다른 순서
```

**까닭** — `KakaoPickerParser.parse` 는 지역 토막을 `[도착 시, 출발 시, 출발 동, 도착 동]` **고정 순서**로 보고
`locations[1]+[2]` 를 출발지, `locations[0]+last` 를 도착지로 나눈다. 묶는 쪽(`groupListNodes`)은 좌표로
바르게 정렬하는데(`compareBy(top, left)`), **픽커가 같은 화면에서도 글자 좌표를 다르게 준다.**
그래서 순서를 믿는 순간 무너진다. 날짜 배지·«N분 내» 는 그 위에 얹힌 증상이었다 (그 둘은 고쳤다).

**고칠 방향 (안)** — 순서를 믿지 않는다.
- [ ] 지역 토막을 **좌표로** 가른다 — 한 카드 안에서 요금 글자보다 위/아래, 왼/오른쪽 자리로 출발·도착을 정한다
- [ ] 또는 상세 주소를 원천으로 삼는다 — 서버에 이미 `pickerDetailAddresses` 가 있다 (상세는 «픽업지» 라벨이 붙어 순서가 필요 없다)
- [ ] 어느 쪽이든 **검사 먼저** — 실물 원문 여러 판(같은 콜의 3가지 순서)을 넣어 같은 답이 나오는지

**함께 나온 것**
- [ ] **경유 콜** — 목적지가 여럿이다: «송파, 송파, 송파 문정2, 문정2, 장지» · 62,986원짜리는 일곱 곳.
      지금 파서에 이 개념이 없어 주소가 통째로 엉킨다 («경유» 는 꼬리표 목록에 넣었다)
- [ ] **퀵 배송 탭을 리스트로 못 알아본다** — 기사님 라이브: *"지금도 리스트 화면인데 관제앱은 모르는 페이지라고 한다."*
      `KakaoPickerKeywords` 의 리스트 판별에 그 탭 글자를 더한다

**찾는 법** — `pnpm db parse kakaopicker` (장부의 `intel.rawText` 로 되짚는다 · 폰 없이 된다)
