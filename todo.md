...
### ⬜ 남은 것 — 4판: 시뮬 문제지에 볼첨지 표

`onedal-sim/packages/core-simulator/src/presets.ts` 에 12줄을 얹는다.
좌표는 이제 읍면동 centroid 로 정확히 뽑을 수 있다 (지도가 들어왔으므로).
채점 기준은 [노하우_추출.md](docs/자료/노하우/일하는_법/노하우_추출.md) 의 표 —
**볼첨지가 정답이 아니다.** 7번은 잡았다 취소했고(패널티), 11번은 열어 보고 포기했다.
