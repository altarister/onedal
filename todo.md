...
- [ ] 가르는 법: **첫짐 상태**(콜 0건)에서 같은 화면을 본다. 거기서 현위치 둘레에 원이
      뜨면 ①이나 ②, 안 뜨면 ③이다. 실험실도 같은 판으로 나란히 본다 (`pnpm lab:both`)
...
> ⚠️ **2026-09-11 오후 `pnpm lab:both` 는 「빈 칸 0개」로 초록이었다 — 안심하면 안 된다.**
> 그날 선 콜은 **갈마동·문지동 둘뿐**이고, 이 구멍은 **세 번째 콜(오송 · 직접콜)**에서 났다.
> 즉 그 케이스를 **안 밟은 것**이지 메워진 것이 아니다. 재현하려면 `scripts/lab-both.mjs` 의
> `CALLS` 에 오송 콜을 더해 셋을 세운다.
...
### 🔧 대조하는 법 — ✅ **정착시켰다** (2026-09-11)

```bash
cd onedal-web && pnpm lab:both        # 같은 콜을 양쪽에 세우고 격자를 나란히 읽는다
```
`pnpm lab` 은 목업만 본다. 이쪽은 **실물과 목업을 번갈아** 보며 격자 다섯 칸을 대조한다
...
- [x] **줄 → 칸 함수와 검사** — `server/src/core/plugins/kakaopicker/pickerScreenOcr.ts`
      ...
      (`tests/core/pickerScreenOcr.test.ts` · 원자료 `ex_images/카카오픽커/실물_2026/33_…png`).
