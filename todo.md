...
## 🕸️ ⬜ 합짐·첫짐 목록은 아직 옛 계산이다 — 그물은 필터를 만질 때만 돈다 (2026-09-14 전수 조사 · 같은 날 정정)
...
- [ ] 🔴 **먼저 `orderKm`·`progressKm` 의 원천을 정한다** — 지금은 옛 경로 버퍼(`getDetourRegions`)에서만 나온다.
      앱 역주행 검사(`orderKm`)와 지나온 곳 빼기(`progressKm`)가 거기 걸려 있어, 목록만 옮기면 **역주행 검사가 꺼진다**
- [ ] 옮기기 전후로 `pnpm net:compare` — 통과 동 목록이 얼마나 달라지는지 먼저 잰다
- [ ] 🔴 **판정 색이 바뀔 수 있다** (규칙 ⑤-3) — 따로 한다
- [ ] 옛 계산은 자동 반경(§10-1)을 안 쓴다 — 옮기면 자동 반경이 처음으로 판정 목록에 걸린다
