...
## Phase 7 — 영업일 경계 (Day Rollover) 📅  🔴 계획 (미착수)

> 2026-08-09 기사님 요청: *"날짜가 바뀌어 어제까지 정보를 저장해 두고 오늘 데이터를
> 리셋해서 첨부터 시작하는 로직이 필요할 듯하다."*
>
> 착수 전 조사에서 **날짜 경계 자체가 이미 깨져 있다**는 것을 발견했다.
> 롤오버를 그 위에 얹으면 잘못된 경계로 리셋하게 되므로 7.0을 먼저 한다.

### 7.0 시각 표준 정리 (선행 필수) 🔴

현재 "오늘"을 정의하는 방식이 **세 가지**이고 서로 다른 답을 낸다.

| 위치 | 계산 방식 | 실제 경계 |
|---|---|---|
| `dispatchEngine.ts:615` (세션 복구) | `new Date().setHours(0,0,0,0).toISOString()` | 서버 TZ에 따라 다름 |
| `orders.ts:33` (오늘 오더 조회) | 위와 동일 | 서버 TZ에 따라 다름 |
| `statService.ts:28` (매출 집계) | `new Date().toISOString().slice(0,10)` | **UTC 날짜 = 09:00 KST** |

- [ ] **㉠ 앱이 KST 벽시계에 가짜 `Z`를 붙인다** 🔴
  - `HijackService.kt:52` `ISO_TIMESTAMP_FORMAT = "yyyy-MM-dd'T'HH:mm:ss'Z'"`
    — 작은따옴표 안의 `Z`는 **UTC 표시가 아니라 리터럴 문자**다.
    `Locale.getDefault()`(KST)로 포맷하므로 `17:32 KST`가 `2026-08-09T17:32:09Z`로 저장된다.
    이 값을 문자 그대로 읽으면 `02:32 KST(다음날)` — **9시간 거짓말**
  - `InsungParser.kt:177`, `Hwamul24Parser.kt:189` 도 동일
  - 실측: `local.db` 16건 **전부** 이 포맷
- [ ] **㉡ 같은 컬럼에 서버 포맷이 섞일 수 있다**
  - `dispatchEngine.ts:906`(귀가콜), `OrderRepository.ts:35`, `orders.ts:93`, `detail.ts:48` 는
    `new Date().toISOString()` — **진짜 UTC + 밀리초**. 앱 포맷과 9시간 어긋난다
  - 지금은 앱이 항상 값을 채워 넣어 드러나지 않았을 뿐 (귀가콜에서 터질 자리)
- [ ] **㉢ 서버 TZ 미설정 — 로컬과 실서버가 다르게 동작한다** 🔴
  - `process.env.TZ` 미설정. 맥은 KST, EC2는 UTC(추정, 미확인)
  - **로컬(KST)**: 임계값 `2026-08-08T15:00:00.000Z` vs 앱 값 `2026-08-08T20:00:00Z`
    → 어제 **15:00 이후 콜이 오늘 복구에 딸려온다**
  - **실서버(UTC)**: 임계값 `2026-08-09T00:00:00.000Z` — 두 오류가 서로 상쇄돼 **우연히 맞는다**
  - ⚠️ 로컬에서 재현되는 버그가 실서버에는 없고 그 반대도 성립한다. 가장 위험한 종류
- [ ] **㉣ 매출 집계는 양쪽 다 09:00 KST에 리셋된다**
  - `capturedAt LIKE '2026-08-09%'` + `toISOString()`(UTC 날짜)
  - 00:00~09:00 KST 사이엔 어제 날짜 문자열이 나온다 → 그 시간대 매출이 어제로 잡힘

**해결 방향**
- [ ] `@onedal/shared`에 시각 유틸 신설 — **경계 계산은 여기 한 곳에서만**
  - `nowKst(): string` — `+09:00` 오프셋을 **실제로** 붙인 ISO
  - `businessDate(iso?): string` — KST 기준 `YYYY-MM-DD`
  - `businessDayRange(date)` — `[시작, 끝)` ISO 쌍
- [ ] 앱 3곳 포맷 수정 — `"yyyy-MM-dd'T'HH:mm:ssXXX"` + `TimeZone.getTimeZone("Asia/Seoul")`
      (`'Z'` 리터럴 제거). 앱 버전 올리고 `:app:compileDebugKotlin` 필수
- [ ] 서버 기동 시 `process.env.TZ = 'Asia/Seoul'` 고정 — 로컬/EC2 동일 동작
- [ ] 기존 데이터 1회 백필 **(부팅 경로 아님, 별도 스크립트)**
      `...T HH:MM:SS Z` → `...T HH:MM:SS+09:00`. 전부 KST 벽시계임이 확인됐으므로 안전
- [ ] 읽기 측 3곳(`dispatchEngine` / `orders` / `statService`)을 새 유틸로 교체

### 7.1 영업일 상태를 세션에 도입

- [ ] `UserSession.isRestored: boolean` → **`businessDate: string | null`** 로 교체
  - 지금은 `isRestored`가 한 번 `true`가 되면 프로세스가 살아 있는 한 다시 복구하지 않는다.
    그래서 **관제탑을 켜둔 채 자정을 넘기면 어제 상태가 그대로 남는다**
  - 부트스트랩 진입 시 `session.businessDate !== businessDate()` → 롤오버 후 재부트스트랩
- [ ] `bootstrapUserSession`의 조기 return 조건을 `businessDate` 비교로 변경

### 7.2 롤오버가 무엇을 지우고 무엇을 남기는가 (핵심 정의)

> ⚠️ **진행 중인 콜은 절대 지우지 않는다.** 화물 기사는 야간 운행이 잦고,
> 자정에 배달 중인 짐을 메모리에서 날리면 결재도 완료 처리도 못 하게 된다.
> 어제 잡아 오늘 새벽에 내리는 짐은 **오늘로 이월(carry-over)** 한다.

| 대상 | 처리 | 근거 |
|---|---|---|
| `myOrders` / `pendingOrdersData` 중 **종료된 콜** | 🗑️ 메모리에서 비움 | DB에 남아 있으므로 유실 아님 |
| `myOrders` 중 **진행 중인 콜** | ✅ 이월 | 자정 넘긴 운행을 끊으면 안 됨 |
| `activeFilter` | ♻️ `baseFilter` 복사본으로 재생성 | 어제 회랑/차종 제약을 물려받지 않게 |
| `dispatchPhase` / `isSharedMode` | ♻️ 이월된 콜로부터 **재파생** | 이월분이 있으면 GATHERING 유지 |
| `destinationKeywords` | ♻️ `rebuildDestinationKeywords()` 재실행 | 이미 단일 생성처 (EE) |
| 기기 통계 `stats.grabbed/canceled` | 🗑️ 0으로 | `devices.ts:141` — 지금 리셋이 없어 계속 누적 |
| `pendingDecisions` / `activeTimers` | 🗑️ 정리 | 데스밸리 타이머 누수 방지 |
| DB `orders` | ✅ 그대로 | 운행일지·매출 원천. **절대 손대지 않는다** |
| 관제탑 화면 | ♻️ 새 `filter-init` + `sync-active-orders` | 부트스트랩과 같은 경로 재사용 |

### 7.3 트리거 3종 (하나로는 부족하다)

- [ ] **㉠ Lazy — 부트스트랩·`/api/scrap` 진입 시 날짜 비교** ← 주 경로
      스케줄러가 죽어도, 서버가 재시작해도 동작한다. 가장 신뢰할 수 있는 경로
- [ ] **㉡ 스케줄러 — 00:00 KST +1분** ← 관제탑을 켜둔 채 자정을 넘길 때
      Lazy만으로는 화면이 갱신되지 않는다. 접속 중인 유저에게만 롤오버 후 재전송
- [ ] **㉢ 수동 — 관제탑 "영업 종료" 버튼** ← 기사님이 일찍 마칠 때
      자정 전에 정산하고 싶은 경우. 다음 날짜로 넘기지 않고 **현재 영업일을 닫는다**
- [ ] 세 경로 모두 **같은 함수 `rolloverBusinessDay(userId, io)`** 를 부른다.
      EE에서 배운 것 — 갈래가 셋이면 셋이 어긋난다

### 7.4 검증

- [ ] 유닛: `businessDate()` 경계값 — `23:59:59 KST` / `00:00:00 KST` / `08:59 KST`(UTC 함정)
- [ ] 유닛: 앱 포맷·서버 포맷·오프셋 포맷 3종을 모두 같은 영업일로 해석하는가
- [ ] 스모크: 세션 `businessDate`를 어제로 강제 → 접속 → 종료 콜만 비워지고
      진행 중 콜은 이월되는가, 기기 통계가 0이 되는가
- [ ] 실기기: 자정 직전 콜 확정 → 자정 통과 → 결재/완료가 정상 동작하는가

### 7.5 기획: 세 축을 분리한다 (2026-08-10 기사님 논의 반영)

기사님 시나리오: *"전날 잡은 콜이 다음날 09시 도착. 전날 밤 상차하고, 다음날 07시에
일어나 합짐을 잡아야 하니 관제화면에 어제 잡은 콜이 보여야 한다. 하지만 일 정산은 하고 싶다."*

혼란의 원인은 **서로 다른 세 가지를 하나의 '날짜'로 풀려는 것**이다. 축을 나누면 각각 답이 명확해진다.

| 축 | 질문 | 기준 | 현재 상태 |
|---|---|---|---|
| **① 화면 노출** | 지금 뭘 보여줄까 | **상태** (`!isTerminal`) — 날짜 무관 | ✅ 이미 맞다 |
| **② 영업일** | 이 콜은 며칠 실적인가 | **04:00 KST 경계** | ❌ 없음 |
| **③ 입금** | 돈이 언제 들어왔나 | `settledAt` / `settlementStatus` | 🟡 컬럼만 있고 미사용 |

**① 화면은 날짜로 자르지 않는다.** 기사님 시나리오의 답이 여기 있다. 어제 잡은 콜이
오늘 07시에 보여야 하는 이유는 "어제 것이라서"가 아니라 **"아직 안 끝나서"** 다.
Phase 7.2의 "진행 중인 콜은 이월"이 정확히 이것이다. 날짜 경계를 어떻게 잡든
진행 중인 콜은 계속 보인다. **이 축은 롤오버와 무관하게 이미 옳게 동작한다.**

### 7.6 영업일 경계를 04:00 KST 로 하는 근거

- [ ] 경계를 **기사님이 자는 시간대**에 둔다. 자정은 화물 기사에게 한창 일하는 시간이라
      한 번의 운행이 이틀로 쪼개진다
  - 23:00 상차 → 02:00 하차: **자정 경계면 잡은 날과 끝낸 날이 다른 영업일**
  - 04:00 경계면 둘 다 같은 영업일 → "그날 밤 일한 것"으로 한 덩어리
- [ ] 기사님 시나리오(07:00 기상)는 04:00 이후이므로 **정상적으로 새 영업일**이 된다.
      어제 콜은 ①축(상태)으로 계속 보이고, 실적만 날짜가 갈린다 — 의도한 대로
- [ ] `user_settings.businessDayStartHour` 로 설정 가능하게 (기본 4).
      기사님 운행 패턴이 바뀌면 코드 수정 없이 조정

### 7.7 매출을 "입금 시점"으로 잡으면 안 되는 이유

기사님 질문: *"정산은 돈 입금된 시점으로 하면 될까?"* — **아니오. 다만 둘 다 필요하다.**

- **우리 시스템은 입금을 알 수 없다.** 앱이 스크래핑하는 배차망 화면에 입금 정보가 없다.
  `settlementStatus`는 전부 `'미정산'`(16/16)이고 채우는 코드가 없다
- **배차망 정산은 보통 주·월 단위**라 입금 기준으로 하면 하루 매출이 0원인 날과
  몰아서 들어오는 날만 생긴다 → **오늘 운행을 판단하는 데 못 쓴다**
- 기사님이 매일 보고 싶은 건 *"오늘 얼마 벌었나"*(노동의 결과)이지
  *"오늘 얼마 들어왔나"*(현금흐름)가 아니다
- [ ] → **매출(영업일 기준)과 입금(③축)을 분리해 둘 다 제공한다.**
      매출은 운행 판단용, 미수금은 현금흐름 관리용. `unpaidTotal`은 이미 있다

### 7.8 🔴 매출 귀속 시각 — 현실적 문제 발견

원칙은 **완료 시각(`completedAt`)의 영업일** 이 맞다. 마감된 영업일 숫자가
나중에 바뀌지 않기 때문이다.
  - 확정(잡은) 시각 기준이면, 마감 후에 그 콜이 취소되거나 운임이 바뀌면 **지난 실적이 소급 변경**된다
  - 완료 기준이면 영업일이 닫히는 순간 그 날 숫자는 확정된다

**그런데 실측 결과 완료 처리를 거의 하지 않는다.**

```
전체 16건 · completedAt 채워진 건 1건 (6%)
ORDER_RELEASED 9 · ORDER_CONFIRMED 4 · ORDER_FORCE_CANCELED 2 · ORDER_COMPLETED 1
```

완료 기준을 그대로 적용하면 **하루 매출이 거의 0원으로 나온다.**

- [ ] **단기**: 귀속 시각 = `completedAt ?? capturedAt` (완료했으면 완료 시각, 아니면 확정 시각)
- [ ] **일 정산 화면에 `미완료 N건` 을 크게 표시** — 숫자가 왜 안 맞는지 기사님이 알 수 있게
- [ ] **중기**: Phase 4(GPS 자동 도착 감지)가 들어오면 완료가 자동 기록되어
      자연히 완료 기준으로 수렴한다. 그때 fallback 제거
- [ ] 취소·방출은 매출 0이지만 **취소 시각의 영업일**로 건수만 집계 (기기별 취소율 추적)

### 7.9 🔴 시각 포맷이 네 종류다 (7.0 조사 보강)

| 컬럼 | 생성처 | 실제 값 | 문제 |
|---|---|---|---|
| `capturedAt`·`timestamp` | 앱 `SimpleDateFormat` | `2026-08-09T17:32:09Z` | KST 벽시계 + **가짜 Z** |
...
- [ ] 매출 귀속을 `completedAt` 기준으로 바꾸려면 **이 4종을 먼저 하나로 통일**해야 한다.
      `completedAt`은 정렬·비교조차 다른 컬럼과 호환되지 않는다
      (`'2026-08-09 17:32:09' < '2026-08-09T17:32:09Z'` — 공백이 `T`보다 작다)
- [ ] 통일 규격: **`YYYY-MM-DDTHH:MM:SS+09:00`** (오프셋 명시)
      — 문자열 정렬이 시간 순서와 일치하고, 사람이 읽어도 KST임이 분명하다

### 7.10 일 정산 화면 (신규)

- [ ] 영업일이 닫힐 때 스냅샷을 남긴다 (`business_day_summary` 테이블)
      마감 후 원본이 수정돼도 그날 정산은 고정
- [ ] 표시 항목: 완료 건수 / 매출 / 주행거리 / 원단가(원/km) / 합짐 건수 /
      취소·방출 건수 / **미완료 N건 경고** / 미수금 누계
- [ ] 관제탑 "영업 종료" 버튼(7.3-㉢)을 누르면 이 화면으로 이동

### 7.5 착수 전 확인 필요 ❓

- [ ] **EC2의 실제 TZ** — SSH로 `date` 한 번. ㉢의 "우연히 맞는다"가 사실인지 확정해야 한다
- [x] ~~영업일 경계를 자정으로 할 것인가~~ → **04:00 KST 로 결정** (7.6). 설정 가능하게 구현
- [ ] **`04:00`이 기사님 실제 수면 시간대와 맞는가** — 새벽 3~5시에 운행하는 날이 잦다면 조정
