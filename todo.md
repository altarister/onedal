...
## 🚨 지금 최우선 — 배포 보류 중 (2026-08-11)

시나리오 검증에서 **배포하면 안 되는 결함 4건**이 나왔다.
`tsc`·`jest`·`vite build`·`audit:socket` 을 **전부 통과한 채로** 숨어 있었다.

| # | 증상 | 심각도 |
|---|---|---|
| A | **상차한 콜이 새로고침하면 사라진다** — 복구 쿼리에 `ORDER_PICKED_UP`·`ORDER_DELIVERED` 누락. 시스템이 빈 차로 착각 | 🔴 치명 |
| B | **어제 잡은 콜이 통째로 사라진다** — 상태로 한 번, 날짜로 또 한 번 걸러짐 | 🔴 치명 |
| C | **잔여 용량이 짐 신고를 무시** — `sizeClass` 관문이 옛 필드에 남아 항상 차종 추정으로 폴백 | 🔴 높음 |
| D | **불일치 경고가 절대 안 뜬다** — 같은 원인. `CargoMismatchBanner` 전체가 도달 불가 | 🔴 높음 |
| E | 짐을 저장해도 필터가 재파생 안 됨 | 🟡 중간 |

**→ 계획서: [docs/2026-08-11_긴급수정_계획서.md](docs/2026-08-11_긴급수정_계획서.md)**

C·D 는 같은 실수다 — 단위 체계를 `sizeClass`→`unit` 로 옮기면서
**계산 함수는 고치고 그 입구의 관문 두 개를 안 고쳤다.** 세 번째 반복이라
개별 수정이 아니라 **재발을 막는 구조**(단일 출처 · 관문을 계산 함수로)까지 범위에 넣는다.
