...
### 0-I. 💸 **통행료를 이미 받고 있는데 버린다 · 설정 셋이 죽어 있다** (기사님 물음 2026-09-04)

기사님: *"차종도 중요한데.. **오토바이는 고속도로 진입하면 안 되거든.**
그리고 **통행료도 중요해** — 그 값도 가져올 수 있는 건가?"*

**둘 다 답이 나왔다. 실제로 카카오를 불러 확인했다** (집→가산동, 09-03 실제 구간).

#### 🟢 차종은 이미 지켜진다 — 우리가 `car_type` 을 넘기고 있어서다

| `car_type` | 거리 | 시간 | 통행료 |
|---|---|---|---|
| **1** (승용/1t) | 47.5km | 103분 | **1,900원** |
| **7** (이륜차) | **54.5km** | **132분** | **0원** |

7km 더 돌고 29분 더 걸리며 통행료가 0 이다 — 카카오가 **고속도로를 빼고** 짠다.
👉 이 실측이 [내비_갈아타기](docs/기획/내비_갈아타기.md) 의 `vehicleType` 을 뒷받침한다.

#### 🔴 통행료는 **이미 응답에 온다 — 우리가 안 읽을 뿐이다**

```
summary.fare = { taxi: 65500, toll: 1900 }
```
그 **1,900원이 카카오내비 앱 화면의 1,900원과 같다** (2026-09-04 교차검증).
`kakaoService.ts` 가 `raw: summary` 로 들고만 있고 `fare` 를 **아무도 안 읽는다.**

⚠️ `orders.tollFare` 는 **다른 값이다** — 배차망 화면에서 긁은 「탁송료/경유비」
(`parser.ts` 의 `extractField(lines, "탁송료")`). **섞으면 안 된다.**

#### 🔴 설정 셋이 DB 에만 있고 URL 에 안 실린다

| 설정 | 지금 | 넣어 보니 |
|---|---|---|
| `avoid_toll` | ❌ 안 보냄 | **52.9km / 130분 / 0원** — 27분 더 걸리고 공짜 |
| `car_hipass` | ❌ 안 보냄 | 이 구간은 차이 없음 |
| `car_fuel` | ❌ 안 보냄 | — |
...
CLAUDE.md 는 톨비를 **정산 페이지** 몫으로 뒀다(관제앱은 «그냥 매출»). 맞다 —
**판정 색에 넣자는 말이 아니다**(규칙 ⑤-1: 돈은 앱이 이미 걸렀다).
다만 **지금 안 쌓으면 정산을 만들 때 과거 톨비를 영영 모른다.**
마이그레이션을 안 하기로 했으니 더 그렇다.

👉 **⑤-4 를 먼저 채운다.** 특히 **⑤ 읽는 곳** — 「카카오 통행료」와 「배차망 탁송료」가
   각각 무슨 질문의 답인지 갈라 놓지 않으면 한 칸에 섞인다.
