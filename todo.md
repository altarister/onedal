...
## 🎯 판정을 한 함수로 — 여섯 축, 전부 가중치 (2026-08-29 계획 · 기사님 «프로젝트의 핵심»)

기사님: *"누가 보더라도 N가지 기준으로 판정하는군.. 모든 값은 이 기능만 통해서만 항상 같은 값을
가질 거야. 먼발치서 가장 효율적인 구조가 되어야 해."*

**계획서: [docs/기획/판정_한_함수.md](docs/기획/판정_한_함수.md)** — ⑤-4 넷(스키마·값·시점·화면)까지 채워 뒀다.

전수조사에서 나온 것 (전부 코드로 확인):
- 판정이 **두 갈래** — `scoreDryRun`(가중치 있음) · `reasons.push` 13곳(가중치 없음)
- 색이 **문자열로** 전달된다 (`kakaoTimeExt` → 화면이 `includes()`)
- 「약속 보존」이 **통화 없으면 구조적으로 실패 불가** (`timing.ts:1089` `declared &&`)
- 「같이 못 실음」은 **0점으로만 존재** (통과 100점이 평균에 못 든다)
- 돈을 **세 번** 센다 (앱·서버 Stage3·미리보기)
- **모르면 점수가 후해진다** (축이 빠져 분모가 준다)
- **지리에 점수가 없다** — 역주행 검사는 앱에만
- 기사님이 **못 고치는 값 11개** · 죽은 값 9개

- [ ] **기사님 결정 셋** (계획서 §5) — 바닥을 몇 축에 둘까 · 「근거」 축을 넣을까(⑤-2 와 충돌 소지) ·
      「지리」 점수를 어떻게 낼까(`progressKm` 이 서버에 없다)
- [ ] 이행 7단계 (계획서 §3) — 단계마다 게이트 초록. **2단계는 병행**(옛것을 안 지운다)
