...
**🔴 먼저 볼 것 — 판정·화면에 닿는다**
- [ ] `routeComposer:267` — *"`planArrivalStops` 는 같은 `optimizeWaypoints` 를 쓰므로
      카카오 경유 순서와 같다"* → **08-25 에 갈라졌다**(최근접 탐욕). `applyRoute` 는 **길이만**
      대조하므로 순서가 어긋나도 통과한다. #32·#35·#36 「두 계획이 갈라진다」와 같은 자리
