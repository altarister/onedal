...
### 🆕 FF. 요금 폴백 파서가 `40,000원`을 `40원`으로 읽는다 🔴 (2026-08-10 발견)

- [ ] `parser.ts:70` `/(?:요금|금액)\s*[:]?\s*(\d+(?:\.\d+)?)(만|천)?/`
  - 실제 화면 값은 **`요금 : 40,000(신용)`** — 쉼표가 들어간다
  - `\d+`가 쉼표에서 멈춰 **`40`** 만 잡는다 → `fare = 40원`
- [ ] 죽은 코드가 아니다. `detail.ts:63`에서
      `if (!pendingOrder.fare || pendingOrder.fare <= 0)` 일 때 호출된다
  - 앱이 리스트에서 요금을 못 긁었을 때 이 폴백이 돈다 → **똥콜 판정이 통째로 뒤집힌다**
    (하한가 3만 원 필터에 `40원`이 걸리니 무조건 탈락 → 놓친 꿀콜이 있었을 수 있다)
- [ ] 수정: 쉼표 허용 + 결제방법 동시 추출
      `/(?:요금|금액)\s*[:]?\s*([\d,]+(?:\.\d+)?)\s*(만|천)?\s*(?:\(([^)]+)\))?/`
- [ ] 같은 파일의 `parseMockupDistance` · `parseMockupVehicleType`도 쉼표·괄호 케이스 점검 필요
- [ ] 회귀 테스트 추가: `"요금 : 40,000(신용)"` → `{ fare: 40000, paymentType: '신용' }`
