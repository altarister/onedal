...
## 🔴 화물24시 파서가 시뮬레이터 콜 카드를 **한 장도 못 읽는다** (2026-09-14 폰 확인에서 드러남 · 옆길이라 미뤄 둠)

폰(앱 `2.9.2-screennet`)에서 스캔앱이 시뮬레이터 화물24시 리스트를 **24시로는 알아봤는데**
`👁️ [리스트 스캔] 텍스트노드 51 · 콜그룹 0 · 통과 0` — 카드를 못 묶어 `intel` 에 `hwamul24` 가 안 쌓였다.

**원인 (로그 · 코드로 확인):** 시뮬레이터가 요금을 `200,000` 과 `원` **두 조각**으로 그린다
(`Hwamul24DispatchBoard.tsx` — `{fare}<span>원</span>`). 파서 `Hwamul24Parser.groupListNodes` 는
`^\d{1,3}(,\d{3})*원$` — **한 조각**이어야 요금으로 본다. 거리도 `14` · `Km` 로 갈려 못 뽑는다.

**모르는 것:** 실제 화물24시 앱이 요금을 한 조각으로 주는지 — 실물 화면 구조 추출이 레포에 없다.
파서 머리 주석은 «24시 준비 중(파서 복구 전)», 화물24시 파서 검사는 **하나도 없다.**

- [ ] 방향을 고른다 (기사님)
      A 시뮬레이터가 «200,000원»을 한 조각으로 — 실물이 두 조각이면 실물에서 같은 병
      B 파서가 «숫자» + 다음 조각 «원» 도 요금으로 — 🔴 머리 줄 «잔액 : 544,864 원» 을 카드로 오인하지 않게 막는 조건 필요 (추천 · 검사 먼저)
      C 실제 화물24시 앱 화면을 먼저 추출
- 폰 로그 원문: 2026-09-14 04:31:55 `🔎 [UNKNOWN 화면 진단]` 줄 · 04:33:34 `👁️ [리스트 스캔]` 줄
- ⏭️ **픽커 시뮬레이터 2단계는 이것 없이 간다** (기사님 2026-09-14) — 계획서 1단계 통과 기준 ⑤ 를 뺐다.
  픽커 화면은 화물24시 파서와 안 엮인다. 이 칸이 풀리면 1단계 ⑤(화물24시 문제지 한 바퀴 → `intel` 에 `hwamul24`)를 돌린다
