...
## 🔥 [우선순위 높음] 단계별 가격 전략 — 하한가가 하나면 안 된다

기사님 (2026-08-13):

> *"목적지 **첫짐**은 좀 기다리더라도 **비싼 것을 잡아 기준을 세우고**,
> **합짐**은 추가로 얻는 매출이니 **가격을 조금 낮추더라도** 합짐을 하는 비중을 높여야 할 듯하고,
> **마지막 합짐**은 금액이 낮더라도 **경로 등이 최대한 같은 것**이 더 유리한 것 같은데."*

지금은 `minFare` 하나가 모든 상황에 똑같이 걸린다. 그래서 단계마다 다른 전략을 쓸 수 없다.

| 단계 | 무엇을 노리나 | 판정 기준이 어떻게 달라져야 하나 |
|---|---|---|
| **첫짐** | 그날의 **기준 단가**를 세운다 | 하한가 **높게**. 기다려서라도 비싼 것 |
| **합짐 (중간)** | 추가 매출 | 하한가 **낮춰서** 비중을 높인다 |
| **마지막 합짐** | 이미 가는 길 | 금액보다 **경로 일치도**가 우선 |
| **관내** | 시간 때우기 | 별도 하한가 (짧고 싸다) |

**"마지막 합짐"을 어떻게 아는가** 가 관건이다. 적재가 거의 찼거나, 출발이 임박했거나,
마감 여유가 얼마 안 남았을 때다. 판정 신호를 정해야 한다.

- [ ] 단계를 어떻게 셀 것인가 정의 (첫짐 / 합짐 N차 / 마지막 합짐 / 출발 후 / 관내 / 복귀)
- [ ] 단계별 하한가를 필터에 넣는다 (지금은 `minFare` 하나)
- [ ] 마지막 합짐에서 **금액보다 경로 일치도**를 우선하는 판정식
- [ ] 관제탑 요약줄에 **지금 몇 번째 짐인지** 표시 (기사님: *"그런 상태를 알아야 할 것 같다"*)

> 이건 수익에 직결된다. 첫짐 단가가 그날의 기준을 만들고, 합짐이 그 위에 얹히는 구조다.
