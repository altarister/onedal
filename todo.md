...
- [ ] `paymentType: '신용' | '선불' | '착불' | '카드' | '현금'` 은 **이미 정의·파싱되어 있다**
      (`utils/parser.ts:183` — "결제방법" 필드). 다만 실측 데이터는 전부 `null`이라
      **인성앱 상세 화면에 이 필드가 실제로 뜨는지 확인 필요**
...
### 8.6 상차/하차 보고 자동 감지 (앱) — 🔜 화면 확보 후

> 기사님: *"아직 그 화면을 확보하지 못했다. 그래서 지금 할 수 있는 것처럼
> 내가 관제웹 화면에서 직접 클릭할 수 있어야 함."*

**따라서 자동 감지는 뒤로 미루고, 관제웹 수동 클릭(`MANUAL_WEB`)을 먼저 만든다.**
8.2가 진입점을 추상화해 두므로 나중에 `AUTO_SCRAPE`를 붙일 때 본체는 손대지 않는다.
...
- [ ] **㉡ `scheduleText` → 구조화된 시각 파서** (`"낼09시"` → 내일 09:00,
      `"11일)09시"` → 해당 월 11일 09:00, `"급송"` → 시간 없음 + 긴급 플래그)
...
- [ ] 🆕 **`scheduleText` 실제 값 샘플** — `"낼09시"` 외에 어떤 형태가 있는지.
      파서를 짜려면 실물이 필요하다 (저장을 켜면 며칠 안에 모인다 → ② 먼저 하는 이유)
- [ ] 🆕 **크기 [소/중/대/초과]를 몇 점으로 환산할지** — 기사님 감각이 필요.
      예: 1t(30점) 기준 소=2 / 중=5 / 대=10 / 초과=30
