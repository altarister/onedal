...
**개발 계획서: [docs/시간체계_개발계획.md](docs/시간체계_개발계획.md)** — 다섯 단계 · 조사 결과 · 위험.
🔴 조사에서 나온 것: `arrivalMarginMin` 이 **DB에 없다**(코드 상수 30 고정) ·
관제웹이 판정 기준을 **받아 놓고 시간 계산에 안 쓴다**(둘을 같이 안 고치면 서버·화면이 갈라진다) ·
단독 경로는 **이미 저장돼 있다**(`kakaoSoloDurationMin` — 새 컬럼 불필요)
