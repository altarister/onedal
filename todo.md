...
- ⚠️ **2026-08-10 조건부 철회 예고** — 기사님이 *"모든 콜은 배송 완료 시 이벤트를 받게 될 것"*
  ...
  그 전까지는 이 항목(확정 시각)을 유지 — 자세한 근거는 8.7
...
### 8.4 전화 + 통화 결과 기록 (Call Sheet)

- [ ] 관제탑 콜 카드에 **상차지/하차지 원터치 전화** (`tel:` — Capacitor에서 동작)
      `places.phone1`이 이미 21건 저장돼 있다. 노출만 하면 된다
- [ ] 통화 후 입력 폼 — **짐 양 / 짐 종류 / 상하차 방법(지게차·수작업) / 약속 시간 / 담당자**
- [ ] 이 데이터가 쓰이는 두 곳 (입력만 받고 안 쓰면 기사님이 안 채운다)
  - **㉠ 잔여 적재 용량 정밀화** — 지금은 차종으로 *추정*한다(`getRemainingCapacityTypes`).
    실제 짐 양을 알면 "1t인데 파렛트 1개뿐" 같은 경우를 정확히 계산 → **합짐 기회가 늘어난다**
  - **㉡ 경로 시간창** — 8.6
- [ ] `places`에 누적 → 다음에 같은 곳 갈 때 미리 표시.
      `visitCount` · `rating` · `blacklistMemo` 컬럼이 이미 있다 (예: *"이 창고 지게차 없음, 수작업 40분"*)

### 8.5 상차/하차 보고 자동 감지 (앱) — 단계적으로

배차앱에 보고하는 것은 **실제 업무 행위**다. 잘못 누르면 업무 사고이므로 한 번에 자동화하지 않는다.

- [ ] **1단계 — 감지만**: 기사님이 배차앱에서 보고를 누르면 화면 변화를 감지해 서버에 알림.
      우리가 누르지 않는다. 여기까지만 해도 완료 이벤트가 자동으로 들어온다
...
### 8.6 시간창(Time Window) 기반 경로 재최적화
...
- [ ] `orderStops.requestedTime`이 현재 **0건** — 파싱/저장 연결이 선행 (8.4)

### 8.7 🔄 Phase 7 결정을 되돌린다 — 매출 귀속을 완료 기준으로
...
- [ ] **인성앱의 상차/하차 보고 화면이 어떻게 생겼는지** — 스크린샷 필요.
      `onedal-app/docs/`의 화면 정의에 이 단계가 없다
- [ ] **`requestedTime`이 왜 0건인지** — 파서가 안 긁는지, 서버가 안 저장하는지
- [ ] **짐 양을 어떤 단위로 받을지** — 파렛트 수 / 박스 수 / 무게(kg) / 적재율(%).
      기사님이 통화 중에 **한 손으로 3초 안에** 입력할 수 있어야 한다
- [ ] **정산(미수금) 흐름** — 하차 보고 후 인수증 제출·입금까지 어떤 단계가 있는지
