...
- [ ] **0단계 · 다섯을 먼저 정한다** (규칙 ⑤-4 — 코드부터 짜지 않는다)
      · 작업 단계를 **몇 개**로 올릴 것인가 (앱엔 6개가 있다. 관제에서 뜻이 있는 것만 추릴지)
      · 모드 «대답»의 형태 — 필터처럼 **버전**인가, 값 그대로인가
      · 「적용중」이 안 풀릴 때 **몇 초에 버튼만 풀 것인가** (기사님 «10초쯤» · 아직 미확정)
      🔴 **«반영 안 됨»이라 쓰면 거짓말이 될 수 있다** — 홈에 있으면 실제로 40초 뒤 반영된다.
         「아직 안 받음 · 마지막 통신 N초 전」 처럼 **사실만** 적는 쪽으로 검토

- [ ] **1단계 · 관제웹만** (앱 무관 — 오늘 바로 보인다)
      · 6+9 배지 합치기 — 화면 꺼지면 `[💤 화면 꺼짐]`
      · 3번 «조용한가» — 서버가 **마지막 두 보고의 간격**만 봐도 안다 (앱 안 고쳐도 됨).
        이게 있으면 **딤드가 얼마나 걸릴지**가 미리 보인다
