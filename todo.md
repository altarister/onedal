...
👉 **A-3 이 방향 전환이다 — 조사 완료 (2026-09-03 밤).**

카카오내비를 여는 공식 길은 **카카오 JS SDK 의 `Kakao.Navi.start()`** 다 (맨손 URL 스킴은 비공식).

| | 카카오맵 (지금) | 카카오내비 (갈아탈 곳) |
|---|---|---|
| 여는 법 | `kakaomap://route?…` URL 하나 | JS SDK `Kakao.Navi.start({...})` |
| 경유지 | 5개 (`vp`~`vp5`) | **3개** (`viaPoints` 최대 3) |
| 좌표 | 위도,경도 텍스트 | `coordType:'wgs84'` 로 우리 좌표 그대로 |
| 차종 | 없음 | `vehicleType` 1~7 — **1t 카고를 뭘로 할지 기사님과 확인** |
| 출발지 | `sp` | `sX`,`sY` |
| 준비물 | 없음 | **JS 앱 키 + 카카오 콘솔에 `1dal.altari.com` 웹 도메인 등록** |
...
👉 남은 것: ① 카카오 콘솔 JS 키·도메인 등록(기사님 손) ② `vehicleType` 확정 ③ `/navi` 버튼 교체.
...
### 0-H. ⏱️ **남은 주행 분이 늙는다** — 살아 있는 값으로 (기사님 물음 2026-09-04)

기사님: *"카카오가 값을 주었지만 표시하기 어렵겠군.. 구간별 위치값을 나누어서
계산하면 나올 수 있을까? 그건 너무 자주 계산되어 부하가 많을 수 있겠다."*

**부하는 문제가 아니다 — 카카오를 다시 부를 필요가 없다.**

| | |
|---|---|
| 진짜 문제 | `StopLead.driveMinutes` 는 **카카오 호출 시점부터의 누적**이다 (`shared/src/timing.ts` 주석). 30분 달린 뒤에도 `~30분` 이라고 말한다 |
| 재료 | 선은 **이미 손에 있다** — `routeHolder.routePolyline` (09-03 실주행 939점). 지도가 그걸 그리고 있다 |
| 방법 | 누적 거리표를 **경로가 바뀔 때 한 번** 만들고, GPS 마다 «가장 가까운 점»을 찾아 **뺀다**. `nearestIndex()` 가 `hooks/useMockGpsSimulator.ts` 에 이미 있다 |
| 분 | `남은 km × (카카오 구간 distance ÷ duration)` — **도로 기준**이라 지어낸 값이 아니다 (규칙 ④) |
| 부하 | 939점 실측 — GPS 1회당 **1.2μs**, 5초마다 재도 **1시간에 0.9ms**. 지도 한 번 다시 그리는 것이 수천 배 비싸다 |

### 🔴 이탈은 «값을 지울 이유»가 아니라 «값을 새로 받을 신호»다 (기사님 지적 2026-09-04)

처음엔 *"선에서 멀어지면 분을 지우고 «경로 이탈»이라 적는다"* 고 적었다. **셋 다 틀렸다.**

기사님: *"경로를 이탈하긴 했지만 **실시간 내비가 다른 길로 안내했다면?** 경로 이탈된 것은
내가 **이미 인지하고 있을 거야**. 경로 이탈했을 때 **남은 시간 혹은 거리가 궁금할 건데**..
경로이탈로만 표시하는 건 문제가 있어."*

| | 왜 틀렸나 |
|---|---|
| «경로 이탈»이라 적기 | 기사님이 **이미 아는 말**이다. 1~2초를 쓰고 아무것도 안 돌려준다 |
| 숫자 지우기 | 벗어난 그때가 **숫자가 가장 필요한 순간**이다 |
| 애초에 «이탈»이라 부른 것 | 내비가 다른 길로 안내했으면 기사님은 **멀쩡한 길** 위에 있다. 틀린 건 **우리 선**이다 — 우리 버그에 기사님 이름을 붙이는 셈 |

**부하 걱정은 반대로 풀린다** — 비싼 쪽이 드문 쪽이다.

| | 얼마나 자주 | 무엇을 |
|---|---|---|
| 선 위 (거의 항상) | GPS 마다 | 뺄셈 — 1.2μs, 공짜 |
| 선 밖 (드물다) | 하루 몇 번 | **카카오를 다시 부른다** |

🟢 `recalculateActiveKakaoRoute()` 는 **이미 있다** (`services/dispatchEngine.ts`) —
취소·방출·비상에 걸려 있고 **GPS 에만 안 걸려 있다.** 새 기계가 아니라 **안 이어진 선** 하나다.

**화면에는 모드가 없다.** 같은 식이 양쪽을 다 답한다:
```
남은 = (현위치 → 선의 가장 가까운 앞점) + (그 점 → 목적지, 도로 기준)
                    ↑  선 위면 6m ≈ 0  ·  벗어나면 800m
```
숫자가 끊기지 않고 **서서히 나빠진다.** 모드 전환도 이름표도 빈칸도 없다.
«이탈»은 **지도가 이미 말한다** — 계획선과 궤적이 눈에 띄게 벌어진다 (㉰과 같은 이유).

🟢 **덤** — 벗어났을 때 다시 부르면 지도 선이 **내비를 따라간다.** 09-03 요청 A-1
(*"기존 경로가 어떻게 바뀐다가 보여야"*)이 여기서 함께 풀린다.

**09-03 실측** — 그날 선 73벌에 GPS 8,470점을 견줬다:
중앙 **6m** · 300m 밖 4.3% · 500m 밖 0.3% · **1km 밖 0점.**
성남시청 두 바퀴도 잘못 든 길도 800m 를 못 넘었다.
⚠️ 이건 «아는 어느 선엔가 가까웠다»이지 «지금 홀더의 선에 가까웠다»가 아니다 — **아직 못 쟀다.**

**남은 함정 둘**
- **터널** (09-03 에 길었다) — GPS 가 얼었다 튀면 한 점은 이탈로 보인다.
  → **연속 N회** 벗어나야 부른다. 한 점은 무시한다
- **카카오가 세 번째 길을 줄 수 있다** — 기사님 내비가 어느 길을 골랐는지 알 방법이 없다
  (카카오내비는 되돌려 주지 않는다). 그래서 숫자는 **언제나 `~`(추정)** 이다.
  푸는 게 아니라 **한계로 적어 두는 것**이 맞다

> 🟢 상태바 쪽 준비는 끝났다 — 문장을 `client-app/src/lib/sheetStatus.ts` **한 곳**으로 모았고
> (`sheetStatus()`), 분은 `driveMinutes` 하나로 들어간다. 살아 있는 값이 생기면 **그 인자만** 바꾼다.
