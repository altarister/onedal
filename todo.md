...
- **보호 선택의 검증을 완성한다.** 결박을 골랐는데 무너졌다 → 사진이 그 인과를 증명한다
  ([도착_사유_기획](docs/도착_사유_기획.md) §0 의 고리가 여기서 닫힌다)
...
- [x] ~~**①** GPS 도착이면 경로에서 뺀다~~ — 2026-08-19 완료 (`hasVisitedStop` 한 곳으로) — "다녀왔다"의 정의가 두 벌이라(경로는 `status`,
      시각은 마일스톤) 같은 자리를 세 번 고쳤다. 클래스를 없애는 수정 ([버그 대장](docs/버그_대장.md) #24 연장)
...
- [ ] **[기획·판정]** 후속 콜 빈도 — 하차지 방향이 "다음 콜을 부르는 힘"을 판정 요소/표시로.
      재료: intel × targetApp × 시각. 근거: docs/노하우/노하우_추출.md §1 (세 영상 공통 최대 노하우)
...
> ⚠️ **2026-08-13 오후 확정으로 이 섹션은 대체됐다** → **[docs/필터_재설계_명세.md](docs/필터_재설계_명세.md)**
...
**의도는 유효하다.** 원 계획서(`docs_backup/implementation_plan copy 2.md`)의 경고:
