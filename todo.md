...
전수 목록과 실행 로드맵(Step 5.5 ~ Step 9)은 **[docs/기획/국면별_판단_직행_로드맵.md](docs/기획/국면별_판단_직행_로드맵.md) 한 곳**으로 전수 취합되어 관리됩니다.
- [ ] 🔴 **[Step 5.5] 막는 그물**: 제외 키워드 파이프 연결(픽커 무방비 해소) · 약속 분 숫자 전달 · 상차 역주행 1층 🔴 차단 · 실콜 어긋남 수집
- [ ] 🏗️ **[Step 5.6] 기반 공사**: 판정 엔진에 `DispatchPhase`('GATHERING' | 'DELIVERING') 국면 분기 축 신설
- [ ] 🅿️ **[Step 6] 정차 합짐**: PROMISE 임시 지연(bufferAfterMin < 0) 비례 감점 · 소박스 정차 5분화
- [ ] 🚗 **[Step 7] 주행 합짐**: 고속도로 IC 접근성 보정 · 라인 띠(경유) 평가 강화 · 장거리 단가식 거리 보정
- [ ] 🏁 **[Step 8] 첫짐 탈출력**: 갇힘 지역 하차지 감점 · 첫짐 거리 하한 가드
- [ ] 🔲 **[Step 9] 공통 체감**: 누적 묶음 시급 실시간 표시 · pnpm db judge 전수 뷰 정식화
