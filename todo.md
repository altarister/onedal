...
### 📷 그림으로 읽기 — 1단계는 섰고, **아직 아무도 안 부른다** (2026-09-13)
...
- [ ] **폰이 찍어 보내는 길** — 접근성 설정에 `canTakeScreenshot` + `takeScreenshot`(API 30↑),
      가로 540·JPEG60 으로 줄여 전송. 실측 29KB → 시간당 22장이면 **월 133MB**
      (유튜브 480p 30분어치). 통신비는 걸림돌이 아니다
- [ ] 🔴 **서버 OCR 엔진이 아직 안 정해졌다** — 09-13 에 잰 428ms 는 **맥의 Vision** 이고,
      라이브는 EC2 **리눅스라 그 엔진이 없다.** tesseract·PaddleOCR·클라우드 중 골라 **다시 재야 한다**
