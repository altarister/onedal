...
## 🔴 #78 — 경로 순서가 지리를 뒤집는다 (2026-08-30 발견 · 다음 작업)

7지점 실측에서: 경로 끝이 신둔(~17km)인데 `progressKm` 가 **곤지암읍 19.2km** —
큰 읍의 **중심점 투영**이 순서를 뒤집어 03 미탐·07 오탐. 합짐 국면은 상차 반경을
룰로 건너뛰므로 **경로 순서가 유일한 문지기**라 무게가 크다.
전말·진단 방향은 [버그 대장 #78](docs/기록/버그_대장.md) · 시작점 `buildAppProgressKm`.
⚠️ 08-25 실측·아래 «주행 중 기점으로 다시 재기» 항목과 같은 언저리 — 같이 풀 것.
...
⬜ **제안 — ⑤-4 에 다섯째 칸**: 「⑤ **누가 이 값을 읽는가**」.
   넷(스키마·값·시점·화면)은 전부 **만드는 쪽**만 묻는다. 오늘 사고 둘(#74·#75)은
   전부 **읽는 쪽**에서 났다. 기사님 승인이 필요하다 (CLAUDE.md 를 고치는 일).
