...
## 🗺️ 지도를 충청권까지 넓힌다 — **VWorld 인증키 만료로 멈춰 있다** (2026-09-06)

기사님 질문에서 나왔다: *"인천 남동공단에서 청주로 목표를 잡으면 몇 개를 잡을 수 있을까?
이 사람은 4개 잡아서 25만원을 이야기하는데.. 우리 시스템도 그렇게 작동할까?"*

**돌려 봤더니 0개다.** 필터가 아니라 **데이터**가 없다 —
`merged_map.geojson` 이 서울·인천·경기(1,239동)만 담아서 「청주」로는 도착 목표가
**빈 배열**이 되고, 빈 목록은 fail-closed 라 앱이 전부 탈락시킨다.

### 🔴 지금 게이트가 빨간불이다 — 하나, 그리고 일부러다

```
tests/rules/mapCoverage.test.ts
  🔴 충청권이 그물에 들어온다 — 청주·천안·대전     ← 실패 중 (청주 0개)
  🟢 나머지 넷 통과
```

「진단 → **빨간불 확인** → 수정 → 초록불」의 빨간불 단계다 (CLAUDE.md).
**지도가 들어오면 저절로 초록**이 된다. 그때까지 `npx jest` 는 이 하나로 빨갛다.

### 막힌 곳 — **기사님만 푸실 수 있다**

```
VWorld API → EXPIRE_KEY «인증키가 만료되었습니다»
키 자리: onedal-map/.env  (vworld_kr · gitignore)
```

[V-World](https://www.vworld.kr) 에서 인증키를 갱신하면 그다음은 명령 두 줄이다:

```bash
cd onedal-map
./.venv/bin/python scripts/generate_bjd_data.py --regions 11 28 41 30 36 43 44
node scripts/merge_intel_to_geo.js        # ← 빼면 intel 이 날아간다
```

### ✅ 키 없이 해 둔 것 (2026-09-06)

| | 무엇 | 결과 |
|---|---|---|
| 🔴 **빈 산출물 가드** | 1,000개 미만이면 안 쓰고 `exit 1` | 실사고를 막았다 — 아래 |
| 출력 경로 | 두 스크립트 다 `onedal-map/out/` 로 (`ONEDAL_MAP_OUT`) | 원본은 읽기만 |
| `REGION_NAMES` | `30 대전 · 36 세종 · 43 충북 · 44 충남` 추가 | 「미분류」 방지 |
| **intel 병합기 검증** | 지금 지도를 넣고 돌려 **해시가 그대로** (`7d5a3129…`) | 2단계는 이미 옳다 |
| `.venv` | `shapely` 격리 설치 | 시스템 파이썬 안 건드림 |
| **성능 기준선** | 부팅 **372ms** · 그물(오산+10km, 109동) **19ms** | 확장 뒤 이 값과 비교한다 |

🔴 **가드가 막은 것** — 키가 만료돼 세 지역이 전부 실패했는데, 원본 스크립트는
`continue` 로 넘어가 **45바이트짜리 빈 지도를 그대로 썼다.**
`map/map` 에서 돌렸으면 **28MB 지도가 그걸로 덮였다.**
`out/` 로만 내보내기로 한 판단이 여기서 값을 했다.

### 남은 순서

| 판 | 무엇 | 상태 |
|---|---|---|
| 0 | 검사를 먼저 만들어 빨간불 확인 | ✅ **끝** |
| 1 | 지금 셋으로 재생성 → 현재 파일과 대조 | 🔴 **키 만료로 막힘** |
| 2 | 일곱(＋30 대전·36 세종·43 충북·44 충남)으로 생성 | ⬜ |
| 3 | 서버 `mapData/` 에만 넣고 검사가 초록이 되는지 | ⬜ |
| 4 | 시뮬 문제지(`onedal-sim/presets.ts`)에 볼첨지 표 12줄 | ⬜ |

⚠️ **1판을 건너뛰지 않는다.** VWorld 원본이 8월 이후 바뀌었을 수 있어서,
지금 셋으로 먼저 맞춰 보지 않으면 «충청 때문인가 원본이 움직인 건가»를 못 가른다.

⚠️ `REGION_NAMES` 에 네 줄(30·36·43·44)을 추가해야 이름이 «미분류»로 안 떨어진다.
