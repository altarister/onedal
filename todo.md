...
## 🔴 요율표를 바꿔도 앱에 **옛 단가**가 내려간다 (2026-09-11 전수조사에서 나옴 · 미확인)

필터 값 그릇 전수 조사 중에 나왔다. **이식과 무관한 별도 결함**이라 여기 적어 둔다.

```
ratePerKm = 콜할인율 × user_filters.vehicle_rates × agency_fee_percent   (파생값)
```

이 파생을 다시 만드는 자리는 둘뿐이다 — `recalculateDerivedFields` 가 **`'callDiscountPct' in changes` 일 때만**
(`filterManager.ts` — `netKeywordsOf()`) 과 타겟 전환(`dispatchEngine.ts` — `setCallTarget()`). 그런데 `PUT /api/settings/pricing` 은
`vehicle_rates`·`agency_fee_percent` 를 **DB 에 직접 UPDATE** 하고(`routes/settings.ts:341-352`)
`saveBaseFilter` 에는 `minFare`/`maxFare`/`excludedKeywords` 만 넘긴다(`:355-362`).

⇒ **요율표를 고쳐도 `session.activeFilter.ratePerKm` 은 옛 표 그대로**이고, 앱에는 그 값이
계속 내려간다 (할인율을 건드리거나 국면이 바뀔 때까지). 로그인 때는 맞게 채워진다.

- [ ] **먼저 재현한다** — 요율을 바꾸고 `curl localhost:4000/api/health` 뒤 앱 피기백의
      `ratePerKm` 이 옛 값인지 확인. 재현되면 그걸 잡는 검사부터 (규칙: 진단 → 빨간불 → 수정)
- [ ] 고치는 자리 후보: `routes/settings.ts` 가 요율을 바꾼 뒤 `recalculateDerivedFields` 를
      부르게. ⚠️ 파생은 `filterManager` 한 곳에서만 만든다 (server/CLAUDE.md)
