...
- [ ] 하차 — 먼 목적지는 하차 동에서 **상차 목록 동을 뺀다**(목록 빼기) · 가까이 온 목적지는 목적지 영역 전체 — 서버 목록과 지도 «하차» 레이어 둘 다 (지도는 빠진 동을 보여야 목록과 같다)
- [x] 🎯 목적지 가까이 옴 — 판단은 shared `filterArea.isNearGoal` · `withNearness` 한 곳 · 상차는 지도 · 서버 목록 둘 다 «가까이 온 목적지가 있으면 A 전체» (2026-09-15)
- [ ] 🎯 가까이 옴 — 하차: 지도 «하차» 레이어는 목적지 원만 그린다 ✅ · 서버 하차 목록은 아직 옛 그물 (아래 «상차 목록 동을 뺀다»와 함께)
- [ ] 관내를 따로 재지 않는다 — `netForGoal` 의 `local`(목적지 원만)을 걷고 «목적지 가까이 옴»으로 (기사님 2026-09-15 *"그쪽에 도착했으니 다른 곳을 정하지 않았으면 그곳에서 일 있으면 하자"*) · `localMode` 를 읽는 곳(`dispatchEngine` · `StageView` · `OrderFilterModal` · 현황판)을 함께 본다
...
- [ ] 🔴 **그 9건을 새 규칙으로 다시 쓴다** — 국면별 저장이라는 개념이 없어졌으니
      「🔍 필터에서 손대면 `activeFilter`, `💾 서버 저장`을 누르면 `baseFilter` 까지」
      (client-app CLAUDE.md 의 지금 규칙)를 묻는 검사로 바꾼다
- [ ] 지울 것이면 **왜 지웠는지**를 그 자리에 남긴다 — 지운 검사는 되살아나지 않는다
