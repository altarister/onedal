...
### ❓ 기사님 판단 대기 — **코드로 정할 수 없는 것** (2026-08-29)

전수조사에서 나왔지만 **어느 쪽이 옳은지가 값·정책의 문제**라 손대지 않은 것들.
규칙 ⑤-4 대로 «무엇을 정해야 하는가»만 적어 둔다.

**① 상세 전송이 실패하면 즉시 취소할까, 30초를 기다릴까**

`ApiClient.sendDetail` 은 전송 실패 시(비2xx · 재시도 소진 · 예외) `CANCEL` 콜백을 부르는데
`HijackService` 가 그걸 버린다. 대신 30초 안전취소 타이머가 받는다 — **안 잡히는 게 아니라
30초 늦게 잡히는 것**이다.

| | 즉시 취소 | 지금 (30초 대기) |
|---|---|---|
| 좋은 점 | 30초를 아낀다. 그 시간에 다음 콜을 본다 | 일시적 오류면 그사이 판결이 올 수도 |
| 나쁜 점 | **통신 한 번 끊겨도 취소 1회 소진** (배차망 10회 한도) | 30초 동안 콜을 물고 있다 |

🔴 **취소 횟수는 되돌릴 수 없는 자원이다.** 지하주차장·엘리베이터에서 한 번 끊길 때마다
취소가 늘면, 정작 필요한 순간에 못 버린다. → **지금(30초 대기)이 안전한 쪽으로 보이나,
하루에 몇 번 일어나는지를 모른다.** 먼저 셀 것: 실주행 로그에서 `Detail 전송 실패` 빈도.

**②「자동 콜 잡기」 — 이 이름이 맞나** (화면 문자열을 이미 바꿨다)

설정 첫 탭의 스위치 이름을 「🚀 무인 서핑 모드 (Full Auto)」 → **「🚀 자동 콜 잡기 (Full Auto)」**
로 바꿨다 (`3d7f01d`). 옛말이라 바꾼 것인데, **용어집이 지정한 대응어(「상세 수집」)도 아니다** —
이 스위치는 `isActive`, 즉 **콜 잡기 전체를 켜고 끄는 것**이라 상세 수집과 다른 것을 가리킨다.

- 화면에 보이는 글자라 기사님이 **다르게 부르고 계셨다면 그 이름이 맞다.**
- 확정되면 용어집 §1 에 등재한다 (지금은 `isActive` 에 대응하는 용어가 용어집에 없다)

**③ 24시 단가 판정을 이을까 — 이으면 콜이 줄어든다**

`Hwamul24Parser` 가 `ratePerKm` 를 안 읽어 24시는 단가 판정이 **아예 없다**
(자세한 것은 아래 「💰 가격 필터는 둘 다 건다」 절). 잇는 것 자체는 한 줄이다.

🔴 **다만 잇는 순간 24시에서 통과하던 콜이 줄어든다** — 지금은 단가를 안 봐서 헐겁다.
그게 «놓치던 똥콜을 막는 것»인지 «잡던 꿀콜을 잃는 것»인지는 **표본을 봐야 안다.**
→ 먼저 셀 것: 24시로 들어온 콜 중 단가 미달이 몇 %인가 (`pnpm ledger` · `targetApp`).
⚠️ 24시는 Phase 5(파서 복구)가 먼저라, 그 뒤에 재는 것이 순서다.

**④ 실제 경로를 어느 순서로 달릴까** — 본문은 위 「🔴 그래서 남은 질문」 절에 있다.
경로(상차 먼저)와 도착 감지(길목부터)가 갈라져 있어 **도착이 안 찍히고 근접 예고가
엉뚱한 곳에서 울릴 수 있다.** 1차 측정(`pnpm route:order`)은 지금 순서가 낫다고 나왔으나,
기사님이 지적한 «주행 중 합짐» 상황은 아직 못 쟀다.
