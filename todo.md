...
좌표를 담는 표가 **없다.** 소켓으로 흘려보내고 메모리에만 산다.
1회차는 `🚨 [위치 점프]`(속도가 튈 때만 찍히는 **경고**) 줄에서 187점을 긁어냈을 뿐이다.

- [ ] 스키마 확정 (⑤-4 ①) — 어느 표에 · 얼마나 자주 · 얼마나 보관 · 누가 지우나
- [ ] *"뒤로 가는 경로 판정"* 을 따지려면 **촘촘한 궤적**이 있어야 한다
- ⚠️ 하루 7콜 × 몇 시간이면 점이 수천 개다. **간격과 보관 기간을 먼저 정한다**
