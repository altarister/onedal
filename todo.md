...
전체 기록은 [docs/필드테스트.md](docs/필드테스트.md). 여기에는 **고칠 것**만 둔다.
...
> 판정 자체를 본 게 아니다. 자세한 것은 [버그 대장 #29~#35](docs/버그_대장.md).
>
...
**기획: [docs/콜_시한과_합짐_예산.md](docs/콜_시한과_합짐_예산.md)** — 예시 검산까지 끝나 있다.
...
**말로 푼 로직: [docs/노선_고르는_법.md](docs/노선_고르는_법.md)** — 목표에서 출발해
...
⚠️ [기획서 §2](docs/콜_시한과_합짐_예산.md) 는 이 둘을 하나로 써 뒀다 — 그대로 구현하면
...
- [ ] **이름 확정** — 용어집의 `상차버퍼`·`경유버퍼`와 다른 것이라 구분되는 이름이 필요하다
      (`시한 버퍼`? `납기 버퍼`?) → [용어집](docs/용어집.md) 등재
...
- [ ] **[기획·판정]** 후속 콜 빈도 — 하차지 방향이 "다음 콜을 부르는 힘"을 판정 요소/표시로.
      재료: intel × targetApp × 시각. 근거: docs/노하우/노하우_추출2.md §1 (세 영상 공통 최대 노하우)
...
> ⚠️ **2026-08-13 오후 확정으로 이 섹션은 대체됐다** → **[docs/필터.md](docs/필터.md)**
...
[docs/troubleshooting/배차_동기화_장애.md](docs/troubleshooting/배차_동기화_장애.md) 가 **결함 #3 "이중 타이머 경쟁"**
...
`docs/architecture/안전모드_구조.md` 가 **이미 구현된 것처럼** 적어 두고 있었다
(*"시간차 6초 이상이면 수동 클릭으로 재분류합니다"*). 코드에는 없다. 2026-08-13 대조 확인.
문서에서 미구현 표시하고 계획을 여기로 옮긴다 — **구현된 것만 아키텍처 문서에 쓴다.**

**의도는 유효하다.** 원 계획서([docs/troubleshooting/수동배차_동기화장애.md](docs/troubleshooting/수동배차_동기화장애.md))의 경고:
...
근거: [docs/결정_이력.md](docs/결정_이력.md) «파생값은 계산 함수 하나에서만 나온다»
...
> **루트 `docs/작업_목록.md` 첫 줄 "디바이스에 풀오토가 자꾸 풀려"의 근본 원인**
> **예상**: 반나절 / **효과**: 체감 최대
...
- [ ] **EC2 퍼블릭 IP가 바뀌었는데 스크립트가 옛 IP를 보고 있음**
  ...
  - `docs/로컬_개발_안내.md`가 "긴급 시 이걸 쓰라"고 안내하는 수동 배포 경로가 **동작하지 않음**
