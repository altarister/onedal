...
### 🔤 옛말 검사에 구멍이 둘 있다 (2026-08-28 발견)

`glossary.test.ts` 를 믿고 *"코드에 옛말은 없다"* 고 말해 왔는데 **둘 다 틀렸다.**

**구멍 ① 주석을 안 본다** — `codeOnly()` 가 주석을 걷어내고 검사한다. 그래서 주석은 **한 번도 검사된 적이 없다.**
**구멍 ② 금지어 목록이 용어집보다 짧다** — `광클` · `무인 서핑` · `팝업 서핑` 이 빠져 있다.

전수조사 결과 주석에 **112회** (`docs/`·`log/` 제외, `.ts/.tsx/.kt/.js/.mjs`):

| 옛말 | 주석 | 판정 |
|---|---|---|
| `여유` | 71회 | ⚪ **위반 아님** — 아래 별도 항목 |
| `mainCall`/`subCalls` | 14회 | 🔴 진짜 위반 (`geoService` 4 · `dispatchEngine` 2 · `VehicleStatusPanel` 2 …) |
| `무인 서핑` | 12회 | 🔴 위반. 게다가 **`PopupSurfingMachine` 은 클래스 이름**이라 주석 밖에도 산다 |
| `광클` | 9회 | 🔴 폐기어인데 금지어 목록에 없다 (`HijackService` 3 · `shared/index.ts` 2 …) |
| `본콜` | 3회 | ⚪ *"본콜은 폐기했다"* 고 설명하는 주석 — 정당 |
| `선빵` · `hunt` | 3회 | 🔴 1회 위반(`lifecycle.mjs`). 나머지는 검사 스크립트가 패턴을 적은 것 |

**고칠 순서** — ⓐ 금지어 목록을 용어집과 맞춘다 → ⓑ 검사를 주석까지 넓힌다 → **빨간불로 몇 개가 터지는지 먼저 센다** → ⓒ 고친다.
⚠️ ⓑ 를 먼저 하면 무더기로 터져 커밋 게이트가 막힌다. **세고 나서 시작한다.**
