...
- [ ] 6. 옛 UI 일괄 삭제 + 규칙 검사·문서 갱신 + 푸시
      🔴 **전수 조사로 드러난 것**: 통째로 지울 파일은 **없다**. 새 화면(`StageView`)이 옛 화면의 본체
      `PinnedRouteBody`(406줄)를 그대로 쓰고, 그 본체가 카드·덱·지도·배지를 전부 쓴다.
      지울 것은 **겉껍데기 4줄 + `sheetOnly` 분기 4곳 + `Dashboard` 분기 13곳 + `App` 2곳 + 토글**(~200줄).
      옛 화면에만 있던 넷 중 셋(머리 줄·미니맵·판정석)은 새 화면에 대체가 있고, **탭 줄은 5-a 서랍이 받았다**
