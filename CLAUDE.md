...
- 🔴 안 적는다: **끝난 사연 · 날짜별 경위 · 이미 없어진 함정** → 커밋 메시지로
...
- ✅ 코드 주석 · `CLAUDE.md` 에는 **지금 규칙과 까닭 한 줄** — 기사님 말씀은 한 마디로, 날짜 없이
- ✅ 지금도 빠지는 함정은 «🔴 하지 말 것 + 까닭 한 줄» (기준: 그 착각에 지금도 빠지는가)
- 🔴 **날짜별 경위**(«🔄 날짜 개정» · «예전 ~였다» · 사고 경위 · 날짜 붙은 인용)는 커밋 메시지로
...
**② 안전장치는 빼지 않는다**
- 앱의 안전취소(잡은 뒤 정한 시간 안에 위약금 없이 취소)는 지우지 않는다 — 시간은 배차망마다 설정값(기본 30초 · `waitTimes`)이고, 픽커는 수락이 곧 계약이라 안전취소가 없다
...
- **만드는 도중에는 전체 검사를 돌리지 않는다** — 바꾼 파일을 읽는 검사만 돌린다(`grep -rl <경로> onedal-web/server/tests` 로 찾는다 —
  검사가 소스 글자를 읽으므로 이름으로는 못 찾는다). 전체 게이트는 **커밋 직전 한 번**. `pnpm` 명령은 jest 와 동시에 돌리지 않는다
