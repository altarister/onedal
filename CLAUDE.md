...
## 코드 주석 · 문서에 무엇을 적나 — 한 가지는 한 곳에만

- ✅ 코드 주석 · `docs/지금/` · `CLAUDE.md` 에는 **지금 규칙과 까닭 한 줄** — 기사님 말씀은 한 마디로, 날짜 없이
- ✅ 지금도 빠지는 함정은 «🔴 하지 말 것 + 까닭 한 줄» (기준: 그 착각에 지금도 빠지는가)
- 🔴 **날짜별 경위**(«🔄 날짜 개정» · «예전 ~였다» · 사고 경위 · 날짜 붙은 인용)는 [버그 대장](docs/기록/버그_대장.md) · 커밋 메시지로
- 검사 파일 머리는 «무엇을 막나» 두세 줄
- 걷을 때: 대장에 없는 사고는 옮긴 뒤 걷는다 · 그 파일을 읽는 검사를 `grep -rl <경로>` 로 모아 돌린다 ·
  주석까지 든 원문을 위치·길이로 자르는 검사를 만나면 주석을 걷어낸 원문(`codeOnly`)으로 바꾼다
- ✅ `pnpm audit:docs` ⑥ 이 문다 — 바꾼 파일의 경위 줄이 HEAD 판보다 늘면 빨간불. 이미 쌓인 줄은 그 파일을 만질 때 줄인다
...
| `docs/아카이브/` | 안 읽는다 | **끝난 계획 · 한 번 쓴 안내문 · 옛 판 초안** — 지금 기준과 다른 관점이 섞여 있다. 목록은 `docs/아카이브/README.md` |
| `docs/비공개/` | — | 외부 공유 금지 |
