...
> 🔴 **"사냥"이라는 말을 쓰지 않는다** (기사님 2026-08-16):
> *"업계 사람들이 사냥이라 하면 알아먹을까? 그냥 **콜 필터**로 하고 사냥이란 단어는 앞으로 쓰지 마."*
> 새 코드·문서·대화에서는 **콜 필터**라고 쓴다. (기존 코드의 `huntPhase` 식별자는 아직 안 바꿨다)
...
타겟명은 `huntPhase`(DEST·LOCAL·HOME), 인덱스는 `getActiveCalls()` 순서와 대응한다.
