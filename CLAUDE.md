...
문서(`CLAUDE.md` · README · 스킬 · 코드 주석)는 쓰기 전에 코드와 대조한다.
`pnpm audit:docs` 는 없는 파일·식별자·링크를 잡지만 **문장이 틀린 것은 못 잡는다.**

📅 임계 경로는 코드가 아니라 **사업자**다.
...
- 🔴 지우는 것은 **손으로, 의도적으로** 한다 — 서버가 켜질 때 조건을 보고 표를 지우는 코드를 새로 만들지 않는다.
  `onedal-web/server/src/db.ts` 에 옛것이 둘 남아 있다 (`order_judgments` 재구성 · `orders` 상태값 변환 V7 — 지금 DB 에서는 조건이 안 맞아 안 돈다)
...
- **운행 동작이 바뀌거나 새 설정값·저장 칸이 생기는 수정**은 근거를 제시하고 합의된 뒤에 한다 — 순서·방법·검사·커밋은 묻지 않는다
