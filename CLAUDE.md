...
- 🔴 **고를 일이 아니면 묻지 않고 «이렇게 합니다»로 정해 쓴다** — 부딪힌다고 느끼면 부딪히는 두 문장을 나란히 놓고 내 결정과 까닭 한 줄을 적는다.
  기사님이 정하실 것은 운행 동작이 바뀌는 것과 새 설정값뿐이다 (아래 「일하는 순서」)
...
- **만드는 도중에는 검사를 돌리지 않는다** — 「버그는 순서를 뒤집어 고친다」의 **빨간불 확인 한 번**과 **커밋 직전 게이트 한 번**이면 족하다.
...
- 🔴 **새 폴더를 만들기 전에 기사님께 묻는다** — 「구성」 표와 실제 폴더가 갈라지면 이 문서가 매 세션 틀린 구성을 가르친다
