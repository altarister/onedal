 ...
 * 누가: 에이전트
 * 언제: 화면을 고친 뒤 기사님과 같은 것을 보려 할 때
 * 어디서: cd onedal-web && pnpm shot '<경로>' <png 파일>
 * 무엇을: 한 화면을 폰 폭으로 찍어 png 로 남긴다
 * 왜: 코드만 읽고 «옮겼다»고 하면 틀린다 — 찍은 것을 열어 본다
 * (잡는 것 · 못 잡는 것 · 검수는 onedal-web/CLAUDE.md 스크립트 표)
 *
 *
 * 기사님: *"목업에 잘 만들어져 있는 걸 네가 잘 보지
