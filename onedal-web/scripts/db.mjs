#!/usr/bin/env node
/**
 * 📖 **DB 보기 — 입구 하나, 무엇을 볼지는 인자로** (기사님 지시).
 * 누가: 에이전트 · 기사님
 * 언제: DB 에 무엇이 어떻게 저장됐는지 볼 때
 * 어디서: cd onedal-web && pnpm db <이름> (인자 없이 부르면 목록)
 * 무엇을: 입구 하나로 `lib/db-*.mjs` 를 골라 띄운다
 * 왜: 보는 도구가 명령 여럿으로 흩어지면 아무도 못 찾는다
