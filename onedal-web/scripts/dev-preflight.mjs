#!/usr/bin/env node
/**
 * 🚦 **띄우기 전에 자리를 확인한다** (기사님 확정)
 * 누가: `pnpm dev` 가 먼저 부른다
 * 언제: 서버를 띄우기 직전
 * 어디서: pnpm dev 안에서 자동 (직접은 node scripts/dev-preflight.mjs)
 * 무엇을: 포트 자리에 옛 서버·감시자가 남아 있나 보고, 있으면 죽일 명령을 적어 주고 멈춘다
 * 왜: 겹쳐 뜨면 묵은 코드가 돌아 «고쳤는데 안 돈다»가 된다
 * (잡는 것 · 못 잡는 것 · 검수는 onedal-web/CLAUDE.md 스크립트 표)
 *
 *
 * 이 검사가 지키는 사고: 감시자(`tsx watch`)가 여럿 쌓이면, 포트를 못 잡은 것이 `lsof :4000` 에
 * **안 보인 채로** 놀다가 자리가 비는 순간 자식을 띄워 **묵은 코드로 서버를 되살린다.**
 * 기사님 화면에서는 방금 고친 것이 안 돌고, 그걸 모른 채 판단하게 된다.
 *
...
const PORTS = [
    ...
    { port: 3000, who: '관제웹' },
    /* 🔌 `pnpm dev` 가 함께 띄운다 (루트 README.md 「포트」) */
    { port: 3001, who: '운행일지' },
];
