...
const KEEP = {
     ...
     *    「안 불린다 = 필요 없다」로 읽어 지우면 그 생각까지 사라진다 (버그 대장 #61).
     *    todo 「관제웹 전면 개편」 에 «이어야 할 것»으로 올려 뒀다.
     */
    MovingBadge: '잇는 자리가 빠졌다 — 현황판으로 간다 (todo). 모의/실측을 가리는 유일한 표시라 안 지운다.',
    /* 🔄 #133 개정 (2026-09-15) — 모의 주행이 «집으로 떠나기»를 안 하게 되며 아무도 안 부른다. 파일 지우기는 기사님 말씀 때만이라 남겨 둔다 */
    fetchHomeLeg: '안 부른다(#133 개정). 파일 지우기는 기사님이 말씀하실 때만 — 그때 homeLeg.ts · homeLeg.test.ts 와 함께.',
    /**
     * 🔴 **틀린 답을 주게 됐다 — 그래서 옮겨 갈 자리를 적어 둔다** (2026-09-05).
     *
    ...
    /**
     * 2026-09-09 에 지도 실험실의 「길 찾기」를 걷어내며 화면 쪽 호출부가 사라졌다
     * (기사님: *"길찾기는 지워"* — 이제 **잡은 콜들이 라인을 만든다**).
     * 파주 길은 카카오 실측 폴리라인이라 다시 만들려면 카카오를 또 불러야 한다.
     * 여주 길(`YEOJU_ROADS`)은 라인 띠 검사가 쓰고 있고, 이건 그 검사를 파주로 넓힐 때 쓴다.
     */
    PAJU_ROADS: '파주 길 카카오 실측 폴리라인. 길 찾기를 걷어내며 화면에서 빠졌다 — 라인 띠 검사 재료로 남긴다.',
    /**
     * 2026-09-04 에 `/navi`(어제 만든 임시방편)를 지우면서 호출부가 사라졌다.
     * **개인폰이 받는 화면은 새로 만든다** (docs/기획/경로.md §4-0-1) —
    ...
    /**
     * 🔴 **아래 다섯은 «지울 게 아니라 이어야 할 것»이다** (2026-08-29 전수조사).
     *    2026-08-20 에 «타입만 세워 두고 연결은 다음 단계에서» 라고 스스로 적어 둔 것들이다.
     *    그날 예고한 증상이 **2026-08-29 에 실제로 커졌다** — `getStopTiming` 의 인자가
     *    또 늘었고(`unk`·`order`·`cfg`), 「화면과 판정이 다른 값」 사고(#67②)가 났다.
     *    잇는 일은 [todo.md] 에 크기와 함께 적어 뒀다. **캐 봤고, 이래서 남긴다.**
     */
    EMPTY_CARGO: '짐을 묶는 이름(`CargoSpec`)의 바닥값. 지금 코드는 짐을 낱개로 들고 다녀 '
               + '함수 인자가 계속 는다 — 그 병을 고치려고 2026-08-20 에 세워 둔 구조다.',
    ...
    effectiveCargo: '「지금 무엇을 믿어야 하나」(실측 > 계획)를 한 곳에서 판단한다. '
                  + '지금은 읽는 쪽마다 따로 해서 화면과 판정이 다른 값을 볼 수 있다 — 2026-08-29 #67② 가 그 사고다.',
    ...
    groupCallOptions: '콜 옵션을 갈래별로 묶어 화면이 그리게 하려던 것. DB 표(`call_options`)와 '
                    ...
                    + '화면을 만들지 말지는 기사님 판단이라 todo 에 적어 두고 남긴다 (2026-08-29 캠).',
};
