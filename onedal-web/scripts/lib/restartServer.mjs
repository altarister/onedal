/**
 * 🔄 **서버를 다시 띄운다 — 감시자를 깨워서**
 * 누가: `reset-calls.mjs` 가 부른다
 * 언제: 장부를 비운 직후
 * 어디서: 직접 부르지 않는다
 * 무엇을: 감시자(`tsx watch`)가 보는 파일의 수정 시각을 건드려 서버를 다시 띄운다
 * 왜: 장부를 비워도 세션 메모리의 콜은 남는다
 ...
 *
 * 🔴 왜 필요한가: 장부(`orders`)를 비워도 **세션 메모리의 옛 콜은 남는다.** 사람에게 «재기동해 주세요»
 *    하고 맡기면 지워진 콜이 관제웹에 유령으로 남는다. **장부를 비우는 쪽이 메모리도 함께 비운다.**
 */
