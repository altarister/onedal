 ...
 * 누가: `pnpm db track` 이 부른다
 * 언제: GPS 궤적이 콜에 붙었는지, 끊긴 구간이 있는지 볼 때
 * 어디서: cd onedal-web && pnpm db track [콜 id 앞부분]
 * 무엇을: 궤적의 점 · 끊긴 구간 · 상하차지 근접을 보여 준다
 * 왜: 도착 감지의 검증 재료가 여기 있다
 * (잡는 것 · 못 잡는 것 · 검수는 onedal-web/CLAUDE.md 스크립트 표)
 *
 *
 * «궤적에 콜이 붙는가»를 확인할 때 묻는 질문이 그대로 이 도구다 —
