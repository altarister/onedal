 ...
 * 누가: `pnpm reach sweep` 이 부른다
 * 언제: 도달 계수를 확정하려고 거리 구간을 고르게 덮은 표본이 필요할 때
 * 어디서: cd onedal-web && pnpm reach sweep
 * 무엇을: 기준점 쌍을 카카오로 재어 `reach_samples` 에 표본을 쌓는다 (카카오 API 비용)
 * 왜: 콜이 떠야만 쌓이는 표본은 느리고 근거리에 쏠린다
