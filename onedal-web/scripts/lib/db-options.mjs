 ...
 * 누가: `pnpm db options` 가 부른다
 * 언제: 화면 값이 이상할 때 그 계산의 재료를 볼 때
 * 어디서: cd onedal-web && pnpm db options [갈래]
 * 무엇을: 콜 옵션 선택지와 지금 값을 보여 준다
 * 왜: 단위 환산 · 상하차 분 · 보호 시간이 여기서 나온다
 * (잡는 것 · 못 잡는 것 · 검수는 onedal-web/CLAUDE.md 스크립트 표)
 *
 *
 * 기사님: *"db 를 터미널에서 보려니 너무 복잡하다."*
 *
