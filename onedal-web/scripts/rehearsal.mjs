...
const PRESETS = [
    ...
    /**
     * 🧭 **경로 순서 — `pnpm drive` 가 검사하는 그 상황을 화면으로 본다** (2026-08-29).
     *
     * `drive` 는 기계가 ✅/🔴 를 찍고 끝난다. 눈으로 확인하려면 같은 상황을 화면에
     * 만들어야 하는데, 기존 문제지 16종에는 **그 상황이 없었다** (전부 광주→파주 축이라
     * «가까운 하차지 + 먼 상차지» 가 안 생긴다).
     *
     * 2026-08-25 기사님 실측 그대로 — 곤지암 하차가 4km 앞인데 가남 상차가 30km 밖:
     * ```
     *   17 을 올려 KEEP → 상차 완료   (하차지가 곤지암, 가깝게 남는다)
     *   18 을 올려 KEEP               (상차지가 가남, 30km 밖)
     *   → 관제웹 방문 순서가 «곤지암 하차» 부터여야 한다
     * ```
     * 🔴 «가남 상차» 가 먼저 나오면 옛 규칙(상차 먼저)이 돌고 있는 것이다.
     *    그때는 서버가 옛 코드인지부터 본다 (리허설 시작 점검이 이미 보지만).
     *
     * ⚠️ 18 번은 여주라 콜 필터(파주 노선)에 막힌다 — "그래도 올릴까요"에 **y**.
     *    여기서 보려는 것은 필터가 아니라 **순서**다.
     */
    { key: '17', label: '🧭 순서① 곤지암 상차 → 곤지암 하차 (3만/다마스) — 먼저 KEEP 하고 **상차 완료**까지',
      pickup: pick('곤지암성당'), dropoff: pick('쿠팡 곤지암1센터'), fare: 30000, vehicleType: '다마스' },
    { key: '18', label: '🧭 순서② 여주 가남 상차 → 이천 (4만/다마스) — 17 다음에 올린다. 상차지가 30km 밖',
