...
const PRESETS = [
    ...
    /**
     * 🧭 **`pnpm drive` 와 «같은 콜»** — 검사가 통과한 그 상황을 화면으로 본다 (2026-08-29).
     *
     * 좌표·요금·차종이 `drive` 와 같다. 그래서 **검사 결과와 화면을 나란히 놓고** 볼 수 있다.
     * ```
     *   집 ─2.5km─ 초월읍상차 ─4.9km─ 곤지암하차 ─26.0km─ 가남상차 ─6.8km─ 세종대왕면하차
     *                                    ↑
     *              합짐이 붙는 순간 여기서 3.6km 앞 · 가남은 29.2km 밖
     *              길목부터 36.4km  vs  상차먼저 59.5km  → 23.1km 차이
     * ```
     * ⚠️ **17 을 먼저, 출발한 뒤에 18 을 올린다.** 순서가 뒤바뀌면 «주행 중 합짐»이 아니라
     *    «처음부터 둘 다 아는» 상황이 되어 8/25 사고와 다른 그림이 된다.
     */
    { key: '17', label: '🚚 [1] 첫짐 · 초월읍 → 곤지암 (5만/1t) — KEEP 하고 **출발**까지 누른다',
      pickup: pick('초월읍행정복지센터'), dropoff: pick('쿠팡 곤지암2센터'), fare: 50000, vehicleType: '1t',
      start: { label: '집 (초월읍 서쪽 2.5km)', lat: 37.3900, lng: 127.2600 },
      expect: [
        '판정 색 — 🔵 또는 🟢 (가까운 첫짐이라 우회가 없다)',
        'KEEP → **출발** 까지 누른다. 목업 주행이 곤지암 쪽으로 달리기 시작한다',
        '내가 위치를 지켜보다가 **곤지암 3.6km 앞**에서 18 번을 올린다',
      ] },
    { key: '18', label: '🚚 [2] 합짐 · 가남 → 세종대왕면 (5만/1t) — **달리는 중에** 올린다 (29.2km 밖)',
      pickup: pick('무신사로지스틱스 여주3물류센터'), dropoff: pick('쿠팡 여주 VF61'), fare: 50000, vehicleType: '1t',
      expect: [
        '🔴 **여기가 2026-08-25 사고가 난 자리다.**',
        '   3.6km 앞에 내릴 짐이 있는데 29.2km 밖 상차지로 끌려가던 그 순간이다',
        '',
        '판정 색 — 🟡 이 정상이다 (29km 를 더 가는 큰 우회)',
        'KEEP 을 누르면 방문 순서가 다시 짜인다. 그 순서를 본다:',
        '  ✅ 곤지암 하차 → 가남 상차 → 세종대왕면 하차   (36.4km · 지금 규칙)',
        '  🔴 가남 상차 → 세종대왕면 → 곤지암 하차        (59.5km · 옛 규칙 = 실패)',
        '',
        'CANCEL 을 눌러도 된다 — 그때는 첫짐 경로가 그대로 남는지를 본다',
      ] },
];
