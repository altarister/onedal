...
async function inject(t) {
     ...
     */
    /**
     * 🔴 **낡은 필터로 판단하지 않는다** (2026-08-29 실측 · 세 판을 날리고 찾음).
     *
     * `lastFilter` 는 5초마다 갱신된다. 그런데 서버는 **콜을 선점하는 동안**
     * `isActive` 를 잠깐 끈다(폰이 그 사이 다른 콜을 물지 않게). 그래서 KEEP 직후
     * 합짐을 올리면 **1초 전에 다시 켜진 값을 못 보고** «콜 잡기 OFF» 로 막았다.
     *
     * ```
     * 20:11:48.9  서버: isActive → false   (선점 중)
     * 20:11:51.5  KEEP → 서버가 다시 true
     * 20:11:52.8  합짐 올림 → 리허설은 5초 전 값(false)을 보고 차단
     * ```
     *
     * 🔴 게다가 그 «그래도 올릴까요?» 질문이 **다음 명령을 답으로 먹어서** 합짐 둘이
     *    통째로 사라졌다. 판을 세 번 날린 뿌리다.
     *
     * → **올리기 직전에 한 번 더 받아 온다.** 앱도 매 통신에서 최신 필터를 받는다.
     */
