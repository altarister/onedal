...
const PRESETS = [
    ...
    /**
     * 🧭 **경로 순서 — `pnpm drive` 와 «같은 조건»을 화면으로 본다** (2026-08-29).
     *
     * 🔴 `drive` 는 기계가 ✅/🔴 를 찍고 끝난다. 눈으로 보려면 **같은 자리에서 같은 콜**을
     *    올려야 비교가 된다. 그래서 좌표·현위치를 `drive` 와 맞췄다:
     * ```
     *          drive (검사)                      리허설 17·18 (화면)
     *   현위치  127.29, 37.37                     같음 — 17 번이 GPS 를 거기로 옮긴다
     *   A      곤지암 상차 → 하차 (이미 상차)     17  곤지암성당 → 쿠팡 곤지암1센터
     *   B      가남 상차 (30km 밖)               18  무신사 여주3센터 (가남)
     *   기대    A하차 → B상차 → B하차             같은 순서가 화면에 나와야 한다
     * ```
     * 거리 검산 (현위치 기준): **곤지암 하차 6.6km · 가남 상차 29.9km**
     *
     * ⚠️ **순서대로 해야 한다.** 17 을 «상차 완료»까지 눌러야 그 하차지가 «가까운 정거장»으로
     *    남는다. KEEP 만 하면 상차지가 아직 남아 그림이 달라진다.
     */
    { key: '17', label: '🧭 순서① 곤지암 (3만/다마스) — 📍현위치를 초월읍으로 옮기고 올린다. KEEP 후 **상차 완료**까지',
      pickup: pick('곤지암성당'), dropoff: pick('쿠팡 곤지암1센터'), fare: 30000, vehicleType: '다마스',
      start: { label: '초월읍 (drive 의 현위치와 같은 자리)', lat: 37.3700, lng: 127.2900 },
      expect: [
        '판정 색 — 🔵 또는 🟢 (짧고 가까운 콜이라 우회가 없다)',
        'KEEP 을 누르고 **상차 완료**까지 누른다 → 하차지만 6.6km 앞에 남는다',
      ] },
    { key: '18', label: '🧭 순서② 여주 가남 (4만/다마스) — 17 을 상차 완료한 **뒤에** 올린다. 상차지가 29.9km 밖',
      pickup: pick('가남읍'), dropoff: pick('동원대학교'), fare: 40000, vehicleType: '다마스',
      expect: [
        '앱 필터 — 🔴 차단이 정상이다 (여주는 파주 노선 밖). "그래도 올릴까요"에 **y**',
        '   → 여기서 볼 것은 필터가 아니라 **순서**다',
        '판정 색 — 🟡 이 정상이다 (30km 를 되돌아가는 큰 우회)',
        '🔴 **성공 판정** — 관제웹 방문 순서가',
        '     ⑴ 곤지암 하차  ⑵ 가남 상차  ⑶ 동원대 하차   ← 이러면 맞다',
        '     ⑴ 가남 상차 가 먼저 나오면 **옛 규칙**이다 (실패)',
        '   서버 로그로도 보인다:  🧭 [경로 순서] ⑴ …하차 ⑵ …상차',
      ] },
];
