...
try {
    ...
    if (realGrid?.length && realGrid.length === mockGrid.length) {
        /**
         * 🔴 **무엇을 견주고 무엇을 안 견주나** — 여기서 한 번 정직해야 한다.
         *
         *   견준다  · **지명 매핑** — 같은 콜이 같은 상·하차를 말하는가
         *           · **순번의 꼴** — 1..2N 이 빠짐없이 이어지고, 한 콜 안에서 상차가 하차보다 앞인가
         *   안 견준다 · **시각**(약속·±·예상) — 양쪽의 «지금»이 다른 판이다
         *           · **상차끼리의 앞뒤** — 🔴 재배치는 «내 위치»에서 가까운 것부터다.
         *             목업은 대전 진차이나에서, 실물은 기사님 GPS 자리에서 잰다 — 두 상차지가
         *             5km 안에 붙어 있으면 **어느 쪽이 먼저인지가 정당하게 갈린다.**
         *             그걸 실패로 세면 «떨어질 줄만 아는 검사»가 되어 아무도 안 본다.
         *             달라지면 **알리되 실패로 치지 않는다** (2026-09-11 실측으로 배운 것).
         */
        for (const [i, cells] of realGrid.entries()) {
            ...
            const rp = +r.상[0], rd = +r.하[0];
            ok(`${i + 1}번 줄 — 상차가 하차보다 앞이다`, rp < rd, `실물 ${rp} → ${rd}`);
            if (r.상[0] !== m.상[0] || r.하[0] !== m.하[0])
                console.log(`  ℹ️ ${i + 1}번 줄 순번이 다릅니다 — 실물 ${r.상[0]}·${r.하[0]} ↔ 목업 ${m.상[0]}·${m.하[0]}`
                    + ` (재배치 기점인 «내 위치»가 달라서다 — 실패 아님)`);
        }
        /** 🔢 순번은 양쪽 각각 **1부터 빠짐없이** 이어져야 한다 — 그건 위치와 무관한 불변이다 */
        const seqOf = g => [...new Set(g.flatMap(c => [+c[0], +c[6]]))].sort((a, b) => a - b);
        for (const [name, g] of [['실물', realGrid], ['목업', mockGrid]]) {
            const q = seqOf(g);
            ok(`${name} 순번이 1부터 빠짐없이 이어진다`, q.every((v, i) => v === i + 1), JSON.stringify(q));
        }
