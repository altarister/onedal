 ...
 * 누가: 에이전트 — 앱의 스크래핑·터치·확정 흐름을 고친 뒤
 * 언제: 폰에 앱이 깔려 있고 시뮬레이터(`cd onedal-sim && pnpm dev`)가 떠 있을 때
 * 어디서: cd onedal-web && pnpm e2e:app
 * 무엇을: 크롬으로 시뮬레이터를 열고 logcat 을 지켜보며 `STEPS` 배열의 단계가 차례로 일어났는지 판정한다
 * 왜: 앱이 화면을 읽고 터치하고 서버에 보고하는 구간은 다른 검사가 못 본다
 * (잡는 것 · 못 잡는 것 · 검수는 onedal-web/CLAUDE.md 스크립트 표)
 *
 *
 * `pnpm scenario` 는 서버·관제웹만 본다. 앱이 화면을 읽고 → 필터를 걸고 → 터치하고 →
 * 서버로 보내는 구간은 이 검사만 본다.
 *
 * 레포 안 배차망 시뮬레이터(`onedal-sim/`)를 **크롬으로** 띄우면 앱이 그대로 스크래핑하고 터치까지 한다.
 * 통과하면 logcat 에 이런 줄이 차례로 찍힌다:
 *
 ...
 *
 * ⚠️ 반드시 크롬으로 열어야 한다. 까닭은 onedal-web/CLAUDE.md 의 «반드시 크롬으로 열어야 한다» 절에 있다.
 *
 * 이 스크립트가 하는 일: 크롬으로 시뮬레이터를 열고, logcat 을 지켜보며
 * 아래 `STEPS` 배열의 단계가 차례로 일어났는지 **단계별로 판정**한다.
 */
...
const STEPS = [
    ...
    { name: '상세 화면 진입',              re: /화면: DETAIL_PRE_CONFIRM/ },
    {
        ...
        /**
         * 🔴 이 단계가 지키는 사고: 자동 터치 직후 LIST 오탐으로 세션이 리셋되면 AUTO 가 MANUAL 로 뒤바뀐다.
         * MANUAL 은 안전취소 없이 즉시 확정되고 아무도 안 치워 유령 콜이 남는다.
         * 이 줄이 이 검사에서 가장 중요하다.
         */
    },
