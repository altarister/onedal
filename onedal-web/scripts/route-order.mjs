 ...
 *
 *
 * **도착 계획**(`planArrivalStops`)은 «지나가는 길목부터»다 — 기사님 실측: *"기사님 위치에서 곤지암 하차 4.0km ·
 * 가남 29.9km 인데 순서가 ⑴가남상차 ⑵가남하차 ⑶세종대왕면하차 ⑷곤지암하차(94분) 로 나왔다"*.
 *
 * 그런데 **실제 카카오 요청**(`planMergedStops`)은 «상차 전부 먼저»다.
 * 두 계획이 갈라져 있으면 구간 주행분이 남의 이름에 붙는다. **어느 순서로 달릴 것인가**가 남은 물음이다.
 *
