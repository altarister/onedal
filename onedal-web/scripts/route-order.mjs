 ...
 * 누가: 에이전트
 * 언제: 경로 순서 규칙을 바꾸기 전에
 * 어디서: cd onedal-web && pnpm route:order
 * 무엇을: 장부의 실제 콜 조합으로 「상차 먼저」와 「지나가는 길」의 직선 총주행을 나란히 잰다
 * 왜: 경로를 바꾸는 일은 숫자를 보고 정한다
 * (잡는 것 · 못 잡는 것 · 검수는 onedal-web/CLAUDE.md 스크립트 표)
 *
 *
 * 지금 경로 순서는 **«지나가는 길목부터»**다 — 실제 카카오 요청(`planMergedStops`)과 도착 계획이 같은
 * `orderByNearest` 를 쓴다. 이 도구는 **«상차 전부 먼저» 규칙**과 지금 규칙의 직선 총주행을 나란히 재어,
 * 규칙을 다시 바꾸려 할 때의 숫자를 준다. (기사님 실측: *"곤지암 하차 4.0km · 가남 29.9km 인데 순서가
 * ⑴가남상차 ⑵가남하차 ⑶세종대왕면하차 ⑷곤지암하차(94분) 로 나왔다"* — «상차 먼저» 규칙의 모습이다)
 *
...
/** «상차 먼저» 규칙 — 상차 전부 먼저, 각각 최근접 (optimizeWaypoints). 지금 코드가 아니라 견줄 기준이다 */
