 ...
 * 누가: 에이전트 · 기사님
 * 언제: 도달 계수(직선 km → 카카오 분)를 다시 볼 때
 * 어디서: cd onedal-web && pnpm reach / pnpm reach sweep
 * 무엇을: `reach_samples` 로 계수를 역산한다. sweep 은 표본을 쌓는다
 * 왜: 앱 필터의 반경 근사가 실측과 맞는지는 숫자로만 판단할 수 있다
 ...
 * (필터 확정안 v2 ②값). 잠정 계수는 1.5분/km 였는데, 실측 첫날에
 * 이미 틀린 모델임이 보였다:
 *
...
/**
 * 🔀 **`pnpm reach sweep` — 표본 쌓기는 같은 입구의 갈래다** (기사님 지시).
 *    «표본 쌓기 → 역산»은 한 흐름이라 입구를 하나로 둔다.
