...
/**
 * 📷 **픽커 상세를 그림으로 읽는다 — 실측 한 장을 통째로 문다**.
 *
 * ── 문제지의 출처 ──
...
describe('픽커 상세를 그림으로 읽는다', () => {
     ...
     *    화면에는 16.9 와 10.1 밖에 없다 — 셋 중 어느 것이 무엇인지 알 길이 없다.
     */
    ...
    /**
     * 🔴 **예약 콜이라는 축은 그림에서만 읽힌다.** 접근성 트리에는 이 줄이 없어서,
     *    여기서 못 읽으면 내일 15:00 콜을 **지금 콜로 알고** 판정한다.
     */
     ...
     *    뒤집혔을 때 「머리 아래부터」로 자르면 **행정동이 통째로 사라져 `null` 이 된다** —
...
/**
 * 🔬 실측 2 — A24 · 폰 안 ML Kit 출력 그대로 (아래 60% 를 540폭으로 자른 그림).
 *    실측 1과 다른 점: **오늘 콜**이라 시각이 「10:00까지 픽업」·「12:39까지 배송」 줄로 온다.
 *    이 줄을 시각으로 못 잡으면 **건물명 자리에 「10:00까지 픽업」이 들어가고 시각은 null** 이 된다.
 */
...
/**
 * 📷 **머리가 줄 «가운데»에 오는 모양** — 실측 실패 (이상 기록 #46·#48·#39).
 *
 * 픽커 상세에는 모양이 둘이다. 하나는 「픽업 2.2km」가 줄 처음에 오고, 다른 하나는 **주소 뒤에** 붙는다.
 * 정규식이 `^` 로 줄 처음만 보면 뒤 모양을 통째로 놓친다 — 사진에 배송지가 또렷이 찍혀 있어도
 * 「머리 둘 누락」으로 버리고 접근성이 읽은 반쪽만 서버로 간다. 그 콜은 하차지가 빈 글자라 좌표를 못 찾는다.
 *
 * 🔴 지도 라벨 방어는 «먼저 나오는 것 하나만» 이다 — `^` 는 그 방어가 아니다.
