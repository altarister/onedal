 ...
 * ── 무엇을 막나 ──
 *
 * 첫짐만 잡은 채 합짐 후보를 선점하면(아직 KEEP 전) 재탐색이 **후보를 붙여 다시 잰 4구간 경로**를
 * 만든다. 그 주행분이 첫짐의 것인 양 나가면 첫짐 하차가 «25분 → 39분»처럼 늘어 보인다.
 * 정거장 목록에는 후보가 **없는데**(2곳), 주행분은 후보를 포함한 4구간 경로에서 온다.
 * 기사님 눈에는 *"경로는 그대로인데 시간만 늘었다"* 로 보인다.
 *
 * ── 왜 길이 검사로는 못 막나 ──
 *
 * 주행분은 **이름으로 맞춘다**(`orderId|stopType`) — 도착해 정거장이 빠져도 남은 주행분이 살게 하려는 것이다.
 * 그래서 «구간 수 ≠ 정거장 수» 검사가 이 섞임을 걸러 주지 않는다 —
 * **4구간 경로가 2정거장 질문에 그대로 답한다.**
 *
 ...
 * ── 어떻게 가르나 ──
 *
 * 경로가 계산된 정거장(`sectionStops`) 중 **활성 콜도, 끝난 콜도 아닌 것**이 있으면 후보가 섞인 것이다.
...
describe('🧭 후보가 섞인 경로의 주행분', () => {
    ...
    /**
     * 🔴 **하차까지 마쳐 빠진 콜도 후보가 아니다**.
     *
     * 위 검사는 «상차를 다녀온» 콜만 본다 — 그 콜은 여전히 활성이라 걸리지 않는다.
     * 그런데 **하차를 마친 콜은 활성 목록에서 빠진다.** 경로가 계산될 때는 그 콜의 하차지가
     * `sectionStops` 에 있었으므로, «활성 콜이 아닌 정거장 = 후보»로만 세면 **방금 끝난 콜이
     * 후보로 잡혀** 남은 정거장의 주행분이 전부 빈다
     * (서버 로그: `⚠️ 길이 어긋남(…) → 전부 null ⚠️ 후보 포함 경로(…)`).
     * 그러면 다음 카카오 호출이 채울 때까지 관제웹 타임라인이 빈다. 그래서 끝난 콜(`isTerminal`)은 후보에서 뺀다.
     */
