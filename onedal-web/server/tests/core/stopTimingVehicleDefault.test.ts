...
/**
 * 🚚 **차종을 알면 짐을 안다 — 판정이 「모른다」고 하면 안 된다** (기사님 지적)
 *
 ...
 * ── 왜 차종에서 짐을 뽑나 ──
 *
 * 단계 장부의 행은 KEEP 해야 태어난다 (`birthFirstStep`). **판정은 KEEP 하기 전에 난다** — 색을 보고 KEEP 을 누르니까.
 * 태어난 행만 읽으면 판정 시점에는 장부가 늘 비어 있어, 정차 시간이 언제나
 * 「미확인 일반값」(상차 15분·하차 10분)으로 떨어진다.
 *
 * 🔴 **차종은 그 전부터 안다.** 앱이 콜을 집을 때 이미 실려 온다.
 *    단계 사슬(`computeChain`)이 차종 기본 짐(`defaultCargoByVehicle`)으로 정차 시간을 담아 두고,
 *    `getStopTiming` 은 장부가 비어도 그 값을 읽는다 (규칙 ③: 같은 값을 한 곳에서).
 *
 * ── 차종별 정차 (실측) ──
 *
 * | 차종 | 짐 | 상차 | 하차 | 합 | 일반값(25분)으로 돌면 |
 * |---|---|---|---|---|---|
 * | 승용차 | 라면박스 5 · 수작업 | 6 | 3 | **9분** | 16분 **과대** |
 * | 1t | 파레트 2 · 지게차 | 8 | 5 | **13분** | 12분 **과대** |
 * | 다마스 | 라면박스 30 · 수작업 | 14 | 11 | **25분** | 우연히 같다 |
 * | 2.5t | 파레트 6 · 지게차 | 16 | 13 | **29분** | 4분 과소 |
 * | 라보 | 라면박스 40 · 수작업 | 17 | 14 | **31분** | 6분 과소 |
 * | 5t | 파레트 12 · 지게차 | 30 | 27 | **57분** | 32분 **과소** |
 *
 * ⚠️ **1t 이 다마스보다 빠르다** — 파레트는 지게차로 뜨고 박스는 손으로 나른다.
 *    일반값은 그 차이를 통째로 지운다.
 *
 * 🔴 **방향이 차종마다 다르다** — 늘 나쁘게 나오는 것이 아니다. 1t 은 지게차라 빠른데
 *    수작업 일반값을 물리면 **우회가 비싸 보이고**, 5t 은 반대로 **싸 보인다.**
 *    색이 곧 결정이므로(규칙 ⑤-3) 양쪽 다 사고다.
 *
 * ⚠️ **다마스는 우연히 25분으로 같다** — 가장 흔한 차종이라 합만 보면 티가 안 난다.
 */
 ...
 * 실제 판정은 그대로 「모른다」**가 된다 — 시나리오 14번(승용차)에
 * 「정차 미확인(일반값)」 딱지가 그대로 붙는 모양이다.
 *
...
maybe('판정 시점의 정차 시간 — 신고가 없어도 차종에서 나온다', () => {
    ...
    it('우회 비용도 같이 움직인다 — 판정이 쓰는 것은 이 값이다', () => {
        ...
        // 1t 은 지게차라 빠르다 — 일반값으로 돌면 우회가 **12분 비싸** 보인다
