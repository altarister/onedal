...
/**
 * 📱 **테스트 시작 전 점검 — 폰이 «실제로 쓰는 값»을 서버 값과 비교한다** (기사님 지시)
 *
 * 시작 전 점검이 «서버가 정한 값»만 보면 **테스트 조건이 틀어져 있어도 초록**이 된다.
 * 그러면 콜이 안 잡힌 까닭을 시스템이 옳게 거른 것으로 잘못 읽는다. 이런 경우다:
 * ```
 * 폰이 서버 응답을 못 읽어 앞 필터 · 직접 모드(MANUAL)로 돈다   ← 서버는 폰이 보낸 모드·지문을 들고 있다
 * 폰에 실제로 적용된 상차 반경은 4.55km (자동)                ← 설정값은 10km 다
 * ```
 * 폰은 통신마다 «적용 중인 모드»와 «들고 있는 필터 지문»을 보낸다. 서버는 그 순간 «보낼 지문»을 계산한다.
 * 점검은 **그 둘을 비교한다** — 이 검사가 그 비교를 잠근다.
 */
