...
describe('🧭 판정까지 이어진다', () => {
    const 색 = (progress: ReturnType<typeof destProgressOf>) => judge(CRITERIA, firstLoadFacts({
        fare: 12_000, totalMinutes: 65, progress, excludedHits: [], pickupBackward: null, tags: [],
    ...
    it('🔴 전진율을 안 넘기면 그 사실이 화면에 적힌다 — 조용히 1.0 이 되지 않는다', () => {
        const 안넘김 = judge(CRITERIA, firstLoadFacts({ fare: 12_000, totalMinutes: 65, excludedHits: [], pickupBackward: null, tags: [] }),
            { ...DEFAULT_JUDGMENT, weights: { ...DEFAULT_JUDGMENT.weights, geography: 1 } });
