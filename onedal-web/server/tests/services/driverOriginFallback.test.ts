...
/** 🔑 «마지막으로 받은 좌표»만 든 세션 — 기점은 `originOf` 가 고른다 (개편) */
...
/**
 * 🔒 규칙 — **기점을 읽는 곳은 «고르는 함수» 하나만 부른다** (개편).
 *
 ...
 *
 * 🔴 **원인: 같은 질문에 답하는 손이 둘인데 한쪽만 고쳤다** (내가 낸 것).
...
/**
 * 🎭 **모의 주행이 좌표를 계속 보내는 동안은 빈 차여도 그 자리가 «지금 위치»다** (다섯 번째 바퀴 · onedal-49 합의).
 *
 * ③ «빈 차인데 가짜 좌표면 집 주소»는 **멈춘 뒤 남은** 모의 좌표(파주 156km)를 막으려던 것이다.
