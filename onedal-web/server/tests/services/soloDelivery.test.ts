 ...
 * 디스플레이한다."*
 *
 * 하차 마감은 `상차 완료 + 단독 배송주행 × 150%` 다. 합짐 콜은 병합 경로만 재므로 따로 재지 않으면
 * 그 주행을 **구조적으로 가질 수 없어**, 매번 «배송주행 추정(일반값)» 딱지가 붙는다.
...
/**
 * 🔒 **`applySoloRoute` 로 대신하지 않는다** — 그것은 폴리라인·구간 주행분·카카오호출시점까지 덮어쓴다.
 * 합짐 홀더에 대고 부르면 **지금 그리고 있는 병합 경로가 단독 경로로 바뀐다.**
 * 이 레포가 이미 여러 번 당한 형태라(«값이 남의 이름에 붙었다») 자리로 막는다.
 */
