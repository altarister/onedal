 ...
 *
 * 사슬 값은 두 시계 모델(시간체계 ⑯ · 기사님 확정)이다:
 *   상차 시계: 통화 약속 > 적요 상차 시각 > 잡음 + 20분
 *   배달 시계: 상차 완료 + 배송 × 150%
 *   16:09 잡음 · 접근 16 · 배송 113 · 상차 8분:
 *     예상 16:25 → 약속 16:29(잡은 시각+20) → 완료 16:37 → 하차 예상 18:30
...
maybe('출생 모델 — KEEP 은 첫 행만 낳는다', () => {
    ...
    it('🔴 안 태어난 다섯도 회색 예정으로 **값은 보인다** — "다음에 뭐가 올지는 알아야지"', () => {
        ...
        expect(kst(preview.row.predicted_at)).toBe('18:30');   // 완료(16:37)+113 — 파생값, 저장 안 됨
    ...
    it('첫 행의 사슬 값 — 예상 16:25 · 약속 16:29 (콜 잡은 시각 + 20분 · 0831 확정)', () => {
        ...
        expect(kst(r.promised_arrival_at)).toBe('16:29');   // 상차 시계 (잡은 시각 + 20분)
    });

    /**
     * 🔴 **약속은 도착 예상을 따라가지 않는다** (기사님 확정) — 상차 시계가 도착 예상보다 일러도
     *    약속은 «잡은 시각 + 20분» 그대로이고, 모자람은 상차버퍼 음수(통화 필수)로 보인다.
     */
    it('🔴 상차지가 멀어 20분을 못 지켜도 약속은 그대로 — 늦음은 버퍼 음수로 드러난다', () => {
        /**
         * 못 지키면 늦은 것이다. 그걸 알아야 그 상차지에 전화를 건다 — 감추면 전화할 기회를 뺏는다.
         * (`tests/shared/timelineDeadlineCap.test.ts` 도 같은 규칙을 문다)
         */
    ...
    /**
     * 🔴 **적요는 출생이 읽는다** (기사님 기획 승인).
     *    태어날 때 한 번 파싱해 계획에 넣고, 화면은 `planned_source` 배지만 그린다
     *    (시트가 열릴 때마다 파싱하지 않는다 — 파싱이 두 곳이면 갈린다). 순서: 실측 > 통화 > **적요** > 차종.
     */
    ...
    it('🔴 주행을 모르면 예상은 null — 0 으로 때우지 않는다 (규칙 ④)', () => {
        ...
        expect(r.promised_arrival_at).not.toBeNull();   // 약속은 상차 시계(잡은 시각 + 20분) — 주행을 몰라도 선다
    });
});
...
maybe('출생 모델 — 단계가 끝나면 다음이 앞 값을 물려받아 태어난다', () => {
    ...
    /**
     * 🔴 **지나쳐진 단계도 태어난다** (기사님 실측 · 3콜 리허설).
     *    통화 없이 GPS 도착이 먼저 올 때 도착 행만 낳으면, 건너뛰어진 하차지 통화가
     *    영영 안 태어나 회색 모형으로 남고 운행 중 통화를 못 한다. 그래서 마감은 **거기까지의 모든 미출생을 낳고** 자기를 마감한다. 빠뜨린 단계는
    ...
    /**
     * 🔴 **합짐도 예측이 있어야 한다** (기사님 실측 · 3콜 리허설).
     *    합짐은 kakaoSolo 가 없다. 그것만 보면 예측이 전부 null 이라 격자가 "주행 시간을 아직
     *    모릅니다"만 띄운다. **경로(타임라인)는 알고 있다.**
    ...
    it('🔴 합짐(solo 없음)도 경로가 알면 예측을 갖고 태어난다', () => {
        ...
        expect(kst(r.promised_arrival_at)).toBe('16:29');          // 상차 시계 (잡음+20)
    });

    /**
     * ⏱️ **배달 데드라인의 기산점은 상차 완료다** (시간체계 ⑯ · 소숙 자막 [09:08]).
     *    상차 전 대기는 배달 시계를 태우지 않는다 — 잡은 시각부터 재면
     *    이 짧은 콜이 16:59 에 잘린다.
     */
