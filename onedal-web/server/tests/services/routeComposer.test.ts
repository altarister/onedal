 ...
 * 짐을 실었으면 그 콜에 남은 일은 **하차뿐**이다. 활성 콜이면 무조건 상차·하차를 둘 다
 * 경유지에 넣으면 **이미 다녀온 상차지를 다시 가는 경로**가 나오고,
 * 거리·시간이 부풀어 그 값으로 잰 우회 예산과 합짐 판정이 통째로 틀어진다.
 *
...
describe('optimizeWaypoints — 지금은 상차를 모두 먼저 돈다', () => {
     ...
     * 그걸 고치는 것은 상·하차 섞기다 — 그때 이 테스트가 깨지면 정상이다.
     */
