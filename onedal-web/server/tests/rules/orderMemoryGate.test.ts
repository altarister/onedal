 ...
 *
 * `TERMINAL_STATUSES` 는 «종결 상태 (더 이상 상태 전이 없음)»이다. 그것을 **관문 하나가 강제한다.**
 * 상태를 정하는 권한이 호출부마다 흩어지면, 늦게 온 요청 하나가 죽은 콜을 심사 중으로 되돌린다.
 *
 * 정리 경로 · 재열람 대조 · 체험(상세 경로)처럼 **경로마다** 분기를 넣어 막으면
 * 경로가 하나 늘 때마다 다시 난다. 그래서 분기를 더하지 않고 **쓰는 문을 하나로** 만든다.
 *
 * 무엇을 막나
