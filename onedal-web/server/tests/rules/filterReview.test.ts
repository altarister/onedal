...
/**
 * 🔍 **필터 화면·저장·그물의 구멍을 잠근다**.
 *
 * 칸을 만든 곳과 쓰는 곳이 갈라지면(컬럼은 있는데 저장 UPDATE 에 줄이 없으면) 자정에 자동 모드와
 * 받을 짐이 조용히 풀린다 (허용 차종이 풀린 사고와 같은 모양).
 *
 * 이 파일은 단계마다 describe 를 하나씩 얹는다. 각 describe 머리에 조사 번호를 적는다.
 */

/* ══════════════════════════════════════════════════════════════════════════
 * 1단계 — 새 필터 칸의 저장 고리 (조사 ①-5·6·7)
 * ══════════════════════════════════════════════════════════════════════════ */
...
describe('1단계 · 받을 짐이 목업과 같은 뜻이다 (조사 ①-6)', () => {
    ...
    it('🔴 «모두» 에서 하나를 누르면 **그것만** 받는다 — 목업 `MapMockup.tsx` 와 같다', () => {
         ...
         * 빈 목록(=모두)에서 1t 을 누르면 [1t]. «1t 만 뺀 넷»으로 만들면 반대 뜻이다.
         */
...
describe('1단계 · 기준 거리 칸 (조사 ①-7 · 기사님: "7번 칸을 만들어줘")', () => {
    ...
    it('🔴 반경 줄이 셋이다 — 현위·목적·라인 (기준거리는 ⚙️ 설정으로 갔다)', () => {
        /**
         * 🔄 **반경 줄은 셋이다** (기사님 지시). 기준거리는 ⚙️ 설정 → 필터에 있다.
         *    셋은 «오늘 조이는 값»이라 성격이 같고, 4칸 격자면 한 칸이 빈다.
         */
    ...
    /**
     * 🔄 **기준거리는 필터에서 바로 바꾼다** (기사님 2026-09-23:
     *    *"40km 반경을 쉽게 바꿔야 할꺼 같아 … 지금은 사용자 설정에 40을 바꿔야 하는
     *    불편한 점이 있다."*). 그전에는 필터가 «말하기만» 하고 고치려면 ⚙️ 설정으로 갔다.
     *
     * 🔴 **한 그리드에 섞지는 않는다** — 반경 셋(현위·목적·라인)은 «오늘 조이는 값»이고
     *    기준거리는 «한 번 정하면 두는 값»이라 층이 다르다. 섞으면 «자동이면 이것만 살고
     *    나머지가 흐려지는» 규칙이 생긴다.
    ...
    it('🔴 ⚙️ 설정에서 고치면 평소값으로 저장된다 — 같은 값을 보는 창이 둘이다', () => {
...
describe('2단계 · 배율이 아니라 «거리»를 싣는다 — 화면이 배율을 낸다', () => {
    /**
     * 배율(`radiusScale`)을 서버가 재서 실어 보내면 화면은 기준거리를 바꿔도 배율을 다시 낼 수
     * 없어, 기준거리를 끄는 동안 지도가 **안 따라온다.**
...
describe('2단계 · 줄인 반경이 앱·요약줄·지도 띠에 간다 — 세 벌을 한 벌로 (조사 ①-4·5)', () => {
    ...
    it('🔴 지도 띠 굵기가 줄인 값이다', () => {
        ...
        /* 지도 띠는 «하차» 레이어 띠(`km`)가 줄인 값을 쓴다 — `lineRadiusKm` 이 다시 나오면 빨간불 */
...
describe('3단계 · 그물의 목적지는 «파생»이다 — 복귀를 켜도 기사님 목적지는 그대로 (조사 ①-1)', () => {
    /**
     * HOME 으로 갈 때 `activeFilter.destinationCity` 를 **집 시로 덮어쓰면**, DEST 로 돌아올 때
     * `activeFilter.destinationCity || baseFilter…` 가 이미 덮인 값에서 끝나 **파주시가 광주시로 굳는다.**
     * 툴팁 «끄면 원래 목적지로 돌아갑니다»가 거짓이 된다.
     *
    ...
    it('🔴 setCallTarget 이 destinationCity 를 안 보낸다 — 덮어쓰기가 사라진다', () => {
        ...
        /* 빈 차 목록도 파생 목적지를 본다 — 부팅·0건·KEEP 이 모두 netFilterOf 한 곳을 지난다 */
        const j = fm.indexOf('function netFilterOf');
        expect(j).toBeGreaterThan(-1);
        /* 목적지 목록(`goalCitiesOf` · 복귀 대기면 둘)을 `netOfGoals` 가 돈다 */
...
describe('3단계 · 집이 있는 시는 «좌표»로 뽑는다 — 주소 글자에 기대지 않는다', () => {
    /**
     * 사람이 적는 주소는 «광주시»라고 안 적는다 — 기사님 주소도 `경기도 광주 초월 동광뷰엘` 모양이다.
     * 집 주소 글자에서 「시」로 끝나는 조각을 찾으면 못 찾아 복귀 토글이 거부된다.
...
describe('4단계 · ⛔ 제외 칩 줄 — 목업 그대로 (조사 ② · MapMockup.tsx:3288~3346)', () => {
     ...
     * 전체 칩을 늘 펼치면 폰에서 여덟 줄이 화면을 먹고, 실수 삭제를 막을 수 없다.
     */
    const modal = readClient('components/dashboard/OrderFilterModal.tsx');
...
describe('4단계 · 목적지 설명줄 · 경로 대기 문구 · 저장 안내 (조사 ②)', () => {
    ...
    /**
     * 🔴 **목적지 설명줄을 두지 않는다** (기사님 확정).
     *    «목적지 X · 🛣️ 노선» 은 «🎯 어디로» 행 머리 요약이 이미 말한다 — 한 화면에 같은 말 두 번은 거짓말 자리다 (규칙 ③).
     */
...
/* 지도는 동 점을 안 찍는다 — 제외는 서버 목록 한 곳이다
   (`phaseUi.test.ts` «지도는 동 점을 안 찍는다 — 제외는 서버 목록 한 곳») */

/* ══════════════════════════════════════════════════════════════════════════
 * 5단계 — 관내 표시 · 노선/동선 · 레이어 열림 · 도구 (조사 ①-8·9·10·11)
 * ══════════════════════════════════════════════════════════════════════════ */
describe('5단계 · 관내 표시를 걷었다 — «목적지 가까이 옴»이 대신한다 (2026-09-15)', () => {
    /**
     * 🔄 **관내는 따로 재지 않는다** — 기사님 *"그쪽에 도착했으니 다른 곳을 정하지 않았으면 그곳에서 일 있으면 하자"*.
     *    목적지 가까이 옴(`filterArea.withNearness`)이 가른다. 서버도 관제웹도 `localMode` 를 두지 않는다.
     */
    const sv = codeOnly(readClient('components/stage/StageView.tsx'));
...
describe('5단계 · 노선/동선은 필터 값이다 — 서버도 알고 저장도 된다 (조사 ①-9)', () => {
    /**
     * `routeMode` 를 화면(`Dashboard.tsx`)의 `useState` 로만 들면 서버가 그 개념을 몰라
     * «동선»을 골라도 **판정·앱 목록은 계속 노선**이고, 새로고침하면 노선으로 돌아간다.
...
describe('6단계 · 칸 바깥 스타일을 목업 어휘로 (조사 ③)', () => {
    ...
    /**
     * 🔴 **행은 구분선 하나로 나눈다** (기사님: *"필터 박스가 있음으로 내부박스를 따로 만들필요가 없을꺼 같아"* ·
     *    *"지금 너무 공간낭비가 많아"*). 네 행 구조에서는 행 사이 틈이 곧 낭비다.
     */
    ...
    /** 🔴 **행 안 소제목을 두지 않는다** — 이름은 행 머리(📐 얼마나 넓게 · 🚫 빼는 곳)가 말한다 */
...
describe('8단계 · 모의 주행 — 현황판이 켜고 끈다', () => {
     ...
     *
     * 🔴 조건이 맞으면 **저절로** 시작하게 두면 끄는 길이 없어 «그만 보고 싶은데 계속 도는»
     *    상태가 된다. 그래서 현황판 버튼으로 켜고 끄고, 속도도 스토어가 든다.
...
describe('8단계 · 상차지까지도 모의 주행으로 간다 (현황판 답신 · 기사님 "출발을 해야 상차를 하지")', () => {
     ...
     *
     * 기사님: *"출발을 해야 상차를 하지"*.
     * 그러면 **테스트에서 가장 필요한 구간(콜 잡고 → 상차지까지)이 막힌다.**
     *
     * 🔴 막을 수 있는 자리가 **두 곳**이다:
     *   ① 관제웹 `canMock` — 국면(`isDriving`)을 보지 않고 경로만 본다
     *   ② 서버 `originOf` — 콜을 쥔 동안(GATHERING·DELIVERING)에는 가짜 좌표를 기점으로 쓴다
     *      → 관제웹만 풀면 서버가 가짜 좌표를 기점으로 안 써 아무 일도 안 난다
     *
     * 🔴 **빈 차(STANDBY)에서 멈춘 뒤 남은 가짜 좌표는 안 쓰고 집으로 물러난다** —
     *    시뮬이 파주에서 멈춘 뒤 가짜 좌표가 남으면 다음 콜 경로가 «파주 → 광주 → 파주» 156km 로
     *    그려진다. 콜을 쥔 뒤(GATHERING·DELIVERING)나 모의 주행이 지금 도는 동안은
     *    **실제로 움직이는 중**이라 가짜 좌표를 쓴다.
     */
    const gps = codeOnly(readClient('hooks/useMasterGps.ts'));
    ...
    it('🔴 서버가 콜을 쥔 동안에는 가짜 좌표를 기점으로 쓴다 — 빈 차일 때만 집으로 물러난다', () => {
        /**
         * 🔄 **세션 좌표를 지우지 않고 고른다** (기사님 지시). `originOf` 가
         *    **물을 때마다 고르므로** 지우는 손이 없다 — 지우는 손이 여럿이면 그중 하나를 놓친다.
         *    그래서 검사도 «고르는 자리»를 본다.
         */
...
describe('8단계 · 궤적이 카카오 길을 따라간다 (기사님 "궤적이 엉망이야")', () => {
    /**
     * 기사님: *"궤적이 엉망이야. **카카오 궤적이 아닌 것 같아.** 목업에서는
     * 지나간 자리를 그려줄 건데 이렇게 그리면 안 되지"* · *"목업에서는 이쁘게 나왔어"* ·
     * *"진짜 GPS로 찍은 건 아니지만 말이지."*
     *
     * 🔴 **지킬 것 둘** (목업과 같은 모양):
     *   ① **그릇** — 구간 배열(`Pt[][]`)이라 2km 넘게 튀면 **끊고 새 구간**을 연다.
     *      한 줄로 이으면 **점프가 그대로 직선**으로 남는다. 모의 주행은 정거장에서
     *      도로 밖 좌표를 그대로 찍으므로(물류센터 601m), 구간을 안 끊으면 **정거장마다
     *      도로 밖으로 튀었다 돌아오는 직선 둘**이 남는다.
     *   ② **걸음** — 거리로 보간해 폴리라인 점을 **다 밟는다**. `idx += 15` 처럼
     *      **점을 건너뛰면** 카카오 곡선이 직선으로 펴진다.
     *
