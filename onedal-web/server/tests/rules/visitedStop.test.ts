 ...
 *
 * **상차 완료를 안 누르면 이미 지나온 상차지로 되돌아가는 경로**가 나온다.
 * 실측: 같은 다산역 콜이 상차 완료 전엔 경유지 5개·+20.0km·🟢56점,
 * 누른 뒤엔 경유지 3개·+0.7km·🔵80점 — **없는 우회 비용 20km를 문다.**
 *
 * «같은 사실을 두 식으로 판단»하는 모양 자체를 두지 않는다.
 *
...
/**
 * ⚖️ **비교 기준(base)도 다녀온 곳을 뺀다** (기사님 실측)
 *
 * 합짐 경로(merged)가 다녀온 상차지를 빼도, 비교 대상인 base 가
 * `현위치 → 첫짐 상차지 → 첫짐 하차지` 로 고정 조립되면 다녀온 곳을 다시 들른다.
 *
 * 실측 로그:
 *   `🔵 총점 80 — 추가 주행 +-13분(100) · 우회 거리 +-23.4km(100)`
 *
 * **우회 비용이 음수**가 된다 — 합짐을 붙였는데 거리가 23km 줄었다는 뜻이다.
 * 정거장이 늘었는데 그럴 리 없다. base 가 이미 다녀온 경안동을 다시 들르는 경로라
 * **부풀어 있고**, 그 부푼 값과 비교하니 모든 합짐이 과대평가된다.
...
/**
 * 🛣️ **단독 경로도 같은 정의를 쓴다** (기사님 실측)
 *
 * 합짐이 전부 하차되어 콜이 하나만 남으면 단독 경로(`calculateSoloRoute`)로 떨어진다.
 * 그 호출부가 다른 정의(`isAlreadyLoaded`)를 쓰면 **사이클 끝마다** 되돌아가는 경로가 나온다.
 *
 * 실측 로그 (가남 콜 `b9f6996a`):
 *   `origin=127.5201,37.2347` (현위치 — 여주)
 *   `waypoints=127.1307,37.4120` ← **성남 야탑 상차지. GPS 도착이 찍힌 곳**
 *   총 이동 104,784m · 섹션 2개
...
/**
 * 🗺️ **관제웹 폴백도 같은 정의를 쓴다** (기사님 실측)
 *
 * 기사님: *"경로 인덱스가 바뀌었어. 다시 경로를 만들었다고 하면 다산동 → 하차1 →
 * 하차2 → 하차3 일 텐데 지금은 뒤죽박죽이 되어 있어서."*
 *
 * 서버는 다녀온 상차지를 뺀다 — `routeStops` 에는 하차만 남는다. 관제웹의
 * **폴백**(routeStops 에 없는 콜을 뒤에 덧붙이는 자리)이 `isAlreadyLoaded`
 * (상차 완료 버튼)를 보면, 상차 완료를 안 눌렀을 때 **뺀 상차지를 도로 넣고**,
 * 그것도 콜 순서대로라 `⑴상차 ⑵하차 ⑶상차 ⑷하차…` 로 번갈아 매겨진다.
 *
 * 정의를 옮길 때는 **호출부를 전부 훑는다.**
 */
describe('관제웹 폴백 — 정의가 서버와 같다', () => {
    it('🔴 PinnedRoute 폴백이 hasVisitedStop 을 쓴다 (isAlreadyLoaded 가 아니라)', () => {
        const src = readFileSync(join(__dirname,
            '../../../client-app/src/hooks/useRouteDerivations.ts'), 'utf8');   // 폴백은 경로 파생을 모으는 훅에 있다
