...
/**
 * 🧾 **심사석이 «기존 콜이 어떻게 되나»를 말한다** (목업 «④ 후보콜에 대한 심사 결론»).
 *
 * 합짐 심사는 이미 후보를 넣은 경로의 정거장 타임라인(`tlAfter`)을 만든다. 결과를 **관문 문장 하나와
 * 최소 버퍼 숫자**로만 쓰면 정거장마다 약속·예정·늦음이 스냅샷에 없어 심사석이 «누가 몇 분»을 못 보이고,
 * 🔴 정거장 순서를 못 받으면 `late` 가 비어 관문이 **통과** — 모르는 것이 «약속 보존»으로 읽힌다. 그래서:
 *
...
describe('🧾 심사석 — 기존 콜 정거장 (4단계)', () => {
    ...
    /**
     * 📞 **심사 카드의 둘** — 목업 시트 심사 카드의 «더 쓰는 시간»(시급의 분모 · 합짐이면 전체 경로가 늘어나는 만큼)과
