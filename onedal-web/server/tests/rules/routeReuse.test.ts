 ...
 *
 * 까닭:
 *   ⓐ `orders` 에 궤적(routePolyline)이 없으면 서버가 재시작할 때마다 `restoreAndRecalculateSession` 이
 *      **카카오를 다시 부른다** (재시작 한 번 = 재계산 한 번 · 실측 하루 여섯 번).
 *   ⓑ 새 콜을 붙였다가 취소할 때 **원래 경로를 또 계산하면** 낭비다. 원래 콜은 아무것도 안 바뀌었다.
 *      미리보기로 "판정만 받고 취소"가 잦아 이 낭비가 크다.
 *
...
describe('🗺️ 가 — 확정 경로는 장부에 남는다', () => {
     ...
     *    없을 때만(궤적 칸이 빈 행·연산 실패) 카카오로 간다 — 안전망은 남긴다.
     */
    /**
     * 🔴 **장부의 문자열이 화면까지 새어 나가면 안 된다** (실측).
     *
     * `routePolyline` 은 DB 에 **JSON 문자열**로 산다. `/api/orders` 가 행을
     * `SELECT *` 로 읽어 **그대로** 관제웹에 보내면, 관제웹은 좌표 배열로 알고
     * `.filter()` 를 부르다 통째로 죽는다:
     *
