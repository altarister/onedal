 ...
 * ── 🔴 이 배지는 A 다 ──
 *
 * 배지가 콜 옵션 표의 「박스당 분」을 **되돌려 계산해** 고치면(**B**)
 * 화면으로는 똑같아 보여도 뜻이 전혀 다르다:
 *
 * ```
...
maybe('✏️ 배지가 고치는 것은 이 콜의 이 정거장이다 (A)', () => {
    ...
    /**
     * 🔴 **안 먹었으면 화면이 알아야 한다** (자기 리뷰).
     *    false 만 돌리고 끝내면 기사님은 **눌렀는데 안 바뀐 화면**만 보신다.
     */
...
describe('⏱️ 실측이 뒤 정거장을 민다 — 기사님의 「−5분」', () => {
    ...
    /**
     * 🔴 **이게 진짜 위험한 경우다** (기사님 리허설).
     *
     * «실제로 몇 분 걸렸나»를 아는 순간은 **상차 완료를 누르는 때**인데, 바로 그 순간
     * 상차지가 **경로에서 빠진다.** «경로에 남은 정거장»만 훑으면
     * 방금 5분 더 걸린 그 정거장을 **아무도 안 세서** 화면에 영영 안 뜬다
     * (기사님: *"하차 완료까지 했는데 «보실 것»을 확인 못 했어."*).
     */
...
/**
 * 🔴 **실측은 «라벨»만이 아니라 약속에도 들어간다** (기사님 리허설 · 로그 대조)
 *
 * 약속에 안 들어가면 화면이 두 말을 한다:
 * ```
 * 신둔면 ~03:15  +5분      ← 「5분 밀렸다」고 적으면서, 03:15 는 안 밀린 값
 * ```
 *
 * 하차 약속의 기산점은 `상차 완료`다. 아직 상차를 안 끝냈으면 **도착 실측 + 정차**로
 * 파생하는데(CLAUDE.md ⑤-5), 그 «정차»는 기사님이 적어 둔 **19** 여야 한다. **계산값 14** 를 쓰면
 * *"다음 도착지가 5분 늦어진다"* 가 라벨로만 남고 시각은 그대로다.
 *
 * 🔴 **이 자리는 정거장이 경로에서 빠진 뒤에만 지난다** — 상차지에 도착하면 그 정거장이
 *    경로에서 빠지기 때문이다 (밀림 누적도 같은 자리다).
 */
describe('⏱️ 실측 정차가 하차 약속까지 민다', () => {
    ...
    /**
     * 🔴 **바탕값에 기대지 않는다.** «기본 대비 +5분» 으로 견주면 신고가 없을 때의 정차
     *    (미확인 일반값 15분)에 따라 숫자가 흔들린다. 그래서 **실측끼리 견준다.**
     */
    ...
    /**
     * 🔴 **라벨과 그 옆 숫자가 같은 말을 해야 한다**.
     *
     * `신둔면 ~03:15 +5분` 처럼 「5분 밀렸다」고 적으면서 03:15 가 안 밀린 값이면 화면이 거짓말한다.
     * 라벨을 다른 자리로 옮기지 않고 **약속이 진짜로 밀리게** 한다.
...
describe('✏️ 화면이 그 규칙을 지키는가', () => {
    ...
    /**
     * 🔴 **± 를 누를 때마다 저장하지 않는다** (자기 리뷰).
     *    누를 때마다 저장하면 1.7초 동안 7번이다(리허설 실측) — 누를 때마다 DB 쓰기 + 브로드캐스트다.
     *    손을 뗄 때(✓) 한 번만 보낸다.
     */
     ...
     *    ① 고친 값을 `text-info` 로 칠하면 칩이 선택됐을 때 배경도 `bg-info` 라 **사라진다**
     *    ② 자릿수가 바뀌어 폭이 변하면 ± 버튼이 밀린다
     */
...
/**
 * 🔴 **통화로 박스 수를 고치면 예측 정차도 따라가야 한다** (기사님 리허설)
 *
 * 박스 수만 고치고 예측 정차를 두면 장부에서 같은 콜의 두 행이 **다른 예측**을 든다:
 *
 * ```
 ...
 * ```
 *
 * 그러면 상차 완료 행이 태어나기 전까지 판정은 **3분 긴 값**을 쓴다.
 * 그래서 `bridgeCargoReport` 는 `planned_quantity` 와 함께 `planned_dwell_min` 도 다시 잰다.
 */
