 ...
 *
 * ── 왜 두 함수인가 ──
 * 한 함수가 **두 질문에 답하면** 한쪽 답이 다른 쪽을 망친다 (규칙 ⑤-4 ⑤ — «읽는 곳이 둘이면
 * 각자 다른 질문을 답하고 있는 것 아닌지 그 자리에서 의심한다»).
 *
 *   · 경로 기점  — 출발점이 없으면 **계산 자체가 안 된다.** 그래서 낡으면 집을 넣는다.
 *                  낡은 좌표를 기점으로 쓰면 몇 시간 전 자리에서 경로를 짠다 (`staleDriverLocation` 검사).
 *   · 내 위치    — 모르면 **모른다**여야 한다. 집을 넣으면 화면이 «집에 있다»고 거짓말하고,
 *                  그 값으로 그물을 치면 **이천에 있는데 파주 콜이 올라온다.**
 *
 * 기사님 확정 — 지도는 «마지막 실제 위치를 흐리게», 그물도 «마지막 실제 위치 기준».
 *
 * 🔴 **`originOf` 는 5분 규칙과 집 대체를 그대로 쓴다** — 그것은 그 질문의 옳은 답이다.
 *    `lastKnownPositionOf` 는 **같은 원자료(`lastFix`)를 다른 질문으로 읽을 뿐**이다 (규칙 ③ — 파생이지 사본이 아니다).
 */
...
describe('마지막으로 아는 자리 — 경로 기점과 다른 값이다', () => {
    ...
    /**
     * 🔴 **이 함수의 핵심이다** — 5분이 넘어도 **집으로 바꾸지 않는다.**
    ...
    /**
     * 🔴 **같은 세션에서 경로 기점은 그 좌표를 안 쓴다** — 낡은 좌표로 경로를 짜면 몇 시간 전 자리에서 출발한다.
     *    ⚠️ 이 시험에는 집 주소가 없어 `originOf` 가 «모른다»(`null`)로 답한다
