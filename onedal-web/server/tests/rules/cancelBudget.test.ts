...
/**
 * 🚫 **취소 예산 10회 — 다 쓰면 알리고 새 판을 연다** (기사님 확정)
 *
 ...
 *
 * ⚠️ 원칙은 *"취소 10회는 **리셋되지 않는다**"* 였다. 그 취지는 **총량이 사라지면 안 된다**는 것이다 (페널티는 못 피하는
...
describe('🚫 한도는 한 곳에서 정한다', () => {
     ...
     */
    /**
     * 🔄 **2026-09-15 개정 — 필터 상황판에서 토스트도 뺐다** (기사님: *"취소한도 토스트는 빼 그건 폰마다 배차망마다
     *    다르기때문에 다른곳에서 해야 할것같다"*). 필터 상황판은 모든 폰에 똑같이 내려가는 **한 벌**을 말하는 줄이다.
