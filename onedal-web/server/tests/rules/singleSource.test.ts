...
describe('정거장 번호를 세는 곳은 하나', () => {
    ...
    it('🔴 번호는 «다녀온 것 → 남은 것»을 이어 세어 만든다 — 기억을 두지 않는다', () => {
        /**
         * 2026-09-01 실측으로 규칙이 한 번 뒤집혔다. 「출발하면 얼린다」로 흔들림은 멎었지만
         * (그날 흔들림 0회), 합짐이 **출발 뒤에** 들어오면 자기 정거장을 끝에 이어 붙여
         * `1 → 3 → 2 → 5 → 4 → 6` 순서로 다니게 됐다 — 번호가 «가는 순서»를 못 말했다.
         * 기사님 확정: **번호 = 가는 순서.** 다녀온 것은 고정, 남은 것은 갈 순서대로.
         *
         * 기억(ref)을 지우는 것이 핵심이다 — 매번 파생시키면 옛 흔들림이 **구조적으로**
         * 못 생긴다(정거장을 다녀오면 남은 목록에서 빠지는 동시에 다녀온 목록에 들어가
         * 자리 번호가 그대로다). 기억을 되살리면 그 보장이 사라지므로 여기서 막는다.
         */
