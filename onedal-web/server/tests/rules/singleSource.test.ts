...
/**
 * 🔒 **«한 곳으로 모았다»가 참인지 검사가 지킨다** (기사님 지시)
 *
 * 기사님: *"아무 생각 없이 수정하지 말고 깊이 생각하고 수정해."*
 *
 * ── «한 곳으로 모았다»가 거짓이 되는 모양은 하나다 ──
 * **새 원천은 만들었는데 부르는 곳을 세지 않는다.** 예:
 *
 *   상차 약속   — 만드는 곳이 셋인데 하나만 고치면 심사 카드는 «+0분», 덱은 «−5분»
 *   경로 홀더   — 관제웹만 고치면 서버 안에 판정이 둘 남는다
 *   주행/정차   — `useDriveMotion` 에서만 mock 특례를 지우고 `MovingBadge` 에 두면
 *                 한 화면에 «정차 중»과 «시뮬 주행»이 나란히 뜬다
 *   정거장 번호 — 세는 재료가 셋이면 «신둔면이 6이었다가 3이었다가» 뛴다
 *
 * 새로 쓴 검사가 **새 원천만** 부르면 남은 자리는 안 불리니 전부 초록이다 —
 * *"있는 검사가 안 불리면 없는 것이다."*
 *
...
describe('상차 약속을 만드는 곳은 pickupClockMsOf 하나', () => {
     ...
     * 그 사다리를 다시 짜면(= 설정값에 60_000 을 곱하면) 적요·통화 갈래가 빠진 반쪽이 되어
     * 그 반쪽이 DB 에 저장된다.
     */
    const FILES = [
    ...
    it('🔴 약속이 도착 예상을 따라가는 옛 식이 되살아나지 않는다', () => {
        /** `max(도착 예상, 시계)` — 여유를 늘 0으로 만들고 늦음을 감추는 식 */
...
describe('«달리는가»를 판정하는 곳은 하나 — 모의 특례는 없다', () => {
     ...
     * 한 곳에서만 특례를 지우면 무대 자막(«정차 중») 옆에서
     * 배지가 «시뮬 주행»이라고 반대말을 한다.
     */
...
describe('정지도 사건이다 — 화면이 «아직 여기 있다»를 듣는다', () => {
    /**
     * 서버 중복 거르기가 **화면 알림까지** 막으면 정차 동안 좌표 알림이 한 번도 안 나가,
     * 정거장에 18초 서 있는데 배지가 «이동 중 87km/h» 가 된다 (실측).
...
describe('도착은 사건이라 덮이지 않는다', () => {
    /**
     * 도착을 포커스 그릇(gpsFocus)에 담아 읽으면, 그 그릇에 «달리는 중 덱 따라가기»도 담겨
     * 도착 직후 다음 정거장이 바뀌면서 **도착을 덮어쓴다** (실측: 도착 6번 중 시트가 2번만 올라감).
     */
    ...
    /**
     * 🔄 **덮이지 않는 것을 «칸을 가르는» 수단으로 지킨다** (기사님 지시).
     *
     * 시트가 소켓에서 **직접** 받게 하면 **듣는 곳이 둘**(스토어·시트)이 되어
     * «덱이 가리킨 콜»과 «시트가 연 콜»이 갈라진다 (기사님: *"지금 그걸 각자 하고 있어서
     * 문제 같은데"*).
     *
...
describe('정거장 번호를 세는 곳은 하나', () => {
    ...
    it('🔴 번호는 «다녀온 것 → 남은 것»을 이어 세어 만든다 — 기억을 두지 않는다', () => {
        /**
         * 「출발하면 얼린다」로 번호를 고정하면 흔들림은 멎지만, 합짐이 **출발 뒤에** 들어올 때
         * 자기 정거장을 끝에 이어 붙여 `1 → 3 → 2 → 5 → 4 → 6` 순서로 다니게 된다 —
         * 번호가 «가는 순서»를 못 말한다.
         * 기사님 확정: **번호 = 가는 순서.** 다녀온 것은 고정, 남은 것은 갈 순서대로.
         *
         * 기억(ref)을 두지 않는 것이 핵심이다 — 매번 파생시키면 흔들림이 **구조적으로**
         * 못 생긴다(정거장을 다녀오면 남은 목록에서 빠지는 동시에 다녀온 목록에 들어가
         * 자리 번호가 그대로다). 기억을 두면 그 보장이 사라지므로 여기서 막는다.
         */
        expect(der).not.toMatch(/stopNoRef/);
        expect(der).not.toMatch(/isDriving \? /);
        // 세는 재료는 화면이 그리는 그 목록이어야 이름표와 지도가 안 갈린다
        expect(der).toMatch(/for \(const p of routePointsRaw\)/);
    });

    it('🔴 지도 캔버스가 스스로 세지 않는다 — 번호는 실려 온다', () => {
        /**
         * 캔버스가 «다녀온 개수 + 남은 목록 인덱스»로 **따로** 세면 이름표는 «1. 곤지암읍»,
         * 지도 마커는 «2 곤지암읍» 처럼 갈린다 (실측). 그래서 파생만이 아니라 캔버스도 본다.
         */
