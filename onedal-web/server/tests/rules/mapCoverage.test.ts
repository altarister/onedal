 ...
 *
 * 목표 도시가 지도(`merged_map.geojson`)에 없으면 `destinationKeywords` 가 **빈 배열**이 된다.
 * 빈 목록은 fail-closed 다 — 앱(`InsungParser.kt`)이 *"도착지 키워드가 비어 있습니다"* 로
 * 콜 잡기를 보류한다 (규칙 ④). 그러면 필터가 멀쩡해도 그 도시 콜은 **0개**다
 * (지도가 서울·인천·경기만 담으면 「청주」가 그렇다 — 반경을 0·10·20·30km 로 넓혀도 동 0개).
 *
 * ⚠️ 그물 자체는 멀쩡하다. 지도에 있는 도시를 넣으면 잘 돈다 (아래 «그물은 돈다» 참조).
 *    **빠질 수 있는 것은 데이터뿐**이다.
 *
 * 🔴 같은 뿌리의 다른 증상 — 지도가 수도권만 담으면 `서구`(인천)를 "유일"로 판정한다
 *    (`districtNameOnList.test.ts`). 하나는 «없는 데이터로 유일하다고 판정», 하나는 «그물이 통째로 안 만들어짐».
 *
 * 이 검사가 하는 일은 둘이다:
 *   ① 지도가 **조용히 바뀌는 것**을 막는다 (생산자는 우리 사정을 모른다 — 아래 참조)
 *   ② 충청권이 **실제로 지도에 들어와 있는지**를 판정한다
 */
...
describe('🗺️ 지도가 담는 범위', () => {
     ...
     * 🔴 **생산자는 우리 사정을 모른다.**
     *
     * 이 파일은 지도 퀴즈 게임에서 만들어 복사해 온 것이다 (`onedal-map/README.md`).
     * 게임의 목적은 **클릭 가능성**이라 작은 지역을 뺄 수 있고(`MIN_CLICKABLE_AREA_PX2`), 그러면
    ...
    it('🔴 지도 파일의 뼈대가 안 바뀐다 — 8자리 읍면동만, 전부 _isEmdGroup', () => {
        ...
        // 리(里)는 8자리로 병합돼 있다 (리는 전면 제외).
        // 콜창도 읍면동까지만 보여준다 (기사님 확인) —
    ...
    /**
     * 🔴 **충청권이 지도에 있어야 한다** — 지도가 수도권만 담으면 이 검사가 빨간불이다.
     *
     * 볼트가 인천 남동공단에서 「청주」를 목표로 넷을 잡아 242,800원을 만들었다.
     * 청주가 지도에 없으면 우리는 0개다 — 그물이 **빈 배열**이 되고, 빈 목록은
     * fail-closed 라 전부 탈락한다.
     *
     * 초록불의 조건: `onedal-map` 이 `--regions 11 28 41 30 36 43 44` 로 만든
     * 지도가 서버 `mapData/` 에 들어와 있을 것.
     */
