...
/**
 * 🧭 **«근소»는 비율이 아니라 «몇 km 차이인가»다** (기사님 실측)
 *
 * 기사님: *"경로가 좀 이상해 보이는데 … 순차적으로 가고 있는거 맞아?"*
 *
 * ── 실측 (서진 문제지) ──
 * 비율만 보면 서버가 **②하차(가산동)를 ③상차(안양)보다 먼저** 놓는다.
 * ```
 ...
 *
 * ── 왜 그렇게 되나 ──
 * `STOP_ORDER_HYSTERESIS`(직전 순서를 **1.20배 안에서 편든다**)에 걸린다.
 * ```
 * 성남 → 안양 석수동   직선 19.2km   실제 도로 22.2km / 23분
 * 성남 → 금천 가산동   직선 21.7km   실제 도로 27.3km / 45분
 * 비율 1.13 < 1.20  →  «근소»로 판정되어 직전 순서(가산동 먼저)가 이긴다
 * ```
 *
 * 🔴 **문턱을 낮추는 것으로는 못 고친다.** #90 을 막는 안정성 검사는 비율이 **1.18**이라
 *    더 크다 — 낮추면 번호가 다시 춤춘다. 두 경우를 가르는 것은 **절대 차이**다:
 * ```
 * 안정성(편들어야 함)   2.03 vs 2.39km → 차이 0.36km
 * 이 경우(편들면 안 됨)  19.2 vs 21.7km → 차이 2.5km
 * ```
...
describe('🧭 먼 거리에서는 직전 순서가 못 이긴다 (2026-09-03)', () => {
    ...
    /** 직전 순서는 «B하차 먼저»라고 말한다 — ②③만 있을 때 굳은 순서다 */
    ...
    /**
     * 🔴 **순서가 굳는 순간은 ④(방화동)가 붙을 때다** — 보낸 순번(②③)이 ④를 모르므로
     *    «가장 가까운 곳»으로 다시 정하는 자리다. 여기서 직전 순서를 편들면 가산동이 먼저 간다.
     *
     * ⚠️ ④ 없이 «매 틱» 모양으로 두면 이 검사는 깨진다 — **보낸 순번이 남은 정거장을 다 덮으면
     *    그 순번을 따르기** 때문이다 (기사님 결정 · `stopOrderStability.test.ts`).
     *    그래서 «새 콜을 붙여 카카오에 보낼 순서를 정할 때»의 모양으로 둔다.
     */
