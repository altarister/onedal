 ...
 * 콜도 함께 보여준다"* — 6단계 채워진 모습을 보기 위한 **화면 규칙**이다. 그래서
 * 사이클이 자정을 걸치면(운행 중 2콜을 남기고 잠들면) 정리하지 않는 한 어제 하차한 3콜이 오늘 아침
 * "진행 중 (5)" 로 계속 보인다 — 성과 기록은 어제로 닫혔는데 화면만 어제를 산다.
 *
...
/**
 * 🔴 **날짜를 박아 두지 않고 오늘에서 거꾸로 센다.**
 *
 * 고정 날짜를 "오늘"이라 적어 두면 하루가 지나 그 콜이 진짜로
 * *"어제 하차분"* 이 되어 정리되고, **서버는 옳게 동작하는데 검사만 빨간불**이 된다.
