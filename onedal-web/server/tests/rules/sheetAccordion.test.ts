 ...
 *
 * 콜 목록 밑에 «어느 콜의 것인지 모르는 스텝»이 이어지면, 스크롤할 때 주어가 사라진다 (S23 실물).
 *
 * ⚠️ `CallDeck` 의 스와이프 갈래는 지금 쓰는 곳이 없다 — 이 검사는 «시트»만 잠근다.
 */
...
describe('🪗 시트 아코디언 — 기사님 확정 2026-09-03', () => {
     ...
     *    (`components/layout/Drawer`). 시트에는 «진행 중»만 산다 —
     *    갈래를 고르는 상태(`viewFilter`·`sheetOnly`)를 두면 시트가 «진행 중» 아닌 것을 그릴 수 있다.
     */
    ...
    it('헤더는 «내용 사이사이»에 끼워 그린다 — 내용이 자기 헤더 바로 밑에 온다', () => {
        // 🔴 헤더를 위에 몰고 내용을 그 아래 따로 그리면 «누구 것인가»가 또 생긴다
        expect(deck()).toMatch(/\{rowOf\(o, i\)\}[\s\S]{0,600}?renderCard\(o\)/);
    });

    /**
     * 🔴 **붙이지(sticky) 않는다** — 기사님 실물: *"여기 겹침이 발생했어."*
    ...
    it('아코디언도 카드를 전부 마운트한다 — 고른 것만 «보일» 뿐 (입력값 보존)', () => {
        /* 성질 — 카드는 늘 만들어지고 `hidden` 으로만 감춘다 (조건부 렌더가 아니다).
              ⚠️ 사이에 높이 클래스 같은 것이 붙을 수 있어 글자 수가 아니라 성질을 본다 */
        expect(deck()).toMatch(/hidden=\{[\s\S]{0,300}?renderCard\(o\)/);
...
/**
 * 🪗 **세로 사슬 — 넘친 것이 밖으로 그려지지 않는다** (기사님 실물:
 * *"시트 반만 열기에서만 겹침이 발생해"*)
 *
 ...
 * | 펼친 판 | **상자 밖으로 안 그린다** |
 * | 위 덩어리(콜 전체) | **안 줄어든다** — 반만 열기는 어떤 콜이 있는지 보려고 여는 것이다 |
...
describe('🪗 시트가 좁아져도 겹치지 않는다', () => {
     ...
     * 🔴 **클래스 «글자»가 아니라 «뜻»을 본다**
     *
     * 시트의 세 단(「나」= 내용만큼)에 따라 두 그릇은 **모양을 바꾼다** —
     * · `h-full` 은 「나」에서 뺀다 (부모 높이가 내용에서 나오는데 자식이 부모를
     *   채우려 들면 서로를 문다)
     * · `flex-1` 은 auto 높이에서 0 으로 주저앉아 `flex-auto` 와 갈라 쓴다
     *
     * 클래스 문자열을 통째로 맞추면 **동작은 그대로인데 검사만 빨간불**이 된다. 그렇게 뜬 빨간불은 사람이 «또 그거네»
    ...
    /**
     * 🔴 **위 덩어리가 이긴다** (기사님: *"위 덩어리는 내용이 다 보여야 해.
     *    지금 시트를 반만 열었다는 건 전체적으로 어떤 콜이 있는지 보기 위함"*).
     *    좁을 때 여기를 줄이면 **정작 볼 것이 사라진다.**
     */
