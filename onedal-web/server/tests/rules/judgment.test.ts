 ...
 * 🎨 **판정색 규칙 검사**
 *
 * 채점 행동 자체는 `judge` 를 부르는 검사들(`judgeCriteria.test.ts` 등)이 지킨다.
 * 여기서 보는 것은 **실측 콜들이 채점기(`judge`)에서 옳게 나오는가**와
 * **판정하는 곳이 한 곳뿐인가**(소스 검사)다.
 */

describe('실측 콜 회귀 — 옛 사고가 새 채점기에서 재발하지 않는다', () => {

    /** 실측 콜 — 99,000원 · 한계 우회 +6분 + 정차 25분 */
    it('🔴 99,000원 · +31분이면 「돈」이 만점이다 (옛 채점기는 마감 여유 0 으로 뭉개 똥이었다)', () => {
        ...
        // ⚠️ 버퍼를 못 재면 색은 🔴 다 — 이 기준을 빼면 꿀이 나온다
    ...
    it('첫짐 — 운행시간이 아무리 길어도 시급이 좋으면 꿀이다 (2026-08-18 실측 그 콜)', () => {
        // 100,000원 · 98분(+정차 25) — 운행시간 문턱이 아니라 시급으로 본다
        const v = judge(CRITERIA, {
            /* 🔴 **첫짐이다** — 빈 차로 서 있을 때의 2시간 단독 콜이라 «우회»가 아니라 본 운행이다.
               긴 우회 감쇠는 합짐에만 걸린다 (`detourDecay.test.ts`). `firstLoad: false` 로 적으면
               «합짐 2시간 우회»로 읽혀 감쇠가 걸리고 보통이 된다 */
...
describe('판정하는 곳은 한 곳', () => {
    ...
    /**
     * 🔴 **채점하는 곳은 한 곳뿐이다** — `judge`.
     *    채점기가 둘이면 같은 콜이 화면마다 다른 색을 받는다.
     */
 ...
 *
 * 빈칸으로 두고 기다리지 않는다. 이 제품은 **미리 채우고 기사님이 확정하는** 방식이다 —
 * 앱이 느슨하게 집어 오면 결재하시고, 적요에서 미리 클릭해 두면 틀린 것만 고치신다.
 */
/**
 * 🏗️ "미리 눌러 두고 기사님이 확정"이 사는 곳:
...
describe('콜 잡은 시각 — 시간대를 잘못 붙인 값도 읽어낸다', () => {
    ...
    it('🔴 세 형식이 **같은 상차 마감**을 낸다', () => {
        ...
        // ⏱️ 상차 약속 = 잡은 시각 + 20분 (확정) · + 상차 미확인 15 = 완료
...
describe('시트 상태 — 콜마다 새로 선다', () => {
     ...
     * 🔴 **콜마다 시트를 새로 그린다**.
     *
     * `key` 가 **단계 이름**(`CALL_PICKUP` 같은 것)뿐이면 콜이 달라도 같다.
     * 그러면 React 가 컴포넌트를 재사용해 **앞 콜의 `deadlineAt`·물량이 다음 콜 화면에 남는다**.
     */
    /** 🏗️ 콜이 달라지면 시트 상태가 새로 선다 */
    ...
    /**
     * 🏗️ "주행을 몰라도 추천"은 **행이 태어날 때**(시딩) 한다: 시딩이 접근을 모르면
     * 상차 약속을 «잡은 시각 + `pickupPromiseMin`(기본 20분)»으로 폴백해 행에 저장하고,
     * 격자는 그 저장값을 그린다 (`stepSeeder.test` 의 '주행을 모르면' 검사).
     */
    it('🔴 주행을 몰라도 약속 폴백이 행에 저장된다 — 시딩이 한다', () => {
        const seeder = readFileSync(join(__dirname, '../../src/services/stepSeeder.ts'), 'utf8');
        // 통화 전 상차 약속의 폴백은 «잡은 시각 + 20분» 하나다 (판정 기준 `unknown.pickupPromiseMin`)
        expect(seeder).toMatch(/pickupPromiseMin \?\? 20/);
    });

    /**
     * 🔴 상차지 통화 화면에 「이어서 — 하차지도 지금 정하기」 블록을 **두지 않는다** (기사님):
