...
/** 주석을 걷어낸 **코드만** — 주석에 적힌 이름이 거짓 초록을 만들지 않게 */
const codeOnly = (src: string) =>
    src.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/gm, '');

/**
 * 🖥️ **현황판이 서버에 요구하는 규격** (현황판 담당 요청 세 건).
 *
 * 기사님이 전해 주신 요청서 그대로다. 현황판(`client-app/src/statusboard/**`)은
 * **다른 담당의 영역**이고 그쪽 화면이 이 규격을 그대로 읽는다.
 * 그래서 **깨지면 안 되는 계약**이다.
 *
 * 🔴 여기 셋은 전부 «현황판이 읽는 것»이다 — 서버 안에서만 쓰는 값이 아니다.
 *    이름이나 모양을 바꾸려면 **그 담당에게 먼저 알린다** (루트 README.md 「여럿이 한 작업 트리」).
 */
...
/**
 * 📍 **한 플래그가 두 사실을 답하지 않게 가른다** (규칙 ⑤-4 ⑤).
 *
 * `lastFixIsMock` 하나로 출처를 답하면 시뮬레이터 좌표(`source:'mock'`)가 `manual`(손으로 찍음)로
 * 적힌다 — **이름이 거짓말한다.**
 *
...
describe('① 내 위치의 출처를 그대로 남긴다 (현황판 ①)', () => {
    ...
    it('🔴 세션에 «출처» 칸이 따로 있다 — 모의와 손찍기를 가른다', () => {
        ...
        /**
         * 🔴 **`home` 은 세션 칸에 없다.** 집 주소는 «받은 좌표»가 아니라
    ...
    it('🔴 sim 이 «고른 것»을 그대로 답한다 — 제 손으로 다시 판단하지 않는다', () => {
        /**
         * 🔴 **화면이 보는 것은 «지금 기점»이다.** 서버가 경로를 그릴 때 쓰는
         *    바로 그 값을 `originOf` 가 골라 내려 준다. 여기서 또 고르면 «화면은 이천인데
         *    서버는 집»이 생긴다 (규칙 ③).
         */
        expect(sim).toMatch(/originOf\(session\)/);
        /* ⚠️ sim 이 파생식(`lastFixIsMock ? 'manual'`)을 따로 두면 «시뮬인데 손찍음»이라고 말한다 */
...
/**
 * 📱 **폰이 들고 온 필터 지문을 대조하고 저장한다** (현황판 담당 요청 ②).
 *
 * 앱은 `filterVersion` 을 매 scrap 에 싣는다. `DeviceSession` 에 두 칸(지문 · 확인 시각)을 두면
 * «메인폰은 새 필터, 서브폰은 두 번 전 필터»가 폰 탭에 바로 보인다.
 *
