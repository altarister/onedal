...
/**
 * 🪟 **개발 브라우저를 관제앱으로 오인하지 않는다** — 포트 없는 localhost 만 앱이다.
 *
 * 무늬가 포트를 받아 주면 개발 브라우저까지 «앱»으로 읽힌다:
 *
 *     /^https?:\/\/localhost(:\d+)?$/     ← `(:\d+)?` 가 `:3000` 을 통과시킨다
 *
 * Capacitor 는 `https://localhost`(포트 없음)에서 번들을 띄운다. 개발 브라우저를 «앱»으로 읽으면
 * `apiBase()` 가 **라이브**를 돌려준다:
 *
 *   · 로그인이 라이브로 가 라이브 토큰이 저장되고
 *   · 기기 이름이 `v2`(라이브 DB · 로컬은 `1234`)로 보이고
 *   · **로컬 서버 로그에는 아무것도 안 남는데** 화면은 멀쩡히 돈다
 *
 * 🔴 **로컬을 고치면서 라이브를 보게 된다.** 이 레포에서 가장 비싼 종류의 착각이다
