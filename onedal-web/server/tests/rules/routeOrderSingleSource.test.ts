...
/**
 * 🧭 **경로 순서의 원천은 서버 하나다** (기사님 동의)
 *
 * 방문 순서는 서버의 `optimizeWaypoints` 가 병합 경로에서 만든 **한 벌**이다.
 *
 * 관제웹이 자기 TSP(`optimizeRouteOrder`)를 **따로** 돌려 인덱스로 끼워 맞추면, 두 순서가
 * 어긋날 때 ETA 가 엉뚱한 정거장에 붙는다 — "파생값 두 벌" 사고 클래스. 그래서:
 *
 *   ① 서버가 `sync-active-orders` 에 `routeStops` [콜ID·정거장·누적주행분] 를 싣는다
 *   ② 관제웹은 TSP 를 돌리지 않고 그것을 그린다 — 이 테스트가 잠근다
 */
...
/**
 * 🧭 **계측 로그는 «바뀔 때만» 한 줄이다** (기사님 실측)
 *
 * `logRouteStops` 는 **값이 바뀔 때만 한 줄** 남긴다 — `sync` 는 매초 나가므로
 * 무조건 찍으면 로그가 묻힌다.
 *
 * ── 그러려면 ──
 * 지문은 **유저마다 따로** 두고, 빈 목록 검사 **뒤**에 저장한다.
 *
 * 1초 인터벌은 `sessions` 의 **모든 유저**를 돈다. 지문이 모듈 전역 하나이거나 빈 목록에서도
 * 덮어쓰면, 남의 서버 토큰으로 생긴 유령 세션(콜 0건)이 매초 지문만 갈아치우고 나가고,
 * 다음 초에 기사님 세션은 "바뀐 것"이 되어 또 찍힌다 — 영원히.
 *
...
describe('관제웹 — 자기 TSP 를 돌리지 않는다', () => {
    ...
    it('🔴 optimizeRouteOrder(클라 TSP)를 아무도 부르지 않는다', () => {
        // 주석은 검사하지 않는다 — 까닭을 적은 주석까지 잡으면 설명을 쓸 수 없다
...
/**
 * 🔴 **경로는 «마지막 콜»이 아니라 «값이 있는 마지막 콜»에서 읽는다** (기사님 실측)
 *
 *   기록: 콜2 KEEP 처리 중 `pickRouteHolder(activeCalls, …)` — 이때 activeCalls 는
 *         아직 [콜1] 이라 `routeComputedAt`·`sectionDriveMin` 이 **콜1** 에 실린다
 *   읽기: KEEP 이 끝난 뒤 activeCalls 는 [콜1, 콜2] — "마지막"인 **콜2** 를 보면 빈다
 *
 * 비면 routeStops 의 주행분이 전부 null 이 되어 **타임라인이 통째로 폴백으로 돈다** —
 * 합짐 시각·카운트다운·지각 검산이 전부 콜별 파생("혼자 간다")으로 계산된다.
 *
 * 관제웹은 `[...liveRoute].reverse().find(r => r.totalDistanceKm != null)` 로
 * **뒤에서부터 값이 있는 콜**을 찾는다. 서버도 같은 방식이다.
 */
