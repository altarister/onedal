...
/**
 * 📜 **창이 화면보다 크면 손이 안 닿는다** (기사님 실측)
 *
 * 기사님: *"일단 필터 옵션창에 스크롤부터 넣어야겠다. 입력할 수가 없어."*
 *
 * 반경·받을 짐·제외지역 3단까지 세로가 길고, 판정 기준 탭에는 「배송 속도」 세 칸이 더 있다 —
 * 창이 화면보다 크면 **입력칸에 닿지 못한다.**
 *
 * ── 스크롤이 걸리려면 ──
 * `overflow-y-auto` 만으로는 안 된다. 둘이 더 있어야 한다:
 *
 *   ① **창에 높이 제한** — 없으면 내용만큼 자라 넘칠 일이 없고, 바깥의
 *      `overflow-hidden` 이 화면 밖으로 나간 부분을 그대로 잘라 낸다
 *   ② **`min-h-0`** — flex 자식은 기본이 `min-height:auto` 라
...
describe('설정 창은 화면 안에서 스크롤된다', () => {
    it('🔴 필터 창 — 반경·받을 짐·제외지역까지 세로가 제일 길다', () => {
