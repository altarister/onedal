...
/**
 * 📋 **앱이 올리는 칸은 하나도 안 버린다** (기사님 지시 2026-09-12).
 *
 * 기사님: *"차종뿐 아니고 상차지거리와 배송거리도 보내고 있고 기타 급송인지도
 * 보내고 있는데.. 그걸 다 받아야 하는 거 아닌가?"*
 *
 * 🔴 **앱은 처음부터 다 보내고 있었다.** 인성 리스트 한 줄에서 여섯 칸을 읽어 그릇
 *    (`SimplifiedOfficeOrder`)에 채워 올리는데 **서버 INSERT 가 아홉 칸을 버렸다** —
 *    차종·배송거리·급송표시·등록시각·원문·좌표 넷. 인성 콜은 넷만 남았다.
 *    그 탓에 현황판 검산이 「차종·배송거리」를 **«못 잰 축»** 으로 적었고, 규칙대로
 *    걸러진 콜이 화면에 **«통과인데 안 잡음»** 으로 보였다 (2026-09-12 실측).
 *
 * 🔴 **`appFilterKeys.test.ts` 의 거울이다** — 그쪽은 «내려가는 것»(서버 → 앱 필터),
 *    이쪽은 «올라오는 것»(앱 → 서버 원장). 내려가는 쪽만 잠겨 있었다.
 *
...
describe('원장 수집 규격 — 앱이 올리는 것과 서버가 담는 것이 같다', () => {
    ...
    it('🔴 INSERT 가 담는 칸은 표에 실제로 있다 — 없으면 실서버에서만 터진다', () => {
         ...
         *    그 함정을 2026-08-26 에 밟았다 — `tsc`·`jest` 는 통과하고 실서버에서만
