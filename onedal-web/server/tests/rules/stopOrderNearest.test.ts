 ...
 *   상차를 전부 앞에 몰면  ⑴ 가남상차 ⑵ 가남하차 ⑶ 세종대왕면하차 **⑷ 곤지암하차(94분)**
 *   → 4km 앞의 하차지를 두고 **30km 동쪽으로 갔다가 되돌아오는** 경로가 나온다.
 *
 * 🔴 «상차 먼저 · 하차 나중»(`[...sortedPickups, ...sortedDropoffs]`)으로 두 통을 따로 정렬하면
 *    새 콜(가남 상차)이 붙는 순간 차를 30km 동쪽으로 끌고 가고,
 *    하차 정렬의 기준점이 **가남**이 되면서 4km 앞이던 곤지암이 맨 뒤로 밀린다.
 *
 ...
 *
 * → 상차·하차를 한 통에 넣고 가까운 순으로 돌되 (`orderByNearest`), **같은 콜은 상차가 하차보다 앞**이라는
 *   것만 지킨다.
 */
