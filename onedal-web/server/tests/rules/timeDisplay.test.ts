 ...
 */
describe('🕐 펼친 카드 — 상차·하차 줄은 철거됐다 (0905 개정)', () => {
    it('🔴 상차·하차 약속 줄을 다시 만들지 않는다 — 타이틀과 중복이다', () => {
        expect(카드).not.toMatch(/function PromiseLines/);
    });

    it('🔴 왜 지웠는지가 코드에 남아 있다 — 다음 사람이 «빠뜨린 것»으로 읽지 않게', () => {
        const 원문 = readFileSync(join(__dirname,
            '../../../client-app/src/components/dashboard/PinnedRouteCard.tsx'), 'utf8');
        expect(원문).toMatch(/PromiseLines[\s\S]{0,200}철거/);
    });
