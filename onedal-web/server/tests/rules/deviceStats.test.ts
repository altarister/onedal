...
/**
 * 📊 **기기 카드의 세 숫자는 각각 참말이어야 한다** (기사님 지적)
 *
 * 기사님: *"서버를 새로 켰으니 리셋되었고 16개의 콜을 받았으면 수집 14 · 수락 2 ·
 * 취소 0 이렇게 표시되어야 하는 거 아닌가?"*
 *
 * 🔴 **수락은 `countKeep` 한 곳에서 `incrementDeviceStats(…, "grabbed")` 로 올린다.** 올리는 자리가
 *    없으면 **항상 0** 이라 화면이 조용히 거짓말한다 (규칙 ⑤-4 ④).
 *
 * 🔴 **수집은 "탈락한 콜"만이 아니라 본 콜 전부를 센다.** 앱은 잡을 콜을 찾으면 `break` 로
 *    루프를 나가므로, `telemetryManager.enqueue` 가 그 아래에만 있으면 **잡은 콜이 안 세어진다.**
 *    이름이 "수집"이라 전부 세는 것처럼 읽힌다.
 *
 * 기사님 확정: **잡은 콜도 센다** — *"16개 중 몇 개를 봤나"* 가 화면에서
...
describe('📊 수집 — 잡은 콜도 센다', () => {
     ...
     *    이름을 물면 **멀쩡한 코드에서 빨간불**이 난다. 무는 것은 «무엇을 올리나»가 아니라
     *    **«올리는가 · 두 번 세지 않는가»** 다.
     */
    ...
    it('🔴 올리는 것은 **판정을 실은 콜**이다 — 화면이 다시 재지 않게 (현황판 의뢰)', () => {
         ...
         *    그 사본은 갈라진다 (요율 모델이 서면 `minFare` 를 안 보는데 사본은
