...
/**
 * 📢 **배너 층(토스트) — 지도 위에 불투명하게, 필터 줄은 가리지 않는다** (기사님 2026-09-15 · 버그 대장 #146).
 *
 * 기사님: *"토스트팝업이 배경을 먹고 나와서 완전오류 같거든"* · *"지도 위로 하자"*.
 *
 * 🔴 배너는 페이지 흐름 안에 끼던 시절의 모양(`bg-primary/10` 같은 10% 바탕)으로 **무대에서만 지도 위에 띄워졌다**
 *    (0831 «뜰 때마다 아래가 들썩인다»). 뒤가 단색이 아니라 지도라 **90% 비쳤다.**
 *    그리고 헤더 바로 아래에 떠 **필터 줄(필터를 여는 유일한 줄)**까지 10초 동안 덮었다.
 */
