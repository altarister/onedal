 ...
 *
 * 앱 payload 로 콜 객체를 새로 만들면 앞에서 넣은 칸이 **조용히 증발한다** —
 *   · `/confirm` 이 넣은 `targetApp` 을 `/detail` 이 새 객체로 덮어쓰면 그 칸이 NULL 이 된다
 *   · 경로 재탐색이 심사 캐시만 고치고 활성 콜을 안 고치면 둘이 갈라진다
 *
 * 필드가 조용히 증발하는 일이라 **타입 검사도 단위 테스트도 못 잡는다.**
...
/** 주석은 걸러 내고 코드 줄만 본다 — 주석의 설명 글이 위반으로 세어지지 않게 */
