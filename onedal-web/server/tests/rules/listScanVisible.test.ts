 ...
 *
 * 실측: `LIST` 화면인데 콜 0개 — 8/21 은 하루 종일 **18,824회 전부 0개**였다.
...
describe('👁️ 리스트 스캔 — 빈 이유를 구분한다', () => {
     ...
     * 🔴 **숫자만으로는 원인을 못 찾는다**.
     *
     * `요금실패 1` 은 알려 주는데 **어떤 글자였는지**를 안 남겨서, 차종을 못 읽은 건지
     * 요금 자리에 딴 게 있는 건지 확답할 수 없었다. 재현해도 또 모른다 —
     * 어제 리스트 진단을 넣기 전과 같은 상황이다.
     *
