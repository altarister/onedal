...
/**
 * 🚚 **단독 배송 주행 — 합짐 콜도 가져야 한다** (기사님 실측)
 *
 * 하차 약속은 `상차 완료 + 단독 배송주행 × 150%` 로 만들어진다. 단독 주행이 비면 기존 콜들의 하차 약속이
 * 없어 *"37분 밀리면 뭐가 깨지나"* 를 잴 수 없고(딱지 **`버퍼 잴 약속 없음`**), 축 하나가 통째로 빠진 채
 * **높은 축만 평균**해 되돌아가는 콜이 파랑이 된다.
 *
 * ── 뿌리 ──
 * 합짐은 병합 경로만 재므로 단독 주행을 따로 채워야 한다 — KEEP 뒤에는 `measureSoloDelivery`
 * (routeComposer)가 카카오로 한 번 재고, 그 전(심사 중)이나 재지 못했을 때는 `soloMinutesOf` 가
 * 배송거리로 추정한다.
 *
 * 하나가 비면 **셋이 죽는다** — 판정 버퍼 축 · 타임라인 추정 약속 · 단계 시딩 마감.
 * 증상이 셋이라 각각 고치려 들면 폴백만 는다. 읽는 곳은 **`soloMinutesOf` 하나**다.
 *
 * ── 재기 전에는 왜 배송거리인가 ──
 * 앱이 **이미 배송거리를 보낸다.** 인성 리스트 최좌측 두 번째 숫자이고, 앱의 단가
 * 판정(`fare ≥ deliveryDistance × ratePerKm`)이 그걸로 돌아간다. 심사 중 콜마다 카카오를 부르지 않고
 * 그 값(`deliveryDistance`)을 쓴다 — 없는 값을 만드는 게 아니라 **받은 값을 쓰는 것**이다 (규칙 ⑤-2).
 *
...
describe('soloMinutesOf — 실측이 없으면 배송거리로 추정한다', () => {
    ...
    /**
     * 🔴 **거리와 시간은 한 짝이어야 한다** — 같은 출처에서 가져온다.
     *
     * 거리는 `kakaoSoloDistanceKm`, 시간은 `kakaoSoloDurationMin` 처럼 **열쇠를 갈라** 두면
     * 한쪽만 있을 때 카카오 거리에 추정 시간이 붙어 **속도가 거짓말한다.**
     */
...
/** 읽는 자리가 여럿이면 갈라진다 — 전부 한 함수를 거치는지 소스로 강제한다 (규칙 ③) */
 ...
 *
 * 하차 약속 = `상차 완료 + 단독 주행 × 150%` 다. 실측(`PICKED_UP`)은 기사님 보고로만 생기고,
 * 예정은 **경로에 상차 정거장이 남아 있을 때만** 채워진다 — 상차지에 도착하면 정거장이 빠지므로
 * 둘 다 사라져 버퍼 축이 통째로 빠진다(딱지 **버퍼 잴 약속 없음**).
 *
 * 장부에는 도착(`step_arrive_pickup`)이 남아 있다 — 규칙 ⑤-5 대로 **도착 + 상차 정차**로 파생한다.
 */
