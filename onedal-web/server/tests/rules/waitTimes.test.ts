 ...
 *
 * ── 왜 필요한가 ──
 * 대기 시간은 여섯 자리가 쓴다 — 원달앱 폰 설정(`safeCancelTimeout`) ·
 * 원달앱 알람 상세 복귀 · 서버 첫 보고 타이머 · 서버 둘째 보고 경고·해제 · 관제웹 판정석·홀드 막대 ·
 * 시뮬레이터. 각자 30초를 박아 두면 한 곳을 바꿔도 나머지는 30초로 돈다.
 *
...
describe('배차망별 대기 시간 — ② 값 (shared 한 곳)', () => {
    ...
    /**
     * 🔴 **1초 미만은 값이 아니라 고장이다**.
     * 관제웹 칸을 비우고 저장하면 `parseInt('') || 0` 으로 **0초**가 들어온다 — 인성 안전취소 0초면 원달앱이 잡자마자 스스로 취소한다.
     * 상한은 걸지 않는다(기사님 확정) · 하한만 막는다. 못 받은 값은 `null` — 설정 경로가 저장된 값을 그대로 둔다(COALESCE).
     */
