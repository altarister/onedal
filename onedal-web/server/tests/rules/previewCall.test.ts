...
/**
 * 👀 **미리보기 콜 — 확정 전에 판정만 받는다** (기사님 확정)
 *
 * 기사님: *"내가 상세 페이지를 보았을 때 팝업 3장을 열어서 정보를 모두 확인하고 그걸 가지고
 * 판단까지 해주면 나는 **페널티 축적 없이** 콜을 판단할 수 있는 장점이 있군."*
 *
 * 확정 전 상세에서 팝업을 안 열면 주소를 모른다. *"리스트에서 본 콜 중 요금이 같은 것"* 을 역추적해
 * 주소를 빌려 오거나 화면 요약 파싱값(*"오파싱 가능성 있음"*)을 쓰면 주소 자리에 적요 조각이 들어간다
 * (실측, 손으로 연 상세 3회 전부):
 *   1차 선점: `계산서필 → 카톤`      ← 주소가 아니라 **적요 조각**
 *   1차 선점: `가전 → 계산서필`
 *   1차 선점: `박스 → 계산서필`      ← 실제로는 경안동 → 문산읍
 *
 * 🔴 그래서 역추적을 고치는 대신 **역추적이 필요 없게** 한다 —
 *    손으로 연 상세에서 팝업 3장을 먼저 읽으면 주소를 직접 읽는다.
 *
 * 그리고 확정을 안 누른 콜은 **인성에서 아무 일도 일어나지 않은 콜**이다 — 취소할 게 없다.
 * 우리 장부에 취소를 쌓으면 취소 카운트(배차망 10회 패널티를 세는 값)가 부푼다.
 */
...
/**
 * 🔴 **«어느 파일에 있나»가 아니라 «앱 어디에 있나»를 본다**.
 *
 * 인성 잡기 수순은 `HijackService` 밖(`plugins/insung/InsungSequence.kt`)에도 산다.
 * 한 파일만 읽는 검사는 코드를 **옮기는 순간 헛돈다**.
 *
 * 그래서 `HijackService.kt` 를 물으면 **잡기 수순 파일을 함께** 읽어 이어 붙인다.
 * 검사가 지키려는 뜻(«앱 어딘가에 이 규칙이 있다»)은 그대로다.
 */
/**
 * 🔪 함수 본문만 잘라낸다 — **선언 모양에 매이지 않는다**.
 * 같은 함수가 `private fun x(` 로도 `fun ScanContext.x(` 로도 선언된다.
...
describe('👀 미리보기 콜 — 규격', () => {
    ...
    it('🔴 없으면 옛 동작이다 — 필드가 선택적이라 옛 APK 가 그대로 돈다', () => {
        const shared = read('shared/src/index.ts');
        // `isPreview: boolean`(필수)이면 이 칸을 안 보내는 앱의 요청이 규격에서 탈락한다
 ...
 *
 * 콜이 끝나는 길(`dispatchEngine` · `detail` · `emergency`)이 저마다 취소를 세지만
 * `incrementDeviceStats(…, "canceled")` 는 `countCancel`(`core/cancelCount.ts`) 하나만 부른다.
 * 길마다 미리보기 조건을 적으면 한쪽만 고쳐져 갈라진다 — 「목록을 손으로 나열」과 같은 모양이다.
 */
describe('🧮 취소 카운트 — 세는 자리는 한 곳', () => {
    ...
    /**
     * 🔴 **판단에 쓸 값을 지운 뒤에 판단하지 않는다**.
     *
     * `forceCancelEvaluatingOrder` 가 `pendingOrdersData.delete(orderId)` 로 캐시를 지운 **뒤**
     * `countCancel` 을 부르면 세션에서 콜을 못 찾아 `isPreview` 를 영영 못 본다 —
     * 미리보기 콜인데 취소 카운트가 오른다(실측 `FORCE_CANCEL` +1).
     *
...
describe('✋ 미리보기 콜 — 확정 전에는 잡지 않는다', () => {
    /**
     * 🔴 **같은 콜을 두 번 판정하지 않는다** (기사님 지적).
     *
     * 미리보기로 한 번 판정한 콜이 확정을 눌러 다시 올라올 때 또 판정하면,
     * 카카오 호출이 두 배가 되고 **관제웹 카드도 두 번 바뀐다** (기사님: *"이상한 것 같기도 하고"*).
     *
    ...
    it('🔴 미리보기 카드에는 결재 버튼이 없다', () => {
        // 직접 갈래 판별은 isManualLineage 한 곳이다
        ...
        // 심사석(JudgmentSeat)의 직접·알람 갈래에도 결재 버튼이 없다 — 버튼은 자동 갈래(else)에만 산다
...
/**
 * 🖥️ **미리보기에게 "결재 대기"는 판정 완료다** (기사님 실측).
 *
 * 기사님: *"평가를 보여주면 좋을 것 같은데 계속 평가중만 깜박이고 있어."*
 *
 * `ORDER_AWAITING_DECISION` 은 관제웹에서 **결재 대기 = 평가중**으로 그려진다. 보통 콜은 그
 * 자리에 KEEP/CANCEL 버튼이 떠서 할 일이 있지만, 미리보기는 버튼을 숨기므로 평가중만 깜박이면
 * 볼 것이 없다. 미리보기에게 이 상태는 **판정이 끝난 것**이다 — 색을 보여줘야 한다.
 */
...
describe('🏄 상세 수집 — 손으로 연 상세는 읽고 나서 올린다', () => {
    ...
    /**
     * 🔴 **확정을 누르면 서버에 알린다** (실측으로 발견).
     *
     * 기사님: *"관제엡의 노랑색을 보고 확정을 눌렀어. 그런데 관제엡은 내가 생각한 것과
     * 다르게 움직이고 있어. 싱크가 전혀 안 되는 것 같아."*
     *
     * 확정 화면(`DETAIL_CONFIRMED`)에는 요청을 막는 자물쇠가 둘 있다:
     *   ① 상세 수집 분기는 `IDLE` 일 때만 일한다 — 미리보기는 이미 `DONE`
     *   ② `sendConfirmOnce` 는 `isDetailScrapSent` 로 재전송을 막는다
     * 알리지 않으면 서버는 여전히 "미리보기"로 알고 30초 뒤 정리해 버린다 — 기사님은 잡았는데.
     * 그래서 `handleConfirmedScreen` 은 미리보기 딱지가 있으면 벗기고 `sendDetail` 로 알린다.
     *
     * 🔴 **딱지는 벗겨지기만 한다**. 확정 화면에 들어간 순간이 그 자리다.
     */
...
/**
 * 🔴 **배차 요청은 한 줄로 나간다** (기사님 지적 · D안).
 *
 * `confirm` 과 `detail` 을 **같은 풀에 던질 때 스레드가 둘**이면 둘이 동시에 출발해
 * `detail` 이 `confirm` 보다 먼저 닿는다(실측 10ms). 서버 계약은
 * *"confirm 이 콜을 만들고 detail 이 승급한다"* 라 **순서가 뒤집히면 안 된다.**
 *
 * 🔴 앱을 기다리게 만들지 않는다(규칙 ② *"HTTP 를 물고 기다리지 않는다"*).
 *    던지는 쪽은 그대로 즉시 리턴하고, **큐가 넣은 순서대로 하나씩 꺼낸다.**
 *
 * 스레드를 둘로 둘 까닭(롱폴링이 스레드를 오래 무는 것)은 없다 — 피기백 V2 에서 `sendDetail` 은
 * 202 만 받고 즉시 리턴한다.
 * ⚠️ 비상 정리·텔레메트리는 각자 전용 스레드라 영향받지 않는다.
 */
