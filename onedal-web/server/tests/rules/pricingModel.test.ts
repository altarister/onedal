...
describe('단가 판정 모델 — 명세 고정', () => {
    ...
    /**
     * 🔴 라면박스 축 (기사님 확정).
     *    적재는 차종마다 라면박스 수(`VEHICLE_CAPACITY`)로 센다 — 오토바이도 1박스를 차지한다.
     */
    it('적재 용량 — 내 1t 트럭 = 100박스, 1t짐 80 · 라보 40 · 다마스 30 · 승용차 5 · 오토바이 1', () => {
        ...
        expect(VEHICLE_CAPACITY['오토바이']).toBe(1); // 오토바이도 자리를 차지한다
    });
    ...
    it('🔴 표시용 slotsUsed 에 옛 칸 환산(÷7.5)이 없다 — 박스가 곧 표시 단위다', () => {
        // ÷7.5(칸 환산)가 남으면 다마스 30박스가 "4/100박스"로 표시된다
        const fm = readFileSync(join(__dirname, '../../src/state/filterManager.ts'), 'utf8')
            .replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/gm, '');   // 주석은 빼고 코드만 본다
    ...
    it('🔴 DB 요율·수수료를 넘기면 그 값으로 계산한다 — 표를 두 벌 두지 않기 위한 통로', () => {
        ...
        // 폴백 상수(1000원)를 쓰지 않았다는 것 — 이게 갈라지면 앱 필터만 폴백 요율로 돈다
