 ...
 *
 * 통화 단계(0·1)도 현장 단계(2~5)도 건너뛰기를 **마일스톤으로 서버에 남긴다** —
 * 세션에만 두면 새로고침할 때 건너뛴 단계가 되살아난다. 그리고 **출처를 삼분**한다:
 *
 *   | source       | 뜻            | 데이터 가치 |
 *   |--------------|---------------|-------------|
...
describe('저장 경로 — 스킵이 장부에 남는다', () => {
     ...
     *
     * **하차 완료 단계**에는 건너뛰기를 일부러 안 둔다(콜의 끝) — 거기서 버튼이 안 보이는 것은 정상이다.
     */
