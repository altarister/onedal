 ...
 *    아직 가설이어도 안전하다. 대신 `기타` + 메모로 **목록을 고칠 재료**를 모은다.
 */
...
describe('짐 폼 — 실어 본 뒤에만', () => {
    ...
    it('🔴 실측(ACTUAL) 짐 입력은 완료 단계 컴포넌트(LiveDone)에만 있다', () => {
        // 🏗️ 단계마다 컴포넌트가 따로 있어, 어느 단계에 무엇이 있나를 컴포넌트 범위로 본다
        const c = code();
    ...
    /**
     * 📝 **완료 단계에는 메모가 없다** (기사님 확정: *"상차 완료, 하차 완료에
     *    메모가 필요 없겠다. 기타가 새로 생겨서"*).
     *
     * 짐 폼의 자유 메모와 사유의 `기타` 메모를 함께 두면 **같은 자리에 둘**이 된다. 무엇을 어디에
     * 적어야 하는지 흐려지고, 같은 사실이 두 곳에 나뉘어 남는다 (규칙 ③).
     * 현장에서 남길 말은 **사유 + 기타 메모**로 충분하다.
     *
     * ⚠️ **통화 단계에는 남긴다** — 거기엔 사유 칩이 없고, 통화에서 들은 말
     *    (*"지하 2층, 경비실 통과"*)은 어딘가 적어야 한다.
     */
    it('🔴 메모 입력은 통화 단계에만, 라벨은 `기타` (현장 사유 갈래와 같은 말)', () => {
        // 통화 단계의 메모 라벨은 «기타»다 (기사님) — 현장 단계의 `기타` 갈래와 짝
        const c = code();
