...
describe('짐 폼 — 실어 본 뒤에만', () => {
    ...
    /**
     * 📝 **완료 단계에는 메모가 없다** (기사님 확정 2026-08-19: *"상차 완료, 하차 완료에
     *    메모가 필요 없겠다. 기타가 새로 생겨서"*).
     *
     * 짐 폼의 자유 메모와 사유의 `기타` 메모가 **같은 자리에 둘**이 되었다. 무엇을 어디에
     ...
     */
    it('🔴 메모 입력은 통화 단계에만 (현장에는 기타 메모가 있다)', () => {
        expect(code()).toMatch(/isCall && [\s\S]{0,300}메모 \(선택\)|메모 \(선택\)[\s\S]{0,200}\)\}/);
    });
});
