...
/**
 * 🧭 **순서는 살아 있되, 근소한 차이로는 안 뒤집힌다** (기사님 승인)
 *
 * ── 왜 버티게 하나 ──
 * `orderByNearest` 는 **가장 가까운 곳부터** 잇는 탐욕법이고, 1초 동기화와 GPS 매 틱이 부른다.
 * 기사님이 달리는 동안 기점이 매초 바뀌므로, 두 후보가 엇비슷하면 **몇십 미터 움직인 것만으로
 * 1번이 바뀐다** — 서버가 같은 카카오호출시점·같은 정거장으로 몇 초마다 다른 답을 내고,
 * 화면에서는 번호가 춤춘다.
 *
 * ── 얼리지는 않는다 ──
 * 저장된 순서(`sectionStops`)를 그대로 되쓰면 새 콜이 끼었을 때 순서가 실제 동선과 어긋나
 * 2.4km 앞 하차지를 두고 먼 상차지로 먼저 간다 (`pnpm drive` 가 잡는다).
 *
 * → **순서는 살아 있어야 한다.** 고칠 것은 «다시 세우는 것»이 아니라 «근소한
...
describe('갈 순서 — 흔들리지 않되 얼지도 않는다', () => {
    ...
    it('🔴 직전 순서가 있으면 근소한 차이로는 안 뒤집힌다 — 기점이 움직여도 그대로', () => {
        ...
        for (const gps of [
            ...
            { x: 127.4260, y: 37.300 },   // B 쪽이 근소하게 가깝다 — 버팀이 없으면 여기서 뒤집힌다
            { x: 127.4270, y: 37.300 },
     ...
     *
     * 보낸 순번이 남은 정거장을 다 덮으면 그 순번을 따른다 (아래 검사). 순번이 모르는 정거장
     * (방금 KEEP 한 콜)이 끼면 덮지 못하므로 가까운 곳부터 다시 정한다 — «영영 얼지는 않는다».
     */
    ...
    /**
     * 🔴 **카카오에 보낸 순번이 남은 정거장을 다 덮으면 그 순번을 따른다** (기사님 결정).
     *
     * 기사님: *"콜이 들어와 경로를 계산하고 그걸로 카카오에 순번까지 보냈으면 그걸로 끝일 거 같은데"*
     *
     * ── 실측 («7지점 한 바퀴» 모의 주행) ──
     * 카카오에 «신둔 하차 → 사음동 상차 → 관고동 하차 → 터미널 하차»로 보낸 뒤,
     * 신둔에 **2초 남은 자리**에서 굽은 길 때문에 사음동(1.91km)이 신둔(2.33km)보다
     * 직선으로 가까워진다 (비율 1.22 > 버팀 20%). 거리만 보면 사음동이 1번을 뺏어 **근접 예고
     * (도착 전 통화)가 사음동으로 가고**, 주행분은 카카오 순서로 붙어 «⑷ 누적 18분»이 나온다.
     *
     * 🔴 **순번을 따라도 도착은 빠지지 않는다** — 도착·지나침은
     *    «순서가 아니라 거리»로 찍힌다 (`arrivalByDistance.test.ts`). 순서가 실제 동선과 달라도
