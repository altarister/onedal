 ...
 *
 * 🔴 **값은 한 벌, 사용자마다 «한 행»이다** — 국면마다 행을 두면 같은 값이 여러 곳에 살아 갈라진다.
 *
 * 🔴 표 하나가 컬럼과 폼을 만들고, **없는 값을 지어내지 않는다.**
 */
...
describe('FILTER_FIELDS — 표 하나가 컬럼·폼을 다 만든다', () => {
    ...
    /**
     * 🔴 **이름이 «한 벌»이다**. 표의 `path` 가 **평면(앱 피기백) 이름 그대로**라
     *    그릇 사이를 옮기는 다리가 없다. 같은 값을 두 이름으로 부르면 옮기는 다리가 생기고
     *    거기서 값이 갈라진다.
     */
     ...
     * 읽으면 **0 이 기본값을 이긴다.** 예를 들어 각도 칸이면
     * 필터 화면에 **출발각 0°** 가 뜨고, 0° 는 그물이 아예 닫히는 값이다.
...
describe('값 다섯의 원천은 «한 행»이다 (이식 C3-3b · 2026-09-11)', () => {
    ...
    /**
     * 🔴 **편집 자리는 🔍 필터 하나다** (기사님 확정).
     * 요율 탭에 노선·반경 칸을 또 두면 같은 값을 고치는 화면이 둘이 되어
     * «두 벌 값»(한쪽 1km · 다른 쪽 15km)이 생긴다.
     * 절대 하한가·상한가 입력도 두지 않는다 — 하한은 단가표 × 콜할인율 파생만.
     */
