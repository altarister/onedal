...
/**
 * 🛫 **반대쪽으로 «얼마나» 멀어지나** — 첫짐 (Step 8-2)
 *
 * ── 왜 ──
 *
 * 노하우 25행: *"먼 첫짐은 비싸도 나쁘다 (합짐 예산이 안 남는다)"* ·
 * *"잘못 고르면 **오전이 1콜로 끝난다**"* — 멀리 나가면 그 길 위에도 그 끝에도 콜이 드물다.
 *
 * 🔴 **그런데 「지리」가 그걸 못 본다.** 전진율은 **나누기**라서 «얼마나»가 약분된다 —
 *    60km 멀어지는 것과 180km 멀어지는 것이 둘 다 `-1` 이다.
 *    나누기 전의 값(`destGainKm`)은 이미 계산하고 **버리기만** 했다.
 *
 * 🔴 **거리만으로 깎으면 안 된다** (노하우 231행: *«장거리 꿀콜이 억울하게 떨어질 수 있다»*).
...
describe('🛫 첫짐 — 멀어진 거리를 본다', () => {
    ...
    /** 🔴 이 검사가 생긴 까닭 — 60km 와 180km 가 같은 점수였다 */
    it('🔴 멀리 멀어질수록 더 깎인다', () => {
