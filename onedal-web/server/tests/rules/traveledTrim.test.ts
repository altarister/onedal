...
describe('지나온 구간 제거 — 다시 그리지 않고 숫자만 비교한다', () => {
    ...
    it('🔴 제거는 파생 계산의 **끝 한 곳**에서만 한다', () => {
        // 회랑을 다시 그리는 길이 여럿인데(경로 갱신·반경 변경·국면 전환),
        // 어느 길로 오든 다시 그리면 지나온 동이 되살아난다
        const derive = fm.slice(fm.indexOf('function recalculateDerivedFields'), fm.indexOf('export function rememberCorridorProgress'));
        expect(derive).toMatch(/applyTraveledTrim\(session\)/);
        expect((fm.match(/applyTraveledTrim\(session\)/g) || []).length).toBe(1);
    });
});
...
describe('옛 방식은 지웠다', () => {
    ...
    it('GPS 이동은 방아쇠만 당긴다 — 제거 로직을 여기서 다시 쓰지 않는다', () => {
        ...
        const body = move.slice(0, move.indexOf('[2] 도착 감지') > 0 ? move.indexOf('[2] 도착 감지') : 1500);
        expect(body).not.toMatch(/destinationKeywords/);
        expect(body).toMatch(/applyFilterCb\(userId, \{\}\)/);
    });
});
