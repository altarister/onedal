...
/**
 * 🔴 **옛말 금지 — 용어집이 유일한 원천이다** (기사님)
 *
 * 기사님: *"새로 정의를 해도 너는 자꾸 이전 용어를 쓰고 있고."*
 * 코드에 옛말이 살아 있으면 그걸 읽는 쪽(사람이든 Claude 든)이 옛말을 다시 배운다.
 * 그래서 **새 코드가 옛말을 쓰면 여기서 빨간불**이 나게 한다.
 *
 * 🔴 **이 검사는 셋을 지켜야 믿을 수 있다** — 하나라도 빠지면 옛말이 살아 있어도 초록불이다.
 *
 *    **① 주석까지 본다** — 이 레포는 설계 의도를 주석에 남기는 것이 규칙이라, 주석을 걷고(`codeOnly()`) 보면
 *      *"읽는 쪽이 옛말을 다시 배운다"* 는 문제가 정작 주석에서 무방비다
 *    **② 금지어 목록이 용어집만큼 길다** — 용어집의 폐기어(`광클`·`무인 서핑`·`팝업 서핑` 같은 것)가 목록에 없으면 그대로 통과한다
 *    **③ 앱(Kotlin)도 읽는다** — `.ts/.tsx/.mjs` 만 훑으면 `onedal-app` 이 통째로 빠진다
 *
 ...
 *      — ⓑ 를 빼지 않으면 일괄 치환이 금지패턴 검사까지 바꿔 멀쩡한 새말을 금지시킨다
 */
...
const ROOTS = [
    ...
    join(__dirname, "../../../scripts"),               // 검증 스크립트
    join(__dirname, "../../../../onedal-app/app/src/main/java"),   // 앱 — ③
];
...
const BANNED: { name: string, pattern: RegExp, allow?: RegExp, until?: string }[] = [
    ...
    { name: 'eyeline → callDiscount', pattern: /eyeline/i },      // DB 컬럼 이름까지 새말이다
    {
    ...
    /**
     * 🏁 **종착지 → 확정콜의 마지막 하차지** (기사님 확정).
     *    «종착지»는 «여기가 끝»으로 읽혀 **목적지와 섞인다.** 그런데 그 자리는 콜을 하나 더 잡으면 바뀐다 —
     *    목적지는 기사님의 **의도**이고, 저것은 지금 확정된 콜이 만든 **상태**다.
     *    기사님: *"중간에 용산 방면의 하차지를 잡는다면 종착지가 바뀌는 것이기 때문이다."*
     */
