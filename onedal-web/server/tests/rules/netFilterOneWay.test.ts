...
/**
 * 🕸️ **필터 목록은 그물 한 벌이 만든다 — 모든 때에** (기사님 확정).
 *
 * 이천 **시 경계**에서 10km 넓히는 계산(turf)은 초월읍까지 담아 «7지점 한 바퀴» 06 콜(중리동 → 초월읍)을
 * 도착지 축에서 통과시킨다 — 그물(`netKeywordsOf`)이면 빠진다(중심 18km). 그래서 목록을 다시 만드는
 * 자리는 모두 그물 한 곳(`rebuildNetFilter`)을 부르고, 시 경계 계산을 거치지 않는다:
 *   · 서버 부팅 · 마지막 콜이 끝나 0건 → `rebuildDestinationKeywords`
 *   · KEEP · 경로 재계산 · 재탐색 · 귀가콜 → `syncDetourFilter`
 *   · 합짐 중 반경 변경 → 소켓 · `refreshDetourIfNeeded`
 *
 * 결정:
 ...
 * 🗺️ **지도도 같은 것을 그린다**.
 *
 * 서버는 얼린 라인 위 GPS 진행도로 지나온 동을 앱 목록에서 뺀다. 지도가 그것을 모르고 **지나온 동을
 * 점으로 찍거나** 라인 띠를 통째로 칠하면 화면과 판정이 다른 말을 한다.
 *   · 동 점 = 지도는 찍지 않는다 (`useCallNet` 레이어를 쓰지 않는다)
 *   · 라인 띠 = 이동 중이면 하차 띠를 현위치부터 긋고, 뒤는 평평하게 자른다 (목업과 같다)
 */
...
/**
 * 🧭 **지나온 곳 빼기가 목적지 쪽 동을 먹지 않게** 두 가지를 지킨다.
 *
 *   ① 서버가 부팅 때 되살린 묵은 위치로 KEEP 순간 빼면, 새 경로의 앞부분(19.2km)을 «지나왔다»며
 *      목록을 21 → 10곳으로 줄인다
 *      → 빼기는 **지금 위치(`originOf` · 묵었거나 집 주소로 대신한 것 아님)**로만 한다
 *   ② «경계가 걸치면 넣기»가 그물이 이미 넣은 목적지 영역 동(관고동·사음동)에도 순서를 붙이면 빼기에 걸린다.
...
/**
 * 🧩 **필터 영역을 서버 목록으로** (기사님 확정).
 *
 *   · 출발 전(`session.departedAt` 없음)이면 내 영역을 그물에 넘긴다 — 출발하면 안 넘긴다
 *   · 관내를 따로 재지 않는다 — 가까이 온 목적지는 목적지 원에 걸친 동이다. 각도를 360° 로 바꿔 마름모를
 *     원으로 만들면 여주·용인 처인까지 35곳이 들어온다
...
describe('🧩 필터 영역 — 출발 전 내 영역 · 관내 목적지 원 (2단계)', () => {
    ...
    it('🔴 지도도 같은 두 값으로 그린다', () => {
        const stage = client('components/stage/StageView.tsx');
        /* 🔴 «출발했나»는 목적지 묶음이 아니라 **조각**이 본다 — 지도도 서버와 같은 사실을 쓴다 */
...
/**
 * 🚀 **주행이 감지되면 출발이다** (목업 `MapMockup.tsx` 의 `if (driving) setDeparted(true)` 와 같다).
 *
 * «🚀 지금 출발» 버튼만 출발을 켜면, 운전 중에는 누를 수 없어(기사님: 먼발치 1~2초 · 무입력에도 일이 되게)
 * 국면이 «콜 쥠»에 머문다. 출발이 안 켜지면 필터 영역이 «출발 전»에 머물러 내 영역이 운행 내내 남는다.
 */
...
describe('🏠 복귀 대기 — 목적지 둘 (3단계)', () => {
    ...
    it('🔴 하차 목록은 살아 있는 목적지마다 조각을 만들어 합친다 — 먼 목적지는 상차 목록 동을 뺀다 (2026-09-15 개정)', () => {
        ...
        /* 상차 목록은 시 · 군 · 구와 짝지어 뺀다 (`activeFilter.pickupGroups`) */
...
/**
 * 🧵 **필터 값을 바꿔도 콜을 쥐었으면 경로 영역이 남는다**.
 *
 * `recalculateDerivedFields` 의 지리 재계산 분기(복귀 켜기 · 각도 · 마름모반경 · 현위반경 · 제외 지역)가
 * 목록을 **라인 없이**(`null`) 다시 만들면, 뒤따라 라인으로 다시 만드는 길이 없어서(`refreshDetourIfNeeded` 는
 * 라인반경만 본다) 다음 KEEP·하차까지 **경로 영역이 빠진 목록**이 폰에 간다 — 복귀를 켜는 순간 가는 길의 동이 사라진다.
