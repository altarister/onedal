 ...
 *
 * 🔴 `InsungParser` 안에서 그 목록을 쓰는 자리는 셋이다 — 요금 앵커링 · 뭉친 텍스트 폴백 · `groupListNodes`.
 *    자리마다 `(오|다|라|승|1t|…)` 를 따로 적으면 한 곳에서 «승»이 빠져도 모른다.
 *
 * `groupListNodes` 는 **카드를 묶는** 자리다. 여기서 빠지면 그 카드는 그룹이 안 생기고, 그룹이 없으면
