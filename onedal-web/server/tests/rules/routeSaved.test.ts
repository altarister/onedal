...
/**
 * 🗺️ **잰 경로는 장부까지 간다 — 메모리에만 두지 않는다** (기사님 판 실측 2026-09-12 밤).
 *
 * ── 무엇이 틀렸나 ──
 * 경로 칸들은 `insertOrder` 가 **콜을 확정할 때 한 번** 저장한다. 그런데 경로는 **그 뒤에**
 * 계산되고 합짐이 붙을 때마다 **다시** 계산된다. 그래서 나중에 홀더가 된 콜은 그 값이
 * **영영 장부에 안 들어갔다** — 어드민 실측: 사이클마다 **첫 콜만** 값이 있었다.
 *
...
describe('경로는 장부까지 간다', () => {
    ...
    /**
     * 🔴 **이 한 건이 이 판의 핵심이다.** 부르는 자리가 여덟이라 «잊으면 조용히 실패»한다 —
     *    그래서 싣는 일과 적는 일을 **한 이름**으로 묶었다. `applyRoute` 를 직접 부르면
    ...
    /**
     * ⚠️ **범위를 좁혀서 센다.** 처음에는 `slice(i, i+400)` 으로 봤는데, 두 감싼 함수가
     *    가까이 있어 **한쪽에서 저장을 빼도 옆 함수 것이 잡혀** 초록이었다 (변이로 드러났다).
     *    지금은 «함수 본문 안에 한 번씩, 합쳐 두 번»을 센다.
     */
     ...
     *    주행분이 남의 이름에 붙는다 (버그 대장 #60). 한 번에 쓴다.
     */
    ...
    /**
     * 🔴 **모르는 값으로 아는 값을 덮지 않는다.** 주행분 없이 다시 잰 판이 옛 주행분을
