...
describe('🛣️ 우회 시간 — 길수록 값을 깎는다', () => {
     ...
     */
    it('🔴 우회 235분 15만원은 꿀로 안 보이고 한계에서보다 더 깎인다 — 하루를 거는 일이다', () => {
        /* 🔴 색이 아니라 **숫자**가 말한다 — 「보통」은 꿀 아래 전부다 (판정 1단계).
           🔴 **숫자를 박지 않는다** — 설정에서 오는 꿀 경계와, 같은 콜의 한계(180분) 값에 견준다.
              옛 판은 `< 40` 이었고 그 40 은 색 경계(`color.normalMin`)에서 왔다 — 그 값이 색을
              안 만들게 되면서 근거를 잃었다. */
        expect(score(150_000, 235)).toBeLessThan(cfg.color.honeyMin);
        expect(score(150_000, 235)).toBeLessThan(score(150_000, cfg.detour.hardMin));
    });
    ...
    it('🔴 한계(180분)에서 값이 0.39 다 — 10만/h 짜리도 39점이다', () => {
        /* 🔴 위 「주의 한계에서 3/4」와 **한 벌**이다 — 감쇠 계수를 점수로 못박는다 */
        expect(score(300_000, 181)).toBe(39);               // 10만/h 만점 × 0.39
    });
