 ...
 *
 * 보유 0건에서 강제 정리하면 불변식이 콜 잡기를 되켠다. 그런데 KEEP 콜을 보유한 채
 * 다른 콜을 강제 정리하면 **불변식이 침묵**해, 뒤 콜이 «평가 보류»만 반복하고 콜 잡기가 통째로 멈춘다.
 *
...
describe('#80 선점 잠금은 심사 중인 콜만 본다', () => {
    ...
    it('🔴 확정 콜을 보유 중이어도, 심사 중인 콜이 0건이면 콜 잡기를 되켠다', () => {
        ...
        // KEEP 콜을 보유한 채 강제 정리한 세션: 01·03 은 KEEP 으로 보유 (캐시에도 남는 것이 정상 동작),
