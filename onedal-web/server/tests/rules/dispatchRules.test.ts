 ...
 *   · MANUAL 에 안전취소가 없는 것을 "누락"으로 읽음 → 지우자고 제안
 *   · 6단계 시퀀스를 "중복"으로 읽고 두 단계를 한 번에 저장
 *   · 요약 줄의 콜 번호를 "군더더기"로 읽고 제거
 *
...
describe('규칙 ① 콜의 주인은 기사님이다', () => {
    ...
    /**
     * 출처: CLAUDE.md 규칙 ① *"직접콜(MANUAL)은 심사하지 않는다 — 기사님 의도다"*
    ...
    /**
     * 출처: CLAUDE.md 규칙 ① *"KEEP 된 콜은 절대 취소하지 않는다"*
...
describe('규칙 ③ 데이터를 변조해서 동작을 바꾸지 않는다', () => {
    /**
     * 출처: CLAUDE.md 규칙 ④ *"합짐이라고 상차반경을 999km 로 덮어쓰지 않는다 — 룰로 건너뛴다"*
