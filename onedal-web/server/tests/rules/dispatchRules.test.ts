 ...
 *   · MANUAL 에 데스밸리가 없는 것을 "누락"으로 읽음 → 지우자고 제안 (2026-08-13)
...
describe('규칙 ① 콜의 주인은 기사님이다', () => {
    /**
     * 출처: `docs/architecture/filter_system.md` §6
    ...
    /**
     * 출처: `docs/sequenceDiagram_MANUAL.md`
     *   *"[MANUAL TRACK] 기사님 수동 콜 포획 (사용자 의도 → 데스밸리 불필요)"*
     *   *"기사님 의도이므로 묻지 않고 확정"*
    ...
    /**
     * 출처: `docs/architecture/safety_mode_architecture.md` §핵심 방어 원칙 1 (Phase 1)
...
describe('규칙 ② 안전장치는 겹쳐 둔다', () => {
    /**
     * 출처: `docs_backup/troubleshooting/Dispatch_synchronization_failure.md`
     ...
     */
    it('데스밸리 타이머는 ID 를 저장해 취소할 수 있어야 한다', () => {
        const src = read('routes/detail.ts');
...
describe('규칙 ③ 데이터를 변조해서 동작을 바꾸지 않는다', () => {
    /**
     * 출처: `docs/architecture/filter_system.md` §5 · 변경 이력 2026-04-23
