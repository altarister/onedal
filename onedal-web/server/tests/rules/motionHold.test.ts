...
/**
 * ⏱️ **«주행·정차로 굳는 시간»은 기사님이 정한다** (화면규칙 S16)
 *
 ...
 * 10초가 코드에 박혀 있으면 빠름(40배) 모의 주행에서 줄곧 `idle` 이고,
 * 「주행이면 시트가 내려간다」가 한 번도 발화하지 않는다.
 *
...
describe('S16 — 주행·정차로 굳는 시간은 고칠 수 있다', () => {
    ...
    it('🔴 **설정을 바꾸면 그 자리에서 따른다** — 클로저에 가두지 않는다', () => {
        /**
         * 🔴 **설정값을 첫 렌더에 가두지 않는다** (기사님 실측).
         *    `holdMs` 를 빈 의존성 `useEffect` 안에서 읽으면 **첫 렌더의 10초가 갇힌다** —
         *    설정을 1초로 바꿔도 판정은 처음 값으로 돌아 주행 판정이 안 뜬다.
         *    화면이 조용히 거짓말하는 모양이다.
         *
