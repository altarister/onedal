...
/**
 * 📍 **직접콜의 주소는 팝업 상세가 채운다 — 승격은 한 곳에서**
 *
 * 🔴 직접콜은 규칙 ①(심사하지 않는다)로 심사(`OrderEvaluator`)를 안 지난다. 팝업의 «위치» 를 콜 주소로
 *    올리는 승격이 **심사 안에만** 있으면 직접콜은 승격을 못 받아 장부에 `수집중(상세확인필요)` 로 남고,
 *    그 주소 없는 콜이 경로·progressKm 를 오염시켜 **미탐·오탐**을 만든다.
 *    재료(`pickupDetails[0].addressDetail`)는 detail.ts 가 **모든 콜에서** 뽑는다.
 *
 * ⚠️ 앱의 캐시 매칭(요금으로 리스트 콜 역추적)이 리스트의 진짜 주소를 재활용해 이 구멍을 **가릴 수 있다.**
 *    상세 요금이 파싱을 빠져나가면(`45,000(선불)` → `fare=0`) 매칭이 무너지고 구멍이 드러난다.
 *
 * → 승격을 `promoteDetailAddresses` **한 함수**로 꺼내 detail 수신 직후(분기 전)에 두고,
 *   심사도 같은 함수를 쓴다. 필터콜·직접콜·미리보기가 **같은 문**으로 주소를 얻는다.
 */

/** 🧾 실물 증거 — 폰(OneDalPrefs)에 남은 실제 상세 원문의 뼈대 */
const REAL_RAW_TEXT = [
