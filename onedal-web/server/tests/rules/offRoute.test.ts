...
/**
 * 🛣️ **부여받은 경로 ↔ 실제 궤적 — 얼마나 벗어났나** (기사님 지시).
 *
 * 기사님: *"카카오 라인과 내 궤적이 같이 있어야 **얼마나 잘못 갔는지** 확인할 수 있을 것 같아."*
 *
 * ── 이 값이 `gps_tracks` 표의 원래 이유다 ──
 * `gps_tracks` 는 이 질문에 답하려고 쌓는다 — *"네비게이션이 가리키는 경로를
 * 놓쳐서 지나치면 **얼마나 우회하게 되는 건지**… 부여받은 경로와 현실의 주행 궤적을
 * 매칭해야 차이를 확인할 수 있을 듯."* 점만 쌓아서는 답이 안 나오고, **경로와 대 봐야** 한다.
 *
 * 🔴 **재료는 한 번에 온다.** `nearestPointOnLine` 이 진행도(`location`)와
 *    떨어진 거리(`dist`)를 **함께** 주고, `snapToRoute` 는 둘 다 내놓는다(`progressKm` · `offRouteKm`).
 *    거리를 버리면 「얼마나 벗어났나」를 답할 값이 매 좌표마다 손에 들어왔다 사라진다.
 */
...
describe('경로 이탈 — 부여받은 길에서 얼마나 떨어졌나', () => {
    ...
    /**
     * 🔴 **`progressAlongPolyline` 은 `snapToRoute` 를 감싼 한 줄이어야 한다** (규칙 ③ — 두 벌이면 답이 갈라진다).
