...
/**
 * 🎯 **도착은 «거리»가 정하지 «순서»가 정하지 않는다** (기사님 지시).
 *
 * ── 왜 순서를 안 보나 ──
 * 정거장 순서는 **직선거리 탐욕법**(`orderByNearest`)이라 굽은 길에서 뒤집힌다 — 한 정거장이
 * 수십 초 사이에 ⑴ → ⑷ → ⑴ 로 오간다. «순서상 첫 정거장»(`nextStopOf`) 하나만 보면
 * 그 차례가 안 와서 도착이 안 찍힌다. 그래서 도착 후보는 `arrivalCandidates` 가 목록 전부에서 고른다.
 *
 * 🔴 **500m 안에 들어왔으면 목록에서 몇 번째든 도착이다.** 거리는 순서를 안 본다.
 *
 * 🔴 **이 갈래는 모의 주행으로 못 본다.** `evaluateArrivalTick` 이 `source === 'mock'`
 *    이면 스톱워치 없이 «서 있다»고 온 틱에 발화해서, 30초 스톱워치 갈래가 **모의 주행에서 안 돈다.**
 *    실 GPS 에서만 드러나므로 여기서 순수 함수로 잠근다.
 *
 * ⚠️ **떼는 것은 «순서»뿐이다.** «제 짐을 싣기 전에는 못 내린다»는 물리 규칙이라 남긴다 —
 *    어드민 실측(궤적 1,294점): 아직 안 실은 콜의 하차지가 **500m 안이던 점 22개**,
 *    그중 셋은 **0m**(거기 정차). 순서만 보면 `orderByNearest` 가 그 하차지를 후보에서
 *    빼 주어 **덤으로** 지켜지지만, 순서를 떼면 그 보호가 함께 떨어지므로 `arrivalCandidates` 가 따로 지킨다.
 */
...
describe('도착 후보 — 순서가 아니라 거리가 정한다', () => {
    ...
    /**
     * 🔴 **순서상 둘째·셋째도 후보다.** 첫 정거장 하나만 후보로 두면,
     *    둘째가 500m 안에 들어와도 «다음»이 될 차례가 올 때까지 도착이 안 찍힌다.
     *    순서가 흔들리면 그 차례가 영영 안 올 수도 있다.
     */
