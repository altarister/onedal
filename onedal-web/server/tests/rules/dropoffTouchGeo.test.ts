...
/**
 * 🔵 **먼 목적지의 하차 조각도 «걸친 동»을 넣는다 — 실제 지도로** (기사님 «영역에 지역이 걸치고 있으면 들어가는거야»).
 *
 * 그물(`netForGoal`)은 동을 **중심점 하나**로 본다 — 그것만 쓰면 원 · 마름모 가장자리에 걸쳤는데 중심이 밖인
 * 넓은 읍 · 면이 빠진다 (인천 조건 실측 45곳). 걸침은 상차 목록과 같은 식이다 (`geoService.regionsTouchingAreaGrouped` — 격자 점 ∪ 동 꼭짓점).
 ...
 * (`filterManager.netKeywordsOf`). turf 버퍼는 **끝이 둥글다** — 자르지 않으면 라인 시작(출발 자리)
 * 뒤로 띠 반경만큼 반원이 붙어, 앞에서 `lineZoneOf` 로 잘라낸 뒤쪽 동이 여기서 **다시 들어오고**
 * 「도척 상차 → 경안동 하차」 같은 역방향 콜이 통과한다.
 *
 ...
 *    `lineZoneOf` 와 이 띠 계산으로 갈라지므로 둘 다 도려내야 한다 — 안 도려내면 **운행 중에만** 상차가
 *    하차에 통째로 잠긴다 (상차 동이 전부 «둘 다»가 되고 «상차만»이 0곳이 된다).
