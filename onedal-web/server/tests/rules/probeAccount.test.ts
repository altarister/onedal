...
/**
 * 🔬 **실측은 기사님 계정을 만지지 않는다** (2026-09-12 · 사고 뒤 신설).
 *
 * 🔴 **그날 두 번 당했다.** CDP 화면 실측 스크립트가 `/api/auth/bypass` 로 로그인하는데
 *    그 라우트는 **«DB 의 첫 번째 유저»** 를 준다 — 곧 **기사님 계정**이다.
 *    그래서 스크립트가 누른 「↩︎ 되돌리기」와 「도 고르기」가 **기사님이 맞춰 두신
 *    오늘의 필터(파주시 · 제외 9곳)를 실제로 날렸다.**
 *
 * 🔴 **화면을 실제로 눌러 보는 것은 포기할 수 없다** — 그날 버그 #108 을 찾아낸 것이
 *    코드 읽기가 아니라 그 실측이었다. 그러니 **도구를 고친다.**
 *
...
describe('실측 전용 계정 — 기사님 것을 안 만진다', () => {
    ...
    it('🔴 부탁했을 때만 낸다 — 기본은 지금 그대로 «첫 번째 유저»다', () => {
        ...
        /* 안 달고 오면 옛 길 그대로 — 시나리오·e2e 가 이 길로 돈다 */
    ...
    /**
     * 🔴 **실측 계정이 시뮬레이터를 막으면 안 된다** (2026-09-12 실측에서 바로 드러났다).
     *
     * `/driver-location` 과 `/preflight` 는 *"로컬 판은 기사님 한 분이다 — 세션이 여럿이면
     * **고르지 않는다**"* 로 지켜져 있다. 그런데 실측 계정이 붙는 순간 세션이 **둘**이 되어
