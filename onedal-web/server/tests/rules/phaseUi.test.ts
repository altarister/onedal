 ...
 *
 * 🔄 C4-11b(기사님 «지도와 같은 수로»)는 지도가 그린 그물 수를 store 에 올려 먼저 봤다.
...
describe('필터 디자인 — 목업 순서 (C4-6)', () => {
    ...
    it('🔴 순서가 목업 그대로다 — 목적지 → 그물 → 반경 → 값 → 제외지역', () => {
         ...
         *
         * 고르는 것은 여전히 «집으로 갈지 말지» 하나다(`homeOn`). 다만 **자리가 바뀌었다** —
         * 목적지 줄 «안»에 두었더니 그 행이 **접힌 채로** 열려서 «펼치기 → 누르기» 두 겹이 됐다.
...
describe('국면 전환 — 입구는 하나, 확인창을 거친다', () => {
     ...
     */
    /**
     * ⚠️ **모양이 버튼 셋에서 «고르기 칸 하나»로 바뀌었다** — 확인창은 그대로다.
    ...
    /**
     * 🔴 **관내 표시는 걷었다** (기사님 2026-09-15) — 관내는 국면이 아니라
...
describe('경유 갱신 — 구현은 하나여야 한다', () => {
    ...
    it('경유을 부르는 자리가 늘어나도 계산은 filterManager 한 곳이다', () => {
        /**
         * 🔴 **0 이다**. 예전엔 `syncDetourFilter` 가 자기 몫으로 하나를
