...
/**
 * 🔍 **코드 리뷰가 잡은 것** — 직접 확인해서 진짜인 것만 여기 검사로 세운다.
 *
 * 🔴 넷 다 깨지면 판정이 틀리거나 사라진다:
 *   ① 점수가 `null` 이어도 판정 저장이 **터지지 않는다** — 칸이 `NOT NULL` 이면 크래시
 *   ② 콜 옵션 표의 정차 값이 **판정 경로에 닿는다**
 *   ③ 못 잰 기준을 화면에 **「0점」으로** 그리지 않는다 — 점수는 `null` 로 둔다
 *   ④ 「약속」이 깨진 이유에 «0분 늦음» 을 붙이지 않는다
 */
...
maybe('② 🔴 콜 옵션 표의 정차 값이 **판정 경로에** 닿는다', () => {
    ...
    /**
     * 🔴 **표를 고치면 판정이 쓰는 정차가 따라 움직인다** — 정차 값은 콜 옵션 표(`call_options`)에 있다.
     *    `dwellMinutes` 를 **직접** 부르는 검사로는 «표를 고쳤는데 판정이 안 읽는» 끊김을 못 잡아,
     *    판정이 쓰는 `getStopTiming` 을 태운다.
     */
...
describe('④ 「약속」이 깨진 이유에 «0분 늦음» 을 붙이지 않는다', () => {
    /**
     * 문장으로만 온 조건(`lateMinutes: null`)은 «몇 분 늦는지»를 숫자로 못 꺼낸다.
     * 자리표시자 `0` 을 넣으면 «…12분 깨집니다 **0분 늦음**» 이 되어
     * 문장이 스스로 모순된다 (규칙 ④).
     */
