...
/**
 * ⛓️ **약속은 경로를 타고 뒤로 전파된다**
 *
 * 기사님: *"이 부분 아주 중요한 부분이야. 모든 약속이 달라지거나 할 수 있어."*
 *
 * 검산 예: 상차 약속을 11:12(까지)로 확정하면
 *   상차 11:12 + 상차 8분 + 주행 97분 = **12:57** 이 하차의 물리적 최소다.
 * 타임라인이 확정 약속을 **그 정거장에만** 쓰고 누적에 안 넣으면
 * (정차와 "부터" 대기만 누적하면) 하차 추정이 11:51 처럼 이르게 뜬다.
 *
...
/**
 * 🏗️ 확정 약속이 잘못 굳거나 되살아나는 사고는 이 파일 밖에서 막는다:
 *   · 시트는 저장된 행을 그리기만 한다 (복원 분기가 없다 — 규칙 ③)
