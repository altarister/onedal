...
/**
 * 🗣️ **통화 시트의 문구는 기사님이 실제로 하는 말이어야 한다** (기사님 확정)
 *
 * «지금 출발한다»고 가정한 문구(*"지금 출발하면 17:57 도착 · 상차 31분 소요 · 약속까지 대기 30분 가능"*)는 쓰지 않는다 —
...
const sheet = () => readFileSync(
    join(__dirname, '../../../client-app/src/components/dashboard/StepSheetMock.tsx'), 'utf8');   // 🏗️ 문장 규칙은 단계 시트가 진다

/** 주석(쓰지 않는 문구를 예로 적어 둔 곳)은 검사 대상이 아니다 */
...
describe('통화 시트 문구 — 지금 출발한다고 가정하지 않는다', () => {
    ...
    /**
     * 🔴 **하차지는 누적이 아니라 구간이다** (실측).
     *    `driveMinutes` 는 카카오호출시점부터의 누적(129분 = 접근 16 + 단독 113)이라,
...
describe('문장이 검산된다 — 출발 + 주행 + 대기 = 도착', () => {
    ...
    it('도착 시각은 저장된 약속·예측을 그대로 그린다 (시트 계산 금지)', () => {
        // 시트는 시각을 만들지 않는다 — 행(약속·예측)과 타임라인 값만 그린다 (규칙 ③)
        expect(code()).not.toMatch(/Date\.now\(\)\s*\+/);
    });

    /**
     * 🔴 **출발 시각을 «지금 + 상차»로 세지 않는다** (기사님 실측).
     *
     * `지금` 은 **시트를 연 시각**이라 열 때마다 값이 달라지고, 상차지 시트가 말하는
     * 출발(`상차 약속 + 상차`)과 **44분**까지 어긋난다 — 한 화면 안에서 두 시트가 다른 시각을 말한다.
     *
     *   하차지 시트: *"…에서 8분 상차하고 **16:19** 출발, 주행 129분 … = 18:26 도착"*
     *   참값:        *"…에서 8분 상차하고 **17:03** 출발, 주행 113분 … = 18:56 도착"*
     *
     * 그래서 **시각을 시트가 만들지 않는다** — 타임라인의 `departPrevMs` 를 그대로 그린다
...
/**
 * 🗺️ **통화 시트도 경로 타임라인을 읽는다**
 *
 * 덱 요약 줄이 합짐 하차 ~05:56 을 아는데 같은 화면의 하차지 통화 시트가
 * "주행 시간을 아직 모릅니다"라며 03:28 같은 **물리적으로 못 지킬 칸**을 추천하면,
 * 한 화면의 두 곳이 다른 세상을 보고 화면이 거짓말한다 —
 * 카드가 시트에 넘기는 주행·선행 시간도 타임라인에서 온다.
 */
describe('통화 시트 — 경로 타임라인 연결', () => {
    /**
     * 🏗️ 시트는 타임라인 값을 그리기만 한다 — 단계 화면에서는
     * 카드(`PinnedRouteCard`)가 `departPrevMs`·`segmentDriveMinutes` 를 타임라인에서 뽑아 넘긴다.
     */
 ...
 * 섞어 보인다 (약속은 도착 시각 — 상차를 섞지 않는다는 규칙과도 어긋난 표시).
 * 숫자가 우연히 맞아 보여서 더 위험하다 — 근거가 거짓말하면 기사님이 검산을 못 한다.
 */
describe('추천 근거 문구 — 실제 계산 그대로', () => {
    // 🏗️ 격자 ⚠️ 는 두 시계(⑯)별 문구로 근거를 말한다 — «여유 30분» 한 줄은 두 시계를 못 가른다
    ...
    it('🔴 카운트다운 추정 설명이 두 시계 기준을 말한다 — 여유30 카피 금지 (⑯)', () => {
        ...
        // 숫자는 판정 기준 탭 값을 그대로 말한다 — 30·150 을 박아 두면 설정을 바꿔도 문구가 안 바뀐다
