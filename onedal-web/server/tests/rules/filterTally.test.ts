...
/**
 * 👁️ **필터가 왜 하나도 안 잡는지 화면이 말해 준다** (기사님 확정)
 *
 ...
 * 그것만 있으면 기사님이 매번 물으셔야 하고, 답은 로그를 열어야만 있다.
 *
 * 🔴 **앱은 이미 답을 알고 있다.** 매 스캔마다 축별로 판정한다:
 *
 *     🔍 차종=✅ 도착지(342중 진위면)=❌ 요금=✅ 상차지=✅ 블랙=✅
 *
 * 그 숫자를 서버로 올려 화면이 이렇게 말한다:
 *
...
describe('👁️ 앱 — 축별로 몇 개가 떨어졌는지 센다', () => {
     ...
     *    호출자가 그릇을 넘긴다 — 기본값 null 이라 그릇을 안 넘기는 호출부도 그대로 돈다.
     */
    ...
    /**
     * 🔴 **한 스캔의 성적표는 한 번만 간다** (기사님 지적).
     *
     * `filterTally` 를 보낸 뒤에도 남겨 두면 **하트비트를 포함한 모든 전송**에 같은 숫자가
     * 실려 가고, 서버는 올 때마다 새 시각을 찍는다. 그러면 폰이 `알 수 없는 화면` 에 있어
     * **스캔을 아예 안 하는데도** 화면은 *"방금 훑었다"* 고 말한다.
     *
...
describe('👁️ 서버 — 받아서 기기 세션에 둔다', () => {
    ...
    it('🔴 서버가 받은 순간을 찍는다 — 앱 시계를 믿지 않는다', () => {
        ...
        // 성적표가 실제로 온 스캔에서만 — 하트비트가 시각만 밀어 올리면 지난 숫자가 새것처럼 보인다
        expect(d).toMatch(/if \(filterTally\)[\s\S]{0,200}filterTallyAt/);
    });
});

/**
 * 🔴 **그 숫자의 주어는 "폰"이지 "필터"가 아니다** (기사님 지적).
 *
 * 기사님: *"이것이 필터에 들어가면 안 될 것 같아. 폰이 2개 이상이어도 상관없는 건가?
 * 1번 폰은 작동하는데 2번 폰은 작동하지 않는다면?"*
 *
 * 필터는 **서버가 만들어 모든 폰에 똑같이 내려보내는 한 벌**이고, 성적표는 **폰마다 다르다.**
 * 한 벌짜리 카드에 폰마다 다른 값을 놓으면 둘 중 하나를 골라야 한다. `devices.filter(…).pop()` 처럼
 * **배열 순서상 마지막 폰**을 집으면 멀쩡한 1번 폰의 숫자가 멈춘 2번 폰을 가린다.
 * 그래서 폰 카드가 **자기** 숫자를 그린다.
 */
describe('👁️ 관제웹 — 왜 안 잡는지 한 줄로 말한다 (폰마다)', () => {
    ...
    /**
     * 🔴 **마지막 보고에 함께 온 성적표만 그린다** (기사님 확정 — 한 줄로 합침).
     *
     * 기사님: *"아래 줄은 없어져야 해"* — 폰이 `알 수 없는 화면` 인데 `👁️ 06:28:15 1건 → 통과 0` 이
     * 떠 있으면, 스캔을 안 하는 폰이 *"방금 훑었다"* 고 말하는 것이다.
     *
