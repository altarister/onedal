...
const sheet = () => readFileSync(join(__dirname,
    '../../../client-app/src/components/dashboard/StepSheetMock.tsx'), 'utf8');   // 도착시간 격자를 그리는 시트

/**
 * 🕐 **도착시간 격자 — 흔들리지 않는 기준**
 *
 * 시트가 격자를 자기 시각(Date.now)으로 만들면 분 틱마다 칸이 밀리고,
 * 저장 약속을 추천이 덮는다. 그래서 방어 자리를 이렇게 나눈다:
 *   · 칸 생성 규칙 → shared `buildArrivalSlots` (순수 함수 — 아래에서 직접 검사)
 *   · 격자의 밑값(도착 예상·약속) → **저장된 행** — 시트는 계산하지 않는다 (규칙 ③)
 *   · 저장된 약속·"부터" → 격자가 읽어 불을 켠다 (추천이 덮을 자리가 없다)
 */
 ...
 *
 * 자정을 걸친 콜처럼 약속이 크게 깨지면(약속이 8시간 깨진 콜) 격자 다섯 칸이 전부
 * 취소선이라 **재약속을 잡을 칸이 없다.** 통화로 확정하는 것이 이 제품의 순서인데
 * 그러면 통화의 도구가 죽는다.
 *
