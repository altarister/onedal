...
/**
 * 🧰 **검증 스크립트는 표에 올라야 산다** (기사님 지시 2026-09-14).
 *
 * 기사님: *"스크립트를 만들면 CLAUDE.md 에 추가하라 같은 기준이 있으면 잊지 않고 너가
 * 만들어 넣겠군. 만들고 나면 항상 쓰는 거라 검수도 필요할 것 같은데?"*
 *
 * 🔴 **글로 된 기준만으로는 또 잊는다.** `scripts/shot.mjs`(화면을 찍어 눈으로 대조)는
 *    2026-09-06 에 만들어져 **명령 이름도 없고 표에도 없는 채로** 살았다. 그 닷새 뒤
 *    같은 목적의 `lab-both.mjs` 를 새로 만들었다 — 있던 도구를 아무도 몰랐다.
 *
 * 🔴 **«못 잡는 것»이 비면 틀린 안심을 준다.** 2026-09-14 대조에서 `audit:docs` 가
 *    초록인데 `CLAUDE.md` 구성 표가 틀려 있었다 — 그 검사는 파일·식별자·링크만 본다.
