 ...
 *
 * 같은 목록을 두 곳이 그린다 — ☰ 서랍(`Drawer`)과 옛 화면의 탭 줄(`PinnedRoute`).
 * 거르는 조건을 양쪽에 적으면 한쪽만 고쳐져 **«탭에는 있는데 서랍에는 없는» 콜**이 생긴다.
 * 그래서 규칙은 `lib/finishedCalls` 한 곳에 있고, 두 화면은 그것을 부르기만 한다 (규칙 ③).
 *
 * 함께 잠그는 것:
...
describe('📋 끝난 콜 — 거르는 규칙이 한 곳이다', () => {
    ...
    it('🔴 옛 화면 탭도 같은 규칙 함수를 부른다 (두 벌 금지)', () => {
        expect(OLD).toMatch(/callsInView/);
        for (const s of TERMINAL_LITERALS) {
