 ...
 * ── 왜 검사가 필요한가 ──
 *
 * `shared` 에 상수가 있는데 서버 시딩(`stepSeeder`)이 **글자를 손으로 다시 적으면** 이 셋이 **두 벌**이 된다:
 *
 * ```
 * shared   DEFAULT_CARGO_TAG = '일반화물'
 * 서버      … ?? ['일반화물']            ← 같은 글자를 또 적은 모양
 * ```
 *
 * 🔴 **리허설로는 이걸 못 잡는다.** 두 벌이 **같은 값**인 동안은 화면이 똑같이 나온다.
 *    사고는 **한쪽만 고치는 날** 난다. 그래서 재현이 아니라 **구조를 잠그는** 검사를 둔다.
 *
 * ⚠️ 이 검사는 **글자를 본다.** 값이 아니라 «상수를 쓰는가»를 보는 것이라 그렇다.
 */

const 시딩 = readFileSync(join(__dirname, '../../src/services/stepSeeder.ts'), 'utf8');
/** 주석 속 예시는 세지 않는다 — 설명으로 적은 글자일 수 있다 */
