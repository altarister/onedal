...
/**
 * 🧭 **경로를 든 콜을 «추측»하지 않는다** (기사님 확정 · 잔상 수리)
 *
 * 기사님: *"자꾸 먼 잔상이 남아서 경로를 망가트리고 있어."*
 *
 * ── 왜 이름으로 보내나 ──
 * 폴리라인은 «홀더» 한 콜에만 실린다(`applyRoute`). **누가 홀더인가**를
 * 세 곳이 각자 판정하면:
 ...
 * 그 사이 관제웹이 **직전 콜의 지난 선**을 그린다. 잔상의 뿌리다.
 ...
 *    payload 가 두 배가 되고(초당 수백 KB 면 브라우저가 죽는다),
...
describe('경로를 든 콜 — 판정은 서버 한 곳', () => {
    ...
    /**
     * ⚠️ **철자가 아니라 성질을 본다**. `routeHolder?.routePolyline…` 같은 **문장을 그대로**
     *    맞추면, 이름이 바뀔 때(`drawHolder` 처럼) **멀쩡한 코드에서 검사가 깨진다.**
...
/**
 * 🧹 **시뮬 방문 장부는 회차를 넘기지 않는다** (기사님 실측).
 * 7지점 문제지는 회차마다 정거장 **좌표가 같다** — 지난 회차의 «들렀다»가 남으면
 * 이번 회차에서 그 정거장의 정차 연기를 통째로 건너뛴다 (어제 상태가 오늘 되살아나면 안 된다).
 */
describe('모의 주행 — 사이클이 닫히면 방문 장부를 비운다', () => {
    it('🔴 정거장이 하나도 없으면 장부를 초기화한다', () => {
        const sim = codeOnly(read('hooks/useMockGpsSimulator.ts'));
        expect(sim).toMatch(/simRef\.current = simStateForStops\(simRef\.current, stops \?\? \[\]\)/   /* 🔄 빔은 장부만 비우고 자리는 둔다 (`simStateForStops` · 판단은 simStep.test 가 문다) */);
    });
});
