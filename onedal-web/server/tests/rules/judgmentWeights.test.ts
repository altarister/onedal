...
/**
 * ⚖️ **판정 기준은 다섯이고, 전부 가중치로 켜고 끈다** (기사님 확정)
 *
 ...
 *
 * ── 왜 ──
 * 「기존 콜 약속 보존」과 「짐 동승」이 축 밖의 **문지기**면 통과/실패만 있고
 * **끌 수가 없어서**, 경로만 보려는 검사에서도 끼어들어 색을 덮는다 —
 * 축점 57점(보통)인데 색은 «사고»가 된다.
 *
 * 그래서 다섯 다 가중치를 갖는다 (`CRITERIA` 순서):
 * ```
 *   ① 돈 — 순증 대비 우회(revenueDetour)   ② 약속 — 기존 콜 약속 보존(promiseGuard)
 *   ③ 공간 — 적재(slots)   ④ 성질 — 짐 동승(cargoCompat)   ⑤ 지리 — 목적지로 전진(geography)
 *   약속·성질은 통과 100 · 실패 0
 * ```
 * 🔴 **안전을 뺀 것이 아니다.** 가중치가 0 이 아니면 실패 시 색은 여전히 «사고» 다.
 *    끄는 것은 **명시적으로 0 을 넣었을 때만**이고, 기본값은 1 이라 켜져 있다.
 */
