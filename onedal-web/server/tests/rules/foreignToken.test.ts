...
/**
 * 🪪 **서명이 맞다고 «이 서버의 유저»는 아니다** (기사님 실측)
 *
 * 라이브와 로컬 두 서버가 같은 JWT 비밀을 쓰므로, 라이브에서 발급된 토큰을 든 클라이언트가
 * 로컬에 붙어도 **서명 검증은 통과한다** — 같은 기사님인데 **로컬 DB 에는 없는 id** 다.
 * 서명은 *"위조가 아니다"* 만 말할 뿐 *"어느 서버가 발급했나"* 를 구분하지 못한다.
 *
 * ── 막지 않으면 ──
 * `getUserSession` 이 세션을 만들고, 외래키 오류(`SQLITE_CONSTRAINT_FOREIGNKEY`)는 `catch` 가 삼키고,
 * 그래도 `sessions.set` 은 실행된다. 소켓이 끊겨도 **메모리 세션은 안 지워지므로**
 * (`clearUserSession` 은 명시적 로그아웃에서만 불린다) 1초 인터벌이 **없는 유저까지** 매초 돈다.
 *
 * 같은 `🧭 [경로 순서]` 줄이 쌓이는 로그는 증상이고 (`routeOrderSingleSource.test.ts` 가 그 짝을 막는다),
 * 뿌리는 **남의 서버 토큰이 문을 통과하는 것**이다.
 *
 * ── 지킬 선 ──
 *   ① 문에서 막는다 — 세션 저장소가 아니라 **인증**의 일이다. 세션 저장소는
 *      테스트용 가짜 id 도 받아야 하는 단순한 그릇이다(`'midnight-cycle-test-user'`).
 *   ② 판단은 **한 곳**(`isKnownUser`)뿐이다 (규칙 ③) — REST 와 소켓이 각자 판단하면 한쪽만 고쳐진다.
