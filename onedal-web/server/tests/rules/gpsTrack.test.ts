 ...
 *
 * ── 왜 저장하나 ──
 * 좌표를 소켓으로 흘려보내기만 하고 **메모리에만 두면** 주행이 끝난 뒤 궤적이 없다 —
 *   · 상차지 GPS 자동 감지가 빗나가도 **몇 미터 차이로 빗나갔는지를 모른다.**
 *     반경 500m 를 늘릴 일인지 장소를 바꿀 일인지 판단이 안 된다
 *   · 「위치 점프」 경고 줄은 시각이 없고 «이상할 때만» 찍혀
 *     **궤적이 아니라 흔들린 순간 모음**이다 — 궤적 대신 읽으면 오독한다
 *
 * ── 비용을 누른 세 가지 (기사님 승인) ──
 ...
 *   ③ **7일 보관** 부팅 때 정리 — 8일째 부팅하면 1일차가 지워진다. 7일이면 약 5MB 다
...
/**
 * 🚨 **경고가 너무 자주 울리면 경고가 아니다**
 *
 * `속도 = 거리 ÷ 시간` 이라 표본이 작으면(1m 를 0.1초에) 속도가 폭발해 `46395km/h` 같은
 * 경고가 줄줄이 찍히고, 진짜 점프와 구분이 안 된다. 그래서 50m 이상 · 1초 이상 움직인
 * 표본만 속도를 잰다 (`isSpeedSampleUsable`).
 */
...
/**
 * 🧭 **궤적에 «그때 어느 콜이었나»가 붙는다**
 *
 * `gps_tracks.order_id` 는 *"그때 어느 콜을 향하고 있었나 — 경로 대조의 열쇠"* 다.
 * `geoService` 가 `bufferGpsPoint` 를 부를 때 `gpsPointOf` 로 `orderId`·`stopType` 을 싣는다.
 *
...
/**
 * 📖 **읽기 — 궤적을 콜별로 묻는 SELECT**
 *
 * 콜별 점 수 · 공백 · 상하차 구분을 `summarizeTrack`·`trackOfOrder`·`trackSegmentsOf` 가
 * 답한다. 서버에 읽는 코드가 없으면 EC2 에 들어가 손으로 물어야 한다. 여기서 그 답을 고정한다.
 *
 * 공백 문턱의 근거: 저장 조건이 «50m 이동 **또는** 15초 경과»라 폰이 살아 있으면
 * 정차 중에도 점이 온다. 그러므로 5분+ 공백은 «서 있던 것»이 아니라
 * **폰이 좌표를 안 보낸 것**이다 (실측: S23 배터리 최적화로 5분+ 공백 7회, 최대 17분).
 */
