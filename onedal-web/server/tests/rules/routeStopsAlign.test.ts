...
/**
 * 🧭 **정거장을 지나도 남은 주행분은 살아 있어야 한다** (기사님 실측)
 *
 * 정거장에 도착하면 `planArrivalStops` 가 그곳을 목록에서 빼는데, 경로 연산이 남긴
 * `sectionDriveMin` 은 **계산 시점의 정거장 수** 그대로다. 인덱스로 맞추면 길이가 어긋나
 *
 *   `⚠️ 길이 어긋남(주행분 4 ≠ 정거장 3) → 전부 null`
 *
 * 안전장치가 주행분을 통째로 비우고 — **도착할 때마다 다음 재계산 전까지 타임라인이 죽는다**
 * (예산 줄·검산 문장·카운트다운이 전부 이 값을 먹는다).
 *
 * 🔴 남은 정거장의 누적 주행분은 **여전히 옳은 값이다** — 카카오호출시점(routeComputedAt)에서 잰
 *    상대값이라 낡지 않는다. 어긋나는 것은 값이 아니라 **자리 맞추기(인덱스)**다.
 *
