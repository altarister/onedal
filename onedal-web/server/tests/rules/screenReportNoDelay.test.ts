...
/**
 * 👀 **화면이 바뀐 보고는 예약하지 않고 그 자리에서 보낸다** (계측으로 확정)
 *
 * 200ms 뒤로 예약해 두면 안드로이드가 **제때 안 깨운다** — 실측 2초~13초.
 * flush 안이 아니라 «깨어나기»가 늦다. 기사님 체감 «페이지 바뀌고 수초 이상»이 이것이다.
 */
...
/**
 * 🟢 **서비스가 붙자마자 한 번 보고한다** (기사님 실측: *"폰이랑 서버랑
 * 연결이 안 되는데?"*)
 *
 * 생존신고 시계(60초)만 걸어 두면 **첫 보고는 그 60초 뒤**다. 그동안 관제웹에서는
 * 폰이 아예 없는 것처럼 보인다. 접근성을 껐다 켜면 그 시계가 **처음부터 다시** 시작하므로,
 * 몇 번 토글하는 사이 1분 넘게 조용할 수 있다. 그래서 `start()` 직후 `forceHeartbeat()` 를 한 번 쏜다.
 */
describe('🟢 붙자마자 첫 보고', () => {
