...
/**
 * 🎯 **판정 기준은 DB 에 산다** (기사님 확정)
 *
 * *"1. 판정 기준 표로 DB에 새로운 스키마로 테이블을 만들고 기본값을 넣는다.
 *   2. 관제웹에서 새로고침할 때 DB에 있는 값을 가져와 접속한 사용자 세션에 적용해 준다.
 *   3. 콜 받으면 그 세션의 값으로 판정한다."*
 *
 * 🔴 그리고 **콜 필터와 완전히 분리·격리**된다 —
 *    `user_filters`(집기 전) ↔ `user_judgment`(집은 뒤 · 한 벌 · «오늘만» 없음)
 */
...
describe('DB 값이 실제로 색을 바꾼다', () => {
    ...
    const 콜 = {
        ...
        drive: { driveKm: 40, driveMinutes: 31 },
    ...
    it('🔴 상하차 일반값을 바꾸면 계산이 따라온다 (DB 컬럼이 죽어 있지 않다)', () => {
        ...
        // 방법과 **수량**을 아는 짐은 일반값과 무관하다 (새 축: 수량 비례)
...
describe('세션이 DB 값을 읽고, 판정이 그 값을 쓴다', () => {
    ...
    /**
     * 🔴 **판정 시점에는 `orders` 행이 없다**. `upsertOrder` 는 KEEP 할 때
     *    도는데 판정은 그 전이다. 콜을 함께 넘기지 않으면 차종을 못 읽어 정차가
     *    통째로 「미확인 일반값」이 된다 — 다마스도 5t 도 같은 25분이 되고 «정차 미확인» 딱지가 붙는다.
     */
    it('🔴 판정은 **메모리의 콜**을 함께 넘긴다 — DB 행은 아직 없다', () => {
        const ev = codeOnly(read('core/engine/OrderEvaluator.ts'));
        const calls = ev.match(/totalDetourCost\([^)]*\)/g) || [];
        expect(calls.length).toBeGreaterThan(0);
        // 🔴 판정 설정까지 함께 넘긴다 — 정차 값이 사슬까지 가야 한다
...
/**
 * 🔴 **콜 필터와 판정 기준은 화면·소켓·스토어까지 갈라져 있다** (기사님 확정)
 *
...
describe('콜 필터 ↔ 판정 기준 — 화면까지 갈라져 있다', () => {
    ...
    it('🔴 관제웹 스토어도 콜 필터와 따로다', () => {
        ...
        // 구독은 한 번만 건다 — 여러 번 걸면 같은 이벤트를 여러 겹으로 받는다
    ...
    it('설정 모달에서 「필터」와 「판정」이 **다른 탭**이다 — 층이 다르다', () => {
         ...
         *    고쳐도 빨간불이 켜진다 (탭을 표로 옮기기만 해도 그렇다).
         */
        expect(m).toMatch(/["'>]판정["'<]/);
        expect(m).toMatch(/["'>]필터["'<]/);
        /* 한 탭이 둘을 겸하면 안 된다 */
...
/**
 * 🔴 **접속 순간에 한 번 보내는 값은, 놓친 뒤에도 받을 수 있어야 한다**
 *
 * 기사님: *"값을 바꿀 수 없다."*
 *
 * 서버는 `judgment-init` 을 **소켓 접속 순간에 한 번** 보낸다. 관제웹이 ⚙️ 설정 → 「판정」 탭을
 * **여는 순간**에야 구독하면 그때는 이미 지나간 뒤라, `loaded` 가 false 로 남고 **폼이 통째로 잠긴다.**
 *
 * 그래서 대시보드에서 미리 구독하고, 못 받았으면 `request-judgment` 로 다시 달라고 한다 —
 * 콜 필터의 `request-filter-init` 과 같은 방식이다.
 */
