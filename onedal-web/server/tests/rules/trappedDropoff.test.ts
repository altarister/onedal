...
describe('🏔️ 갇힘 지역 — 첫짐의 탈출력', () => {
    ...
    /**
     * 🔴 **화면 눈금과 배수가 같은 말을 한다** — 눈금에도 갇힘·멀어짐 곱이 태워져야 한다.
     *    안 태우면 «배수는 ×0.6 인데 화면은 50» 이 되어, 기사님이 보는 숫자와 점수가 어긋난다.
     *    전진율을 못 쟀을 때가 특히 그렇다 — 방향 몫만 1 로 두고 흘려보내므로 눈금의 바탕값이
     *    50 인데, 거기에도 같은 곱이 걸려야 «갇힘이 깎았다»가 화면에 보인다.
     */
    it('🔴 눈금과 배수가 같은 방향으로 움직인다 — 전진율을 못 쟀어도', () => {
        const 잰다 = (trapped: boolean) => {
            const o = GEOGRAPHY.measure({ firstLoad: true, progressRatio: null, unknownWhy: '못 쟀다', trapped } as never, cfg);
            if (o.kind !== 'scored') throw new Error('점수가 아니다');
            return { 배수: o.multiplier!, 눈금: o.score };
        };
        const 갇힘 = 잰다(true), 안갇힘 = 잰다(false);
        expect(갇힘.배수).toBeLessThan(안갇힘.배수);
        expect(갇힘.눈금).toBeLessThan(안갇힘.눈금);      // 🔴 눈금도 함께 내려간다
    });
