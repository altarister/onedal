...
/**
 * 🚚 **하차지 통화 문장의 출발 시각과 주행분** (기사님 실측)
 *
 * 하차지 통화 시트는 *"○○에서 8분 상차·대기하고 **HH:MM 출발**, **주행 N분**, 휴게 M분 = HH:MM 도착"* 이라고 말한다.
 * 두 항이 함께 틀리면 **서로를 가려** 그럴듯한 값이 나온다 — 실측에서 하차 약속을 **30분 이르게** 잡았다.
 *
 *   ① 출발 시각 — **앞 정거장 약속 + 그 정거장 정차**다. 시트가 `Date.now() + leadMinutes` 로
 *      **자기가 계산**하면 시트를 열 때마다 달라진다.
 *   ② 주행분 — 앞 정거장을 떠난 뒤의 **구간** 주행이다. 누적(`driveMinutes`)을 쓰면
 *      접근 분을 **두 번** 센다.
 *
 * 🔴 **값은 타임라인이 만들고 시트는 그리기만 한다** (규칙 ③). `timing.ts` 가
 *    *"한쪽만 고치면 카운트다운과 통화 화면이 다른 시각을 말한다"* 고 경고한 그 모양을 막는다.
 */
...
describe('하차지 문장 — 앞 정거장을 떠나는 시각은 타임라인이 만든다', () => {
    ...
    it('🔴 상차 약속은 16:29 — 콜 잡은 시각 + 20분 (0831 확정)', () => {
        const p = run().find(e => e.stopType === 'pickup')!;
        expect(kst(Date.parse(p.promisedUntil!))).toBe('16:29');
    });

    /**
     * 🔴 **이 값이 있어야 시트가 자기 계산을 하지 않는다.**
     *    출발 = 앞 정거장 약속 + 그 정거장 정차. 16:29 + 8분 = 16:37.
     *    상차지 시트도 같은 말을 한다 — 두 시트가 갈라질 수 없어야 한다.
     */
    it('🔴 하차지 문장의 출발 시각은 16:37 — 상차 약속(16:29) + 상차 8분', () => {
        const d = run().find(e => e.stopType === 'dropoff')!;
        expect(d.departPrevMs).not.toBeNull();
        expect(kst(d.departPrevMs!)).toBe('16:37');
    });

    /**
     * 🔴 `driveMinutes` 는 **카카오호출시점부터의 누적**이라 문장에 쓰면 접근을 두 번 센다.
     ...
     *    16:37 + 113분 = 18:30 이 물리적 도착이다.
     */
    it('🔴 출발 + 구간 주행 = 18:30 — 시트가 자기 계산하던 값이 아니다 (약속 20분 룰로 10분 당겨짐)', () => {
        const d = run().find(e => e.stopType === 'dropoff')!;
        const arriveMs = d.departPrevMs! + d.segmentDriveMinutes! * 60_000;
        expect(kst(arriveMs)).toBe('18:30');
    });
});
