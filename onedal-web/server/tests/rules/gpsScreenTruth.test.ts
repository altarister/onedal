 ...
 *
 * GPS 가 상차지 500m 안을 지나가기만 해도 상차 완료로 간주하는 자체 세트(`pickedUpSet`)를 두면,
 * 장부(orders.status)는 ORDER_CONFIRMED 인데 상단 요약이 "상차 1건 (1t)" 이라 적는다.
 * 지나가기만 해도 실은 것이 되는 추측이고, 카드·장부와 다른 말을
...
describe('덱 — 근접·도착한 정거장의 콜로 화면이 따라간다', () => {
    it('🔴 PinnedRoute 가 근접/도착 이벤트를 받아 덱에 넘긴다', () => {
        // 근접/도착 구독은 파생 훅(`useRouteDerivations`)·`gpsFocusStore` 에 있다 — 셋을 합쳐 본다
