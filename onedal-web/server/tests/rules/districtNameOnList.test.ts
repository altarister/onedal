...
/**
 * 🏙️ **리스트에 "구"로 뜨는 콜을 버리지 않는다** (기사님 지적)
 *
 * 기사님: *"지금 합짐 중인데 경안동 → 분당구 같은 건 잡아야 하는 거 아닌가?"*
 *
 * 실측 — 합짐 국면에서 경유 목록이 읍/면/동만 담으면 **구 단위로 표기된 콜이 전멸한다:**
 *
 *     도착지(409중 분당구)=❌   중구=❌   동작구=❌   단원구=❌   강동구=❌
 *     🧭 [경로 순서] 차단 — 경로 밖 — 상차지(분당구)가 경유 목록에 없음
 *
 * 경로는 **분당을 지난다** (`대장동`·`금토동` 이 경유 목록에 있고 20km 지점).
 * 경안동(6.4km) → 분당(20km) 은 순방향이라 **잡아야 하는 콜**이다.
 *
 * 🔴 배차망 리스트는 서울·성남·안산 같은 대도시를 **구**로 표시한다. 첫짐(파주 목적지)은
 *    동·읍·면뿐이라 안 걸리지만, **합짐은 경로가 서울·성남을 지나므로 경유 목록이 동만 담으면
 *    그 지역 콜을 통째로 놓친다.** 그래서 경유 목록에 구 단독형도 싣는다 —
...
describe('🏙️ 구 단독형 — 유일한 것만 목록에 넣는다', () => {
     ...
     * 🔴 **방위 이름은 고유명이 아니다.**
     *
     * 지도가 **수도권만** 담고 있어서 «지도에서 세어 유일하면 넣는다»만으로는
     * `서구`(인천)가 유일로 보인다. 실제로는 대전·광주·부산·대구에도 있다 —
     * **없는 데이터를 근거로 유일하다고 판정하게 된다.**
     *
