...
/**
 * 📍 **낡은 좌표는 «지금 위치»가 아니다** (기사님 실측)
 *
 * ── 실측 ──
 *   모의 주행이 **여주 세종대왕면**(127.5847, 37.2920)에서 끝나고 4시간 25분 뒤 **광주**에서 콜을 잡았다.
 *   그 사이 GPS 가 한 번도 안 오면, 서버가 그 자리를 «지금 내 위치»로 믿는다 —
 *   경로 요청 origin 이 세 번 다 소수점 14자리까지 같았다:
 *     origin=127.58473548568698,37.29198716004579
 *
 ...
 *
 * 🔄 **지우지 않고 고른다** (기사님 지시). **원자료(`lastFix`)를 그대로 두고** `originOf` 가
 *    물을 때마다 고른다 — 지우는 손이 여럿이면 그중 하나를 놓친다.
 *    그래서 이 검사도 «지워졌나»가 아니라 «무엇을 골랐나»를 본다.
 *
...
describe('낡은 현위치 — 경로 기점으로 쓰지 않는다', () => {
    ...
    it('🔴 4시간 25분 지난 좌표는 비운다 (2026-08-25 실측)', () => {
        ...
        /* 🔴 집 주소가 없는 세션이라 «모른다»(null)로 답한다 — 여주를 쓰지 않는 것이 요점 */
        expect(originOf(s, 콜잡은시각)).toBeNull();
        /* 🔴 **원자료는 그대로 있다** — 아무도 지우지 않는다 */
    ...
    it('🔴 낡음을 재는 곳은 한 곳뿐이다', () => {
        ...
        /* 호출부는 «고르는 함수»(originOf) 하나만 부른다. 비우고 채우는 손은 없다 */
