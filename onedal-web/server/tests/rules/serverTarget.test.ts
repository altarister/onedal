...
/**
 * 🎯 **어느 서버를 보는가 — 판단은 한 곳뿐이다** (기사님 확정)
 *
 * 기사님: *"볼륨 버튼을 클릭해서 라이브인지 로컬인지 바꿀 수 있으면 더 좋을 것 같은데."*
 *
 * ── 왜 ──
 * 관제앱(APK)은 `https://localhost` 에서 자기 번들을 띄운다. 상대 경로 `/api` 는
 * **자기 자신**에게 간다 — 서버가 아니라. 실측 로그캣:
 *
 *     SocialLogin.login → accessToken 수신 → `서버에게 id_token 전달` → **조용히 되돌아옴**
 *
 * 토큰을 받고도 보낼 곳이 없다. 브라우저에서는 Vite 프록시가 받아 주니 안 드러나고
 * **앱에서만** 난다 (*"앱으로 로그인이 안 돼 크롬으로 갔다"* 도 같은 뿌리다).
 *
 * ── 지켜야 하는 것 ──
 *   ① 주소를 정하는 곳은 **하나** — `apiClient` 와 `socket` 이 각자 읽으면
 *      서버를 바꿀 때 **HTTP 와 소켓이 서로 다른 서버**로 갈라진다 (규칙 ③).
 *   ② 바꾸면 **새로고침** — 소켓은 붙을 때 주소가 정해진다. 반씩 맞는 화면이 제일 나쁘다.
 *   ③ **지금 어디를 보는지 화면에 적는다** (규칙 ⑤-4 ④) — 모르면 어느 서버 이야기인지 헤맨다.
 ...
 *    `//` 앞에 `:` 가 있으면 URL 이다 — 자르면 URL 리터럴이 사라져 검사가 제 자신에게 속는다.
 */
...
/**
 * 🔴 **한 화면이 두 서버를 보면 안 된다** (기사님 실측)
 *
 * 위 검사들은 `apiClient`·`socket`·`roadmapLogger` **세 파일만** 확인한다. 그래서
 * 다른 파일이 `fetch("/api/orders")` 처럼 **주소를 손으로 적어도** 못 잡는다.
 *
 * 브라우저에서 서버를 안 바꾸면 상대 경로가 곧 정답이라 **아무 증상이 없다.**
 * 볼륨 업 스위치로 서버를 바꾸면 드러난다:
 *
 *     로그인·소켓·기기목록  → apiBase() = 라이브   (기기 이름 `v2`, filter-init 라이브)
 *     이력(orders) 한 줄만  → 상대경로  = 로컬     → 라이브 토큰이라 401
 *
 * 화면 하나가 두 서버에 걸치고, **로컬 서버 로그에는 그 한 줄만 찍힌다.**
 * 그래서 *"관제탑이 접속을 안 했다"* 와 *"관제웹은 잘 붙어 있다"* 가 동시에 참이 된다.
 *
 * 🔴 **관제앱(APK)에서는 더 나쁘다.** 거기서 상대 경로는 `https://localhost` —
 * 자기 번들이다. 이력이 **영영 안 온다**(취소·완료 탭이 조용히 빈다).
 *
 * → 세 파일을 지목하는 대신 **전부 훑는다.** 지목하는 검사는 지목 밖 파일을 못 잡는다.
 */
describe('관제웹 전체 — 서버 주소를 손으로 적은 곳이 없다', () => {
    ...
    /**
     * 🔴 **`apiBase()` 뒤에 `/api` 를 또 붙이지 않는다** (지도 실험실 실측)
     *
     * `apiBase()` 는 **`/api` 를 포함해** 돌려준다 (`'/api'` 또는 `https://…/api`).
     * `${apiBase()}/api/sim/route` 로 부르면 **`/api/api/…`** 가 되어 서버는
     * 404 — 그런데 fetch 의 catch 가 직선 폴백이라 **화면은 멀쩡히 직선을 그린다.**
     * 위의 리터럴 검사는 apiBase() 를 «거치기만 하면» 통과시켜서 이걸 못 잡는다.
     */
