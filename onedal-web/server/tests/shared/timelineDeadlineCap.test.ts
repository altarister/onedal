...
/**
 * ⏱️ **타임라인도 두 시계다** (시간체계 ⑯ · 기사님 확정)
 *
 * 시딩(`stepSeeder`)과 타임라인이 다른 식을 쓰면 카운트다운·덱이 시트와
 * **두 목소리**를 낸다. 추정을 만드는 규칙은 하나다:
 *
 * 🔴 **상차 약속은 도착 예상을 따라가지 않는다** (기사님 확정). «잡은 시각 + 20분»으로 못박고,
 *    못 지키면 **여유가 음수**로 드러난다. (`max(도착 예상, 잡음+…)` 로 두면 늦는 것을 0으로 눌러 감춘다)
 *
...
describe('타임라인 두 시계 — 추정 약속의 규칙은 하나다', () => {
    ...
    it('🔴 상차지가 멀어 20분을 못 지켜도 약속은 그대로 — 늦는 것이 음수로 드러난다', () => {
        /**
         * 🔄 약속을 도착 예상으로 눌러 올리지 않는다 (캡 바닥 없음).
         *    기사님 확정: 약속은 «잡은 시각 + 20분»이고, 못 지키면 늦은 것이다 —
    ...
    it('🔴 상차를 실제로 마쳤으면 데드라인은 실측 기산 · 못 지키면 음수로 드러난다', () => {
        /**
         * 🔄 하차 약속도 도착 예상(16:10)으로 눌러 올리지 않는다.
         *    기사님 확정: *"상차하고는 150%를 꼭 지켜야 한다."* 그러니 못 지키는
         *    것이야말로 드러나야 한다 — 상차와 같은 규칙이다.
         */
