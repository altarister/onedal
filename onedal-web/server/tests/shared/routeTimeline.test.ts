...
/**
 * 🗺️ **시각의 원천은 "지금 경로" 하나다** (기사님 동의)
 *
 * 콜마다 따로 파생하면 두 가지가 깨진다:
 *   ① 합짐 콜의 도착시각이 안 나온다 — 합짐은 kakaoSolo·approach 가 아예 계산되지
 *      않아 콜별 파생(deriveCallTiming)의 사슬이 끊긴다
 *   ② 카운트다운이 첫짐만 본다 — 합짐은 출발 마감이 null 이라 후보에서 조용히 빠진다
 *
...
// 합짐 B 는 실제 합짐 모양 그대로 — kakaoSolo·approach 가 없다
const orders = [
...
describe('pickBindingDeparture — 어떤 콜이건 가장 빨리 나가야 하는 것', () => {
    it('🔴 합짐의 출발마감이 첫짐보다 이르면 합짐이 기준이 된다', () => {
        // 카운트다운이 첫짐만 보면 합짐의 이른 마감을 놓친다
        // A 의 추정 약속이 20분 룰로 10분 당겨졌으므로 B 도 그만큼 빡빡해야 «B 가 기준»이 된다
...
/**
 * 🧾 **왜 그 시각인지 내역이 함께 나온다** (기사님 실측)
 *
 * 예: 콜 잡은 시각 17:14, 상차 약속 18:00, 접근 주행 15분이면
 * `출발마감 = 약속 18:00 − 접근 주행 15분 = 17:45` — 약 30분 뒤다. 46분은 주행을
 * 빼지 않은 값이다. 화면이 그 15분을 **어디에도 안 적으면** 기사님은 계산을
 * 확인할 방법이 없다.
 *
 * 🔴 계산이 아니라 **근거(내역)** 의 문제다. 타임라인 경로도 `driveMin` 을 비우지 않고
 *    내역 줄을 만든다 — 폴백 경로(콜별 파생)만 적으면 지금 돌고 있는 타임라인 쪽 내역이 빈다.
 *
...
describe('출발마감의 내역 — 주행분이 결과에 실린다', () => {
    ...
    it('🔴 화면이 그 값을 그대로 적는다 — 다시 계산하지 않는다 (규칙 ③)', () => {
        ...
        // 타임라인 분기에서 내역을 비우는 모양(`driveMin: null`)이 없어야 한다
