...
describe('buildArrivalSlots — 30분 단위 도착 협상', () => {
    ...
    /**
     * 🔄 **격자를 :00 / :30 경계로 옮겼다** (기사님 실측 2026-08-19).
     *
     * 예전엔 첫 칸이 도착 예상 그대로였다(08:39) — "가장 빨리 갈 수 있는 시각"을
     * 정확히 표현하려던 것이다. 그런데 격자가 **여는 시각을 따라 움직여서**,
     * 되돌아보기로 다시 열면 저장해 둔 약속(11:05)이 새 격자(11:06…)와 어긋나
     * 따로 끼워졌다 — `11:05 · 11:06` 처럼 1분 차이 칸이 둘씩 생겨 **칸이 7개**가 됐다.
     *
     * 대가: 첫 칸이 도착 예상보다 최대 29분 늦다. "가장 빨리"를 약속으로 고를 수는
     * 없지만, 그 시각은 문장이 그대로 말해 준다 (`주행 14분 … = 11:00 도착`).
     * 통화 대사와도 맞는다 — *"11시까지 갈게요"* 지 *"10시 39분까지"* 가 아니다.
     */
    it('첫 칸은 도착 예상 **이후의 첫 경계**다', () => {
        const s = buildArrivalSlots(NOW, 28, 5);   // 도착 예상 08:39 → 09:00
        expect(s[0].label).toBe('09:00');
        expect(s[0].minutesFromNow).toBeGreaterThanOrEqual(28);
    });
    ...
    it('주행이 길면 그만큼 뒤에서 시작한다', () => {
        expect(buildArrivalSlots(NOW, 124, 3).map(x => x.label))
            .toEqual(['10:30', '11:00', '11:30']);   // 도착 예상 10:15 → 첫 경계 10:30
    });

    it('간격을 바꿀 수 있다 — 경계도 그 간격을 따른다', () => {
