...
/**
 * 기사님이 정한 두 원칙으로 마감을 **통화 전에도** 만든다.
 *
 *   원칙 1 (반드시)  일과시간(17시) 전에 가져다 준다
 *   원칙 2 (가급적)  이동시간을 제외하고 2시간 안에 배송한다
 *
 * 마감이 없으면 여유가 무한이 되어 합짐이 무제한 통과한다.
 */
...
describe('buildArrivalSlots — 30분 단위 도착 협상', () => {
    ...
    /**
     * 🔴 **첫 칸을 :00/:30 경계로 올리지 않고 가장 이른 도착 시각에서 시작한다.**
     *    경계로 올리면 **여유가 제멋대로 변한다** — 도착 예상 17:02 면 28분,
     *    17:29 면 1분 (기사님: *"여유 시간의 디폴트 값이 막 변화하는 거잖아"*).
     *    칸이 겹치는 원인은 눈금이 아니라 **기준점이 열 때마다 달라지는 것**이고,
    ...
    it('🔴 지각 칸이 없다 — 가장 이른 도착에서 출발하므로 못 지킬 시각을 고를 수 없다', () => {
        // 정시부터 만들면 "고르면 지각" 칸이 섞인다. 가장 이른 도착에서 시작하므로
        // 그런 칸도, 그것을 표시할 플래그도 없다.
 ...
 * 🔴 시간 파생의 **유일한 지점**.
 *
 * `PinnedRouteCard` 와 `DepartureCountdown` 이 같은 계산을 각자 하면
 *   단독 구간 선택 · 접근 거리(총 − 단독) · 상차 정차
...
describe('deriveCallTiming — 시간 파생의 유일한 지점', () => {
    ...
    /**
     * 🔴 **단독 구간 — 실측이 없을 때 배송거리로 채우되, 실측이 있으면 추정이 덮지 않는다**
     *    (soloMinutesOf · 규칙 ⑤-2).
     */
    ...
    /**
     * 🔴 **마감은 상차에서 순산한다** (기사님) — 하차에서 역산하면 여유가 부풀려진다:
     * ```
     ...
     *
     * 🔴 그래서 **`capturedAt`(콜 잡은 시각)이 없으면 마감을 만들 수 없다.**
     *    지금 시각으로 대신하면 화면을 열 때마다 마감이 뒤로 밀려
     *    *"영원히 여유가 있다"* 고 거짓말한다.
     */
     ...
     *
     * 주행만 빼면(81분) 기사님이 상차지에 정시 도착해도 **약속보다 상차 정차만큼 늦게** 보내게 된다.
     * 통화값이 이긴다 — `pickupDeadlineAt` 은 통화값이다.
     */
