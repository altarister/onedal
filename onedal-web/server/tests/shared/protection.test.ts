 ...
 *
 * 그래서 기본 시간(찾기·대기 명목의 10~20분) 없이 **수량에만 비례**시킨다.
...
describe('방법 — 옮기는 행위만 (수량 비례)', () => {
    ...
    /**
     * 🔴 기본 시간이 0 이라 생기는 함정 — `수량 0 × 박스당` 이 그대로 0분이 된다.
...
/**
 * 🧹 **정리는 무조건 한다** (기사님) — 보호의 `결박` 과 같은 성격이다.
...
describe('통화 시트 — 하차 방법 미리 채움', () => {
    // 🏗️ «하차 방법 = 상차와 같다고 본다»는 규칙은 두 곳에 있다:
    //    시딩(stepSeeder 의 computeChain)과 타임라인(timing) — 둘이 같은 규칙이어야 한다
