...
/**
 * 🔄 **파생 치환 ① — 카운트다운·타임라인이 새 장부를 읽는다** (기사님 승인)
 *
 * `deriveRouteTimeline`·`deriveCallStep` 은 통화신고·마일스톤 모양을 먹는다.
 * 그 계산은 검사로 두껍게 덮여 있으니 그대로 두고, **새 장부(여섯 단계 행)를
 * 그 모양으로 바꿔 주는 어댑터** 하나가 사이에 선다. 읽는 곳의 재료 출처만 장부다.
 *
 * 🔴 약속 규칙이 그대로 건너가야 한다:
