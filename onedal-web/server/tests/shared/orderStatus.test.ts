...
/**
 * 🔴 **복구 대상 상태 목록은 한 곳(shared)에서 파생한다.**
 *
 * 목록을 여러 곳에 손으로 적으면 갈라진다 — 새 상태(`ORDER_PICKED_UP` · `ORDER_DELIVERED` 같은)를
 * 복구 쿼리에 넣는 걸 빠뜨리면 **짐을 실은 채 새로고침할 때 콜이 사라지고,**
 * 서버는 빈 차로 착각해 1t 콜까지 잡으러 간다.
 *
...
/**
 * 🗓️ **미완료 콜은 날짜와 무관하게 되살린다 (3일 상한).**
 *
 * 🔴 복구 쿼리가 `timestamp >= 오늘 자정` 만 보면 **전날 상차한 콜이 사라져**
