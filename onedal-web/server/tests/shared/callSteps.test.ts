...
/**
 * 콜 진행 6단계는 **저장하지 않고 파생**한다.
 *
 * 저장해 둔 값은 실제와 갈라진다(BB·DD·II·JJ·PP·WW 가 전부 그 모양이다).
 * 그래서 단계는 서버에 이미 있는 증거(마일스톤·통화 기록)만으로 판정한다.
 */
...
/**
 * 🔴 하차지 통화 화면은 아직 상차지에 안 갔으면 접근 주행과 상차 작업까지 센다.
 * "상차지 → 하차지" 주행만 세면 도착 예상이 이르게 나오고 `지각` 판정이 낙관적이 된다.
...
/**
 * 건너뛰기를 서버에 남긴다 (`kind: 'SKIPPED'`).
 *
 * 기사님 기준: *"완료 전까지는 페이지별로 기억하고 있어야 하고 수정이 가능해야 한다."*
 * 화면 로컬(`skippedTo`)에만 두면 **새로고침할 때 통화 단계가 되살아난다.**
