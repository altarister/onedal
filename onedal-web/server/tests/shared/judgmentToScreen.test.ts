...
/**
 * 🎛️ **판정 기준 탭의 시간 4칸이 화면 파생까지 닿는다**
 *
 * 판정 기준 10칸은 서버가 다 쓴다. **관제웹 파생(타임라인·카운트다운·심사 버퍼)이
 * 시간 4칸(미확인 정차 2 · 상차 시계 잠정 · 데드라인 배율)을 안 받고 코드 기본값**을 쓰면,
 * DB 값 = 기본값인 동안은 증상이 숨어 있다가 기사님이 탭에서 잠정을 20→45로
 * 바꾸는 순간 서버(45)와 화면(20)이 갈라진다. #33(정차값 두 벌)과 같은 클래스다.
 *
 * 그래서 파생 입력을 `derivationInputsOf(cfg)` **한 곳**에서 만들고(규칙 ③),
...
describe('판정 기준 → 화면 파생 (시간 4칸)', () => {
    ...
    it('🔴 상차 시계 잠정·정차가 타임라인까지 — 45분 시계 + 40분 정차로 데드라인이 선다', () => {
        ...
        // 상차 약속 = 잡음+45 = 12:45 (기본 20이면 12:20)
        expect(kst(Date.parse(p.promisedUntil!))).toBe('12:45');
        // 데드라인 = 완료(12:45 + 정차 40) + 40×1.5 = 14:25 (기본 정차 15면 14:00)
