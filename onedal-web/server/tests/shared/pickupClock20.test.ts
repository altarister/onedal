...
/**
 * ⏱️ **상차 약속은 «콜 잡은 시각 + 20분»이다** (기사님 확정)
 *
 ...
 *
 * ── 무엇을 막나 ──
 * **`max(상차지 도착 예상, 콜 잡은 시각 + 30분)`** 같은 식은 상차지 도착 예상이
 * «잡은 시각 + 30분»보다 늦기만 하면 **약속을 도착 예상과 같게** 만든다 —
 * **상차 여유가 구조적으로 항상 0**이 되어 판정 원장에 «약속 최소 +0분»만 찍힌다.
 * 게다가 `max` 가 음수를 0으로 눌러 **늦는다는 사실 자체가 화면에서 사라진다.**
 *
 * 30분·60분은 «20분 룰»과 다른 가정치라 쓰지 않는다 (기사님).
...
describe('상차 약속 = 콜 잡은 시각 + 20분', () => {
    it('🔴 값의 근거는 20분 하나다 — 30분·60분 가정치는 없다', () => {
        ...
        // 20분 룰이 아닌 가정치(30·60분)가 쓰이면 빨간불 (기사님 지시)
    ...
    it('🔴 상차지까지 25분이면 여유가 **−5분** — 늦는다는 사실이 드러나야 한다', () => {
        /**
         * `max(도착 예상, 잡은 시각+30분)` 식은 이걸 «0분»으로 눌러 감춘다.
    ...
    it('🔴 약속이 도착 예상을 따라가지 않는다 — 여유가 늘 0이던 병', () => {
        // 40분이든 60분이든 약속은 9:20 그대로 — 약속이 도착 예상을 따라가면 여유가 늘 0이다
