...
describe('단위 — 기사님이 통화에서 실제로 쓰는 말', () => {
    it('파레트 2개 = 1t짐 80박스 — 자투리 20박스는 남는다 (용어집 §5)', () => {
        expect(unitPoints('파레트', 2)).toBe(80);
        expect(unitPoints('톤백', 1)).toBe(40);   // 톤백은 만재가 아니다 (기사님 정정 2026-08-17)
    });

    it('마대·쇼핑백·서류봉투는 실제 적요에 나오는 단위 그대로다 (용어집 §5)', () => {
...
describe('단위 개편 (2026-08-12)', () => {
    ...
    it('🔴 옛 단위도 계속 읽힌다 — 기존 DB 행의 부피를 잃지 않는다', () => {
        expect(unitPoints('톤백', 1)).toBe(40);          // 라면박스 축 (2026-08-17)
        expect(unitPoints('쇼핑백', 2)).toBeCloseTo(0.4);
    });
