 ...
 * 그래야 합짐을 잡을 수 있을 듯."*
 *
 * 그래서 우회 허용치는 고정 상수가 아니라 **실린 짐의 마감**에서 나온다.
 */
...
describe('allowedDetourMinutes — 가장 촉박한 짐이 기준', () => {
    ...
    /**
     * ⚠️ **이미 늦은 짐이 있으면 우회 여력은 음수로 나온다** — 0 으로 깎지 않는다.
     *
     * 0 으로 깎아 **한계**로 쓰면 `+0분` 짜리 콜조차 `0 >= 0` 으로 똥이 된다
     * (예: 요금 99,000원 · 우회 `+1.1km` · 주행 `+6분` 짜리가 🟡).
     *
     * 또 **"모른다"와 "늦었다"가 같은 0 으로 뭉개진다**. 기사님 확정:
...
describe('상하차 소요 시간 — 경로 시간에 더해야 하는 값', () => {
    /**
     * 🔴 **방법 축은 *옮기는 행위만* 센다** (기사님) —
     *    찾기·대기 명목의 기본 시간은 두지 않고, 안전 조치는 `보호` 축이 맡는다.
    ...
    /**
     * ⚠️ 방법을 모를 때 비관적 기본값(상·하차 각 20분)을 쓰면
     *    40분이 주행에 얹혀 꿀콜이 똥이 된다.
     *
     * 기사님 확정: *"**일반적인 값**을 넣어두고 미확인으로 표시하면 좋을 듯.
...
describe('단위 — 기사님이 통화에서 실제로 쓰는 말', () => {
    it('파레트 2개 = 1t짐 80박스 — 자투리 20박스는 남는다', () => {
        expect(unitPoints('파레트', 2)).toBe(80);
        expect(unitPoints('톤백', 1)).toBe(40);   // 톤백은 만재가 아니다 (기사님 정정)
    });

    it('마대·쇼핑백·서류봉투는 실제 적요에 나오는 단위 그대로다', () => {
        expect(unitPoints('마대', 5)).toBe(5);          // 마대 = 박스 1개
        expect(unitPoints('쇼핑백', 2)).toBeCloseTo(0.4);
        expect(unitPoints('서류봉투', 1)).toBeCloseTo(0.2);
    });

    it('라면박스는 100개가 1t 만재 — 축의 기준 단위다', () => {
        expect(unitPoints('라면박스', 100)).toBe(100);
        expect(unitPoints('라면박스', 40)).toBe(40);
    });
...
/**
 * 기사님 결정 — 짐 선택지는 톤백·쇼핑백 대신 `기타` 를 둔다 (톤백·쇼핑백은 `LEGACY_CARGO_UNITS`).
 *
 * 🔴 선택지에서 뺀 것과 **읽을 수 있는 것**은 다르다.
 *    DB 에 톤백·쇼핑백 행이 남아 있을 수 있고 적요 파서도 그 낱말을 읽는다.
...
describe('단위 개편 (2026-08-12)', () => {
    ...
    it('🔴 옛 단위도 계속 읽힌다 — 기존 DB 행의 부피를 잃지 않는다', () => {
        expect(unitPoints('톤백', 1)).toBe(40);          // 라면박스 축
        expect(unitPoints('쇼핑백', 2)).toBeCloseTo(0.4);
    });
...
describe('검수 — 하차의 후작업으로 이사 (2026-08-18)', () => {
     ...
     * 검수는 방법(옮기는 행위)이 아니라 **하차 후작업**이고 60분이다 (기사님 확정).
     */
