...
/**
 * 🧭 앱에 내려보낼 경로 순서 맵 (기사님 확정 — 여유 0km, 역주행은 버린다)
 *
 * 막는 것: 파주 도착 직전 `초월읍(광주) → 금촌동(파주)` 이 앱 필터를 통과하는 것.
 * 세션의 순서 맵은 **지나온 동도 계속 들고 있다** — 그대로 보내면 지나온 동이
 * "경로 위"로 남아 차단이 안 된다. 그래서 키를 지금 목록(destinationKeywords)으로
 * 좁혀 보내는 것이 이 함수의 존재 이유다.
 *
 * 🔴 원천은 `detourOrderKm`(순서 전용 · 순수 스냅점)이다.
 *    트림용 `detourProgressKm` 을 쓰면 pad·Infinity 가 순서에 섞여
 *    길목 동(예: 6km 곤지암읍)이 «경로 끝»으로 나가 순방향 콜이 차단된다.
...
describe('buildAppOrderKm', () => {
    ...
    it('키는 지금 목록으로 좁힌다 — 지나온 동(목록에서 빠진 것)은 실리지 않는다', () => {
        ...
        expect('초월읍' in out).toBe(false);                        // ← 지나온 동은 빠진다
    });

    /**
     * 🔴 하차원 안 동이 전부 null 로 나가면 판정이 통째로 죽는다.
