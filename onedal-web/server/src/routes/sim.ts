 ...
 *    (에 무인증 `GET /api/scrap` 을 «정찰 정보 노출»로 지운 것과 같은 이유).
...
/**
 * 🚚 **개별콜 — 현황판이 낸 콜을 들고 있다가 시뮬레이터에 넘긴다** (기사님 지시 · `core/simCallQueue.ts` 머리)
 *
...
/**
 * 🎬 **시나리오콜 — «이천 왕복 하루»를 서버가 사건순으로 낸다** (기사님 지시).
 *
...
router.post("/route", async (req, res) => {
    ...
    try {
        ...
        for (let i = 1; i < points.length; i++) {
            ...
            //    (실측: 한 구간 103 에 전체가 502 → 멀쩡한 구간까지 직선 폴백 + 반복 재시도)
            try {
...
router.post("/roads", async (req, res) => {
    ...
    try {
        ...
        // 4개만, 대안 없이 (기사님 확정: «추천·최단거리·최단시간·톨게이트 피하기 이렇게 4개만»)
...
/**
 * 🧭 **실험실 전용 — 정거장 전부를 한 번에, 구간별로 그대로** (기사님 확정).
 *
 * 기사님 순서(②~⑥ · ⑫~⑭) 그대로다:
