...
router.post("/", async (req, res) => {
    try {
        ...
        // 🔴 상태값 목록에 타입을 붙인다 — 그냥 문자열 배열이면 없는 상태값 오타를 tsc 가 못 잡아,
        //    "서버가 연산 중인 콜"을 이 잠금 검사가 인식하지 못한다.
        ...
        /**
         * 🚪 **종결된 콜은 되살리지 않는다**. 상세가 늦게 닿으면 이미 치운 콜을
         ...
         *
         * ⚠️ **이 값은 100% 믿을 수 없다** — 앱의 `isAutoActive` 는 화면 전이 경합으로 뒤집힐 수 있다
         *    (자동 터치 직후 LIST 오탐 → 세션 리셋 → AUTO 가 MANUAL 로 보고). 그래서 두 겹을 덧댄다:
         *      · [P3] 서버가 못 읽은 값(요금·주소)은 조용히 넘기지 않고 화면에 띄운다
         *      · [C′] 평가가 실패해도 확정은 진행한다 (실패가 콜을 유령으로 만들지 않게)
         */
        /**
         * ✋ **미리보기 콜은 잡지 않는다** (기사님 확정).
         *
         * 규칙 ① 의 *"직접콜은 심사하지 않는다"* 때문에 MANUAL 콜은 여기서 **즉시 KEEP** 된다.
         * 그런데 미리보기 콜은 기사님이 **확정을 누르기 전**이라 아직 안 잡은 콜이다.
         *
         * 🔴 실측: 팝업 3장을 읽고 올라온 미리보기 콜이 그대로 KEEP 되어
        ...
        if (isSimulated) {
             ...
             *
             * **클라이언트는 언제든 거짓말할 수 있다** — 악의가 아니라 화면 전이 경합 때문에도.
             *
             ...
             * 🔴 **평가 실패가 확정을 막지 않게 한다.**
             *
             * 평가는 **정보 수집**(경로·요율)이고 확정은 **기사님의 의지**다. 확정을 평가 뒤에 `.then` 으로 걸면
             * 평가가 실패할 때(예: 주소 없는 콜) 확정이 안 돌아 **DB 에도 안 들어가고 메모리에서도 안 빠진다.**
             * MANUAL 은 안전취소 타이머도(아래 코드가 이 분기에서 `return` 한다) LIST 이탈 정리도
             * (`devices.ts` 가 MANUAL 을 일부러 제외한다) 없어서, 치울 사람이 없이 관제웹에 영원히 남는다.
             *
             ...
             */
            /**
             * 👀 **미리보기로 이미 판정했으면 다시 계산하지 않는다** (기사님 지적 19:04).
             *
        ...
        const timeoutTimer = setTimeout(() => {
            ...
            if (decision) {
                ...
                //
                // 🔴 **지우기 전에 장부에 남긴다** — 안전취소는 배차망 취소 횟수(10회)에 들어가므로 한 건도 새면 안 된다.
                session.pendingDecisions.delete(payload.order.id);
                /* 🔴 저장 · 캐시 · 기기 짝 정리 · order-canceled · **취소 셈** 전부 이 한 줄 — 뒤에서 또 세면 두 번 센다.
                      미리보기면 셈도 장부도 건너뛴다 */
