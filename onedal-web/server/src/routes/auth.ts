...
router.post("/google", async (req, res) => {
    ...
    try {
         ...
         *    구글 로그인이 없어 재로그인 수단이 없다. 기사님 판단이 필요한 값이다
         */
...
router.post("/bypass", async (req, res) => {
    ...
    try {
         ...
         * 🔬 **실측 전용 계정**.
         *
         * 🔴 이 문의 기본은 **«DB 의 첫 번째 유저»** — 곧 **기사님 계정**이다. 화면을 눌러 보는 스크립트가
         *    그 계정으로 들어가면 기사님의 오늘 필터를 실제로 지운다. 그래서 스크립트는 `probe` 계정으로 들어온다.
         *    화면을 눌러 보는 일은 포기할 수 없다 — 코드 읽기로 못 찾는 버그가 거기서 나온다.
         *
        ...
        if (req.body?.probe) {
            ...
            if (!userRow) {
                 ...
                 *    («재료가 하나라도 없으면 null» · 옛 `useCallNet` — 걷음) **실측이 빈 화면을 찍는다.**
