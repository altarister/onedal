 ...
 * 여기에 목록을 한 벌 더 두면 shared 에 상태를 더해도 반영되지 않아 하차한 짐이 계속 적재 중으로 세어진다.
 */
 ...
 *
 * ⚠️ 적재 판정이 이 값을 먹으므로 숫자를 여기 다시 적지 않는다 — 원천은
 *    `VEHICLE_CAPACITY`(shared/vehicles.ts) 하나다 (규칙 ③)
 */
export function computeLoadedPoints(
    ...
    for (const c of calls) {
        ...
        // 🔴 관문을 **필드(`sizeClass`)가 아니라 점수로** 건다 — 화면(StopCallSheet)은 `unit` 만 보내므로
        //    필드로 걸면 관문이 영원히 닫혀 **기사님이 신고한 짐 양을 무시하고** 차종 추정으로 떨어진다
        //    (합짐 2건이면 만재로 추정해 허용 차종이 [오토바이] 하나만 남는다). cargoPoints 는 이미 unit 을 우선 보고
...
/**
 * 🎨 합짐의 우회 허용치는 따로 계산하지 않는다 — 채점기가 버퍼 소비를 후보 포함 타임라인에서 직접 잰다.
 ...
 *      **모르는 게 있다는 것이 버그 아닐까?**"*
 *
 *    맞다. 행은 KEEP 해야 태어나는데 **판정은 KEEP 하기 전에 난다** — 태어난 행만 읽으면 판정 시점에는
 *    정차가 언제나 「미확인 일반값」(15·10)이라, 1t(실제 13분)은 우회가 비싸 보이고 5t(실제 57분)은 싸 보인다.
 *    색이 곧 결정이라(규칙 ⑤-3) 양쪽 다 사고다.
 *
 ...
 *
 * 저장된 약속과 화면의 도착 예상이 어긋날 때, 서버가 그렇게 줬는지 관제웹이 다른 값을 썼는지 가리려면
 * 경로 순서와 구간 주행분이 남아 있어야 한다 — 카카오의 `sections[0]` 이 곧 상차지까지다.
 *
 ...
 */
/**
 * 🔴 **지문은 기사님마다 따로 기억한다** (기사님 실측).
 *
 * 1초 인터벌은 `sessions` 의 **모든 유저**를 돈다. 지문을 모듈 전역 하나로 두면 콜이 0건인 세션 하나가
 * 매초 지문을 갈아치워, 다음 초에 기사님 세션이 "바뀐 것"이 되어 같은 줄이 또 찍힌다.
 * 저장도 빈 목록 검사 **뒤**에 한다.
 *
...
function logRouteStops(
     ...
     *    «하차 39분» 이라고만 적으면 **그 구간이 39분 걸린다**로 읽혀, 합짐 후보가 붙어 누적이 는 것을
     *    «재탐색이 경로를 나쁘게 만들었다»로 오진한다.
     */
...
/**
 * 관제탑으로 보낼 오더 스냅샷. **진행 중과 종료된 것을 나눠서** 보낸다.
 *
 * 🔴 한 배열에 진행 중 콜과 취소·완료된 콜을 섞어 보내면 **받는 쪽마다 `isTerminal` 을 기억해야** 하고,
 *    잊으면 조용히 틀린다 (적재 수 · 재탐색 · 운임 합산).
 *
...
export function buildOrderSync(session: { userId: string; myOrders: MyOrder[]; pendingOrdersData: Map<string, any>;
    ...
    //
    //    한쪽만 읽으면 다른 쪽만 갱신하는 길이 생길 때 **관제탑에 낡은 상태가 간다**
    //    (예: 하차 완료했는데 카드에 "상차 완료"로 남는다).
    //
     ...
     * 경로는 **어디에도 쓰지 않는다** — 매초 실어 보내면 종료 콜은 하루 종일 쌓이므로 **오후로 갈수록
     * 무거워져** 브라우저가 죽는다.
     */
    const stripPolyline = (o: any) => {
        if (!o?.routePolyline?.length) return o;
        // 🔴 구간 경계(`sectionEnds`)도 함께 뗀다 — 선이 없으면 자를 것이 없다
        const { routePolyline, sectionEnds, ...rest } = o;
        return rest;
    };

    /**
     * 🧭 **경로 순서 — 원천은 서버 하나다** (기사님 동의).
     *
    ...
    /**
     * 🔴 **자리가 아니라 이름으로 맞춘다** (기사님 실측 · 3콜 리허설).
     *
     * 정거장에 도착하면 목록에서 빠지는데 주행분 배열은 계산 시점 그대로라, 자리로 맞추면 도착할
     * 때마다 길이가 어긋나 **주행분 전체가 null** 이 된다 (예산 줄·검산 문장·카운트다운이 전부 이 값을 먹는다).
     *
     * 남은 정거장의 누적 주행분은 여전히 옳다 — 카카오호출시점에서 잰 상대값이라 낡지 않는다.
     * 그래서 **인덱스가 아니라 이름으로** 맞춘다. 경로 연산이 남긴 `sectionStops`
     ...
     * ```
     *
     * 이름으로 맞추므로 «구간 수 ≠ 정거장 수» 검사가 이걸 막아 주지 않는다 — 그래서 여기서 가른다.
     *
    ...
    /**
     * 🔴 **끝난 콜은 후보가 아니다** (기사님 실측 · `routeStopsPending.test.ts`).
    ...
    /**
     * 🚫 **취소 예산 — 한 판(10회)에서 몇 번 썼나** (기사님 개정 · 확정안 구현 5).
     *
     * 잘못 집힌 콜 하나 = 취소 1회 소진, 망별(인성/24시). 파생값이라 저장하지 않고
     * 장부(`orders`)에서 센다 (규칙 ③). **저장하는 것은 리셋 시각 하나뿐**이다.
     *
     * ⚠️ **리셋 이후만** 센다 — 전 기간을 세면 `47/10` 처럼 한도를 몇 배 넘긴 값이 떠 *"조여라"* 도
     *    *"괜찮다"* 도 알려 주지 못한다. 총량은 판수(`cancelRounds`)가 지킨다.
     */
    ...
    return {
         ...
         *
         * ── 왜 여기에 얹나 ──
         * 🔴 지도가 **제 손으로** 위치를 정하면(클라 GPS → 없으면 집) 서버가 아는 자리와 어긋난다 —
         *    기점은 서버가 경로를 짠 그 자리여야 한다.
         ...
         * ⚠️ **궤적은 여기 안 싣는다** — 이 페이로드는 매초 나가 무거워지기 쉽다. 궤적은 `logbook/gps-track` 문이 있다.
         *
         * ── 🔴 «내 위치»는 여기 안 싣는다 ──
         * 이 봉투는 «콜이 바뀔 때» 나가는데 위치는 **1초마다** 바뀐다. 화면이 봉투 값을 따르면 봉투가 올 때마다
         * **옛 좌표가 내 점을 뒤로 당겨** 모의 주행이 멈춘 것처럼 보인다 (기사님 실측).
         * 위치는 **제 시점**을 가진 가벼운 이벤트(`driver-position`)로 간다 (규칙 ⑤-4 ③).
         *
         * 🧭 **`routeOrigin` 만 남는다** — 그것은 바로 위 `routeStops` 를 짠 기점이라
         *    **경로와 한 벌**이다. 정거장 순서가 바뀔 때 같이 와야 «1번 정거장이 내 뒤에 있다»가
         *    안 난다. «내가 지금 어디 있나»와는 다른 질문이다 (규칙 ⑤-4 ⑤).
         */
        ...
        /**
         * 🟡 **심사 중인 콜의 «미리보기 궤적»은 홀더를 따로 둔다** (기사님 실물).
         *
         ...
         *
         * 🔴 **주행분이 없는 콜도 궤적은 그린다** — 주행분이 빠진 콜이 있어도 **그릴 것을 잃지 않게** 한다
         *    (주행분은 `orders.sectionDriveMin` 칸에 저장되고 복구 때 읽힌다).
         *
         * ⚠️ 위 홀더를 넓히지 않는다. 심사 중 콜의 빈 `sectionDriveMin` 이 타임라인을
         *    통째로 폴백으로 돌린다 — 그래서 **갈라 둔다**.
         */
