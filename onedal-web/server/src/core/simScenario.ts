 ...
 *    (첫 시험). 그래서 좌표가 비면 상차·하차 동 이름이 들어 있고 요금이 같은가로 본다.
 ...
 * ⏱️ 75초 = 원달앱 하트비트(60초) + 여유 15초 (여섯 번째 바퀴 · onedal-49 합의) — 폰이 새 필터를 받는 가장 긴 간격이
...
function judgeSent(row: ScenarioRow, rs: RowState, w: ScenarioWorld, claimed: Set<string>): RowState {
    ...
    if (row.kind === 'keep' || row.kind === 'cancel') {
        ...
        /**
         * 🔴 **폰이 막아도 곧바로 끝내지 않는다** (onedal-49 합의) — 폰은 새 콜을 직전 필터로 먼저 판정하고
...
/**
 * 🫳 **거둘 콜 번호** — 끝난 줄(`doneAt`)이 낸 콜 중 아직 안 거둔 것 (일곱 번째 바퀴 · onedal-49 합의).
