/**
 * 📍 **픽커 상세 화면 글자에서 전체 주소를 뽑는다** (폰 시험)
 *
 * ── 왜 ──
 * 픽커 리스트의 주소는 줄임 이름(«광주 초월읍»)이다. 그대로 쓰면 서버의 지역 불일치 방어가 첫 낱말 «광주»를
 * 광주광역시로 보고 카카오의 맞는 답(«경기 광주시 초월읍»)을 버려, 경로도 판정도 없다.
 ...
 * 🔴 «오더» — 수락 뒤 화면의 «오더 확인» 버튼 · «오더 확인 <번호>» 글자가 건물명에 붙었다 (04 폰 시험 · 서버 장부 «… 곤지암점 오더 확인»)
 */
