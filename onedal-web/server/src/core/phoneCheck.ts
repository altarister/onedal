...
/**
 * 📱 **폰이 «실제로 쓰는 값»을 서버 값과 비교한다** — 테스트 시작 전 점검용 (기사님 지시)
 *
 * 기사님: *"지금 여러 번 같은 지점에 오류가 계속되고 있어 … 먼발치에서 근본적인 원인을 찾아 수정해야 할 것 같아"*
 *
 * 🔴 **점검은 «서버가 정한 값»만 보면 안 된다** — 폰이 서버 응답을 못 읽으면 옛 필터 · 직접 모드(MANUAL)로 도는데
 *    점검은 초록이다. 폰은 통신마다 «적용 중인 모드»(`appliedMode`)와 «들고 있는 필터 지문»(`filterVersion`)을
 *    보내므로, 서버가 «보낼 지문»과 비교하면 잡힌다.
 *
