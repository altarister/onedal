...
/**
 * 🪪 **이 서버의 유저인가** — 서명 검증과 **다른 질문**이다 (기사님 실측).
 *
 * 로컬과 라이브가 같은 JWT 비밀을 쓰므로, 라이브에서 발급된 토큰도 로컬에서 **서명은
 * 통과한다.** 서명은 *"위조가 아니다"* 만 말할 뿐 *"어느 서버가 발급했나"* 를 모른다.
 *
 * 그래서 라이브 토큰을 든 클라이언트가 로컬에 붙으면 **로컬 DB 에 없는 유저의 메모리 세션**이 생긴다.
 * 소켓은 끊겨도 세션은 안 지워지고(`clearUserSession` 은 명시적 로그아웃 전용), 1초 인터벌이 없는 유저까지 돈다.
 *
