...
export class SettingsRepository {
    ...
    public static loadPricingConfig(userId: string): PricingConfig {
        const row = db.prepare("SELECT vehicle_rates, agency_fee_percent, call_discount_pct FROM user_filters WHERE user_id = ?").get(userId) as any;
        ...
        return {
            ...
            /**
             * 🔴 2026-08-13 — **`max_discount_percent` 를 `call_discount_pct`(콜할인율)가 대체한다.**
             *
             ...
             */
            maxDiscountPercent: row?.call_discount_pct ?? 10
        };
    }
