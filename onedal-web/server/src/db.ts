 ...
 * 지금까지 판정은 소켓으로만 날아가 새로고침하면 근거가 사라졌다. 카드 접이와 문제지 채점 회귀가 이걸 읽는다.
 * ⚠️ orders FK 없음 — 심사는 KEEP 전이라 orders 행이 아직 없다 (steps 의 FK 교훈).
 */
/**
 * 🔴 **`score` 는 `null` 일 수 있다** (코드 리뷰에서 잡힘).
 *
...
//    → CREATE 문 바로 뒤로 옮겼다 (검사 `schemaOrder.test.ts`)
...
// ═══════════════════════════════════════
// [9] 취소 예산 판 나누기 — **리셋 시각만** 저장한다 (기사님 확정)
//
