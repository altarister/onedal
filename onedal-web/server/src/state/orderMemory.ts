 ...
 *
 * 🔴 증상은 여럿이어도 클래스는 하나다 — 예: 심사 캐시만 고치고 활성 콜을 안 고쳐 앱이 옛 지역으로 거르거나,
 *    `/confirm` 이 넣은 `targetApp` 을 `/detail` 이 새 객체로 덮어써 NULL 로 남는다. 그래서 자리를 하나로 없앤다.
 *
 ...
 *
 * 🔴 **문은 하나다** — 경로마다 분기를 넣으면 경로가 늘 때마다 같은 사고가 다시 난다.
