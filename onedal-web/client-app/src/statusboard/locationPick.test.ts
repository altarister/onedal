...
/**
 * 📍 **「위치 찍기」가 서버에 무엇이라고 말하나** (서버 지적 2026-09-12).
 *
 * 🏠 집 · 주소로 찾기로 찍은 좌표가 **`'mock'` 으로** 나가고 있었다. 서버는 `manual` 을
 * 이미 알아듣는데(소켓 문에 화이트리스트가 없어 온 그대로 통과하고, `originOf`·
 * `gps_tracks.source` 가 그 낱말을 쥔다) 클라 타입에 없어서 못 보냈다.
 *
 * 🔴 **그래서 궤적에서 «달린 것»과 «찍은 것»을 가를 수 없었다.** 2026-08-14 파주 156km
 *    사고가 «가짜를 진짜로 읽어서» 난 것이라, 출처가 한 이름으로 섞이는 것은 그 씨앗이다.
 *
...
describe('📍 위치 찍기 — 출처를 바로 말한다', () => {
    ...
    /** 🔴 «못 보낸다»고 적어 두었던 변명이 남아 있으면 화면이 옛 사실을 말한다 */
