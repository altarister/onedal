/**
 * 🌉 **다리 — 현황판이 관제웹 안쪽에 기대는 «전부»**.
 *
 * 기사님 지시: *"지금꺼 잘 분리해서 **나중에 쓸수 있도록** 잘 만들어줘."*
 *
 * 기사님 구상(아직 안 함): **브라우저 둘** — 프로젝트 / 어드민.
 ...
 *    되고, **의존이 몇 개인지 한눈에 보인다** (아래 `export` 줄을 세면 된다).
 *
 ...
 *
 * 표에 없는 `useDriverPositionStore` · `useSettingsStore` · `KM_PER_TICK`·`STOP_OFF_ROAD_KM` ·
 * `LAB_EVENING` 은 아래 각 줄의 주석이 무엇인지 말한다.
 *
 * 🔴 **위치는 `socket.emit` 을 직접 하지 않고 `publishLocation` 을 지난다** — 그 함수가
 *    **서버에 위치를 알리는 유일한 자리**다 (`lib/gpsBridge.ts`). 두 곳에서 각각 쏘면 시뮬 좌표와
 *    실제 좌표가 섞여 경로가 수백 km 로 튄다.
 * ⚠️ `publishLocation`·`apiClient` 는 주소 찾기와 집 버튼이 쓴다 — 기사님이 빼신 것은 **지도**뿐이다
...
/* ── 🧪 **여기부터 셋은 «테스트용»이다 — 어드민으로 갈 때 함께 걷는다** ──
   ...
   🔴 **셋의 성질이 같다 — 전부 «서버로 보내는» 것**이다. 위의 것들은 읽기만 한다.
      화면에서도 맨 위 **🧪 테스트용 구역** 하나에 모여 있다 (`TestOnlySection`).
   🔴 **걷는 법** — 아래 `export` 줄 전부 + 그 구역 + `<TestOnlySection/>` 호출 한 줄. 그게 전부다. */

/**
 * 🎭 **모의 주행 스위치** (기사님 — *"경로가 생기면 현황판도 알게 될 거고
...
/**
 * 🖐️ **손으로 콜 하나를 올릴 때 쓰는 문제지** (기사님 지시 · 「콜 생성」 버튼).
 *
 * 🔴 **콜 값을 지어내지 않으려고 여기를 지난다** — 이것은 기사님이 **실제로 도신 하루**다
 *    (볼트 8/10 저녁 · 좌표까지 카카오 기록 그대로). 현황판이 제 손으로 콜을 만들면
 *    그 순간 **네 번째 가짜 콜 원천**이 된다 (규칙 ③·④).
 * ⚠️ 어드민으로 옮길 때는 **이 줄만 걷으면 된다** — 그쪽엔 「콜 생성」이 없다.
 */
export { LAB_EVENING } from '../pages/labProblems';
