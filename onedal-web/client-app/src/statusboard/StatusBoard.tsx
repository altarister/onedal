...
function LocationPickCard() {
    ...
    const send = (lng: number, lat: number) => {
        /**
         * 🔴 **`source:'mock'` 으로 나간다 — 손으로 찍은 것인데도.**
         *    서버는 `manual` 을 알아듣는데 `gpsBridge` 의 `GpsSource` 에 그 낱말이 없다.
         *    🎭 모의 주행이 생긴 뒤로 «달리는 중»과 «찍은 것»이 한 이름으로 섞인다 —
         *    그 한 낱말은 프로젝트 쪽이 연다 (답신 ⑥).
         * 🔴 모르면 **화면이 그 사실을 말한다** (규칙 ④).
         */
        const r = publishLocation(lat, lng, 'mock');
        setSentNote(r.sent ? '보냈다 · 서버는 🧪 모의 주행으로 적는다 (manual 을 못 보낸다)'
            : r.reason === 'mock-running' ? '모의 주행 중 — 안 나갔다'
                : '같은 자리 — 안 나갔다');
    };
