 ...
 * 에게 가서 **구글 인증 토큰을 받고도 보낼 곳이 없어** 로그인이 조용히 되돌아온다.
 * 브라우저에서는 프록시가 받아 주니 안 드러나고 앱에서만 난다.
 *
 * ── 화면 규칙 (⑤-4 ④) ──
 * 🔴 **지금 어디를 보는지 항상 적는다.** 안 적으면 «어느 서버를 보고 있나»를 몰라
 *    로컬을 고쳐 놓고 라이브를 보며 *"왜 안 고쳐졌지"* 를 반복한다.
 *
...
export function ServerSwitch() {
    ...
    useEffect(() => {
         ...
         * 🔴 **토글이 아니라 «항상 연다»**.
         *
         * 토글이면 여러 번 누를 때 **누를 때마다 열렸다 닫혔다** 하고, 짝수 번이면
         * 닫힌 채로 끝나 *"아무 반응이 없다"* 가 된다.
         *
