 ...
 * ── 까닭 ──
 *
 * 지도 배경 + 3단 시트 화면에는 **탭 줄**이 없어, «완료됨 · 취소 · 방출»을 볼 자리가
 * 이 서랍뿐이다. 현황판에도 같은 목록이 있지만
 ...
 * ── 규칙 ──
 *
 * 🔴 **거르는 규칙은 `lib/finishedCalls` 한 벌**이다 — 끝난 콜을 거르는 화면은 모두 이것을 부른다 (규칙 ③).
 *    두 벌이 되면 «취소 탭에는 있는데 서랍에는 없는» 콜이 생긴다.
 * 🔴 **끝난 콜은 콜 카드로 그리지 않는다.** `PinnedRouteCard` 에는 결재 버튼과 단계 시트가
 *    들어 있어 끝난 콜에는 맞지 않다 — 한 줄 요약만 그린다.
 * 🔴 **곁 패널(현황판)은 덮지 않는다.** 서랍은 관제 영역 안에서만 깔린다 —
 *    현황판은 곁에서 지켜보는 화면이라 가리면 지금 무슨 일이 일어나는지를 놓친다.
