...
/* 🔼 **아래 셋은 «위 덩어리» 안에 둔다** (기사님 지시).
   「이 콜 처리」(방출·사무실 취소) · 「배송 주행 · 하차 마감」 · 「판정 근거 · 원본 데이터」.
   🔴 스텝 **아래**에 두면 카드가 셋으로 서고 스크롤 막대가 두 개가 된다.
      위 덩어리 안에 두면 콜을 설명하는 값들끼리 모인다. */

/* 🏗️ **약속·차이만 적는 상차·하차 줄은 따로 두지 않는다** — 타이틀이 다 말한다.
   기사님: *"타이틀하고 중복인 것 같은데 이걸 지우고 타이틀에 다 표현할 수 있지?"*
   ⚠️ 지나간 시각(취소선)은 안 적는다 — 지금 몇 시인지만 알면 되고, «달라졌나»는 **색**이 답한다. */
...
interface Props {
    ...
    indexNum: number;
    /** 🎨 콜 색 — 지도 마커 테두리와 같은 색 점 (한눈 대응 · 기사님 확정) */
    accentColor?: string;
...
export default function PinnedRouteCard({
    ...
    /**
     * 🕐 **경로에 없으면 장부에서 읽는다** (기사님 발견).
     *
     * 기사님: *"완료됨 가서 이전 콜을 확인해 보니 `?. 초월읍 약속? - ?. 신둔면 약속?`
     * 이렇게 나오는데.. **약속시간이 날아가나 봐.**"*
     *
     * `etaMap` 은 **지금 경로**에서 만든다. 끝난 콜은 경로에 없어 통째로 빈다 —
    ...
    /**
     * 진행 단계는 **저장하지 않고 단계 행에서 파생**한다 (`stepCurIdx`).
     * 화면이 따로 저장해 두면 새로고침·재접속·스와이프에서 어긋난다.
     *
     * 로컬로 두는 것은 `stepNav` 하나 — 지난 단계를 되돌아볼 때만 쓰고, 새 기록이 들어와
     * 단계가 앞으로 가면 자동으로 풀린다. «통화를 건너뛰었다»는 서버에 남길 값이 아니다
     * (안 한 일을 기록하면 데이터가 오염된다).
     */
    /**
    ...
    /**
     * 🌱 **콜을 잡는 순간 여섯 단계가 정해진다** — 서버가 만든 단계 행(`steps-synced`)을 받아 둔다.
     *
     * 아래 단계 화면(본 화면)이 이것 하나를 그린다.
     */
    ...
    /**
     * 🏷️ **이 장을 «누가» 열었나** — 로그에만 쓴다 (기사님 실측).
     *
     * 🔴 **손으로 넘긴 것과 트랙이 되돌린 것을 갈라 찍는다.** 트랙이 장을 되돌린 것도
     *    `stepNav != null` 이라, 한 라벨이면 도착이 연 것과 **구별이 안 된다.**
    ...
    /**
     * 🔴 현재 단계 = "첫 미완료"가 아니라 **증거 최전방** (기사님 실측).
     *
     * "첫 미완료"로 세면 GPS 도착이 하차지 통화를 건너뛰고 지나갈 때 화면이 **하차지 통화에
     * 묶여**, 상차 완료·하차 도착이 끝나도 안 따라간다. 가장 멀리 간 증거의 **다음**이 현재다.
     ...
     * 🔴 **이 카드는 도착 사건을 따로 듣지 않는다** — 따로 들으면 상태바·장부와 다른 단계가 뜬다.
     */
    const focusStepIdx = focusStep && seededSteps ? seededSteps.findIndex(x => x.step === focusStep) : -1;
    /* 🎬 끌어올려 상태바의 콜이 되는 순간 손으로 넘긴 단계를 지운다 — 상태바 단계로 맞춘다 (기사님 · #145) */
    useEffect(() => {
        if (focused) setStepNav(null);
    }, [focused, focusStep]);

    /**
     * 📡 **화면이 «지금 어느 단계»를 보여주는가 — 로그로 남긴다** (기사님 지시).
     *
     * 기사님: *"순식간에 열었다 닫았다 스텝은 모두 비슷하고. 어떻게 알아 —
     * 너가 콘솔 로그를 찍으면 되지."*
     *
     * ── 왜 필요한가 ──
     * 다른 로그로 알 수 있는 것은 **시트가 몇 단인가**(`[시트]`)와 **어느 콜을 열었나**
     * (`[시트연콜]`)까지다. 그 안에서 **어느 단계가 펼쳐졌는지는** 이 로그가 아니면 안 남는다 —
     * 없으면 «도착 스텝을 못 봤다»는 말을 확인할 수단이 없다.
     *
    ...
    // 📡 심사 중에는 이 콜의 텔레메트리 핑을 세고 잠깐 깜빡인다.
    // (되돌아보기 자동 해제는 위 `stepCurIdx` 효과가 한다 — 도착을 눌렀는데 화면이 지난 단계에 머물면 무엇이 반영됐는지 알 수 없다)
    useEffect(() => {
        // 평가 중이 아닐 때는 카운터 초기화
     ...
     *
     * 🔴 단독 구간 선택·접근 거리·상차 정차를 이 파일과 `DepartureCountdown` 이
     *    **각자 계산**하면, 한쪽만 고쳤을 때 두 화면이 다른 시각을 말한다. 파생값을 만들었으면 그 입력도 한 곳에서 만든다.
     */
    ...
    return (
        ...
        <div className={`flex flex-col relative overflow-hidden transition-all duration-300 ${isDeck ? 'flex-1 min-h-0' : ''} ${evaluating ? 'bg-warning/10' : 'hover:bg-surface-hover/50'} border-b border-border-card ${isTerminal(route.status) ? 'opacity-50 grayscale' : ''}`}>
            ...
            {/* 1-a. 덱 헤더 — **한 줄**. 폰 한 화면이 목표다.
                🔴 경로·거리·분은 **바로 위 덱 요약 줄이 이미 말한다** — 여기엔 적지 않는다.
                   기사님: *"UI 영역을 아껴 써야 한다."*
            ...
            {isDeck && (
                ...
                <div className="px-3 pt-2.5 pb-1 flex items-center gap-x-1.5 text-[11.5px] text-text-muted tabular-nums whitespace-nowrap">
                    ...
                    {/**
                      ...
                      *    조회용 리스트 헤더는 첫 글자만 적으므로 **덱에서는 온전히 적는다.**
                      */}
                    {route.vehicleType && (
                    ...
                    {/* 🧭 어떻게 잡았나 — 덱 머리글에도 단다 (리스트 헤더에만 달면
                        기사님이 보는 덱에는 안 나온다). 알람 듣고 잡음=알람콜 · 손=직접콜 */}
                    ...
                    {/* 🎨 판정색 칩 — KEEP 버튼 배경에만 색을 두면, 버튼 없는 직접·알람 콜은
                        판정을 받아도 **색이 보일 자리가 없다**.
            ...
            {isExpanded && (
                 ...
                 * 📏 **무대에서는 본문이 스크롤한다** (기사님).
                 *
                 * 🔴 카드가 무대가 준 높이만큼 서는데 본문이 내용대로 자라면, 카드의 `overflow-hidden` 이
                 *    **잘라 감춘다** (규칙 ④ 위반).
                 * 🟢 잘라 감추지 않고 **여기서 스크롤**한다 (`overflow-y-auto`). 콜 줄(헤더)은 밖에 있어 계속 보인다.
                 */
                /**
                 * 📏 **본문은 «위 덩어리 + 스텝» 둘로 선다** (기사님 정리).
                 *
                 * 기사님: *"상단은 콜의 내용을, 하단은 콜의 스텝을 이야기하는 영역이다.
                 * 합짐을 많이 해서 콜 내용만 보이는 경우 **콜 컨텐츠 전체에 스크롤**이 적용되고,
                 * 그렇지 않고 스텝 영역까지 보이면 **스텝만 스크롤**되는 구조야."*
                 *
                 * 🔴 자리가 모자라면 이 본문이 스크롤하고, 자리가 있으면 스텝이 `flex-1` 로 서서
                ...
                <div className={`px-4 pb-4 pt-2 text-sm border-t border-border bg-surface ${
                    ...
                    {/* 🕐 **펼친 자리는 원래 시각을 따로 적지 않는다** — 접힌 줄(덱)은 «틀어졌나»만 기호로 답하고(안 C),
                        ...
                        통화의 대사가 그 줄에서 나온다 — *"원래 3시 15분이라 했는데 20분쯤 되겠습니다."* */}
                    {/**
                      * 🔴 **약속·차이만 적는 상차·하차 줄은 여기 두지 않는다** (기사님:
                      *    *"타이틀하고 중복인 것 같은데 이걸 지우고 타이틀에 다 표현할 수 있지?"*).
                      *
                      *    콜 줄이 이미 «① 초월읍 ~23:23 → ③ 신둔면 ~00:38» 을 다 그린다. 그 밖의 둘은:
                      *      · 지나간 시각(취소선)은 안 적는다 — 지금 몇 시인지만 알면 된다
                    ...
                    {/* 🔴 type 은 확정 전 «MANUAL_CLICK» — !== 'MANUAL' 로 거르면 직접·알람 콜의
                        평가 순간에 결재 버튼이 잠깐 그려진다.
                        결정은 스캔앱에서만 — 직접 갈래(MANUAL*)에는 버튼 자체를 안 만든다.
                        🪧 다만 **근거는 갈래와 무관하게 보인다** (기사님: "왜 똥콜이고 꿀콜인지
                        여기 있는 정보를 최대한 보여줘야 한다") — 버퍼·조건 전수가 버튼 블록 안에
                        갇히면 직접·알람 콜은 근거를 통째로 못 보므로, 근거는 버튼 블록 밖에 둔다. */}
                        ...
                        <>
                            ...
                            <div className="mt-1 flex gap-3">
                                ...
                                {!!route.kakaoTimeExt ? (() => {
                                    ...
                                    /**
                                     * ⚠️ `[...꿀똥콜추천최단거리시간]` 처럼 **낱글자 집합**으로 지우면
                                     *    문장 어디에 있든 그 글자를 지워
                                     *    `총 추가시간(+116분) 초과` → `총 가(+116분) 초과` 로 뭉개진다.
                            ...
                            {/* 🪧 직접·알람 콜의 판정 안내판 — KEEP 버튼이 하던 말(색·판정·근거)을
                                누를 수 없는 안내판으로 그대로 한다. 결정은 스캔앱에서, 근거는 여기서 (기사님) */}
                    ...
                    {/* ══════════════════════════════════════════════════════════
                        ...
                        콜을 알아보고 → 적요를 읽고 → 전화를 걸고 → 다음 단계로 넘어간다.

                          ① 착불 경고 · 퀵사무실          ← 놓치면 안 되는 것 · 보조 연락처
                          ② 짐 · 버퍼 칩 줄               ← «어떤 콜인가»
                          ③ 적요 · 상·하차 줄 · 배송 주행 ← 콜을 설명하는 값
                          ④ 접힘                          ← 판정 근거·원본 데이터 · 이 콜 처리(방출·사무실 취소)
                          ⑤ 스텝(지금 할 일)              ← 현재 단계 하나 + 진행 막대

                        🔴 ④의 내용을 본문에 펼치면 폰에서 세로로 길어져
                           "한 화면에 들어온다"는 목표가 깨진다.
                       ══════════════════════════════════════════════════════════ */}
                    ...
                    <div className={`flex flex-col gap-2 text-[13px] leading-tight mt-3 ${
                        ...
                        {(() => {
                            ...
                            /**
                             * 📦 **짐은 적요와 따로 칩으로 세운다** (기사님 — 목업에서 칩으로 세우셨다).
                             *    `품목 / 적요` 를 한 문장으로 이어 붙이면 **무엇을 싣는지가
                             *    긴 문장 안에 묻힌다.**
                            ...
                            return (
                                <>
                                    ...
                                    {/* 🔴 **결재 전에는 단계 시트를 열지 않는다** (기사님 확정).
                                        ...
                                        저장하면 `FOREIGN KEY constraint failed` 로 **통화 내용이 통째로 날아간다**.
                                        6단계는 확정된 콜의 일이므로 그때부터 연다. */}
                                    {evaluating && (
                                    ...
                                    {/* 🏗️ **통화·현장 기록은 아래 단계 화면(StepSheetMock) 하나가 받는다** (기사님).
                                        같은 문(save-cargo-report · report-milestone)으로 저장하고, 열 때마다 계산하지 않고
                                        저장된 단계 행만 그린다. 통화·현장 규칙은 시딩·StepSheetMock 이 잇는다. */}
                                            ...
                                            return (
                                                <div className="flex items-center gap-1.5 flex-wrap text-[11px] mb-1">
                                                    ...
                                                    {/* 🧮 경로 최소 버퍼 (⑯-1) — 이 콜의 칩이 +60 이어도 **다른 콜 약속**이
                                                        +6 이면 예산은 6분이다 (기사님 실측). 내 칩보다 빡빡할 때만 적는다 */}
                                                    {(() => {
                                    ...
                                    {/* ── 적요 — 통화 전에 읽어야 하는 유일한 텍스트 ──
                                        🔴 적요는 스텝('지금 할 일') **위**에 둔다 — 적요를 읽고 전화를 거는 순서다. */}
                                    {/* 배경 박스 없이 라벨만 둔다 — 한 줄이면 라벨만으로 충분히 구분된다 (UI 영역 아끼기) */}
                                    ...
                                    {/* 🧭 상·하차 줄 — 구간·밀림. 타이틀에 없는 것만 든다 (까닭은 부품 주석) */}
                                    <StopDetailBlock route={route} timeline={timeline}
                                        visitOrder={visitOrder} timing={timing} etas={etas} />
                                    {/**
                                      * 🔼 **셋(배송 주행 · 판정 근거 · 이 콜 처리)은 위 덩어리 안에 둔다** (기사님).
                                      *
                                      * 🔴 **스텝 아래**에 두면 카드가 «위 덩어리 + 스텝» 둘이 아니라 셋으로 서서 길어지고,
                                      *    스텝이 자리를 못 받아 **스크롤 막대가 두 개**로 보인다.
                                      * 🟢 여기(적요 아래)는 **위 덩어리 안**이다 — 콜을 설명하는 값들끼리 모인다.
                                      *    자리가 모자라면 위 덩어리가 스크롤하고, 스텝은 자기 몫(220px)을 지킨다.
                                      */}
                                    {/* 🚚 **배송 주행과 하차 마감 — 접지 않는다** (기사님 확정)
                                    ...
                                    {/* ── 접힘 — 문제가 생겼을 때만 ──
                                        🔴 단독 경로·요금·수수료는 본문에 펴지 않는다 —
                                        덱 헤더가 이미 같은 값을 띄우므로 중복이고, 세로만 잡아먹는다.
                            ...
                            <div className="text-[10px] text-text-muted mt-1.5">
                                되돌릴 수 없습니다. 방출은 그 장소에 사유가 기록됩니다.
                                        ...
                                        <div className={`mt-1 mb-2 ${isDeck ? 'flex-1 min-h-[220px] flex flex-col' : ''}`}>
                                            ...
                                            {/* 🌱 **네비게이션 모양** (기사님: *"새로 영역이 길어지면 스크롤 해야
                                            ...
                                            {seededSteps && (() => {
                                                ...
                                                return (
                                                    <div className={`mt-1 ${isDeck ? 'flex-1 min-h-0 flex flex-col' : ''}`}
                                                        ...
                                                        {/**
                                                          * 🌱 **머리 — 단계명 · 가운데 점 · n/6**.
                                                          * 🔴 **점은 가운데 고정** (기사님: *"단어에 따라
                                                          *    스와이프 네비게이션이 덜컹거려"*). 단계 이름 길이가 달라
                                                          *    («상차지 통화» ↔ «상차 완료») 점이 좌우로 밀린다 —
...
/**
 * 약속 시각 칩(`DeadlineChip`, 이 파일 맨 아래) — 헤더에서 바로 보고, 바로 고친다.
 *
 * 기사님: *"여기에 상차시간, 하차시간을 추가하고 시간을 변경해 보여줄 수 있을까?"*
 *
 * **예상 도착(ETA)** 만으로는 모자라다. 정작 중요한 건
 ...
 *    (기사님: *"목업에서 열리면 보이는 컨텐츠 상하차 부분은
 *    가지고 가서 기존 거에 더하고 싶어"*).
 *
 * 🔴 **약속·차이만 적는 줄(`PromiseLines`)은 두지 않는다** — 그것은 *"타이틀과 중복"* 이다 (기사님).
 *    이 줄은 그와 달라 철거 대상이 아니다 — **타이틀에 없는 둘**을 든다:
 *      · **구간 거리·분** — 그 정거장으로 들어오는 구간 (내 위치→상차 · 상차→하차)
 *      · **밀린 분** — 접힌 줄은 `▲▼` **기호로만** 말한다 (안 C). 몇 분인지는 여기에만 있다
 *    `timeDisplay.test.ts` 가 그 둘을 «자격»으로 묻는다 — 그 둘 없이 약속·차이만 적으면 빨간불이다.
 *
 * ⚠️ **원인별 내역**(`└ 합짐2 경유 +7분`)은 이 줄에 아직 없다 — `shared` 의 `impactOfStop` 을
 *    이 줄은 아직 부르지 않는다.
 */
    ...
    return (
        <div className="flex flex-col gap-0.5 text-[11px] tabular-nums mb-2 px-0.5">
            {rows.map(r => {
                ...
                /**
                 * 🔴 **약속은 타이틀과 «같은 원천»에서 온다** (규칙 ③).
                 *    `deriveCallTiming` 만 보면 타이틀과 이 줄이 다른 약속 시각을 말한다.
                 *    타이틀(`CallDeck.promiseOf`)이 그러듯
                 *    **타임라인의 약속을 먼저** 보고, 없을 때만 콜 파생을 쓴다.
                 */
                const promised = tl?.promisedUntil ?? r.promised;
                /**
                 * 🔴 **지난 곳에 «예정»은 없다** — 통과했으면 빈칸이 맞다.
