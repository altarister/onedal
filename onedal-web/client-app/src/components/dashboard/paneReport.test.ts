...
/**
 * 🌱 **트랙이 «손으로 넘긴 것»만 알린다** (서버 진단 · 기사님 확정 2026-09-12 밤).
 *
 * ── 무엇이 났나 ──
 * 도착이 «지금 할 단계»로 장을 열면, 트랙이 거기까지 넘어가는 **그 도중에** 스크롤
 * 이벤트가 옛 번호를 부모에게 알렸다. 부모는 그 말을 믿고 장을 되돌렸고, 둘이 서로를
 * 밀며 엉뚱한 장에서 멈췄다. 실측 (같은 콜, 254ms 안에 네 번):
 *
 *   23:21:34.358  4 ARRIVE_DROPOFF  →  5 DELIVERED
 *   23:21:34.382  5 DELIVERED       →  4 ARRIVE_DROPOFF   (+24ms)
 *   23:21:34.422  4 ARRIVE_DROPOFF  →  0 CALL_PICKUP      (+40ms)
 *   23:21:34.612  0 CALL_PICKUP     →  1 CALL_DROPOFF     (+190ms)
 *
 ...
 *    *"애니메이션을 얹지 않는다 — 부드러움은 `scroll-snap` 이 하고 손이 멈추면 끝난다"*
 *    고 적어 두고 있었고, 열릴 때는 이미 `auto` 였다. 애니메이션이 없으면 중간 자리도
 *    없으니 이 판단은 **겹쳐 둔 장치**가 된다 (규칙 ②).
 *
...
describe('🌱 트랙이 알릴 장', () => {
     ...
     *    프로그램이 옮기든 손가락이 끌든, 그 자리에서 알리면 부모가 옛 번호로 되돌린다.
     */
