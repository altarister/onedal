/**
 * 🌱 **트랙이 부모에게 알릴 장 — 순수 함수** (서버 진단 · 기사님 확정).
 *
 * ── 왜 있나 ──
 * 도착이 «지금 할 단계»로 장을 열면 트랙이 거기까지 넘어가는데, **그 도중의 스크롤
 * 이벤트는 지나가는 장의 번호를 알린다.** 부모가 그 말을 믿고 장을 되돌리면 둘이 서로를 밀며
 * 엉뚱한 장에서 멈춘다 (한순간에 `4 → 5 → 4 → 0 → 1` 처럼 튄다).
 *
 ...
 *    (관제웹 CLAUDE.md — 깃발은 켜는 건 안 잊어도 끄는 걸 잊어 화면이 거짓말한다).
 *
 * 🟢 **트랙은 `behavior:'smooth'` 를 쓰지 않는다** (기사님 확정) — 장을 옮길 때 애니메이션을 얹지 않는다.
