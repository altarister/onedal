...
/**
 * 🧭 **다음 정거장들(경유 최대 3 + 도착 1)을 QR 로 건넨다** (기사님 확정)
 *
 * ── 왜 QR 인가 ──
 ...
 *    폰을 셋으로 나누는 까닭은 GPS 만이 아니다.
 *
 * ── 흐름 ──
 * ```
 * 도착 → 관제폰 화면에 QR → 개인폰 카메라 → 카카오내비가 그 정거장들로 안내
 * ```
 * 🟢 **우리 서버도 인터넷도 안 거친다.** QR 안의 글자를 개인폰이 읽어 그대로 실행한다.
 *
 * ⚠️ **경유지는 3개까지만 담는다** — 카카오내비 한도다. 한 차례가 3콜이면 정거장이 6개라
 *    QR 을 나눠 찍는다. 넷씩 자르는 것은 부르는 쪽(`StageView` 의 `qrSlice`)이 한다.
 */
