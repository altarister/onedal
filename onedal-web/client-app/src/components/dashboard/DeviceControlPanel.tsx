 ...
 * 📱 **폰 한 대의 카드**
 *
 * 📐 **한 줄 41px 기준** (기사님: *"폰 영역 높이가 115px 인데 41px 정도로"*).
...
function DeviceRow({
    ...
    /**
     * 🎛️ **「적용중」 — 누른 것이 폰에 닿았나** (기사님 확정).
     *
     * 🔴 **폰이 받았다고 알려 올 때까지 「적용중」으로 그린다** — 누르는 순간 바뀐 것처럼
     *    그리면 폰이 받았는지 모르면서 단언하게 된다.
    ...
    /**
     * 👁️ **앱은 켜져 있는데 화면을 못 읽는 중** (기사님 확정 · 크리티컬).
     *
     * 접근성이 막혀 콜을 하나도 못 읽어도 텔레메트리는 계속 오므로 연결 표시는 **파란불**로
     * 남는다. **「연결됐다」와 「읽고 있다」는 다른 말이라 따로 그린다** — 안 그리면
     * 실운행에서 콜을 통째로 놓치는데 기사님이 알 방법이 없다.
     *
    ...
    if (currentFilter) {
        ...
        if (device.mode === "MANUAL") {
            ...
            /**
             * 🔴 **알람은 «찾는다»까지만이고 «잡는다»가 아니다** (기사님 확정).
             *
             * 국면 배지(첫짐·합짐·경로)는 자동·알람이 같은 낱말을 쓰고, 알람인지는 옆의
             * 모드 버튼(알람 녹색)이 말한다 — 그 구분이 없으면 화면이 *"앱이 이 콜을 잡는다"* 고
             * 말하는데, 실제로는 기사님이 직접 누르실 때까지 아무 일도 안 일어난다.
     ...
     *
     * 스캔을 안 하는 폰이 *"방금 훑었다"* 고 말하면 안 된다 — 하트비트만 온 차례에도
     * 지난 성적표는 그대로 남아 있으므로, 성적표가 있다는 것만으로는 지금 훑고 있다는 뜻이 아니다.
     *
        ...
        <div className="flex flex-col rounded-lg border border-border-card bg-surface-alt/30 overflow-hidden mb-1">
            <div className="flex items-center justify-between hover:bg-surface-hover/30 transition-colors px-2.5 py-1">
                <div className={`flex items-center gap-2 flex-1 min-w-0`}>
                    ...
                    {/* 🕐 **마지막으로 이 폰이 보고한 시각**을 숫자 앞에 붙인다 (기사님 형식 확정).
                ...
                {/**
                  * 🎛️ **모드 셋은 돌려 쓰는 토글이 아니라 고르기다** (기사님 확정).
                  *
                  * 🔴 **한 버튼을 눌러 다음 모드로 넘기지 않는다.** 셋을 돌려 쓰면 한 번 잘못 누를 때
                  *    되돌아오는 데 두 번을 더 눌러야 하고, 지금 무엇인지가 «다음에 무엇이 되는지»와
                  *    섞여 읽힌다. 펼친 셋에서 바로 고르면 **지금 상태가 곧 화면**이다 (규칙 ⑤-4 ④).
                  *
                  * 색은 판정 색(🔵🟢🟡🔴)과 겨루지 않는다 — 켜진 것만 진하게 (규칙 ⑤-3).
                  */}
                {/**
                  * 🎛️ **모드는 지금 것 하나만 보인다 — 누르면 셋이 펼쳐진다**.
                  *
                  * 🔴 셋을 늘 늘어놓으면 **한 줄의 절반을 모드가 먹는다.** 달리면서 읽을 것은
            ...
            {/**
              ...
              *    줄에 늘어놓으면 «지금 뭘 하고 있나»가 묻힌다 (115칸 → 41칸).
              * 🔴 **«취소 N»은 테두리로 도드라지게, 버전은 흐리게 둔다** — 버전은 하루에 한 번 볼까
              *    말까고, 취소는 **한도가 있는 값**이라 남은 횟수를 알아야 한다 (기사님).
...
export default function DeviceControlPanel() {
    ...
    return (
        <div className="border-b border-border-card">
            {/* 📐 **왼쪽 세로선을 맞춘다** (기사님: *"타이틀 폰 필터 그리드를 맞춰줘"*).
                헤더·필터가 12px 에서 시작하므로 여기도 12px(px-3)에서 시작한다 —
                하나만 어긋나도 타이틀·폰·필터가 따로 노는 것처럼 보인다. */}
