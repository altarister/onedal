...
/**
 * 🧪 **«있을 수 없는 화면»을 만들지 않는다** (기사님)
 *
 * 🔴 높이를 바꾸는 길은 모두 `sheetTransition` 을 지난다 — 높이만 바꾸면
...
describe('다(full) — 하나가 열려 있어야 한다', () => {
    ...
    /**
     * 🪜 **S13 — 사건이 가리킨 콜은 열린 것을 이긴다.**
     *
     * 🔴 `preferIdx`(약한 추천)와 `focusIdx`(강한 지시)는 **답하는 질문이 다르다**
     *    (규칙 ⑤-4 ⑤). 하나로 겸하면 도착이 가리킨 콜이 «추천»으로 들어가
     *    **열려 있던 딴 콜에 밀리고**, 시트는 올라왔는데 그 단계가 안 열린다.
     *    여는 것은 «그 콜의 그 단계»다.
     */
    ...
    /**
     * 🔴 **콜이 없어도 올라간다** (기사님 확정) — 빈 상태를 보여 준다.
...
describe('✋ 끌 것이 없을 때 (2026-09-05)', () => {
    /**
     * 「① 콜 대기」처럼 콜이 하나도 없어도 시트는 세 단을 다 오간다 — 「다」는 열린 것 없이 빈 채로 선다.
     *
     * 🔴 막아 두면 끌었는데 아무 일이 없어 **고장처럼 보인다.**
     */
...
describe('🪧 심사가 들어오면 시트가 올라온다 (2026-09-05)', () => {
    /**
     * 기사님: *"심사가 들어오면 시트가 2단계로 올라와야 한다."*
     *
     * 🔴 「가」는 상태바만 보이는 높이라 **판정이 들어갈 자리가 없다.** 「가」에 둔 채면 주행 중에 합짐
    ...
    /**
     * 🔄 **「다」도 「나」로 내려온다** — 판정을 보는 높이는 「나」 하나이고, 방향을 따지지 않는다.
     *
     * KEEP 으로 「다」에 올라간 채 후보콜이 뜨면 **지도가 통째로 가려진 채** 결재하게 된다 —
     * 기사님: *"콜의 판정 화면은 **「나」** 여야만 해."*
     */
    it('🔴 다(전체)에 있었으면 나로 **내려온다** — 지도가 절반 남아야 결재한다', () => {
        expect(snapOnJudging('full')).toBe('list');
    });
 ...
 *
 * 🔴 자동 경로(`StageView` 의 `feed()`)도 `sheetTransition` 을 지난다. 아코디언을 여닫는 계산은
 *    `sheetTransition` 안에 있어서, `setSnap` 만 하면 KEEP·도착으로 자동으로 「다」에 올라갈 때
 *    **빈 시트가 지도를 덮는다.** 규칙(S3·S4)과 검사가 있어도 자동 경로가 그 규칙을 안 부르면
 *    화면은 어긋난다 — «규칙 파일을 목업·검사만 쓰고 실물이 안 부른다» 모양이다.
 *
