...
describe('🪧 심사가 들어오면 시트가 올라온다 (2026-09-05)', () => {
     ...
     *
     * 예전 이 검사는 *"이미 보이는 자리면 건드리지 않는다"* 로 **「다」를 예외**로 뒀다.
     * 그래서 KEEP 으로 「다」에 올라간 채 후보콜이 뜨면 **지도가 통째로 가려진 채** 결재하게
     * 됐다 — 기사님: *"콜의 판정 화면은 **「나」** 여야만 해. 지금은 **있을 수 없는 화면**이
     * 나온다."* 화면규칙 S9 가 이미 *"방향을 따지지 않는다"* 고 정해 둔 것이다.
     */
