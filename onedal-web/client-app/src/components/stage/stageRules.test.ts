...
/**
 * 🧠 **시트 높이 규칙(Ⅲ표)을 검사로** (기사님 지시 — *"구멍부터 처리하자"*).
 *
 * 이 규칙은 손으로 확인하기 어렵다. 그동안 난 사고들이 여기 문제로 들어 있다 — 규칙을 만지면 빨간불이 먼저 뜬다.
 */
const T0 = 1_000_000;
...
describe('Ⅲ표 — 신호가 정하는 높이', () => {
    ...
    /**
     * 🪧 **판정석은 «시트 맨 아래»에 있다** (기사님 확정 · 안 ⓑ).
     *
     * 🔴 판정 중이면 시트를 「나」로 올린다 — 결재 버튼이 시트 안에 있어서, 내리면 **누를 것이 안 보인다.**
     * 🟢 «지도가 판정의 근거다»도 함께 지킨다 — 「나」는 58% 상한이라 지도가 절반 남는다.
     *    후보 경로(노란 점선)를 보면서 아래에서 결재한다.
     * ⚠️ 어느 높이에서든 「나」로 간다 — 「다」에서도 내려온다 (`snapOnJudging`).
     */
    it('S4 판정 중이면 주행·정차보다 먼저다 — 「나」로 올려 결재를 보인다', () => {
        ...
        /**
         * 🔄 **「다」도 「나」로 내려온다** (기사님 실측).
         *    「다」에 그대로 두면 KEEP 으로 올라간 「다」에서 후보콜이 뜰 때
         *    **지도가 통째로 가려진 채** 결재하게 된다.
...
describe('마중(KEEP·도착) — 정차에는 이기고 주행에는 진다', () => {
    ...
    it('🔴 도착으로 올라간 시트를 «정차»가 끌어내리지 못한다', () => {
        ...
        // 10초 뒤 정차 감지가 발화해도 신고 시트는 그대로 — 도착 마중이 정차에 이긴다
    ...
    it('🔴 달리기 시작하면 즉시 내려간다 — 시간이 아니라 «무엇에 지는가»다 (0831 18:15 사고)', () => {
        /**
         * KEEP 으로 올린 시트는 시간 유예 없이, 달리기 시작하면 바로 내려간다.
         * 손과 같은 30초 유예를 쓰면 정거장이 가까워 구간이 30초보다 짧을 때
         * 유예가 다음 구간을 통째로 덮는다. 그래서 2초 뒤에도 내려가야 한다.
         */
...
describe('손이 이긴다 — 그리고 유예가 끝나면 잊지 않는다', () => {
    ...
    /**
     * 🔴 **유예 중에 온 «도착»은 유예가 끝나면 다시 묻는다 — 아직 그 정거장 곁이면**.
     *    유예가 끝난 뒤 신호만 다시 물으면 «정차»가 되어, 유예 중에 미룬 도착 마중이 사라진다.
    ...
    /** 🎭 모의 도착은 정차 첫 틱에 찍힌다 — 주행 신호는 굳는 10초 동안 아직 «주행»이다. 신호가 다시 흐르지 않으면 시트는 그대로다 */
...
describe('한 판을 통째로 걸어 본다 — 기사님이 정한 수순', () => {
    it('첫짐 KEEP↑ → 후보 판정↓ → KEEP↑ → 출발↓ → 도착↑ → 재출발↓', () => {
        ...
        now += 20_000;
        /* 🪧 판정석이 시트 맨 아래라 «나»로 올려야 결재가 보인다 (지도는 절반 남는다) */
...
describe('🗺️ 「나」도 지도가 볼 자리를 남긴다 (기사님 확정 2026-09-05)', () => {
     ...
     *    **판정의 재료**다. 그래서 「나」에 상한을 건다 — 상한이 없으면
     *    **콜이 많을 때 시트가 화면을 다 덮는다.**
...
/**
 * 🪧 **판정이 손보다 먼저다 · 판정 중에는 손이 높이를 못 바꾼다 · 늦게 열기도 규칙을 지난다** (기사님).
 *
...
/**
 * 🪜 **비어 있는 「나」는 손으로 끌 때 건너뛴다** (기사님).
 *
 * 콜이 없으면 「나」(내용만큼)가 「가」(72px)와 거의 같은 높이라, 규칙이 「나」로 올려도 첫 끌기가 «딸깍»으로만 보인다.
 * 보일 콜 줄이 없으면(콜 없음 · 지난 콜 숨김으로 전부 가림) 「나」에 설 까닭이 없다 — 손으로 끌면 「나」를 건너뛴다 (「가」↔「다」).
 */
