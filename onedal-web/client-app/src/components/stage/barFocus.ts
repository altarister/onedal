/**
 * 🎬 **시트 상태바가 가리키는 «그 콜 · 그 단계»** (기사님 2026-09-15 · 버그 대장 #143).
 *
 * 기사님: *"시트가 맨위로 올라가면 무조건 현황판(시트 상태바)에 표기된 스텝이 표기 되어야 하는데."*
 *
 * 🔴 **단계를 정하는 곳은 여기 하나다.** 09-12 에는 카드가 도착 사건을 따로 듣고 «도착 단계»를 열어,
 *    상태바·장부와 다른 단계가 떴다. 상태바(`sheetStatus`)가 읽는 재료(`arrivedHere` · `next`)를 그대로 받는다.
