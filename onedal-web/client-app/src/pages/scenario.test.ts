...
/**
 * 🧪 **기사님이 적어 주신 한 사이클이 스스로 어긋나지 않는가**
 *
 * 🔴 이 검사가 잡는 것은 «시나리오 글»과 «목업이 그리는 화면»이 갈라지는 것이다.
 *    단계가 «3콜 잡았다»는데 목업에 콜이 둘이면, 화면은 멀쩡해 보이고 **글만 거짓말을 한다.**
 */
...
describe('시나리오 — 아직 없는 것을 있다고 말하지 않는다', () => {
    /**
     * 🔴 시나리오는 «되어야 할 모습»을 그리므로, 코드에 없는 자리는 `gap` 으로 밝힌다 —
     *    밝히지 않으면 계획이 완료로 읽힌다.
     */
...
describe('🔴 다시 물은 순서가 없는 판 — 터지지 않는다 (2026-09-05 버그)', () => {
    /**
     * 🔴 **무엇을 막나** — 콜이 0·1·2 개인 장면(「① 콜 대기」 등)을 누르면 흰 화면이 되는 것:
     *    `Cannot read properties of undefined (reading 'totalKm')`.
     *
     * 🔴 **왜** — `reaskCost` 가 «목업의 콜 수»를 열쇠로 «다시 물은 순서»를 찾는데,
     *    그 표는 **3·4·5콜만** 갖고 있다. 시나리오 장면은 콜이 0·1·2 개일 수 있다.
     *    `plan.calls` 하나가 «몇 콜인가»와 «어느 재요청 결과를 쓰나» **두 질문을 답한다**
     *    — README ⑤-4 ⑤ 가 잡으라는 바로 그 모양이다.
     *
...
describe('🪧 심사석 — 눌러야 넘어가는 자리', () => {
    /**
     * 기사님: *"첫심사에 심사 목업 ui가 있으면 좋겠고, 첫짐킵을 추가해주면
     * 좋겠어, 합짐킵도 있고 주행중 합짐킵도 만들어줘."*
     */
    /**
     * 🔴 **심사석은 «아직 안 잡은 콜»의 자리다** (기사님 정정:
    ...
    /**
     * 🔴 **«심사»는 국면이 아니다**. 국면에 섞으면
     *    「⑪ 주행 중 합짐2 심사」가 **정차로 읽혀** 달리는 중인데 상태바가 ⏸ 를 단다.
    ...
    /**
     * 🔴 **④ 는 시나리오와 실측이 갈리는 자리다**.
     *    기사님 시나리오는 «고속도로 우선을 누르면 멀어지고 녹색이 된다»인데,
     *    실측에서 **추천과 TIME 이 여덟 구간 중 일곱에서 같았다**.
...
describe('🧭 「출발하기」는 콜을 잡은 뒤라면 늘 있다 (2026-09-05)', () => {
    /**
     * 🔴 **QR 목적지는 «덮개가 열린 장면»에만 잡히지 않는다** — 그렇게 하면
     *    「⑥ 출발 전 — 합짐 대기」에서 **버튼이 통째로 사라진다.**
     *
    ...
    it('사이클이 끝나기 전까지는 갈 곳이 있다 — 덮개가 안 떠도 버튼이 가리킬 데가 있다', () => {
        /* 🔴 마지막 장면(⑰ 사이클 끝)은 다 돌았으니 남은 곳이 0 이 **맞다** —
           그래서 이 검사에서 뺀다. 갈 곳이 없으면 버튼도 없어야 한다. */
