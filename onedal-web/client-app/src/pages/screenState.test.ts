 ...
 * 🖥️ **«없다»의 뜻이 갈라지지 않는가**
 *
 * 🔴 **화면 조각들이 «심사 중인가»를 각자 짐작하면 «없다»의 뜻이 갈라진다** —
 *    상태바는 «정거장이 없으니 사이클 끝», 빈 상태는 «콜이 0이니 콜 없음»,
 *    QR 경고는 «목적지가 없으니 키가 없나 보다»로 읽어 **셋 다 틀린다.**
 *    하나를 고치면 다른 하나가 틀린다.
 *
 * 그래서 사실을 한 곳(`screen`)에서 정한다. 이 검사는 그 사실들이 **서로 어긋나지
...
describe('심사 중과 대기 중은 다른 상태다', () => {
     ...
     *    그 전에 뜬다면 «갈 곳이 없다»를 «끝났다»로 읽은 것이다.
     */
