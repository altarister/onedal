 ...
 *
 * 🔴 **잡은 자리는 기록에 찍힌 값 그대로다** — 카카오 호출의 «⑮ 합짐 고유» 줄 머리
 *    `내 위치 x,y` 가 **그 콜을 잡던 순간 서 있던 자리**다. 동 중심점으로 바꾸지 않는다.
 ...
 * 화면은 이 순서 그대로 **왼쪽에 이벤트 · 오른쪽에 콜**로 늘어놓는다 (기사님).
 */
export type LabStep =
 ...
 *    (규칙 ⑤-4 ⑤) — 「볼트 저녁」은 집이 아니라 **김포 두원타워**(본업 자리)에서 시작한다.
 * 이 값은 카카오 기록 첫 줄의 `내 위치 127.2940,37.3772` 그대로다.
 */
export const LAB_START: LabPt = { lng: 127.29400, lat: 37.37720 };

/**
 * 🌆 **볼트 저녁** — 8/10(월) 저녁, 김포 두원타워에서 시작해 용인 원삼까지 한 줄로 흘린 하루.
 *    (출처: 영상 자막 `KpS4RzgFIbA` + 기사님 콜창·앱 캡처)
 *
 * 🔴 **상·하차지는 «동사무소·읍사무소»다** (기사님 확정:
...
export const LAB_MORNING: LabStep[] = [
    ...
    /**
     * 실측 자리 그대로 — 「대전 유성구 문지로 188」(카이스트 앞 LG화학).
     * ⚠️ 오송 콜들이 라인 밖으로 밀려도 이 자리를 옮기지 않는다 (기사님) —
     *    밀리면 그건 자리 탓이 아니라 필터/경로의 문제다.
     */
    ...
    /**
     * 🔴 **분당에 내리고 그 자리에서 실었다** (13:05 잡고 13:30 상차).
     *    자막은 «여섯 개»라 했는데 **일곱째**다 — 하차 중에 추가로 잡았다.
     *    상차지는 판교소프트웨어드림센터(투루파킹) · 하차는 인천 연수구 갯벌로 120 (기사님).
     */
...
/* ═════════════════════════════════════════════════════════════════════════
   🛣️ 경충대로·서이천로 한 줄 길 — 집(초월) ↔ 롯데아울렛 이천
   ═════════════════════════════════════════════════════════════════════════ */

/**
 * 🔽🔼 **가는 방향과 오는 방향은 다른 지점이다** (기사님 지시:
 ...
 *    필터가 방향을 안 보면 «길 옆 600m»라며 반대편 콜을 꿀콜로 올린다 — 잡고 나면 U턴이다.
 */
...
/**
 * 🔽✅ **이천행 · 순행** — 남행 지점만. 우회가 거의 없는 «잘 되는 문제».
 ...
 * 🔴 **③ 에 바디프랜드를 쓰지 않는다** —
 *    상차가 집에서 18km 라 «상차 반경 10km 밖»으로 **거리 때문에** 떨어진다.
 *    방향을 보려는 문제인데 거리가 먼저 떨어뜨리면 **아무것도 시험하지 못한다.**
 * ⚠️ 그래서 **넷 다 통과하는 것이 지금의 정상**이다 — 그 사실이 곧 «필터가 방향을 안 본다».
 */
 ...
 *    「금강→서민행복」은 **❌ 하차 역주행**으로 떨어진다. 이미 되는 것이라 이 문제에 넣지 않는다.
 *    이 문제가 시험할 것은 **아직 못 잡는 것** 하나다.
 */
...
export const LAB_PROBLEMS: LabProblem[] = [
    ...
    /**
     * 🛣️ **경충대로 한 줄 길 넷** (기사님: *"아울렛갈때 가장 좋은거 2세트하고,
     * 복귀때 2셋트 이렇게 4문제"* · *"**잘되는거 하나 이슈있는거 하나** 이렇게 하는 것이 좋을 것 같다"*).
     *
     * 🔴 **방향마다 짝이다** — 잘 되는 문제 하나로 «좋은 콜을 안 떨어뜨리나»를 보고,
     *    이슈 있는 문제 하나로 «반대편 콜을 잡아내나»를 본다. 한 문제만으로는 둘 중 하나를 못 본다.
     * 🔴 **한자리에서 다 잡히지 않는다** — 상차 반경 10km 인데 길이 23km 다.
     *    콜 사이사이에 「📍 내 위치 찍기」로 달려야 뒤 콜이 반경에 들어온다 (문제는 콜만 세운다).
     */
    {
        name: '🔽 이천행 ✅',
