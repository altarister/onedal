...
export const LAB_MORNING: LabStep[] = [
    ...
    /**
     * 🔴 **상차를 «세종 나성동»으로 옮겼다** (기사님 2026-09-11).
     *
     *    실측 자리는 「대전 유성구 문지로 188」(카이스트 앞 LG화학)이다. 그건 대전 **동남쪽**이라
     *    거기서 오산으로 가는 라인이 **오송을 등지고** 서서, 뒤이어 잡는 오송 콜 셋이 라인 밖으로
     *    밀려난다. 대덕구 문평동으로 한 번 옮겨 봤지만 *"그걸로도 안 된다"* —
     *    문평동도 대전 안이라 라인이 아직 오송을 충분히 향하지 않았다.
     *    **나성동(세종)은 대전과 오송의 사이**라, 여기서 오산으로 그으면 라인이 오송을 지난다.
     * ⚠️ **표(「볼트 이틀 실측」 4번)의 자리와 다르다** — 그쪽은 실측이라 안 고쳤다.
     *    여기만 «문제로 쓰려고» 옮긴 자리다.
     */
    { kind: 'call', where: '세종 나성동 → 오산 황새로 211 (38,500)', confirm: true,
        from: { lng: 127.28572, lat: 36.49806 }, to: { lng: 127.06071, lat: 37.14473 } },
