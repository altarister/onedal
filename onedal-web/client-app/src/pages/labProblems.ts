...
export const LAB_MORNING: LabStep[] = [
    ...
    /**
     * 🔴 **상차를 문지동(유성구)에서 문평동(대덕구)으로 옮겼다** (기사님 2026-09-11:
     *    *"2번 문지동 → 가수동 가는 거 **경로 오류를 막으려면 대전 대덕구 문평동**으로 바꿔야
     *    할 것 같아. 그래야 **오송읍 쪽을 바라볼 수 있거든**"*).
     *
     *    실측 자리는 「대전 유성구 문지로 188」(카이스트 앞 LG화학)인데, 그건 대전 **동남쪽**이라
     *    거기서 오산으로 가는 라인이 **오송을 등지고** 선다 — 뒤이어 잡는 오송 콜 셋이
     *    라인 밖으로 밀려난다. 문평동은 대전 **북쪽**이라 라인이 오송을 향한다.
     * ⚠️ **표(「볼트 이틀 실측」 4번)의 자리와 다르다** — 그쪽은 실측이라 안 고쳤다.
     *    여기만 «문제로 쓰려고» 옮긴 자리다.
     */
    { kind: 'call', where: '대전 문평동 → 오산 황새로 211 (38,500)', confirm: true,
        from: { lng: 127.40224, lat: 36.45239 }, to: { lng: 127.06071, lat: 37.14473 } },
