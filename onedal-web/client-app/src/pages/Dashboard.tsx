...
/**
 * ⏱️ **배너는 10초 뒤 스스로 사라진다** (기사님 지시: *"토스트 팝업이 10초후
 * 닫히도록 해야 할꺼 같아 화면을 다 가려"*).
 *
 * **서버 재시작 복구 알림도** 같다 — 손으로 닫게 두면 닫기 전까지 폰 화면을 덮는다.
 * 운전 중에는 그 닫기 버튼을 못 누른다
...
export default function Dashboard() {
    ...
    /**
     * 🛣️ **노선 ↔ 🔷 동선 — 그물을 어떤 모양으로 볼까** (기사님 지시:
     *    *"노선 동선 버튼도 지도에서 필터로 이사와야해"*).
     *
     * 🔴 **여기서 쥔다** — 고르는 버튼은 **필터**에, 그리는 것은 **지도**에 있다.
     *    한쪽이 제 상태를 들면 «필터는 동선인데 지도는 노선»이 된다 (규칙 ③).
     * 🔴 **브라우저에 기억하지 않는다** — 레이어(🧅)는 «보기»라 localStorage 에 남기지만 이것은
     *    **판정을 바꾸는 값**이라 서버 필터에 둔다 (아래).
     *    기본은 «노선» (기사님 확정).
     */
    /**
     * 🛣️🔷 **노선/동선은 필터 값이다**.
     *    화면 상태로만 두면 **서버가 몰라** «동선»을 골라도 판정·앱 목록은 노선이고,
     *    새로고침하면 노선으로 돌아간다. 그래서 `filter.routeMode` 를 읽고
     ...
     *    **왼쪽 여백**이 남는다. 그 여백이 한 칸(346px · `useSidePanelRoom`)을 담을 만큼일 때만 만든다.
     *
     * 🔴 **폰에서는 아예 안 만든다** (기사님 지시: *"모바일일때는 컨포넌트 호출을
    ...
    /**
     * 🎭 무대 모드 — **문서 스크롤을 잠근다** (기사님 확정 — 안 잠그면 헤더 빼고 다 같이 움직여 들썩인다).
    ...
    // ⏱️ 복구 알림도 스스로 닫힌다 — 손으로 닫게 두면 닫기 전까지 화면을 덮는다
    useEffect(() => {
    ...
    // 기억해야 하고, 잊으면 조용히 틀린다.
    //
    // ☰ 서랍·현황판이 끝난 콜도 보므로 둘 다 필요하고, 여기서만 합친다.
    ...
    /**
     * 📢 **서버가 «대신 한 일»만 잠깐 알린다.**
     * 🔴 도착 감지(`auto-arrived`)·근접 예고(`next-stop-approaching`)는 여기서 토스트를 안 띄운다 (기사님 —
    ...
    // 기사님이 **모르는 채로 콜을 잃으면** 안 된다 — 상한을 두면서 조용한 소실을 만들 수는 없다.
    useEffect(() => {
    ...
    /**
     * 🔬 **곁 패널은 원본 «바깥»에 선다** (기사님 지시:
     *    *"원본에는 어떤 영향도 없어야해.. div 로 완벽하게 분리해줘"*).
     *
     * 🔴 **원본 안쪽을 건드리지 않는다** — 무대만 `mr-auto` 로 당기면 전체 폭을 쓰는 헤더와
     *    어긋난다. 원본(`<main>`)을 통째로 왼쪽 칸에 담고 패널은 오른쪽 칸에 둔다 —
     *    원본은 제 폭 안에서 **패널이 없을 때와 한 픽셀도 다르지 않게** 동작한다.
     *
     * 🔴 **지울 때는 이 감싸개와 패널 한 줄만** 걷어내면 된다. `body` 는 안 건드린다.
     */
    const withPanel = sidePanelRoom;
    /**
     * 🎭 화면 = 상자, 스크롤은 시트 안에서만 일어난다 (기사님 확정).
        ...
        <main className="relative h-dvh overflow-hidden flex flex-col bg-bg-base font-sans"
            ...
            <div className="relative flex flex-col max-w-2xl mx-auto w-full flex-1 min-h-0">
                ...
                <div className={isFilterOpen ? "relative z-40" : "relative"}>
                    {/* ⚙️ 오더 필터 한 줄 현황판 — 심사석과 자리를 바꾸지 않는다 (아래) */}
                    {/**
                ...
                {/* 📢 배너 층 — 무대에서는 흐름 밖으로 띄운다. 🔴 바탕은 불투명(bg-surface) — 10% 바탕이면 뒤 지도가 비친다 (#146). 흐름 안에 두면 뜰 때마다
    ...
    /* 🔴 **패널이 없으면 원본 그대로 내보낸다** — 감싸개조차 만들지 않는다 */
    if (!withPanel) return body;

    return (
        <div className="flex h-dvh overflow-hidden">
            {/**
              * 🪪 **두 영역에 이름표를 붙인다** (기사님 지시:
