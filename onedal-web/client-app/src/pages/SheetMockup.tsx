                ...
                <section className="relative flex-1 min-h-0">
                    ...
                    <StageSheet snap={snap} onSnapChange={setSnap}
                        peekBar={
                             ...
                             */
                            /**
                             * 🖐️ **누르면 그 콜로 데려간다** — 지도 우하단 버튼이 하던 일이다
                             * (기사님 2026-09-04: *"그 버튼은 클릭하면 시트를 최대로 올리고
                             * 그 콜의 아코디언을 열고 해당 스텝을 보여주는 기능을 가지고 있었거든"*).
                             * 자막이 그 자리를 물려받았으니 **기능도 함께** 물려받는다 —
                             * 자리만 옮기고 일을 흘리면 기사님 손이 갈 데가 없어진다.
                             */
                            <button type="button"
                                onClick={() => {
                                    if (!nextStop) return;
                                    ...
                                    setLog(`자막을 눌렀습니다 → 시트를 전체로 올리고 ${nextStop.callNo}번 콜을 «${STEPS[CALLS[nextStop.callNo! - 1].now].k}» 단계로 엽니다.`);
                                }}
                ...
                <div className="flex gap-1.5 flex-wrap">
                    ...
                    {MAP_STOPS.slice(0, 5).map((p, i) => (
                        <button key={p.no} type="button"
                            onClick={() => { setVisitedCount(i + 1); setSnap('peek'); setLog(`「현구간」은 ${p.no}~${MAP_STOPS[i + 1]!.no} — ${p.name} → ${MAP_STOPS[i + 1]!.name} 입니다.`); }}
