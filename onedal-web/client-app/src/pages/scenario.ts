...
export type ScenarioStep = {
     ...
     *
     * 🔴 **«심사»는 여기 넣지 않는다**. 국면에 섞으면
     *    「⑪ 주행 중 합짐2 심사」가 **정차로 읽혀** 상태바가 ⏸ 를 단다 — 달리는 중인데.
    ...
    seat?: '첫콜' | '합짐1' | '합짐2';
    /**
     * ⏱️ **이 콜을 잡으면 뒤가 몇 분 밀리나** (기사님 확정 · «심사 중에 미리»).
     *
...
/* ═════════════════════════════════════════════
   ...
   🔴 **판정색·점수는 시늉이다** — 실제 판정은 서버가 낸다.
      주소·요금·거리는 실주행 실측 그대로다 (`local.db` 의 그 콜들).
   ═════════════════════════════════════════════ */
