...
/**
 * 🔴 **값은 한 벌이다 — 국면(`dispatchPhase`)으로 칸을 가르거나 빼지 않는다** (기사님 확정).
 *
 * 아웃풋은 국면을 파생(`resolvePhaseKey`)하지 않고, «그 국면에서 안 쓰는 칸»도 숨기지(hidden) 않는다.
 * 실물 필터도 한 벌이라, 국면으로 가르면 아웃풋과 실물이 갈라진다.
 * 국면에서 실리는 것은 «콜을 쥐었나»(`isSharedMode`) 상태 하나다.
 */
...
describe('제외지역 — 목록·묶음·좌표 셋에서 같이 빠진다', () => {
    ...
    /**
     * 🔴 **도 통째 제외**(기사님 *"원래 내가 원한 건 서울을 빼는 거였는데"*).
