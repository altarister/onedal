...
/**
 * 🧪 **목업 계획 셋(3·4·5콜)이 스스로 어긋나지 않는가**
 *
 * 왜 있나: 목업을 3콜·4콜·5콜 계획 셋으로 가른다 — *"3콜은 상한이 아니다"*. 갈라진 것끼리
 * **어긋날 자리**가 있다 — 정거장은 4콜인데 목록은 셋인 계획 같은 것.
 */
...
describe('판 셋 — 정거장과 콜 목록이 갈라지지 않는다', () => {
    ...
    /**
     * 🔴 **콜은 번호로 찾는다 — 뺄셈으로 찾지 않는다**.
     *    목록은 **잡은 순서**라서(기사님) `callList[callNo - 1]` 처럼 자리로 찾으면 다른 콜이 나온다.
     */
...
describe('QR 을 몇 번 찍나 — 판에서 파생된다', () => {
    /**
     * 🔴 「6정거장이면 2번」을 **글로 박으면** 3콜 계획에서만 참이다.
     *    이 검사는 «계획이 바뀌면 숫자도 바뀐다»를 잠근다.
     */
...
describe('⟳ 다시 물은 순서 — 무엇이 갈리고 무엇이 안 갈리나 (Q8)', () => {
    ...
    /** 🔴 순서가 흔들려도 **상차 없이 하차하는 순서**가 나오면 안 된다 */
    ...
    /**
     * 🔴 **비교는 같은 시각에 물은 두 순서끼리 한다** — 다른 날 물은 값끼리
     *    견주면 «순서 때문»인지 «시각 때문»인지가 섞인다.
     */
    ...
    /**
     * 🔴 **순서가 흔들린다고 늘 나빠지지 않는다** —
     *    4콜 계획에서는 2분이 준다. 이 검사는 그 사실을 **화면이 계속 말하도록** 잠근다:
...
describe('🎨 지도의 색과 목록의 색은 같은 곳에서 온다 (2026-09-05)', () => {
    /**
     * 🔴 **지도와 목록은 같은 `callNo` 로 칠한다** (기사님: *"지금 색이 지도랑 리스트가 같지 않아"*)
     *
     * 목록이 `callNodeFill(i + 1, …)` 처럼 **목록에서 몇 번째인가**로 칠하면, 3콜 계획에서는
     * 목록이 [1,2,3] 순이라 **우연히 맞지만** 시나리오에서는 콜이 [2,3,1] 차례로
    ...
    /** 🔴 시나리오에서 **실제로 갈라진다** — 이 검사가 그 사실을 붙들어 둔다 */
...
describe('🔴 콜이 없으면 지도에 그릴 것도 없다 (2026-09-05)', () => {
    /**
     * 경로선·궤적을 그대로 넘기면 「① 콜 대기」인데 **정거장 하나 없는 지도에
     * 선만 그려진다.** 없는 것을 그리지 않는다 (규칙 ④).
     */
...
describe('🔴 콜 목록의 순서는 «잡은 순서»다 — 절대 안 바뀐다 (2026-09-05)', () => {
     ...
     *
     * ⚠️ `callList[callNo - 1]` 로 찾기 편하자고 **번호순으로 정렬**하면 «잡은 순서»가 깨진다 —
     *    콜은 번호로 찾는다(`find(c => c.callNo === …)`).
     */
...
describe('📋 콜 내용이 실측과 맞는가 (2026-09-05)', () => {
    /**
     * 목업 콜 내용은 실측 3콜의 `local.db` 값과 맞춘다 (기사님: «아코디언 컨텐츠를 맞춰 보자»).
     * 🔴 실측 품목은 「서류봉투」다 — 「라면박스 20개」 같은 **지어낸 값을 적지 않는다** (규칙 ④).
     */
    ...
    /**
     * 🔴 **수수료는 콜마다 다른 값이다** — 「23%」를 화면에 박으면 어느 콜을
     *    펼쳐도 같게 나온다. 실측 3콜이 셋 다 23% 인 것은 **실측이라서** 같은 것이다.
     */
