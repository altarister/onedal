 ...
 * ── 왜 필요한가 ──
 *
 * 새 화면(지도 배경 + 3단 시트)으로 굳히면 옛 화면의 **탭 줄**이 사라진다.
 * 그 탭이 «완료됨 · 취소 · 방출»을 보는 **유일한 자리**였다 (`PinnedRoute` 의
 * `view !== 'ACTIVE'` 블록). 현황판에도 같은 목록이 있지만 곁 패널은 폭 1018px
 ...
 *    실물로 옮길 때는 `activeRoute` 에서 오는 진짜 콜을 쓰고, 탭 거르는 규칙은
 *    **순수 함수 한 벌**로 꺼내 옛 화면과 함께 쓴다 (규칙 ③ — 두 벌 금지).
 * 🔴 **색을 박지 않는다** — 테마 토큰만 쓴다. 원본 와이어프레임을 그대로 옮겨
 *    밝은 테마에서 글자가 안 보이던 사고를 되풀이하지 않는다.
 */
