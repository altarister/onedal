...
/**
 * 🛣️ **카카오를 부를 때 쓰는 경로 축 — «추천» 하나다** (기사님 *"길찾기는 지워"*).
 *
 * **잡은 콜들이 라인을 만들므로** 길(추천·최단시간·최단거리·고속도로 피하기·톨게이트 피하기)을
 * 미리 고를 이유가 없다.
...
/**
 * 🧳 **정거장에 머무는 분 — 짐을 모르므로 «일반값»이다** (기사님 *"넣어줘"* · 규칙 ⑤-2).
 *
 ...
 */
/**
 * 🧳 **정거장에 머무는 분 — 짐을 넘긴다** (기사님 확정:
 * *"15분(양촌읍 상차)이 잘못된 부분이구나. **볼트는 승용차로 하니까 박스 하나씩** 했을 거야"*).
 *
 * 🔴 **박스 수를 넘긴다** — `dwellMinutes(null, 0, …)` 로 부르면 **늘 «모를 때 값»(상차 15 · 하차 10)** 이 나온다.
 *    그건 **1톤에 짐을 모를 때**의 값이다(*"상차에는 결박이 붙는다"*) — 승용차 박스 하나엔 과하다.
 *    실험실은 콜마다 **박스 수를 알고 있다**(기사님이 넣는다).
 ...
 *    3분은 **실측이 아니라 이 목업의 하한**이고, 실물 상수를 고치는 것은 별개 일이다.
 */
...
/**
 * 🎨 **그물 색 — 한 벌** (기사님 지시: *"선택 영역이 일관성이 없어 보여. 색도 라인도
 * 투명도도"* · *"**사용자가 볼 때 «아 이 영역에서 콜을 부를 거구나»** 하고 생각할 수 있게"*).
 *
 * 🔴 조각마다 색을 달리하면(마름모 하늘색 실선 · 라인 띠 진파랑 반투명 · 원 주황 점선)
 *    **한 영역이 셋으로 보인다.**
 *
 ...
 *
 * 🔴 **면은 한 번에 칠한다** (기사님 *"좀 더 진해도 될 것 같고 마름모 라인을 지울 수 있을까?"*).
...
const LAB_DEFAULTS = {
    ...
    /** 📍 상차 반경 10km · 하차지 주변 15km · 라인 반경 6km (기사님) */
...
const LAB_DEFAULT_EXCLUDED = [
     ...
     * 코드에 박지 않고 여기 둔다 — 필터에 **보이고, 되살릴 수 있어야** 한다
     * (기사님: *"필터에 서울이 없는데 서울 값들이 들어와야 할 것 같아."*).
     */
    'S|서울',
    'R|인천 강화군',            // 빠져나오기 어렵다
    'R|연천군',                 // «거의 연천이야» — 꼭대기 밖
...
/**
 * 💰 **콜이 잘 나오는 곳** (기사님 *"잘 나오는 곳도 표시되면 좋을 것 같은데"*).
 *
...
/**
 * 🗺️ **지도 실험실** — `/mockup/map` (기사님 요청: *"지도만 테스트, 다른 것에 영향 없이"*
 * → *"배경 지도, 오른쪽에 버튼 영역"*)
 *
 * 배경은 **OSM 타일** — 시트 목업의 지도(PinnedRouteCanvas)와 같은 원천이다.
 * 🔴 카카오 지도(SDK)는 **안 쓴다** (기사님 결정: *"미리 지도를 가져오는 것이
 ...
 * 🔴 사각형의 기점은 **내 위치**다 (기사님 재확정 — 하차지 기점은 출발 전
...
/**
 * 🎯 **목적지는 시군구에서 고른다 — 도를 먼저, 시를 다음** (기사님 확정:
 * *"시군구로 표기가 되어야 할 것 같고. 선택이 어려우니 **도를 선택하고 시를 선택**하게 할까?"*).
 *
 * 🔴 전국 시군구를 다 고를 수 있어야 한다 — 몇 곳만 코드에 박으면 나머지는 **아예 못 고르고**,
 *    그것이 «선택이 어렵다»가 된다. 시도가 동 표에 붙어 있어 도 → 시 두 걸음으로 고른다.
...
/**
 * 🚚 **받을 짐 차종을 한 자로** (기사님: *"한 자씩 표현하면 될 듯. 다/라/승/오/1t"*).
 * 칸이 화면의 1/3 이라 «1t · 다마스»가 벌써 잘린다 — **닫힌 줄에서만** 줄이고,
 * 레이어를 열면 온전한 이름이 보인다.
 */
/* 🚚 차종 짧은 이름은 **`shared` 한 벌**이다 — 실물 필터가 같은 글자를 쓴다 (규칙 ③) */

/** 콜 번호별 경로 색 — ①은 프리셋 경로의 기본색과 같은 장미로 잇는다 */
/**
 * ☎️ **통화로 정한 약속의 색** — 보라 (기사님 지정).
...
/**
 * ▒ **정거장 상자의 바탕 — 콜의 색** (기사님 확정:
 * *"상차와 하차 모두 박스를 만들어 넣어 줘. 색을 콜의 색으로 배경을 넣어 주고"*).
 *
 * 🔴 **둘 다 상자다.** 상차에만 옅은 바탕을 깔면 «한쪽만 상자»라 짝이 안 맞는다.
 ...
 */
// 🔴 `stopBoxBg` 도 `styles/callPalette` 에 있다 — 같은 이유

/* 🎨 **콜 선 색은 `callPalette.callLineColor` 한 벌이다** (기사님 확정).
   실물 지도·목록·마커가 `callPalette` 를 쓰므로 실험실 선도 그것을 쓴다 — 여기 따로 색을 박으면
   「색 = 콜 번호」 가 목업에서만 깨진다. */
...
/** 주행 속도 — 한 틱(120ms)에 가는 km. 버튼이 순환시킨다 (기사님 «너무 빨라») */
const SPEEDS = [
...
/**
 * 🧩 **실물로 들고 갈 공용 부품** (기사님: *"왼쪽 필터와 오른쪽 필터옵션은
 * 컴포넌트 단위 구조·레이아웃을 같이 — 그래야 가져가기 편하다"*).
 * 두 사이드바가 똑같이 `FilterPanel > NumRow/TextRow` 로 조립된다. 디자인 최소 — 로직이 주인공.
 * `mode` — 국면×칸 표시 규칙(`PHASE_FIELDS`)을 가리키는 이름이다.
 * ⚠️ 실물은 **값이 한 벌이라 그런 표가 없고** «지금 이 칸이 쓰이나»를 상태에서 파생한다.
 *    목업 아웃풋 표기는 그대로 둔다.
 */
/**
...
type LabCall = {
        ...
        /**
         * 📦 **이 콜의 짐(박스)** — 적재는 **콜이 들고 있고 합은 파생된다** (기사님:
         ...
         * 🔴 **못 쟀으면 값이 없다** (규칙 ④). 직선 km 를 지어내면
         *    그 숫자가 시급(요금 ÷ 분)에 들어가 **색이 틀린다.**
        ...
        /** 잡을 당시의 목적지 — 목적지별 경로 묶음의 열쇠 (기사님: 다음 목적지의 콜을 미리 잡아 공백을 줄인다) */
        destName: string;
};
...
/**
 * ⚖️ **심사 영역 — 시트 맨 아래 붙박이** (v17 B안 · 기사님 확정).
 *
    ...
    return (
        <div className={`relative overflow-hidden rounded-[10px] border ${tone.bd} bg-background`}>
            ...
            <div className="relative px-2 text-[19px] font-black tabular-nums leading-tight">
                ...
                {/**
                  * 🔴 **km 와 분은 같은 것을 재야 한다** — km 는 «이 콜만»(`distanceKm`)이고
                  *    분도 «이 콜만»(`soloMin`)이다. «전체 경로 증가분»(`driveMin`)은 다른 값이다.
...
/**
 * 🎚️ **손잡이 묶음(`KnobGrid`)은 `components/ui/KnobGrid.tsx` 로 옮겼다**.
...
/**
 * 📦 하단 footer — **memo 부품** (실측: 틱마다 화면 전체가 재생성되며 주행이 무거웠다).
...
const FilterFooter = memo(function FilterFooter({ out, callTarget, lineOn, routeMode }: {
    ...
    return (
        <>
        {/* 📦 하단 — 필터로 나갈 값. 축별 6열 분류 (기사님: «키우고 잘 분류해서 — 스크롤 없이») */}
...
export default function MapMockup() {
    const stageIdx = 0;   // 국면은 «대기»로 고정 — 프리셋 버튼이 없다(기사님). STAGES 데이터는 남긴다
    const [pickup, setPickup] = useState<Pt | null>(null);
    const [drop, setDrop] = useState<Pt | null>(null);
    /**
     * 💰📦 **후보콜의 요금과 짐** (기사님 «필터를 만들어 보자» 판).
     *
    ...
    /**
     * 🎚️ 그물 셋업 — 각도 둘·반경 둘 (기사님 요청 «아까처럼 만들어 넣어줘»).
     ...
     *
     * 🔴 **국면마다 값을 따로 두지 않는다.** 국면 다섯 벌(`user_filter_phases` 흉내)이 하는 일은
     *    «값을 여러 벌 두는 것»이 아니라 **«지금 안 쓰는 칸을 감추는 것»**(`PHASE_FIELDS` 의 `hidden`)이 된다.
    ...
    /**
     * ✅ 확정한 콜들 (기사님 요청 «확정하면 콜 리스트로, 지도에 색·경유 번호로»).
     * 확정해도 그물은 안 움직인다 — 그물의 기점은 내 위치이고, 확정은 경로와
     * «첫 콜을 잡았다»(상차 영역이 ∩ 로 조여짐)만 바꾼다. 국면을 바꾸면 처음부터 다시 시작한다.
     */
    ...
    /**
     * ⛔ 제외지역 (기사님) — 그물에 들어도 필터에 안 싣는 곳.
     * 키 두 모양: `R|시군구`(통째) · `D|시군구|읍면동`(하나). 이름만 쓰면 동명이인(창전동)이 섞인다.
     */
    /**
     * 🎛️ 필터 옵션 (기사님: 오른쪽 사이드바 — **실물 필터에 있는 요소만**, 값은 목업).
    ...
    /**
     * 🚫 **제외 단어 — 입력이 필요하다** (기사님 확정: *"제외 단어는 입력이 필요하다.
    ...
    /**
     * ⏸ 주행 중 새 콜이 생기면 멈추고, 처리(확정/지우기)되면 다시 달린다 (기사님).
    ...
    /** 📋 저장된 콜 전부를 날것 그대로 — 제목을 누르면 열린다 (기사님) */
    ...
    const callApi = async (who: string, path: string, body: Record<string, unknown>, reqSummary: string, tag?: string) => {
        ...
        try {
            ...
            const res = Array.isArray(d.roads)
                ...
                //    legInfo 만 읽으면 전체 경로 응답이 «구간 4: » 로 텅 비어 보인다
    ...
    /**
     * 🔴 **합짐의 «지금 경로» 미리보기** (기사님: *"값은 받아 왔는데 경로를 그리지 않는다"*).
     * ⑮ 고유 호출은 «확정 뒤»라 `uploadedLeg`·`approachLeg` 가 **첫짐일 때만** 채워진다.
     * 합짐은 ⑬⑭ 전체 경로를 받아 두므로 **그것을 그린다** — 안 그리면 직선 점선만 남는다.
     * 전체 경로는 재배치한 순서 그대로이므로 이걸 그리면 «이 콜을 끼면 이렇게 간다»가 그대로 보인다.
     *
     * 🔴 **⑭ 에서 전부 실선으로 그린다** (기사님: *"14에서 다 그려야 하는데…
     * 점선이면 사용자가 확정을 해야 할지 취소를 해야 할지 몰라"*). ⑭-1 심사와 ⑭-2 확정은
     * **이 그림을 보고** 하는 것이므로, 전체 경로가 오면 직선 점선은 안 그린다.
    ...
    /**
     * 🐾 지나온 길 (기사님 밤 «내가 지나온 길을 표시해 줘») — 주행이 실제로 밟은 점들.
     ...
     * 🔴 값이 한 벌이라 «관내 인지 → 국면 → 벌 → 관내 인지» 순환이 없다 — 운행 중 벌로 고정하지 않고
     *    지금 손잡이 값을 그대로 읽는다. 바닥값(`Math.max(6, …)`)도 두지 않는다 — 근거 없는 상수다.
     */
     ...
     *
     * 🔴 노선을 «길 찾기로 고른 직행 길»로 두면 그 길은 **잡아 둔 콜을
     *    안 거치므로**, 콜을 쥐면 틀린 답을 낸다. 그래서 **콜이 라인을 만든다.**
     */
    const [routeMode, setRouteMode] = useState(LAB_DEFAULTS.routeMode);
    /**
     * ⛔ **고치는 중(초안) ↔ 실제로 적용된 것** (기사님:
     * *"이걸 누르면 **너무 쉽게 삭제**되는데.. 바로바로 저장되면 안 될 것 같아.
     * 접기 옆에 **저장 버튼**이 있어야 할 것 같아."*).
     *
     * 🔴 칩 하나를 잘못 누르면 그 지역이 **그 자리에서 그물에 들어온다.** 운전 중이면 되돌릴
     *    새도 없다. 그래서 고치는 것과 적용하는 것을 가른다 — 「💾 저장」을 눌러야 그물이 바뀐다.
    ...
    /** ⚖️ 심사창에서 접어 둔 것들 — 캘 때만 편다 (기사님 «심사 부분이 너무 높이가 높아») */
    ...
    /** ⛔ 제외 목록을 펼쳤나 — 닫히면 **한 줄**, 누르면 전부 (기사님) */
    const [exListOpen, setExListOpen] = useState(false);
    /**
     * ⛔ 제외지역은 **노선·동선 공통**이다 (기사님 «공통으로 빼»).
     * 노선에도 마름모가 함께 살아서 넓은 구간이 들어오므로 노선에서도 뺀다.
     */
    ...
    /**
     * 🎨 **그물 전용 캔버스** (기사님: *"지금은 하나씩 그려서 겹치는 부분만 진한 색인데..
     * 한 번에 그리면 어떨까? 많이 연산해야 해?"*).
     *
     * 🔴 마름모·원은 한 path 로 묶어 겹침이 없지만 **라인 띠는 `stroke` 라 그 path 에 못 들어간다** —
     *    그대로 두면 띠와 마름모가 겹치는 자리만 진하게 남는다.
    ...
    /**
     * 🧅 레이어 (기사님 «각각 레이어 처리 — 켜고 끄고») — 그리기 순서의 켜기/끄기.
     * 모든 레이어가 한 투영(줌·원점)을 쓰므로 켜고 꺼도 드래그·줌은 그대로다.
     */
    /**
     * 🔴 **「길」 레이어는 따로 없다** (기사님: *"길 레이어에 라인 반경을 그리고 있는데
     * 이유가 있어? 불필요한 길 레이어 지우고 싶어서"*).
     * 라인 반경 띠는 **그물 그 자체**(노선일 때 콜을 거르는 영역)라
     ...
     *  viewRef 를 한 발 전으로 되감아 이동분을 잃었다 (기사님 «자꾸 당기고 튄다» — 실측 24걸음 중 3걸음 유실) */
     ...
     *  다시 계산하면 앞선 이동분이 사라지며 지도가 뒤로 튕긴다 (기사님 «자꾸 돌아가려 한다») */
    ...
    /**
     * 📌 지금 화면을 그대로 얼린다 (기사님 «탭을 바꾸면 원점이 바뀐다») —
     ...
     *   (기점을 «마지막 하차지»로 옮기면 그물이 목적지를 안 보게 된다.
     *    그물은 목적지를 보는 눈이다 — 기사님: *"목적지를 보고 있어야 해"*).
     */
    const [departed, setDeparted] = useState(false);
    /**
     * 🕒 **모의 시계 — 약속·예정·통과를 한 시계 위에 올린다** (기사님:
     * *"시간이 빨라진 건 지금 모의 주행이라 차 속도가 엄청 빨라서 … 아닐까?"*).
     *
     * «보통»이 0.2km/틱 · 120ms → **약 6000km/h**. 약속·예정은 카카오가 준 **실제 분**인데
     * 통과만 벽시계로 찍으면 늘 «40분 빨라짐» 같은 값이 나온다 — 사실이 아니라 **시계가 둘이라서** 생긴 값이다.
    ...
    /**
     * ⏱️ **실제 통과 시각** (기사님: *"지금 내가 하고 있는 것은 예상 시간인 거고
     ...
     *
     * `confirmed.slice(0, -1)` 로 **통째로 없애면** «언제 무엇이 빠져서
     * 순서가 바뀌었나»가 사이클에서 사라진다 (기사님: *"2번째 콜이 취소될 때"*).
    ...
    /**
     * 🔢 **확정 콜 묶음이 바뀐 횟수** — 확정·취소마다 오른다. 취소는 카카오 응답을 기다렸다가 적립하는데,
     * 그 사이에 또 바뀌면 **인덱스로 만든 라벨이 어긋난다.** 늦게 온 응답을 버리는 열쇠다
     * (`uploadSeqRef` 와 같은 규약).
     */
    const planSeqRef = useRef(0);
    /** 🔎 지나는 순간 얼릴 «마지막 예상» — 키는 `콜id-상차/하차` (실물의 `predicted_at` 자리) */
    const etaRef = useRef<Record<string, number | null>>({});
    /**
     * 🛣️ **확정된 콜들의 실도로 경로 — 한 벌** (기사님 승인).
     *
     * 기사님 설계 그대로다: *"새 경로를 요청할 때는 **새로운 콜이 필터를 타고 왔을 때**야.
     * 그때 지나온 건 빼고 최적화로 섞어 그 값을 카카오에 요청해, 가지고 온 값을 지나온 길과 연결해."*
     *
     * 🔴 선은 **`/sim/chain` 응답에 이미 와 있다** — 콜을 올릴 때 받는 응답에 구간별
     *    **폴리라인(`line`)**이 들어 있다. 그리기·주행용 계보(`effPath`→`legCache`→`drawLegs`)를
     *    따로 돌리면 「내 위치」가 둘로 뜨고 직선이 날아다닌다 — 계보가 둘이 되기 때문이다.
    ...
    useEffect(() => { setTerminated([]); }, [stageIdx]);   // 국면을 새로 열면 취소 기록도 함께 비운다
    /**
    ...
    const effPath = useMemo(() => {
        ...
        /**
         * 🧷 **순번도 «확정 순간의 chain»을 따른다** (기사님 실측).
         * 재배치를 다시 하면 지나갈 때마다 순서가 흔들려 선(동결 chain)과 바퀴가 갈린다.
         * chain 이 성하면 그 순서 그대로, 못 접으면(취소 재측정 전 창 등) 다시 재배치로 물러난다.
         */
    ...
    /**
     * 🐢 **그물용 앵커 — ~300m 격자로 스냅** — anchor 가 매 틱 새 객체면
     * 그물 만들기(buildLineNet, 라인이 길면 수천만 연산)가 **매 틱** 돌아 주행이 무거워진다.
     * 그물은 km 단위 판정이라 300m 스냅은 화면·아웃풋에 안 보인다.
    ...
    /**
     * 🛣️ 실도로 곡선 (기사님 «콜을 잡으면 카카오에서 진짜 길찾기») —
    ...
    /**
     * 🔴 재요청 기준은 **내용(정거장 좌표)**이지 배열 정체성이 아니다 — 주행 시작 때
     * departed 가 켜지며 같은 내용의 새 배열이 만들어지는데, 그걸 «경로가 바뀌었다»로
     * 읽으면 재호출해 곡선이 지워졌다 다시 그려진다.
     */
    ...
    /**
     * 🖌️ **그릴 곡선 — 상태(realLegs)가 아니라 «캐시 우선»으로 꺼낸다** (기사님:
     * *"확정하면 직선으로 깜빡인다"*). 확정 순간 effPath 는 새 경로인데 realLegs 는 아직
     * 직전 값/null 이라, 상태에서 꺼내면 **한 박자 직선**이 그려진다.
     * 캐시에 있으면 그 프레임부터 곡선이다.
     */
    ...
    const drivePath = useMemo(() => {
        /**
         * 🚗 **모의 주행도 «확정 순간의 카카오 경로»를 따른다** (기사님 승인).
         *
         ...
         *    도장이 찍힌다. `foldChainOrder` 로 순번이 chain 을 따르게 접으므로
         *    평소엔 **항등**이고, 순번이 접기에 실패해 다시 재배치로 물러난 때(취소 재측정 전 창 등)
         *    에서만 갈린다 — 그때 주행도 함께 `effPath` 계보로 물러나 세 원천이 같은 쪽에 선다.
         */
    ...
    const routeLine = useMemo<Array<[number, number]> | null>(() => {
        /**
         * 🕸️ **라인 그물도 «확정 순간의 카카오 경로» 한 벌에서** (기사님 승인).
         *
         ...
         * 🔴 chain 이 없거나(프리셋 · 확정 전) 못 잰 구간이 있으면 **`effPath` 계보로 물러난다**.
         */
        ...
        /**
         * 🔴 **라인은 통째로 둔다 — 지나온 자리는 «숫자»로 뺀다** (기사님:
    ...
    const lastDrop = useMemo<NetPoint | null>(() => {
        ...
        /**
         * 🐢 **같은 자리면 같은 객체를 돌려준다** (실측). 통과 도장마다 effPath 가
    ...
    /**
     * 🎯 **목적지마다 그물 하나 — 라인은 하나, 마름모는 목적지마다** (기사님 지적).
     *
     * 🔴 라인을 **노선 목적지에만** 걸면(`g.name === dst.name`) **복귀콜을 잡아
     *    목적지가 «집»으로 접히는 순간 라인이 통째로 빠지고** 「내 위치 → 집」 마름모만 남는다
    ...
    const goalNets = useMemo(() => goals.map(g => ({
        goal: g,
        /**
         * 🔴 **«라인 그물로 만들었나»를 계산이 함께 내놓는다** (기사님 지적).
     ...
     */
    /**
     * 🐢 **지나온 동 «수» — 그물을 다시 합칠지 정하는 숫자 하나** (실측).
    ...
    /**
     * 📦 **앱에 내려갈 필터 아웃풋** (기사님: *"DB 도 서버통신도 없이, 필터 로직을
    ...
    const appFilterOutput = useMemo(() => buildAppFilterOutput({
         ...
         *    화면 판정(`isExcluded`)과 같이 아웃풋도 노선일 때 제외지역을 뺀다 — 안 그러면
         *    노선일 때 «화면에서는 빠진 동이 앱에 내려갈 목록에는 그대로» 남는다.
         */
     ...
     * 그리기가 전역 routeStarted 를 읽으면 «지도에 안 그린 자리의 콜이 통과»한다.
     */
     ...
     *  경계 ±200m 에서 «그물은 든다는데 판정은 탈락»이 생긴다 (리뷰 · 규칙 ③) */
    ...
    /**
     * 🧮 **합짐 우회 — 실물 계산을 그대로 쓴다** (기사님 확정:
     * *"합짐 판단은 기존 경로와 지금 경로가 얼마나 차이가 있는가를 아는 것"*).
     *
     * 🔴 직선 근사를 **쓰지 않는다.** 서버 `/api/sim/detour` → 실물 `calculateDetourRoute` 가
    ...
    /** ✅ 콜 확정 — 리스트에 넣고, 그 콜의 배송 거리·시간·톨비를 실측해 카드에 붙인다 (기사님).
     *  ⚠️ 옵션은 카카오 «추천» 하나다 (서버 calculateSoloRoute 기본값) — 길 찾기의 5옵션과 다르다. 카드에 표기함 */
    /**
     * ☎️ **«통화로 정한 약속»을 켜고 끈다** (기사님).
    ...
    const confirmCall = (p: Pt, d: Pt) => {
        ...
        const caughtDest = goalsVerdict?.wonGoal?.name ?? dst.name;   // 통과한 목적지가 곧 경로 묶음 (⑮ 기준 3)
        ...
        planSeqRef.current++;                    // 확정 콜 묶음이 바뀌었다 — 늦게 오는 취소 적립을 무효로 만든다
        ...
        //    불일치로 재배치에 물러난다 (리뷰: 독이 든 동결이 다음 판까지 살았다)
        ...
        /**
         * 🧾 **이 확정이 기존 정거장을 몇 분 밀었나 — 그 자리에 적립한다** (기사님).
         *
         * «확정 전 경로»와 «확정 후 경로»의 그 정거장까지 누적 차이 — ⑯ 의 우회 정의 그대로다.
         * 이 값을 심사 문단에 **띄우기만 하면 사라진다.** 쌓아 두면 나중에
    ...
    /**
     * 📋 **기사님이 찍어 둔 콜들을 그대로 문제로 뽑는다** (기사님: *"내가 찍으면
     * **그때 걸 기억하고 넣으라고**"*). 좌표가 아니라 **동 이름**으로 읽히게 «어디였나»를 함께 적는다.
     */
    const [captured, setCaptured] = useState<string | null>(null);

    /**
     * 🧪 **문제 버튼은 «목적지·시작 자리만 놓는다» — 콜은 기사님이 누른다** (기사님 확정:
     * *"왜 자동이야. **클릭은 내가 할게. 자동 빼.**"*).
     *
     * 🔴 버튼 하나가 콜을 다 잡고 주행까지 하면 **내가 돌린 것**이지
     *    기사님이 보시는 것이 아니다 — 중간에 멈춰 보거나 다르게 눌러 볼 수가 없다.
     *    그래서 이 함수는 **지도를 비우고 목적지·시작 자리만** 놓는다.
    ...
    const cancelLastCall = () => {
        ...
        callApi('🧹 취소 후 경로 다시 재기', '/sim/chain',
            ...
            .then(d => {
                if (seq !== planSeqRef.current) return;   // 그 사이 확정 콜 묶음이 또 바뀌었다 — 이 적립은 어긋난다
    ...
    /** 🔴 올리기 경합 가드 — 늦게 온 앞 콜 응답이 새 콜 값을 덮어쓰면 안 된다 (리뷰) */
    ...
    const uploadCall = () => {
        ...
        // 🔴 **«상차지까지» 구간(현위치→상차지)을 함께 받는다** (기사님: *"상차지 약속을
        //    잡을 수 없다 — 얼마나 시간과 거리가 되는지 모르니까"*). 상차 약속의 재료다.
        const me = { ...myPos };
        /**
         * 🔴 **⑮ 는 여기서 안 부른다 — «콜 확정» 뒤다** (기사님 확정:
        ...
        /**
         * ④⑫ 재배치 — 🔴 **기점은 «지금 내 위치»다** (기사님: *"왜 출발부터 재는 거야?
        ...
        const fromVisited = visitedCountRef.current;   // 🧷 이 경로가 «몇 정거장 지나고» 잰 것인지 — 접기(`foldChainOrder`)의 기준점
         ...
         * (기사님: *"저장하라고 했잖아 … 그림까지 그려져 있는데 모른다는 게 말이 되니"*).
         *
    ...
    /**
     * 📌 **기존 콜이 얼마나 밀리나** (기사님: *"이 합짐이 기존 콜에 얼마나 영향이
     ...
     * 우회하차 = 지금 도착 − 저장 도착
     * ```
     * 🔴 **구간 하나만 보면 안 된다.** `(합짐상차→첫콜하차) − (저장 배송)` 으로 재면
     * 사이에 낀 `(첫콜상차→합짐상차)` 가 통째로 빠진다 — 그 구간이 37분이면
     * 50분 지연을 13분으로 답하게 된다.
    ...
    const detourRows = useMemo(() => {
         ...
         * 🔴 **분끼리 빼지 않는다 — 시각끼리 뺀다**.
         *
         * `지금 누적분 − 저장 누적분` 으로 빼지 않는다. «지금 누적»은 **지금 내 위치**에서
         * 재고 «저장 누적»은 **확정하던 그 자리**에서 잰 값이다 — 기준점이 다른 두 분을 빼면
         * **그 사이에 흐른 시간이 통째로 빠진다**(주행 30분 뒤라면 +70분 지연이 +40분으로 보인다).
         * 약속은 **절대 시각**으로 못 박아 뒀으니
    ...
    /**
     * 📦 **콜 하나가 한 덩어리** (기사님: *"좌우로 표현하지 말고 첫콜 합짐1 합짐2
     * 이렇게 순차적으로 덩어리감 있게"*).
     *
     * 기존↔지금을 좌우로 놓으면 «어느 콜이 어떻게 되는가»를 눈이 스스로 짜맞춰야 한다.
     * 콜마다 자기 상차·하차의 «기존 도착 → 지금 도착»을 들고 있게 한다 — 마지막 덩어리가
     * 지금 심사하는 후보콜이다.
     */
    ...
    const callImpacts = useMemo(() => {
        ...
        /**
         * ⏱️ **예정 시각은 «계산»한다** (기사님: *"아직 안 지났으면 언제 지날지
         ...
         * «콜 받은 순간»에 고정된다.
         */
        // 🔴 못 잰 응답(legs 빈 채 note 만 온 것 — measuredAt 없음)은 심사 경로로 안 친다 (리뷰)
         ...
         * 지나온 정거장까지 그대로 품고 있다. 그냥 이어 붙이면 지나온 정거장을 두 번 세어
         * 그 수만큼 번호가 밀린다.
         */
        const visitedLabels = visitedNow.map(v => `${circled(v.call)}${v.kind}`);
        /**
         * 🔴 **후보콜은 번호를 안 먹는다** — 지도와 콜 리스트의 남은 번호가 같아야 한다.
         *
         * `chainNow` 는 «후보콜을 낀 전체 경로»라 후보 정거장 **둘**을 품고 있다. 그대로 세면
         * 그 뒤 정거장이 전부 **+2** 로 밀린다. 지도는 후보를 «상/하» 표시로만 그리고 번호를 안 준다 —
         * 잡아야 정거장이 되기 때문이다. 세는 곳이 둘이면 번호가 갈리므로
         * 계산은 `labTime.visitOrder` 한 곳에 두고 검사로 잠근다 (규칙 ③).
         */
        ...
        return Array.from({ length: last }, (_, i) => {
            ...
            return {
                ...
                stops: (['상차', '하차'] as const).map(kind => {
                    ...
                    /**
                     * 🔴 **덩어리는 «그 콜의 것»만 보인다** (기사님: *"이 영역은 첫짐의
                     * 영역이니까 첫짐이 가진 저장된 정보만 보여야 하는데"*).
                     *
                     * 지금 경로에서 그 정거장으로 **들어오는** 구간을 적으면 첫짐 칸에
                     * `②하차 → ①하차 9.4km · 21분` 같은 남의 구간이 들어온다 — «중대동에서
    ...
    /**
     * 🎨 **후보콜의 색 — 실물 엔진이 채점한다** (기사님 «필터를 만들어 보자» 판).
     *
    ...
    const seatOrder = useMemo<(SecuredOrder & { soloMin?: number | null }) | null>(() => {
        ...
        return {
             ...
             *    점수를 못 낸 기준을 전부 담으면 심사석에
             *    «❌ 지리 안 봄 (가중치 0)» 이 거절 사유로 뜬다 — **안 보는 것은 거절이 아니다.**
    ...
    /**
     * ⏱️ **확정 경로의 정거장 시각** (기사님: 콜 리스트에 «(이동시간 − 늦어진 시간, 도착시각)»).
    ...
    const sheetNext = useMemo(() => {
        /**
         * 🔴 **«다음»은 방문 순번(seq)이 정한다 — 콜 번호 순이 아니다** (실측).
    ...
    /**
     * 📜 판정 기록 (기사님 요청 «한 바퀴 돌고 검토») — 처리(확정/버림)된 콜만 남긴다.
    ...
    const retarget = () => {
         ...
         *
         * 경로 **전체**에서 가장 가까운 점을 고르지 않는다. 수도권에서 정거장이 예닐곱이면
         ...
         * 같은 정거장들을 왕복하는 사이 모의 시계만 흘러 마지막 구간을 못 끝낸다.
         *
            ...
            setMyPos(pos => {
                ...
                for (; ti < walked.idx; ti++) {
                    ...
                    { setTargetSeq(t.seq);
                        ...
                        //    (그러면 아직 안 간 정거장이 방문으로 잠겨 방문 순서가 최적과 다르게 굳는다)
    ...
    useEffect(() => {
        const draw = () => {
            ...
            //    내 위치가 화면 밖으로 잘렸다 (기사님 «노선을 클릭해도 함께 노출»)
            ...
            if (layers.net) for (const { goal, net: gn, usedLine } of goalNets) {
                /**
                 * 🔴 **«노선 단추»가 아니라 «라인이 실제로 있는가»로 가른다** (기사님 지적:
                 * *"마름모 없음 — 그리지 않았을 뿐 영역은 있는 것 같음"*).
                 * `routeMode` 로 가르면 노선을 누르기만 해도 라인이 없을 때
                 * **마름모가 화면에서 사라지는데 판정은 그 마름모로 한다** —
                 * 화면이 조용히 거짓말한다 (규칙 ⑤-4 ④).
                 *
                ...
                /**
                 * 🎨 **면은 한 path 에 모아 한 번 칠한다** (기사님 «마름모 라인을 지울 수 있을까?»).
                 ...
                 * (기사님: «마름모 라인을 지울 수 있을까?» · «목적지 테두리가 없어»).
            ...
            if (layers.net && lineOn && routeLine) {
                /**
                 * 🔴 **띠도 «지나온 만큼» 짧아진다** (기사님: *"아직도 지나간 길 인식
                 * 못 하고 있거든"*). 판정은 동마다 든 `progressKm` 으로 걸러지므로
                 * **그리는 띠도 같이 줄어야 한다** — 띠를 전체 그대로 칠하면 화면과 판정이 다른 말을 한다.
                 *
             ...
             *
             * «도착하는 정거장»의 색으로 칠하면 `②상차 → ①하차` 가 **①색**으로 칠해진다 —
             * 도착지가 ①이니까. 그런데 그 구간을 만든 건 **②번 합짐**이다. 합짐이 어디를 늘렸는지가
             * 색에서 사라진다. 두 정거장 중 **나중에 잡은 콜**이 그 짝을 만든 콜이다.
             */
            ...
            /**
             * 🛣️ **선은 «확정 순간의 카카오 경로» 한 벌로 그린다** (기사님 승인).
             *
             * 구간마다 콜 색: «to» 정거장의 콜 번호(②상차 → 콜 2)로 칠한다.
             * 🔴 구간 하나라도 못 쟀으면(`failed`) **통째로 `effPath` 계보로** 물러난다 — 반쪽 그림을
             ...
             *    (잰 순간의 자리 + 지금 자리). 출발 전에는 두 점이 겹쳐 안 보일 뿐이다.
             */
            if (layers.route) for (const p of effPath.slice(1)) {
                const [px, py] = S(p.x, p.y);
                if (p.seq) {
                    // 방문 순번 배지 — 콜 색 원 안에 흰 번호 (기사님 «경로에 번호»)
            ...
            if (layers.call && pickup && drop) {
                ...
                //    (기사님 «좀 더 시뮬레이션처럼»). 30초가 지나면 조용한 점선으로
            ...
            /** 상·하차 마커 — 원 안에 「상」「하」 (기사님: 삼각형은 방향으로 오해된다) */
    ...
    const placeAt = (pt: Pt) => {
        ...
        if (!pickup || (pickup && drop)) { if (pickup && drop) pushLog('버림'); pauseForCall(); setPickup(pt); setDrop(null); setUploaded(false); setUploadedLeg(null); setApproachLeg(null); setApproachInfo(null); setChainNow(null); setChainBefore(null); setChainPreview(null); uploadedInfoRef.current = null; uploadSeqRef.current++; setCallSeenAt(null); }
             ...
             *
             * 🔴 «직선 km × 하한 단가»로 채우면 **콜마다 요금이 달라져** 색이 따라 흔들린다.
    ...
    return (
        <div className="h-screen bg-background text-text-primary flex flex-col overflow-hidden">
            {/* 📍 정거장 상세 — 라벨을 펼친다. 퀵사가 «왜 늦었어» 할 때 여는 화면 (기사님) */}
            {stopPeek && (() => {
            ...
            {/* 📋 저장된 콜 전부 — 화면이 파생값으로 말하니, 원본도 볼 수 있어야 한다 (기사님) */}
            {callsPeek && (
                <div className="fixed inset-0 z-50 grid place-items-center bg-black/55 p-4" onClick={() => setCallsPeek(false)}>
                    <div className="w-full max-w-[900px] max-h-[85vh] overflow-auto rounded-[10px] border border-border-card bg-surface p-3 flex flex-col gap-2"
                        ...
                        {/* 🔴 **날것 그대로 찍는다** (기사님: *"이런 형식으로 저장하고 있는 거 확실해?"*).
...
{JSON.stringify(terminated, null, 2)}
            ...
            {/* ⚙️ 상단 — 설정 모음 (기사님 «상단은 설정을 모으고») */}
            {/* ⚙️ 상단 — 설정 (필터는 왼쪽 탭으로 — 기사님) */}
            <header className="shrink-0 border-b border-border-card bg-surface px-3 py-2 flex flex-wrap items-center gap-x-5 gap-y-2">
                ...
                <div className="flex flex-col gap-1 min-w-[300px]">
                    <div className="flex items-center gap-1 flex-wrap">
                        ...
                        {/**
                          * 📋 **지금 판을 문제로** (기사님: *"내가 찍으면 **그때 걸 기억하고
                          * 넣으라고**"*). 잡아 둔 콜들과 지금 찍어 둔 후보를 **순서 그대로** 뽑는다.
                          * 🔴 내가 좌표를 감으로 고르면 맥락이 없다 — 문제는 **기사님이 실제로 찍은 콜들**이라야 한다.
                          */}
            ...
            <div className="flex-1 min-h-0 flex">
                {/* 🗂️ 왼쪽 — 필터 (기사님 와이어프레임: 필터 → 판정 → 콜 리스트) */}
                <aside className="w-[460px] shrink-0 border-r border-border-card bg-surface p-3 flex flex-col gap-2 overflow-y-auto">
                    {/* 🎯 요약줄 — 실물 규격 그대로 (OrderFilterStatus: «🎯 노선행 · 여기서 10km → 서울 1km · 📦 90/100»).
                        라벨은 shared CALL_TARGET_LABEL, 값은 지금 필터 상태에서 파생 (기사님) */}
                    ...
                    {/* 🔓 **콜을 쥔 뒤에도 노선으로 갈 수 있다** (기사님 지시 «노선에 락을 풀어 봐»).
                        노선의 띠는 **잡은 콜들이 만든 실제 경로**(`routeLine`) 주변이다 — 사각형은 콜을 쥔 뒤에도
                        그대로 넓어서 **이미 잡은 콜과 무관한 콜이 통과하므로**(기사님 실측: 탄벌동→고촌읍을 쥔 채
                        의정부 왕복 콜이 통과), 띠로 좁히면 막히는지를 **화면에서 눈으로 확인한다.** */}
                    ...
                    {/* 🔴 **값은 한 벌이다 — 노선·동선이 같이 쓴다** (기사님 확정:
                        *"모두 꺼내 두고 노선이면 라인값을 사용하고 동선이면 사용 안 하면 되니까"*).
                        탭마다 다른 칸을 보이면 «값이 여러 벌»이 아니라
                        «지금 안 쓰는 칸을 감추는 것»이 된다 — 감추면 화면이 조용히 거짓말한다. */}
                    ...
                    {/* 🔴 설명 줄은 이상한 상태 하나만 둔다 (기사님: *"이건 무슨 뜻이야 불필요한 거 같은데"* ·
                    ...
                    {/**
                      * 💰🚚🚫 **셋도 같은 3칸 레이어로** (기사님: *"이 부분도 디자인에 맞춰
                    ...
                    {/**
                      ...
                      *
                      * 🔴 **전국에서** 고른다 — 지금 그물에 든 동만 목록에 띄우면
                      *    그물이 강화군에 닿기 전에는 강화군을 뺄 수가 없다.
                    ...
                    <div className="mt-1 border-t border-border-card pt-2 flex flex-col gap-1">
                        <div className="relative grid grid-cols-3 gap-1">
                            {/**
                              * 🔴 **도 한 층을 둔다** — 없으면 서울을 빼려고 구 **25개**를 하나씩 눌러야 한다
                              * (기사님: *"원래 내가 원한 건 **서울을 빼는** 거였는데.. 지금 서울을 빼고 있는데"*).
                              *
                            ...
                            {/**
                              ...
                              * «고르기»와 통째 제외를 나누면 두 번 눌러야 한 곳이
                              * 빠지고, 한 곳 찍을 때마다 레이어가 닫힌다.
                              *
                              * **누르면 그 시·군·구가 통째로 빠진다**(다시 누르면 되살아난다).
                        ...
                        {/**
                          ...
                          * **닫히면 한 줄, 누르면 전부**다 (기사님 *"결과물을 첫 줄만 보여 주고 클릭하면 다"*).
                    ...
                    {/* 🧪 판정 — 필터와 콜 리스트 사이 (기사님 와이어프레임 확정) */}
                    {/* 🔴 두 층은 완전히 격리되어 각각 따로 작동한다 (규칙: 필터=집기 전 · 심사=집은 뒤).
                        기사님: *"판정영역을 상하로 나눠서 상은 필터가 하는 일, 하는 심사가 하는 일"* */}
                    {/**
                      * 🔴 **제목의 수는 «앱에 실릴 동 수»여야 한다** (화면 실측).
                      *    그물 동 수와 아웃풋 동 수는 다를 수 있다 — 차이는
                    ...
                    <FilterPanel tone="filter" title={`🔍 ① 콜 필터 — 집기 전 · 앱이 하는 일 · 그물 ${netCount}동${dongOut ? ` · ⛔${dongOut}` : ''}${lineOn ? ' · 라인' : ''}`}>
                        {/* 🔴 «관내 모드인가»가 아니라 **«관내 규칙으로 쟀는가»** 를 읽는다 (기사님 지적).
                        ...
                        {verdict && (
                            <div className="flex flex-col gap-1">
                                ...
                                {/* 찍은 좌표 — 화면만 보고 그대로 재현·검산할 수 있게 (기사님 제안) */}
                                {pickup && drop && (
                                ...
                                {/**
                                  * 🔴 **탈락 버튼도 둔다** (기사님: *"필터에서 통과 버튼만 있어.
                                  * **탈락 버튼도 하나** 만들고"*). 없으면 올릴 수만 있고 **버릴 수가 없어서**,
                                  * 안 올릴 콜을 지우려면 지도를 다시 두 번 눌러야 한다.
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <div className="rounded-[8px] border border-border-card bg-background px-2 py-1 text-[10.5px] leading-snug">
                                    ...
                                    {/* 🔴 «목적지까지 3거리»와 «전진/역주행»은 여기 안 적는다 — 바로 위 ① 콜 필터가
                                        같은 숫자와 「하차 전진 / 상차 앞」 칩으로 **이미** 말한다 (기사님:
                                                ...
                                                {ci.stops.map(st => {
                                                    ...
                                                    return (
                                                        <div key={st.kind} className="flex flex-col">
                                                            {/**
                                                              * 🔴 **정거장 하나에 한 줄** (기사님 *"심사 부분이 너무 높이가 높아"*).
                                                              * 「구간 from → to · km · 분」을 윗줄로 따로 두면 콜 셋이면 그것만
                                                              * 여섯 줄이다. `from` 은 **바로 윗 정거장**이라 안 적어도 알고,
                                                              * km·분은 도착 줄 옆에 붙인다. **값이 없어도 «--» 로 남긴다** — 비교해야 하니까.
                                                              */}
                                                            ...
                                                            {/**
                                                              * 🧾 **왜 밀렸나 — 그 자리에서 쪼갠다** (기사님: *"심사 시 보면 좋을 것 같아"*).
                    ...
                    {/* 📋 콜 리스트 — 왼쪽 사이드바 맨 아래 딱 붙임 (기사님: mt-auto) + 방문 순서 */}
                    <FilterPanel title={
                        ...
                        {confirmed.length > 0 && (
                            <>
                                {/* 콜 = 한 덩어리 카드: 윗줄 상차→하차 · 아랫줄 거리·시간·톨비 (기사님) */}
                                                    ...
                                                    {([['상차', c.pickup], ['하차', c.drop]] as const).map(([kind, pt], k) => {
                                                        ...
                                                        return (
                                                            <span key={kind} className="font-black">
                                                                ...
                                                                {/* 🔢 앞 숫자는 «몇 번째로 들르나» — 상차/하차 표시가 아니다 (기사님) */}
                ...
                {/**
                  * 💰📦 **두 점을 찍으면 지도 위에 뜨는 입력창** (기사님).
                  *
                  * 🔴 **실험실 전용이다.** 실제 콜은 배차망이 요금과 짐을 주므로 실물에는 이 창이 없다.
                  *    심사창에 두면 이식할 때 «실물에 필요 없는 것»이 딸려 간다 — 그래서 여기 둔다.
                ...
                {/* 🎛️ 오른쪽 — 필터 옵션 (기사님 와이어프레임: 실물 요소만 · 값은 목업 ·
                ...
                <aside className="w-[400px] shrink-0 border-l border-border-card bg-surface p-3 flex flex-col gap-2 overflow-y-auto">
                    ...
                    {/* 🔴 **«필터 값» 패널을 두지 않는다** (기사님 지시).
                        · 도착 목표 · 상차 반경 · 하차지 주변   왼쪽 🎯 목적지·«현위㎞·목적㎞»와 **같은 값**이다
                        · 우회 허용    실험실은 그 결과(라인 반경)를 기사님이 직접 넣는다 — 두면 손잡이가 둘이 된다 */}
                    {/* 🔬 **적재 패널은 두지 않는다** (기사님: *"적재는 상태값이니 필요 없고"*).
                        쓴 박스는 **잡은 콜들의 짐 합**이라 고를 값이 아니다 — 상단 요약줄에 `📦 n/100` 으로 보인다.
                        ⚠️ 「라면박스 환산이 화물 종류에 맞는가」는 **실측이 필요하다** — todo 에 항목으로 있다. */}

                    {/**
                      * 🧪 **콜 문제를 한 줄씩 — 눌러서 지도에 찍는다** (기사님:
                      * *"오른쪽 사이드바에 콜 리스트를 만들고 **순서대로** 넣어 놔.
                      * **그걸 클릭해서 지도에 찍을 거니까.**"*).
                      *
                      * 🔴 위(상단) 문제 버튼은 «문제를 통째로 부르는 것»이고, 여기는 «한 콜씩 손으로 놓는 것»이다.
                    ...
                    <details className="border-t border-border-card pt-2" open>
                        ...
                        {/**
                          * 🔴 **액션과 콜을 나누지 않는다 — 한 줄이 콜 하나다** (기사님:
                          *    *"엑션과 콜을 나누지 말고 **콜에 설명을 넣어서** 4줄로 잡아 줘"*).
                          *
                          *    좌우 두 칸(이벤트 | 콜)으로 나누면 **줄이 두 배**가 되고
                          *    빈 칸이 절반이다. 주행은 «그 콜을 잡기 전에 해야 하는 일»이라
                    ...
                    {/**
                      * ⚖️ **실물 심사석 두 가지** (기사님 «심사판 2종류를 오른쪽 사이드바에»).
                      *
                      * 🔴 **실물 부품을 그대로 부른다** — `components/dashboard/JudgmentSeat`.
                      *    실물 파일은 **읽기만 한다** (기사님 *"기존 프로젝트는 건드리면 안 돼"*).
                      *    베껴 두면 실물이 바뀔 때 여기만 바뀌기 전 모양으로 남아 실험이 거짓말을 한다.
                      * 🔴 둘을 가르는 것은 **콜의 갈래**다 — `isManualLineage(type) || isPreview` 면
                    ...
                    {/**
                      * 🪟 **시트 — 실물 구조 그대로, 사이드바 하단 고정** (기사님 확정:
                      ...
                      * 🔴 **UI 만 가져온다.** 실물 `StageSheet`·`PinnedRouteBody` 는 고치지도
                    ...
                    <div className="mt-auto sticky bottom-0 -mx-3 -mb-3 px-3 pb-3 pt-2 bg-surface border-t border-border-hover flex flex-col gap-1">
                        ...
                        {/* ── 📋 아코디언 콜 리스트 — 타이틀 한 줄, 열면 컨텐츠 ──
                            타이틀 문법(기사님): [콜번호] [정거장번호]동이름 ~시각 → [정거장번호]동이름 ~시각
                                ...
                                return (
                                    <div key={ci.no} className="rounded-[8px] border border-border-card bg-background overflow-hidden">
                                        ...
                                        <button type="button" onClick={() => setSheetOpenNo(open ? null : ci.disp)}
                                            ...
                                            {/* 🔴 **열기 화살표는 두지 않는다** (기사님 *"열기 아이콘은 지워 버린다"*) —
                        ...
                        {/* ── ⚖️ 심사 영역 — 시트 맨 아래 붙박이 ── */}
                        {/* 🔴 **판정이 쓴 그 값을 그대로 넘긴다** — «더 쓰는 시간 = 주행 + 정차»가 실물 정의라,
                            시트가 주행만 따로 적으면 이름은 같은데 값이 달라진다 (규칙 ③). */}
                    ...
                    {/* 🛰️ 시스템 — 카카오를 언제·무엇으로 부르고 무엇을 받았나 (기사님) */}
                    <details className="border-t border-border-card pt-2" open>
                        <summary className="text-[10.5px] font-black text-text-muted cursor-pointer">
