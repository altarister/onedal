 ...
 *
 * 🔴 **둘 다 «예상»이 아니라 «약속»이다.** 상차를 «잡은 시각 + 접근 실측»으로 세우면
 *    **약속을 예상에 맞추는 것**이라 ± 가 늘 0에 가깝고 «20분을 넘겼나»가 안 보인다
 *    (기사님: *"이걸로는 20분이 넘었는지 아닌지 모른다는 것이다"*).
 * 🔴 하차의 150% 는 **관행 상한이 아니라 고객과의 약속**이다 (기사님 정정:
 *    *"지금 전달해 주시면 150% 안에 가져다 드릴게요 — 그러니 고객과의 약속이 맞아"*).
 * 🔴 병합 경로 누적으로 잡지 않는다 — **빙 둘러 온 시간이 약속에 들어가면**
 *    「예정 − 약속」이 0 이 되어 **진짜 영향이 숨는다** (기사님).
...
describe('⏰ 약속 시각 — 직행 기준', () => {
    ...
    /**
     * 🧳 **하차 약속에는 «상차에 머무는 분»이 들어간다** (기사님 *"정차를 넣어줘"*).
