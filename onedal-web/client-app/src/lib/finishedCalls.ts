 ...
 *
 * 부르는 곳: ☰ 서랍(`Drawer.tsx`) — 폰에서도 보인다.
 *
 * 🔴 **거르는 규칙을 그리는 쪽에 적지 않는다** (규칙 ③). 목록을 그리는 곳이 늘면 모두 이 함수를
 *    부른다 — 따로 적으면 한쪽만 고쳐져 «취소 탭에는 있는데 서랍에는 없는» 콜이 생긴다.
 *
...
/** 콜 목록의 갈래. 서랍은 셋만 쓴다 — `ACTIVE`·`ALL` 은 서랍 탭에 없다 */
