...
/**
 * 🧭 **지도 투영·시점 검사** — 배경 타일과 마커가 같은 자리에 오는지 본다.
 *
 * ① 투영은 **메르카토르**다 — 선형으로 놓으면 마커가 도로에서 밀린다
 * ② 확대·축소의 기준점은 **그리는 쪽과 제스처 쪽이 같다**(`anchorBaseOf`) — 다르면 확대할수록 지도가 옆으로 흐른다
 */

/** 실측 문제지의 정거장들 (drive.mjs 와 같은 좌표) */
...
describe('projectMercator — 타일이 쓰는 투영', () => {
    ...
    /**
     * 🔴 **이 검사가 선형 투영을 잡는다.** 위도·경도를 같은 축척으로
     *    놓으면 이 비가 1.0 이 되어 한국 위도에서 세로가 25% 눌린다.
     */
...
/**
 * 🪟 **시트가 덮은 자리를 피한다** (기사님 요청 — *"반쯤 열리면 같이 볼 수 있을 것 같은데"*).
...
describe('확대 — 누른 자리가 붙잡혀 있다', () => {
     ...
     *    제스처 쪽 공식이 기준점을 (0,0) 으로 알면 이 성질이 깨진다 — 기준점은 `anchorBaseOf` 한 곳에서 온다.
     */
...
/**
 * 🤏 **핀치 줌 — 두 손가락 중간이 제자리에 남는다** (기사님 실주행 지적)
 *
 * 기사님: *"손가락 중간을 기준점으로 줌인이 될 거라 생각했는데..
 * 한쪽 방향으로 치우쳐서 줌인되었어."*
 *
 * 핀치도 휠·버튼(`zoomAround`)처럼 기준점을 잡고 팬을 보정한다(`pinchStep`).
 * **배율만 바꾸고 팬을 안 고치면** 한쪽으로 치우쳐 확대된다.
 */
...
/**
 * 🎨 **`mapTileTone` 의 평소 톤은 배율을 안 받는다** (기사님 «줌인할수록 흐려지는 기능은 필요 없을꺼 같아» · «어»)
 *
 * 영역이 주인공인 지도라 평소 톤은 회색조로 눌러 둔다.
 * ⚠️ 캔버스는 이 함수를 부르지 않는다 — 실제 지도 밝기는 `tileToneFor` 가 «어둡게» 레이어와 배율로 정한다 (아래 🌓).
 */
 ...
 * 배율에 그대로 비례하면(`3 * zoom`) 10배에서 30px 띠가 되어 도로를 통째로 덮는다.
 */
describe('🖊️ 경로선 두께', () => {
    it('기본 배율에서는 3px', () => {
...
describe('🔭 지도 보기 — 전체 · 이번 구간 · 현위치', () => {
    ...
    /**
     * 🔴 구간을 «현위치 → 다음 정거장»으로 잡으면 달릴수록 둘이 가까워져
...
/**
 * 🌓 **밝기는 «어둡게» 레이어와 배율이 정한다** — 보기 모드와 묶지 않는다 (`tileToneFor`).
 ...
 * 🔴 아주 빼지는 않는다 — 가까운 영역은 보여야 한다 (다 빼면 «영역이 너무 짤린다»가 된다).
 */
...
/**
 * 🔭 **짧은 축이 화면을 통째로 줌아웃하지 않는다** (기사님 실측)
 *
 * 기사님: *"4~5 가산동은 다른 지점이 보일 만큼 줌 아웃 되어 있어."* ·
 * *"지도가 보이는 영역에 지점이 꽉 차서 보여야 하는데.. 그렇지 못해."*
 *
 * 짧은 축에 고정 폭(예: 22km)을 채우면 세로로 뻗은 구간은 가로가 짧다는 이유로
 * 22km 를 뒤집어쓰고 `min()` 이 그 축을 골라 화면이 통째로 축소된다. 짧은 축은 제약 없음으로 둔다.
 */
describe('🔭 뷰포트 맞춤 — 짧은 축이 화면을 줄이지 않는다', () => {
    const W = 400, H = 560, OCC = 72, NO = { x: 0, y: 0 };
    /** ④가산동 → ⑤구로동 — 가로 1.0km · 세로 4.1km 의 «세로 구간» (실제 좌표) */
    ...
    it('🔴 세로 구간이 세로에 맞춰진다 — 두 점이 화면에서 멀리 벌어진다', () => {
        ...
        // 짧은 가로에 22km 를 채우면 세로 간격이 60px 남짓으로 눌린다
...
/**
 * 🔭 **보기 버튼 — 언제나 되돌리고 언제나 다시 그린다** (버그)
 *
 * 기사님: *"현위치에서 드래그하고 **다시 현위치를 누르면 현위치로 안 와.**"*
 *
 * 🔴 버튼이 `setViewMode` 만 부르고 그리기를 **리렌더에 얹으면**, 이미 그 모드일 때
 *    리액트가 상태를 안 바꾸므로 리렌더가 없고, 따라서 **그리기도 없다.**
 *    ref 인 `zoom`·`pan` 만 조용히 되돌아가고 **화면은 제자리에 남는다.** 그래서 `pickViewMode` 가 늘 `draw` 를 부른다.
 */
