/**
 * 🎯 **어느 서버를 보는가 — 판단은 여기 하나뿐이다** (기사님 확정)
 *
 * 기사님: *"볼륨 버튼을 클릭해서 라이브인지 로컬인지 바꿀 수 있으면 더 좋을 것 같은데."*
 *
 * ── 왜 필요한가 ──
 * 관제앱(APK)은 `https://localhost` 에서 자기 번들을 띄운다. 그래서 상대 경로 `/api` 는
 * **자기 자신**에게 간다 — 서버가 아니라. 구글 인증이 성공해 토큰을 받고도
 * **보낼 곳이 없어 조용히 되돌아온다.** 브라우저에서는 Vite 프록시가 받아 주니 안 드러나고,
 * 앱에서만 난다.
 *
 * ── 왜 한 곳인가 ──
 * `apiClient.ts` 와 `socket.ts` 가 **각자** `VITE_API_URL` 을 읽으면
 * 주소를 바꾸는 길이 생길 때 두 곳이 갈라진다 — 이 레포가 반복해 당한 형태다 (규칙 ③).
 * 여기서 정하고 둘 다 여기서 읽는다.
 *
 * ── 왜 새로고침하나 ──
