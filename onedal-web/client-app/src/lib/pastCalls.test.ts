...
/**
 * 🙈 **지나간 콜 숨기기** (기사님 지시: *"오른쪽 끝에 지나간 콜 숨기기가
 *    있으면 좋겠는데… 아코디언의 콜타이틀중 배송이 끝난콜을 숨겼다 보였다"*).
 *
 * ── 왜 필요한가 ──
 * 덱은 **하차를 마친 콜도 사이클이 끝날 때까지 함께 보여 준다**(`deckOfCycle`) — 그래야 6단계가
 * 채워진 모습이 보인다(기사님). 그런데 콜이 여섯이면 끝난
 ...
 * 🔴 **언마운트하지 않는다** — 접힌 콜처럼 마운트한 채 숨긴다(`CallDeck`).
 *
...
/**
 * 🗺️ **시트와 지도가 «같은 집합»을 본다** (기사님 지시:
 *    *"지도에 있는 역인 부분과 순번도 같이 숨겨줘"*).
 *
 * 🔴 숨길 id 를 **시트를 넘기는 JSX 안에서** 만들면, 지도까지 숨기려 할 때 그 식이
 *    **두 곳**이 되고, 한쪽에 조건이 붙는 순간 **«목록에선 접혔는데 지도엔 남는»** 상태가
 *    된다 — 같은 값을 두 곳에서 따로 계산하는 모양이다.
