...
/**
 * 🛣️ **남은 거리를 «길을 따라» 잰다** (기사님 지적: *"frontend에서 다 알고
 *    있는 값일껀데."*).
 *
 * ── 왜 ──
 * 시트 상태바가 정차 중에 «얼마나 더 가야 하나»를 말해야 한다. 직선 거리는
 * *"우리가 아는 값 중 가장 부정확"*(화면규칙 B5 ㉱)하므로 쓰지 않고, 서버에 도로 기준
 * 거리를 새로 달라고 할 필요도 없다.
 *
 ...
 *    서버가 점마다 `off_route_m` 을 이미 쟤 두고 있으니, 그 값이 화면까지
