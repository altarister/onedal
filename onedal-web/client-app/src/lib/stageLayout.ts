/**
 * 🗺️ **무대의 셈 — 시트가 덮는 만큼 지도가 비켜 준다**
 *
 * ── 왜 따로 두나 ──
 * 🔴 **지도가 시트를 직접 알면 안 된다** (기사님 지시). `PinnedRouteCanvas` 가 `StageSheet` 의
 *    `sheetOccludedPx`·`SheetSnap` 을 import 하면 **부품이 부품을 직접 참조**하게 된다.
 *
 ...
 * 🔴 기사님 확정: *"판정 시트나 시트 아래 나타날 때 시트는 「나」 위치로
