...
/**
 * 👣 **다녀온 정거장의 순서 — 번호를 세는 목록** (기사님 실측).
 *
 * ── 왜 따로 두나 ──
 * 이력만 남은 렌더에서는 화면 번호가 통째로 무너질 수 있다. 계측 로그로는 이렇게 보인다:
 *
 ...
 *
 * 🔴 **그래도 번호가 무너질 이유는 없다.** 한 목록이 두 질문에 답하게 두지 않는다 (규칙 ⑤-4 ⑤):
...
export function visitedSequenceOf<T extends DeckCall>(
     ...
     *    (규칙 ④).
     * 🔴 **정렬은 안정적이어야 한다.** 시각이 **다 사라진 렌더**(이력만 남은 렌더)에서는 열쇠가
