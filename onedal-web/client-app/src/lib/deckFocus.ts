 ...
 *
 * 🔴 `평가중 먼저 → 최신순` 으로 놓으면 평가중 콜이 확정되는 순간 맨 앞에서 제 자리로 튀면서 **덱 전체가 재배치된다.**
 *    스와이프 도중에 그게 일어나면 손가락과 화면이 어긋난다.
 *    (기사님: *"스와이프 오작동한다. 새 아이디 들어온 것을 시간순으로 인덱스 주면 해결될 듯"*)
 *
 * 시간순이면 **새 콜은 뒤에 붙기만** 하고 기존 위치는 절대 안 밀린다.
 * 새로 들어온 평가중 콜로 옮기는 일은 정렬이 아니라 `pickAutoFocus` 의 자동 이동이 맡는다 —
