 ...
 * 둘이 각자 계산하면 한쪽만 여백을 넣게 되어
 * **확대할수록 지도가 옆으로 흐른다** (규칙 ③: 파생값을 만들었으면 그 입력도 한 곳에서).
 *
 ...
 * 🔴 그리고 **시트가 아래를 덮으면 그만큼 위로 올라온다** (기사님 요청).
...
export function computeViewport(
    ...
    /**
     * 🔴 **짧은 축에 고정 폭(예: 22km)을 채우지 않는다** (기사님 실측:
     *    *"4~5 가산동은 다른 지점이 보일 만큼 줌 아웃 되어 있어"*).
     *
     *    짧은 축에 고정 폭을 주면 **세로로 뻗은 구간**은 가로가 짧다는 이유로
     *    넓은 가로 범위를 뒤집어쓰고, `min()` 이 그 축을 골라 화면이 통째로 축소된다.
     *
 ...
 *
 * 🔴 **핀치도 휠·버튼(`zoomAround`)처럼 기준점을 잡고 팬을 보정한다.**
 *    배율만 바꾸면 확대의 중심이 «두 손가락 중간»이 아니라 화면이 원래 잡고 있던 중심이 되어,
 *    손가락이 가운데서 벗어날수록 **한쪽으로 쏠린다**
 *    (기사님 실주행: *"손가락 중간을 기준점으로 줌인이 될 거라 생각했는데..
 *    한쪽 방향으로 치우쳐서 줌인되었어"*).
 *
 * 🔴 **배율은 거리의 «비»로 잡는다.** 차이를 더하면(`(dist - last) * 0.01`)
 *    화면 크기·손가락 간격에 따라 체감이 달라진다. 비로 잡으면 «두 배 벌리면 두 배»다.
 *
...
/**
 * 🎨 **배경 지도 톤 — 늘 평소 톤** (기사님 «줌인할수록 흐려지는 기능은 필요 없을꺼 같아» · «어»).
 *
 * 배경이 시끄러우면 그 위의 **판정 색 · 마커 색 · 상차 · 하차 영역이 안 읽힌다** (규칙 ⑤-3: 색을 틀리는 것이
 * 가장 큰 사고). 그래서 회색조로 눌러 둔다.
 *
 * ⚠️ 캔버스(`PinnedRouteCanvas`)는 이 함수를 부르지 않는다 — 실제 지도 밝기는 `tileToneFor` 가
 *    «어둡게» 레이어와 배율로 정하고, 확대하면 옅어진다. 이 함수를 부르는 것은 검사뿐이다.
 *
 ...
 *
 * 🔴 **`cand`(후보)는 «경로»와 따로 둔다** (기사님 확정) — 후보는 확정 경로 위에
 ...
 * 🔴 **보기별로 나뉘지 않은 한 벌 저장본(`{ base: true, … }`)도 읽는다** — 세 보기에 같은 값으로 편다. 안 그러면 쓰던 사람이 켜 둔 것이 한 번에 사라진다.
 */
 ...
 * 🔴 **배율에 따라 옅어지는 것은 기사님이 그대로 두기로 정한 동작이다** — 크게 확대하는 것은
 *    «도로를 자세히 보겠다»는 손짓이라 배경을 돌려준다. 배경을 계속 눌러 두고 싶으면 «어둡게»를 켠 채 배율을 낮춘다.
...
/**
 * 🖊️ **경로선 두께 — 확대해도 도로를 덮지 않는다** (기사님 지적).
 *
 * 기사님: *"라인이 너무 두꺼워 길을 잘 간 건지 모르겠어. 줌에 따라 두께가 달라져야 할 것 같아."*
 *
 * 🔴 배율에 그대로 곱하면(`3 * zoom`) 10배로 확대할 때 30px 짜리
 *    띠가 되어 도로를 통째로 덮고, «계획선과 궤적이 겹쳤나»를 볼 수가 없다.
 *    확대는 «자세히 보겠다»는 손짓인데 선이 오히려 자세함을 가린다.
 *
 ...
 * | `leg` | **직전 정거장 + 다음 정거장 + 현위치** | 달리는 중 — 지금 구간만 크게 |
 ...
 *    (기사님: *"지도가 잘 보이는 구간까지 줌이 더 되어야 할 것 같아"*).
 *    1.5km 로 잡으면 동네가 통째로 들어와 «살펴보기»가 안 된다.
 */
 ...
 *
 * 🔴 **버튼은 언제나 다시 그린다** (기사님 실측:
 *    *"현위치에서 드래그하고 다시 현위치를 누르면 현위치로 안 와."*)
 *
 *    **이미 그 모드면 리액트가 상태를 안 바꾼다** — 리렌더가 없다. 그리기를 리렌더에 얹으면
 *    `zoom`·`pan`(ref)만 조용히 1·0 으로 돌아가고 **화면은 제자리에 남는다.**
 *
...
export function viewCoordsFor(
    ...
    if (mode === 'leg' && nextStop) {
        /**
         * 🔴 **구간은 «직전 정거장 → 다음 정거장»이다** (기사님:
         *    *"구간은 지금 진행하고 있는 구간이 다 보여야 해"*).
         *    «현위치 → 다음 정거장»으로 잡으면 달릴수록 둘이 가까워져
