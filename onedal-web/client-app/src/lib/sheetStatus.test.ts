 ...
 *
 * ── 왜 경우마다 문장을 만드나 ──
 * **요소를 하나씩** 돌려주고(기호·번호·지명·분·꼬리) 화면이 조립하면
 * 어느 경우에나 **같은 말투**가 나온다 — «⏸ 1 초월읍 ~30분 2번 콜 · 상차».
 * 달릴 때 궁금한 것과 서 있을 때 궁금한 것이 **다르므로** 한 모양으로 말하면 안 된다.
 *
...
describe('🎬 시트 상태바 — 일곱 경우', () => {
    ...
    /** 🗓️ **사이클 = 하루** (기사님 결정) — 콜 사이 빈 차마다 «사이클을 마쳤다»가 뜨면 거짓말이다. 오늘 한 일을 센다 (화면규칙 E12) */
