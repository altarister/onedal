...
/**
 * 🎬 **시트 상태바 문장 — 경우마다 그 경우의 말을 한다** (기사님 확정).
 *
 ...
 *
 * ── 왜 갈랐나 ──
 * **요소를 하나씩** 돌려주고 화면이 조립하면(기호·번호·지명·분·꼬리) 어느
 * 경우에나 **같은 말투**가 나온다 — «⏸ 1 초월읍 ~30분 2번 콜 · 상차». 그런데 달릴 때
 ...
 * | | 무엇을 | 왜 |
 * |---|---|---|
 * | ㉮ | 지명을 **6자에서 자른다** | 표본 자료 72종 중 평균 3.5자인데 `경기광주자연앤자이점`(10자)이 섞인다 |
 * | ㉯ | «이동»·«정차»를 **기호로** | ▶ ⏸ 🔍 ✅ 로 칸을 던다 |
 * | ㉰ | **«어디서»를 뺀다** | 다녀온 곳은 콜 헤더의 진행 점과 지도 동그라미의 테두리가 말한다 |
 * |  | ⚠️ **«도착»만 예외다** — 지도가 가려진 순간이라 동그라미 테두리를 볼 수 없고, 기사님이 그 모양을 직접 주셨다 (`곤지암성당(하)도착 → 3 이천물류(상)`) |
 *
 * 🔴 **거리가 두 가지다** (규칙 ⑤-4 ⑤ — 한 낱말이 두 질문에 답하지 않게):
 *    · 먼 거리(45km) → **도로 기준**이어야 뜻이 있다. 직선이면 산을 뚫고 간다.
 *      폴리라인을 따라 잰 값(`remainOnRouteKm`)을 받고, **모르면 그 조각을 뺀다** (규칙 ④)
 ...
 *       두 개가 다른 것을 말하면 «화면은 도착인데 단계는 안 넘어감»이 된다 (그 모양).
 *
...
export interface SheetStatus {
    ...
    notice: string | null;
    /**
     * 🚩 **출발 조각** — 어느 경우든 붙는다 (기사님: *"박스는 지우고 내용은 시트 현황 바에 넣어줘 (몇분 지각 / 몇시 출발)"*).
...
/**
 * 🚩 **출발 조각의 말** — 늦었으면 «N분 지각», 아니면 «HH:MM 출발» (기사님).
...
function sheetStatusCase(i: SheetStatusInput): Omit<SheetStatus, 'due'> {
     ...
     * 🗓️ 사이클 = 하루라 콜 사이 빈 차마다 «사이클을 마쳤습니다»가 뜨면 거짓말이다 (화면규칙 E12).
     */
