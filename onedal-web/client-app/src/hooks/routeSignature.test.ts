...
/**
 * 🧬 **경로가 바뀌었나 — 무엇으로 보느냐에 따라 틀린다**.
 *
 * ── 왜 이 검사가 있나 ──
 * 모의 주행은 «지금 달리는 경로»의 인덱스를 들고 걷는다. 경로가 갈리면 인덱스를 다시
 * 잡아야 하는데, **«갈렸나»를 무엇으로 보느냐**에 따라 이렇게 틀린다:
 *
 *   ⓐ `length` 만      → 점 수가 같은 다른 경로에 **앞 경로의 인덱스**를 써 **10.9km** 뛴다
 *   ⓑ 참조(`!==`)      → `sync-active-orders` 마다 새 배열이라 **매 틱 다시 잡아** 걸음이 끊기고 경로를 잘못 돈다
 *   ⓒ 양끝 + 길이      → 느슨하다. **콜이 끝나 경로가 다시 짜였는데 그 셋이 같으면** 또 뛴다
 *                        (한 틱에 10,916m)
 *
 * 🔴 **중간을 봐야 한다.** 같은 두 점을 잇는 길은 여럿이고(콜이 빠지면 경유가 바뀐다),
 *    양끝만으로는 그 차이가 안 드러난다.
 * 🔴 **참조가 아니라 값으로 가른다** — 값이 같으면 같은 지문이어야 한다 (ⓑ).
 */
...
describe('경로 지문 — 갈렸는가', () => {
    ...
    /**
     * 🔴 **이 한 건이 ⓐ·ⓒ 가 놓치는 것이다** — 양끝과 점 수가 같은 다른 길.
