...
/* 📍 구독은 현황판 쪽에서 건다 — 지도는 이 값을 아직 안 쓴다 (아래 🗑️ 주석) */
...
export function useRouteDerivations(
    ...
    /**
     * 🔄 타임라인·카운트다운의 재료는 여섯 단계 행에서 읽는다 (기사님 승인).
     */
    ...
    /**
     * 🧭 **경로를 든 콜 — 추측하지 않는다** (기사님 확정).
     *
     * «진행 중 콜 중 폴리라인 가진 마지막 것»으로 **추측**하면 서버의 판정
     * (`buildOrderSync` 의 holder)과 규칙이 달라, KEEP 직후처럼 둘이 갈리는 순간에
     * **직전 콜의 지난 선**을 그린다(잔상). 그래서 서버가 준 이름을 쓴다 (규칙 ③).
     ...
     *    목적지 상태가 `routed` 가 되고 확정콜의 마지막 하차지가 후보 하차지로 바뀌어, 잡지도 않은 콜 때문에
    ...
    const mockStops = useMemo(() => {
        /**
         * 🔴 **목록에서 빼지 않고 «방문»을 표시한다**.
         *    다녀온 정거장을 목록에서 빼면, 시뮬이 감속하며 다가가는 사이
         *    서버 도착 감지(500m)가 먼저 찍혀 **목표가 눈앞에서 사라진다** — 정차 연기가
         *    안 밟힌다. 다녀온 곳을 안 가는 것은 시뮬 내부 장부(simStep.visited,
         *    가동 시 이 표시를 옮겨 받는다)가 지킨다 — 판단은 여전히 hasVisitedStop 하나다.
         */
    ...
    /**
     * 🔄 **모의 주행이 끝나도 화면은 집으로 돌아가지 않는다** (기사님 지시).
     *
     * 서버는 가상 좌표를 지우지 않는다 — `originOf` 가 물을 때마다 고르고,
     * **콜을 쥔 동안에는 그 자리를 그대로 기점으로 쓴다.** 화면만 집으로 가면 두 곳이
     * 다른 말을 하므로, 화면도 그대로 두는 것이 «같은 말»이다 (규칙 ③).
     */
    useEffect(() => {
    ...
    /**
     * 🗑️ **지도는 «서버가 아는 자리»를 따르지 않는다** (기사님 지시).
     *
     * 🔴 **위치 하나를 여섯이 읽는다** (지도·영역 계산·도착 감지·지나온 구간·궤적·경로 기점).
     *    하나를 건드리면 여섯이 흔들린다. 봉투(`myPosition`)를 따르면 봉투가 «콜이 바뀔 때»만 와서
     *    지난 좌표가 내 점을 뒤로 당겨 모의 주행이 멈추고, 가벼운 이벤트(`driver-position`)를
     *    따르면 영역 재계산이 걸린다 — 이 값은 `StageView` 의 «상차» · «하차» 레이어도 먹는다.
     *    바로잡으려는 것(«PC 지도가 집을 가리킨다»)은 운행에 당장 지장이 없으므로 잇지 않는다.
     *
     * ── 지금 상태 ──
     * 서버는 **준비된 채 조용하다** — `driver-position` 을 쏘고 봉투에 `routeOrigin` 을
     * 싣지만 **지도는 안 듣는다.** 현황판만 «서버가 아는 내 자리»를 **보여준다**(읽기 전용).
     * 🔴 **다시 이을 때는 화면에서 그 값을 하루 보고 나서** 잇는다 — 검증 없이 지도에 이으면
     *    위 여섯이 함께 흔들린다.
     */

    /**
     * 🖥️ 다음 정거장에 가까워지면 그 콜 화면으로 (기사님).
     ...
     * 카드(단계 화면)도 봐야 한다 — 따로 만들면 덱과 시트가 다른 시각을 말한다.
     */
    ...
    const routePointsRaw: RoutePoint[] = useMemo(() => {
        ...
        for (const st of routeStops) {
            ...
            /**
              * 🚏 다녀온 정거장은 여기서도 뺀다 (기사님 실측 — 안 빼면 숫자가 하나씩 밀린다).
              *    도착 직후 서버 재계산 전까지 routeStops 에 남아 있어, 발자취(✅번호)와
              *    남은 목록에 **이중으로** 세어진다. 표시는 발자취가 이어받는다.
              */
        ...
        for (const r of liveRoute) {
            // 🚏 다녀온 정거장은 폴백에서도 되살리지 않는다 (기사님 실측) — hasVisitedStop 하나
    ...
    /**
     * 🕐 콜별 상하차 예상 시각 — 재료는 타임라인 하나다 (기사님 질문).
     * sectionEtas 를 직접 쓰면 정차가 빠져 한 화면이 두 시각을 말한다.
     */
    ...
    /**
     * 👣 **지나온 발자취 — 오늘 하루 남는다** (기사님).
     *    다녀온 정거장은 경로·순번에서 빠지는 게 맞지만(다시 안 간다), 화면에서
     *    통째로 사라지면 «내가 어디를 돌았는지»를 잃는다.
     *    방문 시각(arrivedAt)순으로 ✓1 ✓2 … 를 단다. 취소·방출은 없던 일이라 안 남는다.
     *
     * 🔴 **셈은 `visitedSequenceOf` 한 곳이다**.
     *    번호를 세는 데 **좌표를 요구하면**, 이력만 남아 좌표가 빈 렌더에서 목록이
     *    통째로 비어 **번호가 줄어든다**.
     ...
     *
     * ── 다른 두 규칙이 안 되는 까닭 ──
     * ① «지금 남은 목록의 몇 번째»로 매번 새로 세면, 서버가 경로를 다시 세울 때마다
     *    남은 것들의 자리가 바뀌어 번호가 뛰고, 다녀온 정거장을 목록에서 빼는 순간
     *    **그 콜의 번호가 통째로 사라진다.**
     * ② «출발하면 얼린다»로 하면 흔들림은 멎지만, 합짐이 **출발 뒤에** 들어오면
     *    자기 상차·하차를 **끝에 이어 붙여** 실제로는 `1 → 3 → 2 → 5 → 4 → 6` 순서로 다니게 된다 —
     *    번호가 안 바뀌는 대신 **순서를 못 말한다.** 값이 자기 질문에 답하지 못한다.
     *
     * ── 지금 규칙 ──
     ...
     *    이름표와 지도가 어긋난다 — 캔버스가 따로 세면 그렇게 갈린다.
     */
    ...
    const callColors = useMemo(() =>
         ...
         *    색 배열을 따로 두면 **같은 콜이 지도와 목록에서 다른 색**이 된다.
         *    마커·박스·선이 **같은 콜 번호에서 같은 색상**을 본다 («색 = 콜 번호»).
     ...
     *    스스로 세면 이름표와 다른 답을 한다 (이름표 «1. 곤지암읍» ·
     *    마커 «2 곤지암읍» 처럼). 세는 곳은 stopNoOf 하나다 (규칙 ③).
     */
    const unifiedRoutePoints: RoutePoint[] = useMemo(
        () => routePointsRaw.map(p => ({
            ...p, no: p.routeId
                ...
                : undefined,
            /**
             * 🌈 **몇 번 콜인가** — 색상(hue)이 이걸로 정해진다.
             * 🔴 정거장 번호(`no`)와 **다른 값**이다 — 콜 하나가 정거장 둘을 갖는다.
             * ⚠️ 안 실으면 캔버스가 조용히 한 색 문법(상차 초록·하차 로즈)으로 떨어진다.
             */
    ...
    /**
     * 📡 **여유와 번호를 로그로 남긴다** (기사님 지시 — *"눈으로 확인하지 말고
     *    로그로 확인하자"*). 관제웹 로그는 서버로 중계되므로, 운행이 끝난 뒤 GPS 궤적과
     ...
     *
     * ── 왜 ──
     * 번호는 **두 목록을 이어 붙인 것**이다 (다녀온 것 → 남은 것). 로그가
     * 완성된 문자열 하나만 남기면, 흔들릴 때 **어느 쪽이 흔들렸는지 못 가린다.**
     * 흔들림은 이런 두 모양으로 보인다:
     *
    ...
    useEffect(() => {
        ...
        /* 🕐 «다녀온 것»의 정렬 열쇠 — 이 값이 `null` 이면 맨 뒤로 간다 */
     ...
     * 봉투가 둘로 갈라져 오는 때(부팅 직후 두 번 푸시 · 후보콜 선점)에서 한쪽이 늦게
     ...
     * 📍 이 계측의 코드는 아래 «경로재료» 효과 다음에 있다.
     */
    /**
     * 🔬 **계측 — 「그릴 재료가 있나」** (기사님 실측: *"새로고침하고 나면
     *    경로가 사라져 있어"*).
     *
