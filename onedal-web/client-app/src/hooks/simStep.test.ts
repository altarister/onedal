 ...
 * 목적이다 — 등속으로 정거장을 순간 통과하면 정차 상태가 한 번도 안 나온다.
 */
...
describe('모의 주행 연기 — simStep', () => {
    ...
    it('🔴 지나온 점을 하나도 건너뛰지 않는다 — 카카오 곡선이 직선으로 펴지던 자리', () => {
        /**
         * 한 틱에 점을 배속만큼 건너뛰면(`st.idx += 배속`) 밟지 않은 점은 궤적에 남길 수 없어,
         * 카카오가 준 곡선이 **직선 토막**으로 그려진다.
         * 그래서 목업과 같은 `driveStep` 이 점을 다 밟고 `via` 로 돌려준다.
         */
        const st = initialSimState();
    ...
    /**
     * 🎭 **연기 눈금을 돌리면 시뮬이 따른다** (기사님 지시:
     *    *"모의 주행의 정차시간, 서행하는 거 오른쪽 어드민에서 설정하면 좋겠는데"*).
     *
     * 🔴 인자로 받게만 해 두고 **안 쓰면 조용히 기본값으로 돈다** (설정값이 클로저에 갇히면
     *    그렇게 된다). 그래서 «돌리면 정말 달라지는가»를 문다.
     */
    ...
    /**
     * 🔴 **멀리 있는 정거장으로 순간이동하지 않는다** (기사님 실측).
     *
     * ── 왜 거리도 보나 ──
     * «이번 걸음이 지나친 정거장»을 **인덱스만으로**(`nearestIndex` 가 `[from, to)` 안에 드는가) 고르면,
     * **새 콜이 들어와 그 상차지가 `stops` 에 붙는 순간** 그 정거장의 최근접 인덱스가 우연히
     * 그 구간에 들 때 **실제로 10km 밖인데도** 「지나쳤다」고 보고 `loc = due` 로 **그 자리에 찍는다.**
     * 그래서 거리(닿는 거리 `reach`)로도 거른다.
     *
     ...
     *
     * 닿는 거리를 `stepKm + 이탈폭` 으로 두면 **1배속에서 0.74km ·
...
describe('대기 뒤 새 경로 — 서 있던 자리에서 출발한다 (#133 개정 · onedal-49 검토)', () => {
    ...
    /**
     * 🔴 **정거장이 한 순간 비어도 되감기지 않는다**.
     *    정거장 목록이 잠깐 비었다고(합짐 선점 때 관제웹 목록이 0.2초 빌 수 있다) 걸음 상태를 통째로 0 으로 돌리면
     *    경로 앞으로 되감겨 이미 들른 상차지에서 또 선다. 그래서
     *    «빔»은 **들른 장부만** 비우고(시험마다 좌표가 같은 문제) 자리(idx·at)는 둔다.
     *    다시 채워지면 서버가 «들렀다»고 한 정거장을 장부에 옮겨 적는다.
     */
