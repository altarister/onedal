 ...
 *
 * 후보콜이 판정에 들어오면 서버가 후보를 넣은 합짐 미리보기 선(`OrderEvaluator` 의 `merged.polyline`)을 싣는다.
 * 모의 주행이 그 선으로 갈아타면 좌표가 멀리 튀고, 서버가 다음 틱을 «하차지를 떠났다»로 읽어
 * 확정 콜을 가짜로 하차 완료한다. 실 GPS 는 선을 안 따르니 시험 도구만 막는다.
...
/**
 * 🅿️ **콜이 0건이 되어 선이 사라져도 켜 둔 모의 주행은 그 자리 좌표를 계속 낸다**.
 * 선이 사라졌다고 좌표를 끊으면 서버가 «돌고 있는 모의 주행이 아니다»로 보고 심사 기점을 집 주소로 잡아,
 * 서 있는 차의 심사 경로가 집에서 출발하는 것으로 계산된다.
 */
