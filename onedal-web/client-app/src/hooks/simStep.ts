...
/**
 * 🎭 **모의 주행의 걸음 하나 — 순수 함수** (기사님 확정).
 *
 * 화면 상태 기계는 **시간 조건**(정차 5km/h↓ 10초 · 주행 20km/h↑ 10초)으로 움직이므로,
 * 등속으로 정거장을 순간 통과하는 시뮬에서는 정차 상태(S2·S7)가 **한 번도 안 나온다**.
 *
 * 그래서 걸음에 «연기»를 넣는다:
 ...
 * 순수 함수라 폰·타이머 없이 검사된다 (`hooks/simStep.test.ts`).
 */
...
/**
 * 🚏 **정거장 목록이 바뀌었을 때 걸음 상태**.
 *
 * · 비었으면 **들른 장부만** 비운다 — 시험마다 좌표가 같은 문제지에서 지난 시험에 들른 곳을 «들렀다»로 보지 않게.
 *   🔴 자리(idx·at)는 두고 간다. 통째로 `initialSimState()` 로 되돌리면 합짐 선점 때 목록이 잠깐 빈 틈(#137)에
 *   경로가 되감겨 수 km 를 뛰고 들른 상차지에서 또 선다.
 * · 채워졌으면 서버가 «들렀다»고 한 정거장을 장부에 옮겨 적는다 — 목록이 올 때마다.
 */
...
/**
 * 🛣️ **한 틱(1초)의 기본 걸음(km)** — 배속을 곱한다.
 *    `idx += 배속` 으로 **폴리라인 점을 건너뛰면** 카카오 곡선이 직선으로 펴진다
 *    (기사님: *"궤적이 엉망이야"*). 그래서 목업과 **같은 함수**(`lib/driveStep`)로 거리를 간다.
 */
...
/**
 * 🎭 **연기 눈금 — 시뮬이 «어떻게 달리는가»** (기사님 지시).
 *
 * 기사님: *"모의 주행의 정차시간, 서행하는 거 오른쪽 어드민에서 설정하면 좋겠는데..
 * 코드에 상수로 들어가 있는 거지?"* — **현황판이 돌릴 수 있게** 인자로 받는다.
 *
 ...
 *  (기사님 «정차가 너무 길다»). 모의 좌표의 도착은 500m 안이면 곧바로라
 ...
 *
 * ⚠️ **근거는 표본 둘뿐이다** — 곤지암 물류센터 **601m**와 **525m**.
...
export function simStep(
    ...
    stops: PolylinePoint[],
    multiplier: number,
    /** 🎭 연기 눈금 — 안 주면 기본값 (`DWELL_TICKS`·`APPROACH_KM`·¼) */
    ...
    /**
     * 🔇 **인덱스로는 걸렸는데 너무 멀어 못 밟은 정거장** (기사님 지시).
    ...
    /**
     * 🚗 **걸음은 목업과 같은 함수가 낸다** (`lib/driveStep`).
     ...
     *
     * 🔴 **인덱스만으로는 «먼 것»과 «지나친 것»을 못 가른다** (기사님 실측 — 한 틱에 **10,916m**).
     *
     ...
     *    보고 `loc = due` 로 **그 자리에 찍는다.** 방아쇠는 경로 갈아타기가 아니라
     *    **새 정거장**이라, 경로 지문(`routeSignature`)을 조여도 안 잡힌다.
     *
     * 🟢 **그래서 실제 거리도 함께 본다** — 이번 걸음으로 닿을 수 있는 거리 안이라야
     *    «지나친» 것이다.
     * 🔴 **정거장 좌표를 찍는 동작 자체는 필요하다.** 물류센터가 도로에서 떨어져 있어
     *    (실측 곤지암 **601m**) 그게 없으면 도착 감지가 영영 안 걸린다.
     ...
     *
     * ⚠️ `stepKm + 이탈폭` 만 보면 **1배속에서 0.74km · 서행이면 0.71km** 로
