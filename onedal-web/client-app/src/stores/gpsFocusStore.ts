...
/**
 * 🖥️ **근접/도착 포커스 — 구독은 모듈에서 한 번**.
 * 훅 안에 socket.on 을 두면 훅을 쓰는 컴포넌트 수만큼 구독이 늘어난다 —
 * judgmentStore 와 같은 패턴으로 여기서 한 번만 건다.
 */
/**
 * 🔴 **kind 는 «무엇이 일어났나»다 — «무엇을 하고 싶나»가 아니다**.
 *    KEEP·마커 탭이 덱만 옮기려고 `arrive` 를 빌려 쓰면, 도착 효과가 그걸
 *    진짜 도착으로 읽어 규칙을 두 번 먹이고 로그에 «도착»이라는 거짓 사유를 남긴다.
 *      approach — 근접 예고 (덱만 따라간다)
 *      arrive   — 정거장 도착 (시트가 마중 나간다)
 *      focus    — 사람이 골랐다 (덱만 옮긴다 · KEEP·탭)
 */
/**
 * 🪜 **focus 는 «콜»이 아니라 «콜과 단계»다.**
 *
 * 🔴 서버는 `auto-arrived` 에 `stopType` 을 싣는다 — 여기서 버리면 도착해도 시트가
 *    «어느 단계를 열지» 몰라 기사님이 할 일이 없고, 손으로 내리시면 그 손이 30초 유예를 걸어
 *    **다음 도착 마중까지 먹는다.**
 */
interface GpsFocus {
    orderId: string;
...
/**
 * 🏁 **방금 도착한 정거장** — 시트가 마중 나갈 자리.
 *
 * 🔴 **`gpsFocus` 와 답하는 질문이 다르다 — 한 칸에 넣지 않는다** (규칙 ⑤-4 ⑤).
 *      · `gpsFocus` «지금 보는 콜»   — 덱이 따라간다 (근접·탭·KEEP 이 바꾼다)
 *      · `arrival`  «방금 도착했다»  — 시트가 마중 나간다 (도착만 쓴다)
 *    한 칸으로 겸하면 도착 직후 다음 정거장 근접(approach)이 같은 칸을 **덮어써** 시트가
 *    읽기도 전에 사라진다. 소켓을 따로 들어 가르면 **듣는 곳이 둘**이 되어
 *    «도착이 가리킨 콜»과 «시트가 연 콜»이 갈라진다.
...
export function ensureGpsFocusSubscribed() {
    ...
    subscribed = true;
    // 예고(2km)와 도착은 다른 일이다 — 예고는 카드만 따라가고, 도착은 시트가 마중 나간다
