 ...
 * 라이브에서는 폰의 GPS를 쓸 거야."*
 *
 * 🔴 **저절로 시작하지 않는다 — 버튼이 주인이다.** 조건이 맞으면 자동으로 돌게 두면
 *    끄는 길이 없어 «그만 보고 싶은데 계속 도는» 상태가 되고, 속도를 주소창 `?speed=` 로만
 *    바꾸면 운전석에서는 손댈 수가 없다.
 *
 ...
 *
 * ⚠️ **목업의 «모의 시계»는 쓰지 않는다** (기사님 확정). 목업 주행은 서버를
...
/* 🐢🚗🚀 보통은 3배(≈1,080km/h · 이천 30km 를 1분 40초) — 기사님 «정차가 너무 길고 속도가 너무 빠르다».
   정차 기본은 12초(`DWELL_TICKS`) — 모의 좌표의 도착은 500m 안이면 곧바로라(`evaluateArrivalTick`) 정차가 도착을 막지 않는다. 15배는 🚀 빠름이다. */

/** 🐢🐇 주소창 `?speed=` 는 **첫값으로만** 산다 — 그 뒤로는 버튼이 정한다 (주소창 링크도 계속 쓸 수 있다) */
...
export const useMockDriveStore = create<MockDriveState>((set) => ({
     ...
     *    하차가 끝나 경로가 잠깐 0점이 될 때 여기서 `running` 까지 끄면, 경로가 몇 초 뒤 돌아와도 차가 선다.
     *    엉뚱한 좌표는 `useMasterGps` 의 `useMock = canMock && running` 이 막는다 — 경로가 없는 틈에는 안 달리고,
     *    돌아오면 서 있던 자리에서 가장 가까운 점부터 이어 달린다. 🔴 **끄는 것은 기사님뿐이다** — 경로 끝에서도 안 끄고 그 자리에서 대기한다.
     */
