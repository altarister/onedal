 ...
 *
 * 🔴 **구간 배열이다**. 점 하나짜리 평평한 배열이면
 *    GPS 가 끊겼다 이어진 자리, 시뮬을 다시 켠 자리, 콜이 바뀌어 딴 데서 시작한 자리가
 *    **한 줄로 이어져** 지도를 가로지르는 긴 직선이 생긴다. 쌓는 규칙은 목업과 **한 벌**이다
...
/** 오늘 덱이 비면(자정에 어제분이 빠지면) 자취도 접는다 — 어제 자취가 오늘 지도에 살아나지 않는다 (규칙 ③ · 사이클 = 하루) */
export const clearDrivenTrail = () => useDrivenTrailStore.setState({ segments: [] });

/**
 * 👣 **새로고침·재기동 뒤에도 오늘 달린 자취를 다시 그린다** (기사님:
 *    *"카카오라인과 내 궤적이 같이 있어야 얼마나 잘못갔는지 확인할 수 있을 것 같아"*).
 *
 * 🔴 **이 스토어는 메모리에만 산다** — 위 구독이 `local-gps-update` 를 들어 쌓을 뿐이라,
 *    새로고침하면 `segments: []` 로 시작해 **겹쳐 볼 자취가 아예 없다.**
 *    장부(`gps_tracks`)에는 그날 점이 다 남아 있으므로 거기서 읽어 온다.
 *
 ...
 *
 * 🔴 **한 번만 읽는다** — 그릴 때마다 다시 물으면 라이브로 쌓은 점을 덮는다.
 * 🔴 **라이브 점이 이미 있으면 손대지 않는다** — 모의 주행 중에 새로고침한 경우가 아니라면
 *    비어 있고, 비었을 때만 씨를 뿌린다.
 */
...
export async function restoreDrivenTrail(orderIds: string[]): Promise<void> {
    ...
    try {
        /**
         * 🔴 **`apiClient` 는 쓰는 자리에서 들인다**.
         *
         * 그 모듈은 불려 오는 순간 `window.location.origin` 을 읽는다(`serverTarget`).
         * 최상단에 두면 **이 스토어를 import 하는 검사 파일이 `window` 가 없는 곳에서
         * 통째로 죽는다** — `Tests:` 숫자는 멀쩡한데 스위트 하나가 «없는 것»이 된다
