...
- **두 그릇은 그대로다. 그것을 가르는 것이 화면이 아니라 버튼으로 바뀌었다** (C4-10 · 2026-09-12).
  · 🔍 필터에서 손대면 → `activeFilter` (메모리, 자정에 되돌아감 = **오늘만**)
  · `💾 서버 저장` 을 누르면 → `baseFilter` 까지 (DB, 매일 아침 여기서 시작)
  · `↩︎ 되돌리기` 는 세 번째 저장이 아니라 **서버 값으로 되돌리는 것**이다
  · ⚙️ 설정의 노선·반경 편집은 철거됐다 — 편집 자리는 🔍 필터 하나
  🔴 «저장 안 함»은 깃발(`dirty`)이 아니라 `baseFilter` 와 견주는 **파생**(`unsaved`)이다 —
  켜는 건 안 잊는데 끄는 걸 잊어서 화면이 거짓말했다
  🔴 2026-08-12 에 `updateFilter` 가 **항상** baseFilter 에도 낙관적 반영을 해서
  새로고침하면 두 값이 갈라졌다 (*"설정에는 파주인데 필터를 열면 용인"*) — 지금은 `saveAsDefault` 일 때만
...
- 🔴 **"Should have a queue" / "change in the order of Hooks" 는 대개 코드 버그가 아니다.**
  ...
  (2026-09-05). `MockHeader()` 를 `MockHeader({ st })` 로 바꿨더니 `st` 가 `undefined`
  로 들어왔다 — Vite 가 모듈만 갈아끼웠는데 **props 없이 부르던 옛 부모가 화면에
  남아 있었다.** 소스·`tsc`·검사·프로덕션 빌드가 **전부 멀쩡했다.**
  주소에 붙은 `?t=1788591408758` 이 그 표시다 — HMR 로 갈아끼운 모듈이다.
