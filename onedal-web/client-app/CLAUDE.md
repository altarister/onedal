...
- 🔴 **갈래를 가르는 값은 «부르는 곳이 둘 이상일 때»만 만든다.** 하나뿐이면 갈래를 안 만든다 —
  쓰던 쪽이 떠나도 갈래는 안 따라가서, **한 번도 안 그려지는 자리를 고치며** 시간을 쓰게 된다.
  `variant='deck'|'list'` 가 그랬다: 조회 탭이 서랍으로 옮겨 간 뒤 `'list'` 갈래만 남았고,
  그 안의 머리줄을 고쳐도 화면이 안 바뀌었다 (`document.querySelectorAll` 이 빈 목록이었다).
  🟢 쓰는 곳이 하나면 **그 하나를 그대로 적는다** — 삼항도 깃발도 두지 않는다

- 🔴 **소스 글자를 읽는 검사는 «그 글자가 실제로 그려지는 파일»을 읽는다.**
  부품이 옮겨 가면 **검사도 함께 옮긴다** — 안 옮기면 검사는 초록인데 화면에는 없다.
  하루에 둘이 그랬다: 「밀린 분(−5분)」은 펼친 카드로, 「취소 시각」은 서랍으로 갔는데
  검사는 덱과 옛 머리줄을 보고 있었다. **검사가 «화면에 있다»고 말하는데 없는 것**이 가장 나쁘다 —
  아무도 안 보고, 그 값을 되살릴 계기도 안 생긴다
