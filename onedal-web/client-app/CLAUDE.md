...
- **필터 두 그릇을 가르는 것은 화면이 아니라 버튼이다** (그릇 자체는 [server/CLAUDE.md](../server/CLAUDE.md)).
  ...
  · 편집 자리는 🔍 필터 하나다. ⚙️ 설정에서는 목적지와 반경을 고칠 수 없다.
    ⚙️ 단가 탭의 «반경 기준거리»는 자동 반경을 얼마나 줄일지 정하는 기준이지, 반경 그 자체가 아니다
...
- **직접 갈래(MANUAL·MANUAL_CLICK) 콜에는 KEEP/CANCEL 버튼을 띄우지 않는다**
  ...
  체험 콜은 예외다. 결재 연습이 목적이라, 직접 갈래 콜이나 미리보기 콜이어도 버튼을 띄운다(`route.isSimulated`).
  🔴 `=== 'MANUAL'` 문자열 비교를 새로 쓰지 말 것 — type 이 두 표기(확정 전 `MANUAL_CLICK`)라
  한쪽을 빠뜨려 배지 누락·버튼 잔상이 생긴다

- **기사님의 취소 버튼은 `PinnedRouteCard` 의 «⋯ 이 콜 처리» 안에 일부러 접어 뒀다** (콜을 버리는 것은 기사님만 — README 규칙 ①)
...
- **접속 주소와 `.env` 주의사항은 루트 [README.md](../../README.md) 「무엇이 실제로 돌고 있는가」에 있다** (한 곳에만 둔다)
