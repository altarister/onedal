/**
 * 🎛️ **콜 옵션 — 화면의 선택지와 그 값이 사는 한 곳**
 *
 * 기사님: *"콜 옵션이 모두 들어가 있는 디폴트 값을 정의하는 테이블을
 * 만들 스키마를 만든다. 타입스크립트로도 만든다."*
 *
 * 🔴 **왜 필요한가** — 화면의 짐 관련 값(단위 · 환산 · 차종 정원 · 상하차 방법 · 보호 · 후작업 · 성질)이
 *    코드 상수면 기사님이 못 바꾸신다. 그 값이 기사님이 1~2초에 누르는 색을 정한다 (규칙 ⑤-4 ①).
 *
 * 🔴 **지금 이 표에서 읽는 값은 정차 값뿐이다** — 상하차 방법의 박스당 분 · 후작업 분 (`dwellRatesOf`).
 *    나머지(단위 · 환산 · 차종 기본 짐 …)는 화면·판정이 아직 코드 상수로 돈다(`defaultCargoByVehicle` 등).
 *    하나씩 옮기되, 옮길 때마다 값이 같은지 먼저 확인한다
 ...
 *    컬럼이 크게 겹치고, 갈래가 하나 늘 때마다 스키마가 늘어난다. 갈래는 `category` 칸으로 가른다.
 */
 ...
 *
 * → 그래서 «화면의 기본값이 모두 맞는가»는 **저절로 보장된다.** 남는 확인은 *"빠진 항목이 없는가"* 하나뿐이다.
 */
...
/**
 * ⏱️ **정차 값의 원천은 이 표다** (기사님 확정)
 *
 * ── 왜 여기인가 ──
 *
 * 「지게차 박스당」·「수작업 박스당」·「검수 분」은 **화면의 칩에 붙는 숫자**다 — 통화 시트가
 * 「수작업 10분」·「검수 60분」이라고 그리는 그 값. 판정은 그걸 **쓰는** 쪽이지 정하는 쪽이 아니다.
 * 그래서 판정 기준 탭이 아니라 이 표의 칸(`handling.num2` · `afterwork.num1`)에 둔다 —
 * 두 그릇에 담으면 갈라진다 (규칙 ③).
 *
 * ```
