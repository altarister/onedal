 ...
 * 🧪 **그물 셋업의 계산이 ⑭ 검산과 같은가**
 *
 * 왜 있나: 동선 사각형은 **실시간으로 계산**한다. 검산 값(⑭)은 스크립트로 잰 실측이라,
 * 수식이 어긋나면 **지도와 표가 같이 조용히 틀린다** — 그래서 검산의 숫자를 여기 못박는다.
 * 수식을 고치면 이 검사가 먼저 빨간불이어야 한다.
 * ⚠️ 집 좌표는 실좌표다(폴리곤 평균보다 남서 2.6km) — 꼭짓점이 그만큼 밀리면
 * 동 수가 48 ↔ 53 으로, 부발읍이 «1° 탈락» ↔ **진입(21.7°)**으로 갈린다.
 */
 ...
 * 서버 판정(`makeInNet` · `excludeSrc`)도 같게 센다 — 현위치 원을 **OR 로 더하면** 같은 «하차 영역»을
 * 두 곳이 다르게 세어 화면과 목록이 갈라진다.
 *
 * 🔴 **상차 판정은 그대로 둔다** — 상차는 현위치 원이 있어야 한다 (`judgeTwoStage`).
 *    도려내기는 하차 두 길(`netForGoal` · `netAreaTesterOf`)에서만 켠다.
 */
describe('🔵 하차 조각 — 현위치 원은 도려낸다', () => {
    /** 현위치 원이 20km 로 분명히 있는 설정 — 0 이면 도려낼 것이 없어 검사가 헛돈다 */
...
describe('대기 프리셋 — «여주를 목적지로 느긋하게» (⑭ 검산 2026-09-07)', () => {
    ...
    it('🔴 부발읍이 든다 — 목적지각 21.7° ≤ ±25° (집 좌표 교정으로 «1° 탈락»이 뒤집혔다)', () => {
        // 폴리곤 평균 좌표(산자락)로 재면 26°라 1° 차이로 빠진다. 실좌표면 축이 돌아 3° 여유로 진입 —
     ...
     * 코드가 `region.startsWith('서울')` 로 몰래 빼면 화면 어디에도 안 보인다 —
     * 그래서 ⑭ 의 판단을 **«미리 눌린 제외지역»**으로 둔다.
     */
...
describe('곤지암 판 — 곤지암성당(하차지) → 여주 (기사님 확정 2026-09-07)', () => {
    ...
    it('🔵 부발읍이 이 판에서는 든다 — 하차지 기점으로 축이 옮겨져 목적지각 19°', () => {
        // 초월(집) 기점에서는 목적지각 26° 로 1° 차이로 탈락하는 곳이다 (위 대기 검사)
...
describe('관내 국면 — 목적지 원 안 + 출발지 원 밖이면 방향을 안 본다 (기사님 2026-09-07)', () => {
    it('여주 시내에 도착하면 관내다 · 출발 전 초월은 관내가 아니다', () => {
        ...
        // 복귀(목적지=집): 아침의 집은 목적지 원 안이지만 출발지 원 안이기도 하다 → 관내 아님
...
/**
 * 🏘️ **«관내로 쟀는가»는 잰 쪽이 말한다** (기사님 지적).
 *
 * 화면이 «관내 모드인가»만 보고 배지와 칩을 띄우면 거짓말을 한다 —
 * 복귀로 목적지가 접혀 관내 규칙이 안 도는데도 «둘 다 원 안만»이라고 적고,
 * 「내 위치 반경 밖」인 것을 「상차 **원** 밖」으로 읽히게 한다.
 */
/**
 * 📐 **마름모 반경 — 축에서 좌우로 몇 km** (기사님 확정).
...
describe('📐 마름모 반경', () => {
    ...
    /**
     * ✏️ **그리는 모양도 같은 셈법이어야 한다** (기사님 실측 *"작동 안 해"*).
     * 두 광선의 교점으로 그리면 각도 180 에서 **평행이라 무한대**로 날아가 지도가 통째로 튀고,
     * «마름모 반경»도 안 봐서 판정만 잘리고 그림은 안 잘린다.
     */
    ...
    /**
     * 🔴 **180 을 넘으면 축 뒤쪽까지 담긴다 — 그림도 따라와야 한다** (기사님 확정:
     * *"180도 이상은 더 그리지 않는 이유는 뭐야?"* — 그림만 안 커지고 **판정은 커지면** 둘이 갈린다).
     */
...
describe('라인 띠 — 길 하나가 담는 동 (2026-09-07 카카오 실측 · 수식은 lineZoneOf 로 옮겼다)', () => {
    ...
    // 🔴 길은 이름으로 찾는다 — «모든 길» 재생성(저녁)으로 개수·순서가 바뀔 수 있다
    /* 🔴 띠는 **길 시작점 뒤를 자른다** — 뒤쪽 반원의 광주시 4동은 안 든다 (39동) */
...
describe('노선 — «라인 ∪ 남은 마름모» (기사님 확정 2026-09-09)', () => {
    ...
    /**
     * ✂️ **하차 영역은 «− 현위치 원»이다 — 라인이 있어도** (기사님 확정).
     *
     * 기사님이 그린 그림: *"내 주위에 녹색이 있고, 파랑과 접경에 보라색 지역이 있고,
     * 그 이후 목적지 방향으로 파란 점이 있다."* 곧 **내 위치 원 안은 상차만**이고,
     * 원 경계에 걸친 동만 «둘 다»이며, 그 밖이 하차다.
     *
     * 🔴 라인이 없을 때는 `buildNet(..., excludeSrc=true)` 가 도려낸다. 라인이 생기면
     *    `lineZoneOf` 로 갈라지므로 거기서도 도려낸다 — 안 도려내면 **운행 중에만** 상차가 하차에
     *    통째로 잠기고(상차 동이 전부 «둘 다», «상차만» 0곳) 싣는 자리에 그대로 내리는 콜이 통과한다.
     */
    ...
    it('🔴 마지막 하차지 둘레에는 원을 안 두른다 — 평촌 남쪽 6km 는 지나온 뒤라 안 담는다', () => {
        // 기사님 지적: *"중간 기착지인 평촌동도 점선 라인과 영역에 지역들을 가지고 있는데 이걸 빼야 해"*
        // 🔴 자리를 **꼭짓점 원 안**(반경 7.5km)에 잡아야 이 검사가 뜻을 갖는다 — 원 밖에 잡으면
        //    원을 두르든 안 두르든 거짓이라 **아무것도 안 잡는다**.
     ...
     * 띠 반경만큼 통째로 하차 후보가 된다. 상차 · 지도 · 하차 목록이 같은 `aheadOf`·`isAheadOf` 로
     * 자른다(#151) — 목록만 안 자르면 **점은 있는데 칠은 없는** 자리가 생긴다.
     */
    ...
    /**
     * 🔴 **라인은 목적지에서 나오지 않는다 — 잡은 콜들에서 나온다** (기사님 지적).
     *
     * 화면에서 잡으셨다: *"복귀콜로 집에 가는 중인데 이 모습은 첫짐의 동선과 같다.
     * 노선의 동선이 되어야 할 것 같은데."* 라인을 **노선 목적지에만** 걸면,
     * 복귀콜을 잡아 목적지가 «집»으로 접히는 순간 **라인이 통째로 빠지고** 넓은 마름모만 남는다.
     */
    ...
    /**
     * 🔴 **이 갈림은 화면 밖 `netForGoal` 에 둔다** — 화면 안에 있으면 아무 검사도 못 본다.
     * 여기서 잠근다.
...
describe('판(목적지) 그룹 경로 — 콜마다 잡을 당시 목적지를 기억한다 (기사님 설계 2026-09-08)', () => {
     ...
     * 점에서 공백을 줄이는 좋은 방법."* 복귀 콜들을 잡은 순서대로 뒤에 붙이면
     * 장암동(북) → 광주(남) → 상계동(북) → 매산동(남) 요요가 나온다 (실측).
     */
    ...
    it('🔴 실측 사고 그대로 — 요요가 없다: 장암(북) 뒤에 광주로 되내려오지 않는다', () => {
        ...
        // 83.4km — 목적지별로 갈라 넣는 순서(83.7km)보다 짧다
    });
...
describe('⑮ 동선의 기준 — 목적지 하나당 마름모 하나 (기사님 확정 2026-09-08)', () => {
     ...
     *   · 둘 다 통과하면 복귀가 이긴다
     *   · homeCaught / ∩ 전환 특례는 **두지 않는다** — 목적지는 각자 제 마름모로 살아 있을 뿐
     */
    const PAJU = cityCenter('파주시');
    ...
    it('🔴 실은 짐이 없는 목적지는 ∩ 를 안 건다 — 복귀 대기의 미리 잡기 (2026-09-08 회귀)', () => {
        // 파주 짐을 싣고 가는 중 복귀를 켜도 집 4.2km 하차 콜이 «상차 사각형 밖»으로 잘리면 안 된다 (실측)
...
describe('judgeGoals 의 배선 — 화면이 읽는 값과 목적지별 콜백 (2026-09-08 리뷰: 변이가 안 잡혔다)', () => {
    ...
    it('🔴 won 은 «이긴 목적지»의 판정이다 — 첫 결과가 아니다 (화면 판정 카드가 이걸 읽는다)', () => {
        ...
        // 「복귀(집)」 배지 밑에 이천 숫자가 깔리면 안 된다 — 승자의 값이어야 한다
...
/**
 * 🏘️ **제외지역이 두 층이라 세 칸이 필요하다** (기사님 «도 · 시·군·구 · 보기»).
...
/**
 * ⛔ **제외 키 세 층** (기사님 *"원래 내가 원한 건 서울을 빼는 거였는데"* —
 * 도 한 층이 없으면 서울을 빼려고 구 25개를 하나씩 눌러야 한다).
...
/**
 * 🔴 **복귀 마름모의 동을 «지나온 곳»으로 지우지 않는다** (기사님 · 지도 스크린샷).
 *
 ...
 * 그래서 `buildLineNet` 은 라인 띠 동에만 그 값을 붙인다. **마름모·목적지 원으로 들어온 동에까지** 붙이면
 * 마름모 동은 라인에 **수직으로 멀리** 있어 진행도가 작게 나오고, 파주까지 달려온 기사님의
 * 진행도가 크니 **앞으로 갈 곳이 통째로 «지나온 곳»으로 지워진다.**
 */
 ...
 * 🚫 **제외 지역을 빼는 자리는 하나다**.
 *
 * 실험실과 서버가 각자 거르면 **«화면은 뺐는데 판정은 안 뺐다»** 가 된다.
 * 판별 규칙을 또 쓰지 않게 빼는 일 자체를 여기로 모은다 (규칙 ③).
 */
...
/**
 * 🧩 **필터 영역 — 단계마다 합친다** (기사님 확정).
 *
 *   첫 콜 확정 ~ 출발 전  내 영역 ∪ 목적지 영역 ∪ 마름모 영역 ∪ 경로 영역
 *   출발하면              (마름모 ∩ 내) ∪ 목적지 ∪ 마름모 ∪ 경로   — 내 영역 중 마름모 밖이 빠진다
 *   관내                  따로 재지 않는다 (목적지 가까이 옴은 `filterArea.test.ts`)
 *
 * 노선 그물에 내 영역이 없으면 집 뒤 동(매산동·쌍령동·양벌동)이 경로 띠로만 들어오고, 라인 시작 뒤라
 * 진행도 0 이 붙어 첫 KEEP 순간 «지나온 곳»으로 빠진다. 관내를 따로 재면 각도 360° 마름모가
 * 여주·용인 처인까지 35곳을 담는다 (실측 «7지점» 시험).
 */
describe('🧩 필터 영역 — 출발 전 내 영역', () => {
...
/**
 * ✂️ **운행 뒤 라인 띠는 현위치부터 앞으로만 · 시작 끝은 평평하게** (기사님 «교집합이긴 한데 뒤가 너무 많이 남는다 · 뒤를 자르는 Cap 타입으로» · «2»).
 * 상차 띠가 지나온 길(출발 → ① → 현위치)까지 그리면 역방향이 남는다 — 서버 상차 목록 · 지도 «상차» · «하차» 띠가 같은 두 함수를 쓴다.
 */
