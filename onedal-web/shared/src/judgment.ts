 ...
 *
 * 기사님: *"나중에 실지로 도로에 나가서 데이터를 모아서 쉽게 수정할 수 있도록
 * 사용자 설정 팝업에서 수정 가능하도록 하는 기능이 필요하겠다."* — 그래서 값은 DB 에 살고 설정 팝업에서 고친다.
 */
export interface JudgmentConfig {
    ...
    /**
     * 🚚 **지나침 판정** — 정거장을 지나치면 도착·완료를 대신 찍는다 (기사님 확정).
     *
     * 기사님: *"운전중에 그걸 누르는건 너무 위험하다 … 지나온 목적지를 벗어나면 도착과
     * 상차완료를 순차적으로 종료시켜 주는 것이 맞을 것 같다."*
     *
     * 🔴 **지나가면 실은 것이다 — 상차·하차 완료도 대신 찍는다** (기사님 확정: *"지나가면 실었다가 맞아"*).
     */
    pass: {
    ...
    unknown: {
        ...
        /**
         * ⏱️ **상차 약속** (기사님 확정) — 콜을 잡으면 이만큼 안에 그 상차지에 도착.
         *    잡은 시각 + 이만큼 = 무통보로 봐주는 상차 한계 (주선사의 시계).
         *    통화 약속 > 적요의 상차 시각 > 이 값 순으로 이긴다 (용어집).
     ...
     *
     * 기사님: *"아직 나도 어떻게 가중치를 주어야 할지 잘 모르겠어 그래서 모두 1을
    ...
    weights: {
         ...
         *    가중치가 **0 이 아니면 실패 시 색을 «사고»로 덮는다** (안전은 안 뺀다).
         *    0 이면 끈다 — 경로만 보려는 검사가 색을 덮지 않게.
         */
        promiseGuard: number;
     ...
     * 시급이 나와도 하루를 통째로 건다. 우회 235분 15만원이 보통 위로 올라오던 자리다.
     *
...
export const DEFAULT_JUDGMENT: JudgmentConfig = {
    // 🧪 판정색 확정안 v2 (기사님 확정) — 우회는 가중치 하나(revenueDetour)로 재고,
    //    목표 시급은 문제지 캘리브레이션으로 정했다(3.0만).
 ...
 *
 * 기사님: *"수정할 때마다 문서를 읽어야 할 건데.. **문서가 항상 최종본이 아닐 수
 * 있고**."* → 그래서 역할을 갈랐다:
 *      DB        지금 값이 얼마인가        ← 진실
...
export const JUDGMENT_FIELDS: readonly JudgmentField[] = [
    // 🧪 우회의 절대 크기는 감점이 아니라 사실 표시("우회 +43분", 딱지)다 (판정색 확정안 v2 · 기사님 확정).
    ...
    /**
     * 🔴 `마감 미확인 여유 90분` 을 **두 규칙으로 갈랐다** (기사님).
    ...
    // 🧪 우회는 가중치 하나(**우회 시급**)로 잰다 (기사님 확정 ② — 같은 40분 우회라도 요금이 가른다)
    ...
    { col: 'dest_bonus_max', path: ['destBonus', 'max'], group: '첫짐',
      label: '목적지로 곧장 갈 때 배수', unit: '배', min: 0.5, max: 2, int: false,
      why: '목적지 쪽으로 **완벽히** 전진하는 첫짐의 점수를 이만큼 곱한다. 1 이면 안 깎는다. 옆으로 가는 콜은 이 값과 «완전히 반대로 갈 때 배수»의 **한가운데**다. 🔴 **1 보다 크게 두면 좋은 콜끼리 구분이 사라진다** — 점수가 100 에서 잘려 시급 2.1만과 4.0만이 같은 🔵 100점이 된다' },
    { col: 'dest_bonus_min', path: ['destBonus', 'min'], group: '첫짐',
      label: '완전히 반대로 갈 때 배수', unit: '배', min: 0.1, max: 1, int: false,
      why: '**완전히 반대로** 가는 첫짐의 점수를 이만큼 곱한다. 내리면 역주행 콜이 확 깎이고, «곧장 갈 때 배수»와의 사이가 벌어져 **방향에 따른 차이가 커진다**. 0 으로는 안 둔다 — 그건 «버려라»고 서버가 정하는 것이다 (규칙 ①)' },
...
// 점수 — 🧪 **채점은 `judge.ts` 하나다**
//
// 이 파일은 색 이름 '꿀/보통/똥/사고' 와 경계(color.honeyMin/normalMin)만 들고 있다.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 *
 * 🔴 **`본콜` 은 금지어다** — 한 단어가 세 뜻(잡아 둔 첫 콜 · 첫짐 · 후보콜)으로 읽혀,
 *    후보콜의 문제를 기사님이 *"내가 KEEP 한 첫 콜에 문제가 있나?"* 로 읽으신다.
 */
