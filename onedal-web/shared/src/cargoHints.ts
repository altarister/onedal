 ...
 * 적요의 낱말 → 적재 단위.
 *
 * 단위가 **기사님이 쓰는 말** 그대로라 매핑에 추측이 없다 — 예: `마대`가 곧 단위다.
 ...
 * `가전` 은 성질이 아니다 — 부피도 취급도 제각각이라 묶이지 않는다.
 */
...
const HANDLING_WORDS: Array<[RegExp, HandlingMethod]> = [
    // 🔴 검수는 **하차의 후작업**이다 (기사님) — 방법이 아니라서 여기서 매핑하지 않는다.
    [/지게차|호크리프트|포크리프트/, '지게차'],
    // 🔴 호이스트·크레인은 방법 목록에 없다 (기사님: "해본 적이 없다") — 지게차로 바꿔 넣으면 없는 사실을 지어내는 것이다.
