/**
 * 🧭 **내비 링크 — 개인 폰으로 보내는 카카오맵 경로** (기사님 기획)
 *
 ...
 * | **개인폰** | **내비게이션만** |
 * | **관제폰** | **전화 · 관제앱만** (업무폰) |
 * | **스캔폰** | **원달앱 + 배차망이 공존.** 🔴 배차망에 따라 **추가될 수 있다** |
 *
 * ── 형식 (카카오 공식 문서) ──
 * ```
 ...
 *
 * 브라우저는 **보안 문맥(https)** 에서만 위치를 준다. http 로 열면 화면이 «위치 권한을 허용해 주세요»라고만 해서
 * **손댈 데가 없는 안내**가 된다 — 권한 문제가 아니라 주소 문제다.
 *
...
export interface KakaoNaviLinkInput {
     ...
     *    `coord_type` **하나뿐**이었다.
     * 🔴 **모르는 칸은 보내지 않는다** — 넣으면 카카오내비가 거부한다 (규칙 ④).
...
export function buildKakaoNaviUrl(input: KakaoNaviLinkInput): string | null {
     ...
     *    `vehicleType` 을 넣으면 카카오내비가 거부한다 (*"죄송합니다. 서비스 오류로 대응중입니다"*) (규칙 ④).
