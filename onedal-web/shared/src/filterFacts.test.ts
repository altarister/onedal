...
/**
 * ✂️ **상차 띠는 «얼마나 왔나»로 자른다 — 자름선의 방향은 «남은 길 전체»에서 가져온다** (기사님 확정)
 *
 * 라인을 내 자리에서 끊어(`lineFromPoint`) 지나온 **길**을 버리고, 현위치에서 그은 직각선
 * (`cutTowardGoal` → `isAheadOf`)으로 띠의 둥근 끝이 덮는 **면**을 버린다. 그 직각선의 방향을
 * **마지막 골목 한 구간**에서 가져오면 그 구간이 방향을 지배한다 — 목적지에 가까워 남은 길이
 * 띠 반경보다 짧아지면 그 쏠림이 커진다.
 *
 * 실측: 양재에서 서울로 가는 중 상차 영역이 목적지 반대편 남서쪽 반원이 되어 역삼동(3.3km)이 빠졌다.
 *
 * 무엇을 막나
 * - **자름선 방향을 라인 끝 구간에서 가져오는 것**(`aheadOf(band…)`) — 골목이 영역을 통째로 돌린다
...
describe('✂️ 상차 띠는 방향으로 자르지 않는다', () => {
     ...
     * 그 선의 방향을 **마지막 한 구간에서** 가져오면 흔들리고, 남은 길 전체로 보면 안 흔들린다.
     */
