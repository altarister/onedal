 ...
 *
 * 시간 계산이 여러 파일에 흩어지면 어디를 고칠지 찾기 어렵고 같은 계산이 복제된다.
 *
 ...
 * 상하차에 걸리는 시간(분).
 *
 * 🔴 경로 시간에 **상하차 정차를 더한다** — 주행만 세면 수작업 상하차 두 번의 한 시간이 사라져
 *    «우회 +20분»이 실제로는 +80분이 된다.
 *
 * 점수 축은 **라면박스**다 (1박스=1점 · 파레트=40점).
 ...
 *
 *    **기본 시간은 0 이다** — 찾기·대기 명목의 고정 분을 두지 않는다.
...
/**
 * 방법을 모를 때 쓰는 **일반값** — 상차와 하차가 다르다 (기사님 확정).
 *
 * 🔴 **모를 때는 흔한 값으로 계산한다** — 큰 값을 넣으면 정차가 주행에 얹혀 `+6분` 짜리 콜이 `+46분` 이 되어 똥이 된다.
 *
...
export interface DwellUnknown {
    ...
    /**
     * 📦 **박스 하나에 걸리는 시간(분)** — 판정 기준 탭에서 온다 (화면으로 올림).
...
export function dwellMinutes(
    ...
    /**
     * 🔴 **수량은 늘 있다** (기사님): *"콜이 들어왔다는 건 어떤 차종의 짐을
     *    부른 것이라는 걸 무조건 알 수밖에 없다."* 신고가 없으면 차종 정원이 들어온다
     *    (`defaultCargoByVehicle` — 화면도 서버도 같은 값).
...
/**
 * 도착 예상 시각부터 **30분 간격**으로 고를 수 있게 한다.
 *
 * 기사님이 실제로 하는 통화:
...
export function buildArrivalSlots(nowMs: number, minMinutes: number, count = 5, stepMin = 30): ArrivalSlot[] {
     ...
     *
     * 🔴 `:00 / :30` 경계로 올리지 않는다 — 올리면 **여유가 제멋대로 변한다** —
     *    도착 예상이 17:02 면 여유 28분, 17:29 면 **1분**이다.
     *    기사님: *"격자로 하면 여유 시간의 디폴트 값이 막 변화하는 거잖아."*
     *
     *    칸이 겹치지 않게 하는 것은 호출부다 — **저장된 약속을 기준점으로** 넘긴다 (StopCallSheet).
     */
 ...
 *
 * 음수를 0 으로 깎으면(`Math.max(0, …)`) 그 0 이 한계로 쓰여 *"0분 안에 다녀와라"* 가 되고, `+0분` 짜리 콜조차 똥이 된다.
 * **0 은 "한계 0분"이 아니다** — «모른다»(`null`)와 «늦었다»(음수)를 뭉개면 둘을 구분할 수 없다.
 */
...
/**
 * **영업일 경계 — 자정.** 기사님 결정: *"그냥 하차시간을 기준으로 24시를 기준으로."*
 *
 ...
 * **하차 마감 — 상차 마감에서 순산한다.**
 *
 * 🔴 하차에서 상차를 역산하지 않는다 — *하차 도착 예상 + 120분* 에서 역산하면 100km 콜의 마감이
...
export function departureDeadline(
     ...
     *    (기사님). 주행만 빼면 상차지에 정시 도착해도 **약속보다 상차 시간만큼 늦게** 보내게 된다.
     */
...
/**
 * 🔴 **남은 시간은 어디까지 왔는지로 센다.** 상차 전인데 `상차지 → 하차지` 주행만 쓰면
 * **현위치 → 상차지 이동과 상차 작업이 통째로 빠져서**, 도착 예상이 이르고 `지각` 판정도 낙관적이 된다.
...
/**
 * 🔴 **입력도 여기 한 곳에서 만든다** — 정차 분 · 접근 거리 같은 입력을 화면마다 따로 만들면
 *    한쪽만 고쳐져 **카운트다운과 통화 화면이 다른 시각을 말한다.**
 */

/** 경로 계산에 필요한 오더 필드만. `SecuredOrder` 전체에 묶이지 않는다 */
export interface TimingOrderFields {
    /**
     * 🔴 **콜을 잡은 시각.** 상차 마감(`잡은 시각 + N분`)의 기준점이다 (기사님).
...
export const DEFAULT_DEADLINE_RULES: DeadlineRules = {
    /**
     * ⏱️ **20분** (기사님 확정) — 콜을 잡았으면 20분 안에 그 상차지에 닿는다.
     * 🔴 근거는 배송 원칙(20분 룰)이다.
     */
...
/**
 * 🎛️ **판정 기준 탭 → 시간 파생 입력, 한 곳에서**.
 *
 * 탭의 시간 4칸(미확인 정차 2 · 상차 시계 잠정 · 데드라인 배율)을 rules/unk 로 조립한다.
 * 🔴 **서버와 관제웹이 모두 이 함수로 조립한다** — 한쪽이 기본값 상수로 파생하면 기사님이 탭에서
 *    값을 바꾸는 순간 서버와 화면이 다른 값을 쓴다.
 */
...
/**
 * 🚚 **단독 배송 주행(분) — 값이 태어나는 자리는 여기 하나다** (기사님 확정)
 *
 ...
 *
 * 🔴 **거리와 시간을 «짝»으로 낸다** — 같은 출처에서 가져와야 한다. 열쇠를 갈라 한쪽만 있으면
 *    카카오 거리에 추정 시간이 붙어 **속도가 거짓말한다.** 그래서 한 함수가 둘 다 낸다.
...
export function deriveCallTiming(
    ...
    // 이게 없으면 미확인 15분으로 지어내 서버 데드라인과 갈라진다 (규칙 ③)
    ...
    /**
     * 🔴 **상차에서 순산한다** (기사님 모델 — 하차에서 역산하면 여유가 부풀려진다):
     * ```
    ...
    if (!pickupDeadlineAt && capturedMs != null) {
         ...
         * 🔴 **약속은 도착 예상을 따라가지 않는다** — `max(도착 예상, 시계)` 로 두면 상차 여유가 늘 0 이고
     ...
     *    값을 만드는 자리에서 `null` 로 낸다 — 화면에서 막으면 다른 화면이 그 값을 쓰는 순간 잘못된 카운트다운이 뜬다.
     */
     ...
     * 약속(도착)과 완료는 `완료 = 도착 + 정차` 로 묶여 있으므로 (기사님),
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// 경로 타임라인 — 시각의 원천은 "지금 경로" 하나다 (기사님 동의)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
export interface RouteTimelineEntry {
     ...
     *
     * 기사님: *"상차에 10분으로 되어 있던 것이 15분이 되었다면 다음
     ...
     * 통화 문장에 쓰면 접근 주행을 **두 번** 센다 (기사님 실측: `주행 129분`,
...
/**
 * 🕐 **끝난 정거장의 시각은 «지금 경로»가 아니라 장부에 있다** (기사님 발견)
 *
...
export function deriveRouteTimeline(
    ...
    /**
     * ⛓️ **앞 정거장을 떠나는 시각이 뒤 정거장의 기준이다** (코드리뷰).
     *
     * 🔴 **확정한 "까지" 약속도 뒤로 전파한다** — 정차와 "부터" 대기만 누적하면, 상차를 11:12 로 약속했는데
     *    하차 추정이 물리적 최소(12:57)보다 이른 11:51 로 뜨고, 그 값으로 화주와 약속하면 무조건 지각이다.
     *
     ...
     *
     * 출발마감의 기준은 하나 더 갈린다 (기사님 실측):
     *   `mandatoryMin` = 정차 + **확정 "까지" 약속으로 생긴 지연**
     *     빼지 않으면 요약 줄은 «못 지킨다», 카운트다운은 «여유가 있다»로 화면이 두 말을 한다.
     ...
     *
     * 🔴 **다녀온 정거장의 밀림부터 깔고 시작한다** (기사님 리허설).
     *
     * «실제로 몇 분 걸렸는지»를 아는 순간은 **상차 완료를 누르는 때**이고, 그 순간 그 정거장은
     * **경로에서 빠진다.** `stops` 만 훑으면 방금 더 걸린 그 정거장을 아무도 세지 않는다.
     *
    ...
    for (const st of stops) {
        ...
        const estMs = (() => {
            if (st.stopType === 'pickup') {
                ...
                /**
                 * 🔴 **약속은 도착 예상을 따라가지 않는다** (기사님 확정).
                 *    `max(도착 예상, 시계)` 로 두면 상차 여유가 구조적으로 늘 0 이고 «늦는다»는 사실이 가려진다 —
                 *    늦는 것을 알아야 기사님이 그 상차지에 전화를 거신다.
                 */
                return pickupClockMsOf(order, capturedMs2, rules.pickupPromiseMinutes ?? 20);
            }
            /**
             * 🚚 **상차지에 도착해도 «상차 완료 예정»을 잃지 않는다** (기사님 실측).
             *
             * 하차 약속 = `상차 완료 + 단독 주행 × 150%` 인데 그 앞쪽이 비었다:
             *   · 실측(`PICKED_UP`) — 기사님이 **보고해야** 생긴다
             *   · 예정(`pickupDoneOf`) — **경로에 상차 정거장이 남아 있을 때만** 채워진다
             * 도착하는 순간 정거장이 빠지므로 **둘 다 없어진다** — 그러면 상차 후 후보의 버퍼 축이 통째로 빠져
             * 되돌아가는 콜이 후하게 나온다.
             *
             * 🔴 **도착 실측은 갖고 있다** (`ARRIVED_PICKUP` · GPS). 완료 시각은 **도착 실측 + 지금 추정 상차 소요**로
             *    파생한다 — 없는 값을 만드는 게 아니라 **있는 값에서 파생**한다 (규칙 ③).
            ...
            /**
             * ⏱️ 🔴 **여기서도 실측이 계산을 이긴다** (기사님 리허설).
             *
             * 계산값(`t.pickupDwell`)만 보면 기사님이 상차 완료 시트에 「19분」이라 적어도 하차 약속이
             * 계산값에 머물러, 화면이 **「5분 밀렸다」고 말하면서 안 밀린 시각**을 적는다.
             *
             * 🔴 이 줄은 **상차지에 도착했지만 아직 상차를 안 끝냈을 때만** 지난다 —
             *    도착하는 순간 그 정거장이 경로에서 빠져 `pickupDoneOf` 가 안 채워지기 때문이다.
             */
            ...
            /**
             * 🔴 **하차 약속도 도착 예상을 따라가지 않는다** (기사님 확정).
             *    `max(도착 예상, 마감)` 으로 두면 **150%를 넘겨도 여유가 0**으로 보인다.
         ...
         */
        /**
         * 🚚 **지나간 정거장의 기준은 실제 시각이다** (기사님 실측 · 모의주행).
         *
         ...
         *
         * 🔴 **약속과 출발은 다른 질문의 답이다** (기사님 확정 · 규칙 ⑤-4 ⑤).
 ...
 * 경로에 무엇을 더 실을 수 있는가". 「약속」 기준이 이 값을 먹는다.
 */
