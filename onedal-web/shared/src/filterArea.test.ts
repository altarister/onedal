...
describe('🔵 하차 목록 — 목적지마다 합친다 (상차 목록은 빼지 않는다)', () => {
    ...
    it('🔴 가까이 온 목적지 동은 상차 목록과 겹쳐도 남는다 — 관내콜', () => {
        ...
        // 먼 목적지에서도 안 빼므로 초월읍(상차 목록)도 남는다
    ...
    /** 🔴 상차 목록은 하차 목록에서 아무것도 안 뺀다 — 같은 이름이든 다른 시든 그대로 남는다 */
...
/* 🎯 목적지 눈금(필터값 ∪ 마지막 KEEP 콜의 목표값)은 `filterFacts.test.ts` 가 본다 */

/* 🔵 하차 조각 눈금은 `filterFacts.test.ts` 가 본다 — 조각은 사실 하나(라인이 있나)만 본다 */
