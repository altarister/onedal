 ...
 * ══ 왜 저장하지 않고 파생시키는가 ══
 *
 * 현재 단계를 `useState` 에 담으면 새로고침·재접속·스와이프에서 어긋난다 — 저장해 둔 값은 실제와 갈라진다.
 *
 ...
 *    (기사님) — 출처 삼분(직접·자동·건너뜀)이 여기서 나왔다.
 *
 * ⚠️ 세 앱 모두 건너뛰기를 `SKIPPED` 로 서버에 보낸다 — 하나라도 화면에만 두면 장부가 깨진다
 */
...
export function deriveCallStep(
     ...
     *    않으면 *"내가 확인한 건지 아닌지"* 를 알 수 없다 (기사님).
