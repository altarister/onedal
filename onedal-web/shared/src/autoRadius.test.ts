 ...
 *    원 둘이 거리보다 크니 마름모가 설 자리가 없어 **설정값 셋이 아무 일도 안 한다**
...
/**
 * 📏 **자동 반경의 거리는 하루에 한 번 잰다** (기사님 확정).
 *
 * 기사님: *"자동 반경은 그날 첫짐일 때 적용되는 거다. 위치가 바뀐다고 바뀌어서는 안 된다.
 * 그럼 나중에 관내콜을 할 수가 없다. 그리고 가는 길에 좋은 콜을 못 잡는다."*
 *
 * 합짐 때 «마지막 하차지(중리동) → 이천» 1.2km 로 다시 재면 배율 0.03 — 목적 원 10km 가
 * 0.3km 로 줄어 도착지 목록이 1곳이 된다 («7지점 한 바퀴» 실측).
 */
