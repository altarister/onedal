 ...
 *
 * 그래서
 *   ① **기사님이 통화에서 실제로 쓰는 단위**(파레트·라면박스·마대)를 앞에 둔다
...
//
// 기사님 결정 — 선택지는 아래 다섯이고 화면에 **한 번에** 보인다 (더보기 없음). 톤백·쇼핑백은 선택지에 없다.
...
export const CARGO_UNIT_POINTS: Record<CargoUnit, number> = {
    ...
    '톤백': 40,      // 박스 40개 분량 — 만재가 아니다 (기사님 확정)
    '쇼핑백': 0.2,   // 쌓아올리기 어려운 백
};
 ...
 *
 * 서버는 신고가 없으면 `VEHICLE_CAPACITY[차종]` 을 적재로 잡는다 (`computeLoadedPoints`).
 * 화면도 같은 값을 미리 눌러 둔다 — 빈칸이면 **화면과 서버가 다른 값을 본다.** 기사님은 틀린 것만 고친다.
 *
...
export function defaultCargoByVehicle(vehicleType?: string | null):
    ...
    if (boxes >= perPallet * 2) {
        // 🔴 **파레트면 지게차다** (기사님): *"파레트를 사람 손으로 내리기는 너무 어려우니까."*
...
/**
 * 통화 전 미리 눌러 두는 값 — **정리는 무조건 한다** (기사님).
