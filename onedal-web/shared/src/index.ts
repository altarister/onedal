...
export const TERMINAL_STATUSES: readonly OrderStatus[] = [
    ...
    // 적재 계산·경로 계산·화면 표시가 함께 짐을 내린다 — 빠지면 잔여 용량이 회복되지 않아 다음 짐을 못 잡는다.
    'ORDER_DELIVERED',
    'ORDER_COMPLETED',
    'ORDER_RELEASED_BY_ME',
    'SAFE_CANCEL',
 ...
 *
 * 🔴 **손으로 나열하지 않고 파생시킨다** — 나열하면 새 상태를 복구 쿼리에 빠뜨려, 짐을 실은 채 새로고침하면
 *    콜이 화면에서 사라지고 서버는 빈 차로 착각한다.
 *
 * 평가 중(안전취소 이전)은 메모리에만
 ...
 *
 * 방법은 짐을 옮기는 행위만이다 — 둘뿐이다.
 */
// 🔴 호이스트는 방법이 아니다 (기사님: *"해본 적이 없는데 이건 그냥 뺄까?"*) — 안 해 본 일에 시간 값을 지어내면 그 값이 색을 정한다.
// 🔴 검수는 하차의 `후작업`(AFTERWORK_MINUTES)이다 (기사님: *"검수는 하차할 때 하는 거라 하차로 옮기는 것이 맞을 듯."*)
 ...
 * 정거장 기록의 종류.
 *
 * `SKIPPED` — **통화를 건너뛰기로 한 결정**.
 *
 * 기사님 기준: *"완료 전까지는 페이지별로 기억하고 있어야 하고 수정이 가능해야 한다."*
 * 건너뛰기를 화면에만 두면 **새로고침하면 되살아난다** — 서버에 남긴다.
 ...
 * 다른 정차를 먹고 한 화면에서 두 데드라인을 말한 사고의 수리.
 */
...
export interface CargoReport {
    ...
    /** 🔒 보호 — 호루·결박·그물망·탑박스 (복수 선택 · 기사님) */
    protections?: string[];
    /** 🧹 후작업 — 정리·검수 (하차 전용 · 복수 선택 · 기사님) */
    afterworks?: string[];
    /**
     * **상차지 통화에서 함께 들은 하차지 도착 예정 시각.**
     *
...
export function cargoPoints(r: Pick<CargoReport, 'unit' | 'sizeClass' | 'quantity'>): number {
    // 새 단위(파레트/라면박스/소·중·대)를 우선 쓰고, 없으면 옛 필드 sizeClass 로 폴백
...
export function cargoMismatchRatio(declared?: CargoReport | null, actual?: CargoReport | null): number | null {
    // 🔴 관문을 필드(`sizeClass`)가 아니라 **점수**로 건다 — 화면은 `unit` 만 보내므로 필드로 걸면
    //    불일치 경고가 한 번도 안 뜬다 (cargoPoints 가 unit·sizeClass 를 모두 처리한다).
    //
...
export interface OfficeOrder extends SimplifiedOfficeOrder, DetailedOfficeOrder {
     ...
     *  🔴 원장(orders.targetApp)에 저장 — 배차망별 콜 검색·분석의 근거 (기사님) */
    targetApp?: string;
    /** 🖱️ 잡은 방식 — 6하원칙의 «어떻게» (CAPTURED_VIA · 기록 전용, 보호 분기는 matchType).
     *  구앱·모르는 값은 null — 지어내지 않는다 (기사님 확정) */
...
export interface PendingOrder extends OfficeOrder {
     ...
     *
     * 🔴 **타입에만 없었다** (고침). `orders` 테이블에 `deliveryDistance REAL`
    ...
    /** 🧭 구간마다 어느 정거장인가 — sectionDriveMin 과 같은 길이. 도착으로 정거장이 빠져도 이름으로 맞춘다 */
    sectionStops?: Array<{ orderId: string; stopType: 'pickup' | 'dropoff' }>;
    /**
     * 🎨 **구간이 끝나는 자리** — `routePolyline` 안에서 각 구간의 **끝 인덱스(누적)** 다
     *    지도가 «구간마다 그 콜의 색»으로 그리려면 경계가 필요한데
     *    통짜 배열은 그것을 잃는다.
     *
     * 🔴 **선을 한 벌 더 보내지 않는다** — 같은 점열을 복제하면 초당 수백 KB 가 오가 브라우저가 죽는다. 경계는 숫자 몇 개뿐이고
 ...
 * 심사 카드가 조건 전수를 이걸로 그리고, **색도 여기서 온다** (4단계).
 *
 * import 순환을 피해 `dryRun.ts` 의 `DryRunVerdict` 를 구조로 적는다.
 * 🔴 **여기 한 곳에만 적는다** (규칙 ③) — `SecuredOrder` 에만 두면 `MyOrder` 로 다니는 콜
 *    (재시작 복구가 만드는 것)에 **색이 실리지 못한다.**
 */
...
export interface SecuredOrder extends OfficeOrder {
    ...
    /** 🎨 구간이 끝나는 자리 — `routePolyline` 을 콜 색으로 칠하려면 이 경계가 필요하다 */
    sectionEnds?: number[];
    /** 🧭 구간마다 어느 정거장인가 — `sectionEnds` 와 같은 길이. 구간의 **색 주인**이다 */
    sectionStops?: Array<{ orderId: string; stopType: 'pickup' | 'dropoff' }>;
...
export interface AutoDispatchFilter {
    ...
    /**
     * 🗺️ **상차 영역을 지도에 그릴 재료** (기사님 «현위치 영역에 교집합 영역이 보이지 않는다»).
     ...
     */
    /**
     * 🛣️🔷 **노선 / 동선 — 그물을 어떤 모양으로 볼까**.
     *    노선(true)은 경로 양옆(라인반경) ∪ 목적지, 동선(false)은 내 위치 → 목적지 마름모 하나.
     * 🔴 **필터 값이다** — 화면에만 두면 서버가 몰라 «동선»을 골라도 판정·앱 목록은 노선이고,
     *    새로고침하면 노선으로 돌아간다. DB 자리는 `user_filters.route_mode`.
     ...
     *    **지도가 못 따라왔다** — 배율을 다시 낼 재료가 없었다 (전수 조사 1단계 실측).
     *    서버는 재료만 싣고 셈은 한 함수가 한다.
     * ⚠️ 자동·수동과 무관하게 싣는다 — 그래야 수동에서 «자동이면 얼마가 되나»를 미리 볼 수 있다.
     *    못 재면 **없다** — 0 으로 지어내지 않는다.
     * 🔄 **하루에 한 번 잰 값을 들고 있는다** (`heldRadiusDistanceKm`). `null` 은 «다시 구하기» —
     ...
     *    겹쳐 두면 기사님이 고른 것이 **짐 한 번에 지워진다** (사고).
    ...
    /**
     * 🎯 **그물이 실제로 향하는 시** — 서버가 파생 · 읽기 전용 · DB 저장 안 함 (전수 조사 ①-1).
     *
     * `callTarget` 이 HOME 이면 **집이 있는 시**, 아니면 `destinationCity`. 기사님이 정한
     * `destinationCity` 는 복귀를 켜도 **한 번도 안 바뀐다** — HOME 으로 갈 때 덮어쓰면 돌아올 때
     * 덮인 값이 남아 목적지가 집 시로 굳는다.
     ...
     * 명목값이며 통화 확인 시 갱신.
     * ⚠️ 앱·서버·관제웹이 함께 읽는 칸이라 단위(박스)를 잘못 읽으면 비용이 크다
     */
    slotsUsed?: number;
 ...
 *    **낭비**다 (예: `destinationGroups` 하나가 응답의 27%다).
 */
 ...
 *
 * 🔴 **관내(`'LOCAL'`)는 고르는 것이 아니다** — 목적지에 도착해 다른 곳을 안 정했으면 그곳 일을 한다.
 *    필터 영역이 «목적지 가까이 옴»(`filterArea.withNearness`)으로 알아서 좁혀진다.
 * 🔴 **같은 일을 하는 길을 둘 두지 않는다** (규칙 ③) — 그러면 언젠가 갈라진다.
 * ⚠️ `callTarget` 은 **DB 에 없고**(메모리뿐) **앱이 안 읽는다.**
 */
 ...
 * **이 필터로 콜을 잡아도 되는가.**
 *
 * 🔴 **빈 필터는 "제한 없음"이 아니라 "고장"이다.** 막지 않으면 두 겹이 같은 방향으로 열린다:
 *
 *    앱 (`InsungParser.kt`):
...
export function callFilterBlocker(filter: AutoDispatchFilter): string | null {
    /* 📋 빈 상차 목록은 «제한 없음»이 아니라 고장이다 (규칙 ④). 아직 안 만들었으면(undefined) 옛 흐름이라 막지 않는다 */
 ...
 *
 * `driverAction` 은 **정류장마다 바뀐다** — 그걸로 가르면 하차지에 도착해 `UNLOADING` 이 되는 순간
 * DELIVERING 이 풀리고, 짐이 2건이면 **출발을 네 번 눌러야** 한다. 풀리면 지나온 구간 제거가 멈추고,
 * 🚀 출발 버튼이 다시 나타나고, 요약줄이 "대기"로 바뀐다.
 *
 * 기사님에게 "운행 중"은 *"이제 그만 모으고 간다"* 이고, 그건 **한 번 켜지면 마지막 하차까지
 * 유지되는 상태**다. 중간에 짐을 내리는 건 그 안에서 일어나는 일이지 운행이 끝난 게 아니다.
 *
 * 그래서 판정은 **출발했는가**(`hasDeparted`)다. `driverAction` 은 순수하게
 ...
 *
 * 🔴 부르는 곳마다 `?? N` 을 따로 두면 어느 값이 진짜인지 코드로 알 수 없다. DB 기본값(5)과 같은 값을 여기 하나에 둔다.
 */
...
export function getEffectiveDetourRadius(
    ...
    /**
     * 🔴 **국면으로 덮어쓰지 않는다** — 경유 반경은 기사님이 필터에 저장한 값(`detourRadiusKm`) 한 벌이다.
     *    `DELIVERING` 이라고 0 으로 덮으면 그 설정이 영영 무시된다.
     *
 ...
 *
 * 🔴 **«필터가 도는가» 와 «앱이 누르는가» 는 다른 물음이다** (에 갈랐다).
...
/** 화면에 적히는 이름 — MANUAL 의 화면 이름은 «직접», SIMULATION 의 화면 이름은 «체험» (기사님 확정) */
...
/**
 * 🌐 **배차망 값 표준 — 여기 한 벌뿐이다** (기사님 확정 · 픽커_수집.md §6-전).
 *
 * «어느 배차망에서 온 것인가»는 6하원칙의 **어디서**다. 앱이 모든 요청에 싣고,
 * 원장(orders·intel)의 `targetApp` 열에 저장되며, 화면 배지와 통계 필터가 읽는다.
 *
 * 🔴 코드값·라벨·기본값 전부 **여기서만** 늘린다 — 매핑·폴백을 여러 곳에 두면 배차망을 더할 때마다 갈라진다. (앱은 Kotlin 이라 이 파일을 못 읽는다 —
...
export type ScreenContextType =
     ...
     * **화면이 진행을 말해 준다** — 이 값이 없으면 운행 중 화면이 전부 `UNKNOWN`(«알 수 없는 화면»)으로 떠,
     * 기사님이 가장 알고 싶은 순간에 관제가 가장 모르게 된다.
     *
 ...
 *
 * 앱은 `LIST` 와 `LIST_COMPLETED` **둘 다** 리스트 복귀로 보고 세션을 리셋한다(`HijackService`).
 * 서버가 하나만 인정하면 **앱은 다음 콜을 찾는데 서버는 그 콜을 계속 쥐어**, 결재 카드가 영원히 남고
 * 콜 잡기가 멈춘다. 같은 판단을 두 곳에서 따로 정의하면 갈라진다 — 화면이 늘어나면 **이 배열에만** 넣는다.
 */
export const LIST_SCREENS: ScreenContextType[] = ['LIST', 'LIST_COMPLETED'];

/** 지금 화면이 "콜에서 손을 뗀" 상태인가 (= 서버가 쥐고 있던 콜을 놓아도 되는가) */
/**
 * 👁️ **앱은 켜져 있는데 화면을 못 읽는 중인가** (기사님 확정 · 크리티컬).
 *
 * 기사님: *"분명 폰 이름 1234에 파란불이 들어와 있었어."*
 *
 * 텔레메트리가 온다고 ONLINE 으로만 보면, 접근성이 막혀 콜을 하나도 못 읽는 동안에도 관제웹이 파란불이다 —
...
/**
 * 🚦 **작업 단계의 이름은 여기 한 곳에서 짓는다** (기사님 확정).
 *
 ...
 *
 * 🔴 버튼을 누르는 순간 바뀐 것처럼 그리지 않는다(«낙관적 업데이트») — 폰이 받았는지 모르면서 단언하는 것이다.
 ...
 *
 * 🔴 값 **30초** (기사님 확정). 지어낸 값이 아니라 **생존신고 60초의 절반**이다 —
...
export interface DeviceSession {
    ...
    /**
     * 📵 **왜 오프라인인가** (기사님 지적: *"'접근성 꺼짐' 이렇게 표현되면 좋겠는데"*).
     *
     ...
     * 앱이 **매 텔레메트리에 사실을 싣는다** (규칙 ③) — 이벤트 한 번으로 알리면 다음 하트비트가 `ONLINE` 으로 되돌려 녹색이 된다.
     */
    isScreenOn?: boolean;
    /**
    ...
    /**
     * 🕐 **그 성적표가 서버에 닿은 시각** (epoch ms · 기사님 지적).
     *
    ...
    /**
     * 🎛️ **폰이 «나 지금 이 모드다»라고 대답한 값** (기사님 확정).
...
/**
 * 🚏 **이 정거장에 이미 다녀왔는가 — 판단은 여기 하나뿐이다** (기사님 확정).
 *
 * 기사님: *"현실에서는 내가 지나온 것은 무시할 것 같은데."*
 *
 * 🔴 **경로 조립과 시각 계산이 이 판단 하나를 쓴다** — 둘로 나뉘면(버튼 vs GPS) 상차 완료를 안 누른 콜이
 *    이미 지나온 상차지로 되돌아가는 경로를 만들어 **없는 우회 비용**을 문다.
 *
...
/**
 * 🗓️ **시트의 카드 목록 — 오늘 한 일** (기사님 결정 «시트는 오늘 한 일을 남긴다 · 화면의 사이클 = 하루»).
 *
 * 처음엔 «진행 중이 남은 동안만 하차분을 함께» 보여 줬고(6단계 채운 모습을 보려고),
...
export interface OrderSyncPayload {
    ...
    /**
     * 🧭 **경로 기점 — 정거장 순서(`routeStops`)를 짠 그 자리**.
     *    같이 와야 «1번 정거장이 내 뒤에 있다» 같은 화면이 안 난다.
     *
     * ⚠️ **«내 위치»는 여기 없다** — 이 봉투는 «콜이 바뀔 때» 나가는데 위치는 1초마다
     *    바뀐다. 함께 실으면 **옛 좌표가 내 점을 뒤로 당겨** 모의 주행이 멈춘 것처럼
     *    보인다 (기사님 실측). 위치는 `driver-position` 이벤트로 따로 간다 (규칙 ⑤-4 ③).
     ...
     *
     * ⚠️ **리셋 시각 이후만** 센다 — 전 기간을 누적하면 `47/10` 처럼 한도를 몇 배씩 넘긴 숫자가 떠
     *    *"조여라"* 도 *"괜찮다"* 도 알려 주지 못한다 — 총량은 `cancelRounds` 가 지킨다.
     */
    ...
    /**
     * 🧭 **지금 경로를 든 콜의 id** (기사님 확정 · 잔상 수리).
     *
     * 폴리라인은 «홀더» 한 콜에만 실린다. 관제웹이 홀더를 추측하면 KEEP 직후처럼 갈리는 순간에
     * **직전 콜의 옛 선**을 그린다 — 이름을 실어 추측을 없앤다 (규칙 ③).
     * 🔴 폴리라인 자체를 여기 담지 않는다 — 콜에 이미 있는 것을 한 벌 더 만들면
     *    payload 가 두 배가 되고(초당 237KB 사고) 파생이 두 벌이 된다.
     */
    routeHolderId?: string | null;
    /**
     * 🟡 **심사 중인 콜의 «미리보기 궤적» 홀더** (기사님 실물).
     *
     ...
     * 그래서 갈랐다. 합치면 심사 중 콜의 빈 주행분이 타임라인을 폴백으로 돌린다.
     */
...
/**
 * 🚫 **취소 예산 한 판의 크기** (기사님 확정).
 *
 * 배차망이 세는 취소 한도다.
 * 🔴 서버와 관제웹이 이 값 하나를 본다 — 서버가 "다 썼다"를 판정하려면 같은 값을 봐야 하고, 두 벌이면 갈라진다.
 */
/**
 * 👁️ **필터 성적표 — 마지막 스캔 한 판** (기사님 확정).
...
/** 🏷️ 화면 이름표 — 배차망별로 갈라 둔다 (기사님 설계) */
export * from './screenLabels';
export * from './naviLink';
/** ⏱️ 밀림 — 한 콜이 앞선 정거장을 몇 분 밀었나, 누가 밀었나 (실험실에서 올림) */
export * from './stopImpact';
/** 🎨 구간별 선 — 통짜 폴리라인 + 경계로 자른다 */
...
/**
 * ⏱️ **«주행·정차»로 굳는 데 걸리는 초 — 기본값** (기사님 확정 · 화면규칙 S16).
 *
...
/**
 * ⏱️ **대기 시간 입력 — 1 이상 정수만 값이다** · 아니면 `null` (리뷰).
