...
export interface AutoDispatchFilter {
    ...
    radiusBaseKm?: number;
    /**
     * 📏 **서버가 파생한 배율** — 읽기 전용 · DB 저장 안 함 (`localMode` 와 같은 처지).
     *
     * 🔴 **화면이 이것만 있으면 반경 넷을 다 안다.** 필터 화면은 «내 위치 → 목적지» 거리를
     *    모르므로(모달은 `myLocation` 을 안 쥔다) 서버가 재서 실어 보낸다.
     *    반경 넷을 각각 실으면 칸이 넷 늘고 **기사님이 정한 원값이 가려진다** (규칙 ④).
     * ⚠️ 수동이거나 못 재면 **없다** — «1» 로 지어내지 않는다.
     */
    radiusScale?: number;
