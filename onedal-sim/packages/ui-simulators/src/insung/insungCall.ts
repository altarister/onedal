 ...
 * 🚚 **인성 화면만 쓰는 것** — 표기 함수
 *
 * 인성 화면만 쓰는 것은 공통 코드(`core-simulator`)에 두지 않고 이 폴더에 둔다 — 공통에 두면 배차망이 늘 때마다
 * 공통 파일이 부푼다 (`tests/boundaries.test.ts` 규칙 ①). 화면 글자는 스냅숏(`tests/screens.test.tsx`)이 문다.
 */
...
/** 🚚 인성 화면이 읽는 콜 — 공통 칸(`BaseCall`) + 인성 칸 (칸은 «어느 화면이 읽나»로 가른다) */
 ...
 * (`InsungParser.kt` 의 `VEHICLE_TOKENS`)가 그 약자를 앵커로 요금을 읽으므로,
 * 시뮬이 풀네임("다마스")을 그대로 뿌리면 파서가 요금을 못 읽는다
 * (실측: 여주 문제지의 다마스·라보·승용차 콜이 전부 요금 못 읽음).
 *
 * 승용차는 인성에서 **«승»** 이다 (기사님 확정). 파서 필터 매칭도
 * `"승용차" -> p.contains("승")` 으로 «승» 을 쓰고, 요금 앵커(`VEHICLE_TOKENS`)도 «승» 을 든다.
 */
...
/** 인성 콜 분류 풀 — «보통»이 둘이라 보통이 더 자주 나온다 */
const CATEGORY_OPTIONS = ['보통', '보통', '예약'];

/**
 * 🎨 **인성 칸을 입힌다** — 공통 칸만 있는 콜에 요금·합짐·급송·결제·차종·분류·상태.
 * 공통 생성기는 공통 칸만 만들고, 인성 칸은 인성 화면으로 갈 콜에만 여기서 입힌다.
 */
