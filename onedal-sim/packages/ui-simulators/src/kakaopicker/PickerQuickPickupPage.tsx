/**
 * 🚚 **퀵 — 픽업까지 흰 페이지** — 실물 17-1 «픽업 출발» · 17-2 «픽업 이동» (카카오픽커_시뮬레이터.md §8-2)
 *
 * 퀵과 도보는 잡은 뒤 **루틴은 같고 페이지가 다르다** — 도보는 지도 위 시트 + «밀어서 …»(16~22), 퀵은 흰 페이지 + 바닥 버튼:
 *   17-1 «픽업 출발» ─「픽업 출발하기」→ 17-2 «픽업 이동» ─「픽업 완료하기」→ 배송
 *   ⚠️ 퀵 배송 페이지는 사진이 없어 도보 배송 페이지를 쓴다 (`PickerOngoingScreen`) — 사진이 생기면 따로 만든다
 *
 ...
 * - 🔴 **머리(«픽업 N분 지연 중» · «지금 바로 출발해 주세요» · 마감 · 딱지)와 «길안내»는 실물에서 안 읽힌다** — `aria-hidden` 으로 웹뷰가 넘기지 않게 한다.
 ...
 * - ⚠️ 추정: 늦지 않았을 때의 머리(«픽업 N분 남음») · 17-2 의 읽히는 글자(로그가 없다 — 17-1 과 같게 둔다) · 픽업 장소 «매장 직원에게 문의»(17-2 는 «직접 전달»)
 * - 🔴 모르는 칸은 안 그린다 — 오더 코드(DJYZ914 꼴) · 메뉴 · 고객사 주문번호 · 유의사항 · 물품 설명 딱지(«전자제품이에요»)
 */
import type { PickerCall } from './pickerCall';
import { minutesLeftToday, pickerTagChipClass } from './pickerCall';
import { formatPickerDistance, formatPickerFare } from './PickerDispatchBoard';

interface Props {
  call: PickerCall;
  /** 「픽업 출발하기」를 눌렀나 — 아니면 17-1, 눌렀으면 17-2 */
  departed: boolean;
  onDepart: () => void;
  onPickedUp: () => void;
  onBack: () => void;
...
export const PickerQuickPickupPage = ({ call, departed, onDepart, onPickedUp, onBack, onCancel }: Props) => {
  const left = minutesLeftToday(call.pickupTime);
  /** 픽업 마감이 지난 분 — 안 지났으면 null */
  const late = left !== null && left < 0 ? -left : null;
  const red = late !== null ? { color: LATE_RED } : undefined;
  const places = [
    { detail: call.pickupDetails?.[0], dot: PICKUP_BLUE, name: '픽업지' },
    { detail: call.dropoffDetails?.[0], dot: '#b5b5b5', name: '도착지' },
  ];
  ...
  return (
    <div className="relative w-full h-full bg-white text-[#1f1f1f] select-none">
      <div className="absolute inset-0 overflow-y-auto" style={{ paddingTop: HEADER_PX, paddingBottom: BOTTOM_PX + 12 }}>
        {/* 머리 — 딱지 · 늦었나 · 마감 | 픽업지 거리. 🔴 실물에서 원달앱이 못 읽는다 (A24 로그) */}
        ...
        {/* 픽업 장소 · 오더번호 — 실물 로그에서 한 칸에 이어 읽힌다 */}
        <div className="mx-[16px] rounded-lg bg-[#f5f6f8] px-[14px] py-[12px] flex flex-col gap-[8px] text-[14px]">
      ...
      <div className="absolute inset-x-0 top-0 flex items-center justify-between px-3 bg-white border-b border-[#eeeeee]" style={{ height: HEADER_PX }}>
        ...
        {!departed && <button onClick={onCancel} className="px-2 text-[15px] text-gray-600">배정 취소</button>}
      </div>

      {/* 바닥 — 길안내(🔴 실물에서 안 읽힌다 · 시뮬레이터에서는 아무 일도 안 한다) · 픽업 출발하기 / 픽업 완료하기 */}
      <div className="absolute inset-x-0 bottom-0 flex text-white font-bold" style={{ height: BOTTOM_PX }}>
        ...
        <button className="flex-1 text-[20px]" style={{ background: PICKUP_BLUE }} onClick={departed ? onPickedUp : onDepart}>
          {departed ? '픽업 완료하기' : '픽업 출발하기'}
        </button>
      </div>
    </div>
  );
