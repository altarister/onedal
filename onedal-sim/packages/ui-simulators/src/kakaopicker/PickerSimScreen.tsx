 ...
 * - «내 오더» 카드를 누르면 **수락 뒤 단계** (`PickerOngoingScreen` · 실물 16~31) — 픽업 이동부터 · 완료하면 잡은 콜에서 빼고 리스트로
 * - 상세를 열면 방문 기록에 한 칸 남는다 (`nets.ts` 의 `detailInHistory`) — 원달앱의 «뒤로 가기»가 상세만 닫는다
 * - 🚫 **리스트에 오래 떠 있던 콜은 남이 가져갔다** — 누르면 상세 대신 «이미 배정이 완료된 오더입니다» 토스트 (실물 캡처 03)
 */
...
/**
 * 🚫 **이 콜을 남이 가져가는 시각** — 리스트에 뜬 뒤 몇 ms (시뮬레이터 값).
 *
 * 🔴 **원달앱의 시간과 무관하다.** 원달앱의 «알람 상세 30초 자동 복귀»에 맞춰 두면 시뮬이 원달앱을 아는 셈이 된다 —
 *    진짜 픽커는 원달앱이 몇 초 뒤 돌아오는지 모른다 (기사님: *"시뮬레이터는 진짜 픽커 처럼 작동해야"*). 실제로 언제 가져가는지도 모른다.
...
/**
 * 🔴 **실물 글자 그대로 · 줄바꿈이 든 한 덩어리** — 실물에서 원달앱이 이 모양 그대로 읽어 카드 출발지로 넣는다
 *    (출발지 «이미 배정이 완료된⏎오더입니다.»).
...
export const PickerSimScreen = (p: NetScreenProps) => {
  ...
  if (p.selectedCall && isPickerCall(p.selectedCall)) {
    ...
    /* 🚚 잡은 콜이면 수락 뒤 단계 — 수락 전 상세(«넘기기»·«수락하기»)를 다시 그리지 않는다 */
