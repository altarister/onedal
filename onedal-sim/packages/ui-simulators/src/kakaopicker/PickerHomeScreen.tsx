/**
 * 🏠 **픽커 홈 (출근 전)** — 실물 캡처 01 · 실물 화면 덤프 01
 *
 * 원달앱은 **「시작하기」 버튼**으로 홈을 알아본다 (`KakaoPickerKeywords.STAGE_WORDS` · 기사님 확정).
 * 🔴 미션 문구 «퀵 1건 배송완료하고» 를 **실물 그대로** 둔다 — 원달앱은 홈을 먼저 봐서 이 글자가 «배송 단계»로
 *    잘못 읽히지 않게 한다. 시뮬레이터가 이 글자를 빼면 그 순서를 시험하지 못한다.
 * 글자 덩어리마다 요소 하나 — 픽커는 네이티브 앱이라 덩어리가 따로 온다 (`PickerDispatchBoard` 머리 주석).
 */
interface Props {
  onStart: () => void;
  onMenuClick: () => void;
