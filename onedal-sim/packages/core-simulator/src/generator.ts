 ...
 * ⏰ **상차 시각은 «지금»에서 잰다**.
 *
 * 근무시간(08~16시) 안에서 아무 시각이나 뽑으면, 새벽에 시험을 돌릴 때 적요에 «15:58상차» 처럼
 * 먼 시각이 적히고 관제웹이 그것을 상차 약속으로 그대로 읽어(적요 상차 시각 > 콜 잡은 시각 + 잠정)
 * 버퍼가 수백 분이 된다 — 모든 콜이 먼 예약콜이 되어 **약속 축이 통째로 무의미해진다.**
 *
 * 실제 배차망은 «지금 곧» 실을 콜을 띄운다. 그러니 시뮬도 지금에서 재야 시험이 성립한다.
 ...
 * 🔴 상차지·하차지가 **맨 앞**이다 — 그래서 배차망별 칸의 난수가 바뀌어도 같은 씨앗이면
 *    주소·좌표·거리가 똑같다 (`tests/commonFields.test.ts` 가 씨앗마다 스냅숏으로 문다).
 */
