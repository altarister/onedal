/**
 * 🚚 **화물 콜의 요금·차종·결제 칸** — 화물 배차망 입히기 함수가 함께 쓴다 (2026-09-14 · 카카오픽커_시뮬레이터.md 0단계 0-2 ④)
 *
 * 예전엔 공통 생성기(`generateSimCall`)가 모든 콜에 이 칸을 채웠다. 이제 생성기는 공통 칸만 만들고,
 * 이 칸이 필요한 배차망의 입히기 함수가 부른다. 🔴 이 파일은 **배차망 이름을 모른다** — 쓰는 쪽이 고른다.
 * (픽커처럼 요금 체계가 다른 배차망은 이것을 안 쓴다)
 *
 * 값과 풀은 예전 생성기에서 **그대로** 옮겼다.
 */
...
/**
 * 🔴 **차종은 «표준 이름»으로 낳는다 — 화면 말로 낳지 않는다** (2026-09-11).
 *
 * 예전에는 인성 약자(`오`·`다`·`라`)를 그대로 넣었다. 그러면 **화물24시 화면이 읽을 말이
 * 없다** — 그쪽은 «다마스»·«1톤» 이라 적는다. 값은 한 벌로 두고 각 화면이 자기 말로
 * 옮긴다 (`formatInsungVehicle` · `formatHwamul24Vehicle`). 문제지도 이미 풀네임을 쓴다.
 */
