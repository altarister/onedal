...
/**
 * 🎬 **볼트 일요일 — 유튜브 실제 하루** (기사님 지시)
 *    영상: 「천기누설! 주말 자차부업 일당 30만원 버는 법」 (itfFh0jkf4I)
 *    여덟 콜의 상차·하차지를 카카오가 아는 실제 장소로 찾아 박았다 (`주소생성`·`주소검증` 통과).
 *    🔴 **주소를 일부러 사전에 없는 글자로 둔다** — `findMockEntry` 는 `addressDetail.includes()`
 *       로 찾아 **사전 좌표를 쓴다.** 사전의 비슷한 주소에 걸리면 콜이 볼트 실제 자리가 아닌
 *       엉뚱한 곳에서 뜬다(김포 사우동을 고촌읍으로). 못 찾으면 `fallback` 좌표가 그대로 쓰이므로
 *       **카카오 실측이 살아 있다** — 그것이 이 문제지가 원하는 것이다.
 *    🔴 주소에 **동 이름을 넣는다** — `stopAddressHasRegion`(«주소 전체에 그 동 이름이 들어 있다»)이 문다.
 *    🔴 **주소 사전(`mockLocationData.json`)은 건드리지 않는다** — 문제지는 여기서 좌표를 직접
 *       주므로 사전이 필요 없고, 사전을 늘리면 씨앗 기반 랜덤 콜이 달라져 스냅샷이 깨진다.
 */
const BOLT_GIMPO_P = stop('사우동', '경기 김포시 사우동 사우중로 1', 126.71430750, 37.61606397);
const BOLT_GWANGJIN_D = stop('자양동', '서울 광진구 자양동 강변역로4길 34', 127.09222858, 37.53413911);
const BOLT_GWANGJIN_P = stop('자양동', '서울 광진구 자양동 강변역로4길 10', 127.09340506, 37.53534922);
const BOLT_HWASEONG_D = stop('매송면', '경기 화성시 매송면 서해로 2448-32 화성함백산추모공원', 126.9109, 37.2704);
const BOLT_SEONGNAM_P = stop('성남동', '경기 성남시 중원구 성남동 산성대로', 127.12473185, 37.42563565);
const BOLT_GWONSEON_D = stop('서둔동', '경기 수원시 권선구 서둔동 권선로699번길 7', 127.02779063, 37.25946840);
const BOLT_PANGYO_P = stop('삼평동', '경기 성남시 분당구 삼평동 동판교로177번길 25', 127.11356227, 37.39732076);
const BOLT_SUWON_D = stop('매산로', '경기 수원시 팔달구 매산로 덕영대로 924 수원역', 127.00017993, 37.26559208);
const BOLT_BANWOL_P = stop('성곡동', '경기 안산시 단원구 성곡동 별망로 178', 126.7456, 37.3105);
const BOLT_NAMDONG_D = stop('고잔동', '경기 안산시 단원구 고잔동 광덕대로 193', 126.70624949, 37.38393653);
const BOLT_NAMDONG_P = stop('구월동', '인천 남동구 구월동 남동대로 559', 126.70772327, 37.43228497);
const BOLT_GEUMGWANG_D = stop('금광동', '경기 성남시 중원구 금광동 광명로', 127.16178877, 37.44571131);
const BOLT_JAHYE_P = stop('은행동', '경기 성남시 중원구 은행동 자혜로16번길 56', 127.15925041, 37.44561456);
const BOLT_JONGNO_D = stop('낙원동', '서울 종로구 낙원동 삼일대로', 126.98014999, 37.57025726);
const BOLT_NONHYEON_P = stop('논현동', '서울 강남구 논현동 논현로111길 16', 127.0322, 37.5076);
const BOLT_SOGONG_D = stop('소공동', '서울 중구 소공동 남대문로 81', 126.97489475, 37.56556387);
...
export const PRESETS: Record<string, PresetProblem[]> = {
    ...
    /**
     * 🎬 **볼트 일요일 — 유튜브 실제 하루를 그대로** (기사님 지시)
     *
     * 「천기누설! 주말 자차부업 일당 30만원 버는 법」(itfFh0jkf4I) 에서 읽은 여덟 콜이다.
     * 기사님이 영상 화면(단말기)을 보고 시각·금액·주소를 표로 채우셨고, 그 값을 그대로 옮겼다.
     *
     * 🔴 **요금은 실제 값이다** — 다른 문제지처럼 200,000 으로 부풀리지 않았다. 이 문제지의 목적이
     *    «볼트가 잡은 선택이 옳았나»를 우리 판정과 대조하는 것이라, 요금을 바꾸면 물음이 사라진다.
     *
     * 볼트의 그날: 목표 30만원 · 15:00 에 22만원 · 승용차(전기차) · 단말기 하한 2만원 ·
     * 자동배차 10km 반경 · 3만원~최대 · 도착지 전체.
     *
     * 🔴 **①②는 김포·광진구라 다른 콜과 멀다** — ③부터가 성남·수원 묶음이고, ⑤⑥이 안산·인천,
     *    ⑦⑧이 서울이다. 한 자리에서 다 뜨는 문제지가 아니라 **하루를 따라가는** 문제지다.
     *
     * 무엇을 보나:
     *   · ④ 판교→수원역 — 볼트가 «10분이 안 걸린다»며 잡은 합짐. 꼬리 빼기가 얼마를 남기나
     *   · ⑦ 성남→종로 23,100 — 볼트가 «단가는 좋지 않은데 인서울 물량을 노려» 잡았다
     *   · ⑧ 논현→소공동 20,000 — 볼트가 «10분 차이가 안 나서 15,000원을 더해 억지로 만들었다»
     *   · ① 김포→광진구 — 볼트는 «개꿀짐»이라 했고 시세로는 +903원이다. 43km 48분이라 시급은 2.1만
     */
    '볼트일요일': [
        ride('① 김포 사우동 → 광진구 자양동 · 30,800', BOLT_GIMPO_P, BOLT_GWANGJIN_D,
             '🚚 08:13 에 잡은 첫짐 — 볼트 «개꿀짐»(알루미늄 파이프) · 시세 +903원 · 43km 48분이라 시급은 2.1만'),
        ride('② 광진구 자양동 → 화성 함백산추모공원 · 34,650', BOLT_GWANGJIN_P, BOLT_HWASEONG_D,
             '🚚 09:30 · 1번 배송 중에 잡았다 — 상조 물품 2개 · 픽업 1분 · 카카오 51분인데 실제 138분'),
        ride('③ 성남 성남동 → 수원 권선구 · 24,640', BOLT_SEONGNAM_P, BOLT_GWONSEON_D,
             '🚚 10:10 · 볼트 «길목과 길목» — 2번을 싣고 성남으로 내려가며 잡았다'),
        ride('④ 판교 삼평동 → 수원역 · 30,800', BOLT_PANGYO_P, BOLT_SUWON_D,
             '🚚 10:25 · 볼트 «10분이 안 걸린다»며 잡은 합짐 — 하차가 역사 3층이라 끌차가 필요했다'),
        ride('⑤ 안산 성곡동 → 안산 고잔동 · 23,100', BOLT_BANWOL_P, BOLT_NAMDONG_D,
             '🚚 12:30 · 볼트 «저단가 · 공차로 나갈 수도 있었다» — 인천으로 나가는 길에 주웠다'),
        ride('⑥ 인천 남동대로 → 성남 금광동 · 38,000', BOLT_NAMDONG_P, BOLT_GEUMGWANG_D,
             '🚚 13:13 · 그날 가장 비싼 콜 — 픽업까지 20분 · 고속도로 43분 · 문 앞 하차'),
        ride('⑦ 성남 자혜로 → 종로 낙원동 · 23,100', BOLT_JAHYE_P, BOLT_JONGNO_D,
             '🚚 14:10 · 볼트 «단가는 좋지 않은데» 인서울 물량을 노려 잡았다 — 6번 하차지 세 집 옆'),
        ride('⑧ 논현동 → 소공동 롯데백화점 · 20,000', BOLT_NONHYEON_P, BOLT_SOGONG_D,
             '🚚 14:55 · 볼트 «10분 차이가 안 나서 15,000원을 더해 억지로 만들었다» — 오토바이 오더(통합)'),
    ],
};
...
export const PRESET_REQUIRES: Record<string, PresetRequires> = {
    ...
    /**
     * 🎬 볼트 일요일 — 그날 볼트는 목적지를 말하지 않았다. 「지리」는 그래서 배수 ×1.0 이다.
     *    🔴 도착 목표를 굳이 박지 않는다 — 박으면 그날에 없던 조건을 시험하는 것이 된다.
     *    상차 반경만 볼트 설정대로 10km 로 둔다 (자동배차 «10km 반경 · 3만원~최대»).
     */
    '볼트일요일': { minPickupRadiusKm: 10 },

};

const ALIASES: Record<string, string> = {
    seven: '칠지점', '7': '칠지점',
    bolt: '볼트일요일', sunday: '볼트일요일', 볼트: '볼트일요일', 일요일: '볼트일요일',
    /**
...
export const PRESET_MENU: Array<{ key: string; title: string; desc: string }> = [
    ...
    { key: '볼트일요일', title: '🎬 볼트 일요일 — 유튜브 실제 하루 여덟 콜',
      desc: '8문제 · **요금이 실제 값이다**(30,800 · 34,650 · 24,640 · 30,800 · 23,100 · 38,000 · 23,100 · 20,000) — ' +
            '다른 문제지처럼 20만으로 부풀리지 않았다. «볼트가 잡은 선택이 옳았나»를 우리 판정과 대조하는 자리라 ' +
            '요금을 바꾸면 물음이 사라진다. 🔴 **「📍 내 위치 찍기」로 «경기 김포시»** — 그 목록에 동은 ' +
            '없다(볼트는 김포 사우동에서 시작했고 같은 시 안이라 방향·거리 판정은 같다) · ' +
            '**도착 목표는 비워 둔다** — 그날 볼트가 목적지를 말하지 않았다(「지리」는 배수 ×1.0). ' +
            '상차 반경은 **10km**(볼트 자동배차 설정 그대로). ' +
            '①②는 김포·광진구라 멀고 ③부터 성남·수원, ⑤⑥ 안산·인천, ⑦⑧ 서울이다 — 한 자리에서 다 뜨는 것이 아니라 ' +
            '**하루를 따라가는** 문제지다. ④는 볼트가 «10분이 안 걸린다»며 잡은 합짐이고, ' +
            '⑧은 «10분 차이가 안 나서 15,000원을 더해 억지로 만들었다»는 그 콜이다' },
