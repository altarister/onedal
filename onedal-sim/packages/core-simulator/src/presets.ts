...
export interface PresetProblem {
    ...
    filler?: boolean;
    /**
     * 🧩 **배차망만 아는 칸** — 공통 코드는 이름도 뜻도 모르고 그대로 넘긴다 (카카오픽커_시뮬레이터.md 3단계 3-2).
...
//     그래서 **주소로** 박는다.
//  🔴 `region` 은 **반드시 «동» 이름이다** (실사고). 처음에 «강서구»라 적었더니
...
// ══════════════════════════════════════════════════════════════════════
//  🚚 볼첨지 이틀 — 실측 표를 그대로 문제지로 (11 · 유튜브 자막 + 기사님 화면 캡처)
//
...
//  🔴 `region` 은 **동 이름**이다 (실사고 — 「강서구」라 적었다가 셋 다 떨어졌다).
 ...
 *    (실사고). 화면에 그려지는 글자가 이것이고, 앱은 화면을 읽는다.
 */
...
export const PRESETS: Record<string, PresetProblem[]> = {
     ...
     */
    /**
     * 🌅 **볼트 오전 — 하루가 한 판으로** (지도 실험실에서 옮겨 왔다)
     *
    ...
    /**
     * 🌆 **볼트 저녁 — 앉은 채로 셋, 달리며 하나** (목업에서 옮겨 왔다)
     *
    ...
    /**
     * 🚗 **초월(집) → 이천 롯데아울렛 — 내일 실주행 문제지** (기사님 확정)
     *
...
export const PRESET_REQUIRES: Record<string, PresetRequires> = {
    ...
    /**
     * 🚚 **7지점 한 바퀴** — 정답표 주석의 전제를 옮겼다. 조건이 없어 시작 전 점검 줄이 아예 안 떴다.
...
const ALIASES: Record<string, string> = {
    seven: '칠지점', '7': '칠지점',
    /**
     * ⚠️ **볼첨지 넷·지도청주·축·서진·오탐·초월이천의 별칭을 함께 걷었다**.
...
export const PRESET_MENU: Array<{ key: string; title: string; desc: string }> = [
    /* 🚚 **하루 한 판** — 흩어진 볼첨지 네 조각과 달리 «오전/저녁»을 통째로 밟는다 */
...
/**
 * 📚 **문제지 책** — 문제 · 설정 화면 목록 · 요구하는 판 상태 · 별칭을 한 묶음으로 (3단계 3-2).
