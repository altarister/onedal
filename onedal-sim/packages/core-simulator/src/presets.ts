...
export const PRESETS: Record<string, PresetProblem[]> = {
    ...
    '구로': [
        ...
        /**
         * 🏠 **복귀 콜** (기사님 지적 2026-08-23): *"구로에서 집으로 오는 케이스의 콜도 있어야 해."*
         *
         * 맞다. 갈 때는 전진 국면(DEST), **올 때는 복귀 국면(HOME)** 이다 —
         * 돌아오는 길이 **하루 흐름의 나머지 절반**이고, 굳이 따로 나가지 않아도 붙어 온다.
         *
         * 🔴 **상차지가 구로 쪽이라 갈 때는 저절로 걸러진다.** 시뮬레이터의 콜 목록은 그대로인데,
         *    서버 필터는 **실제 GPS** 를 기준으로 도니까 — 초월에 있을 땐 멀어서 떨어지고,
         *    구로에 닿으면 가까워져 통과한다. **가만히 둬도 국면이 저절로 넘어간다.**
         *    (그래서 이 두 문제는 갈 때 `상차지 N` 으로 세어지는 것이 정상이다)
         */
        {
            label: '⑨ 구로 → 초월 — 복귀 첫짐 (집으로)',
            pickup: '구로구', dropoff: '초월읍',
            fare: 92000, vehicleType: '1t',
            why: '돌아오는 길의 주인공. 하차가 집 방향이라 복귀 국면에서 잡혀야 한다',
        },
        {
            label: '⑩ 영등포 → 경안동 — 복귀 합짐',
            pickup: '영등포구', dropoff: '경안동',
            fare: 85000, vehicleType: '1t',
