 ...
 *    🔴 **좌표는 카카오 실측이고, 주소는 «시 구 동»으로 적는다** — 도로명주소에는 동 이름이
 *       안 들어가서 `stopAddressHasRegion` 검사(«주소 전체에 그 동 이름이 들어 있다»)가 문다.
 *       기존 문제지들도 같은 까닭으로 동 단위로 적혀 있다.
 *    🔴 **주소 사전(`mockLocationData.json`)은 건드리지 않는다** — 문제지는 여기서 좌표를 직접
 *       주므로 사전이 필요 없고, 사전을 늘리면 씨앗 기반 랜덤 콜이 달라져 스냅샷이 깨진다.
 */
const BOLT_GIMPO_P = stop('풍무동', '경기 김포시 풍무동', 126.7234, 37.6122);
const BOLT_GWANGJIN_D = stop('자양동', '서울 광진구 자양동', 127.09222858, 37.53413911);
const BOLT_GWANGJIN_P = stop('자양동', '서울 광진구 자양동 강변역로4길', 127.09340506, 37.53534922);
const BOLT_HWASEONG_D = stop('매송면', '경기 화성시 매송면 서해로 2448-32 화성함백산추모공원', 126.9109, 37.2704);
const BOLT_SEONGNAM_P = stop('성남동', '경기 성남시 중원구 성남동', 127.12473185, 37.42563565);
const BOLT_GWONSEON_D = stop('서둔동', '경기 수원시 권선구 서둔동', 127.02779063, 37.25946840);
const BOLT_PANGYO_P = stop('삼평동', '경기 성남시 분당구 삼평동', 127.11356227, 37.39732076);
const BOLT_SUWON_D = stop('매산로', '경기 수원시 팔달구 매산로', 127.00017993, 37.26559208);
const BOLT_BANWOL_P = stop('성곡동', '경기 안산시 단원구 성곡동', 126.7456, 37.3105);
const BOLT_NAMDONG_D = stop('고잔동', '경기 안산시 단원구 고잔동', 126.70624949, 37.38393653);
const BOLT_NAMDONG_P = stop('구월동', '인천 남동구 구월동', 126.70772327, 37.43228497);
const BOLT_GEUMGWANG_D = stop('금광동', '경기 성남시 중원구 금광동', 127.16178877, 37.44571131);
const BOLT_JAHYE_P = stop('은행동', '경기 성남시 중원구 은행동 자혜로', 127.15925041, 37.44561456);
const BOLT_JONGNO_D = stop('낙원동', '서울 종로구 낙원동', 126.98014999, 37.57025726);
const BOLT_NONHYEON_P = stop('논현동', '서울 강남구 논현동', 127.0322, 37.5076);
const BOLT_SOGONG_D = stop('소공동', '서울 중구 소공동', 126.97489475, 37.56556387);
...
export const PRESETS: Record<string, PresetProblem[]> = {
     ...
     */
    '볼트일요일': [
        ride('① 김포 풍무동 → 광진구 자양동 · 30,800', BOLT_GIMPO_P, BOLT_GWANGJIN_D,
             '🚚 08:13 에 잡은 첫짐 — 볼트 «개꿀짐»(알루미늄 파이프) · 시세 +903원'),
...
export const PRESET_MENU: Array<{ key: string; title: string; desc: string }> = [
    ...
    { key: '볼트일요일', title: '🎬 볼트 일요일 — 유튜브 실제 하루 여덟 콜',
      desc: '8문제 · **요금이 실제 값이다**(30,800 · 34,650 · 24,640 · 30,800 · 23,100 · 38,000 · 23,100 · 20,000) — ' +
            ...
            '요금을 바꾸면 물음이 사라진다. 🔴 **「📍 내 위치 찍기」로 김포 풍무동** (126.7234, 37.6122) · ' +
