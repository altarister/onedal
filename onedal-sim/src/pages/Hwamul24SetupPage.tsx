...
export function Hwamul24SetupPage() {
  ...
  const handleStart = () => {
    ...
    navigate(`/hwamul24/dispatch?${params.toString()}`);
  };

  return (
    <div className="w-full h-dvh bg-gradient-to-br from-[#c62828] via-slate-800 to-slate-900 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-slate-800/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-700/50 overflow-hidden">
        {/* 헤더 */}
        <div className="bg-gradient-to-r from-[#d32f2f] to-[#c62828] px-6 py-5">
          <h1 className="text-white text-xl font-bold tracking-tight">🚚 화물24시 시뮬레이터</h1>
          <p className="text-red-200 text-sm mt-1">시뮬레이션 환경을 설정하세요</p>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-700 text-white rounded-lg px-4 py-3 text-sm border border-slate-600 focus:border-red-500 focus:ring-1 focus:ring-red-500 outline-none transition"
            >
          ...
          <div>
            <label className="block text-slate-300 text-sm font-semibold mb-2">
              📏 상차 반경: <span className="text-red-400">{maxPickupKm}km</span>
            </label>
            <input
              type="range"
              min={5}
          ...
          <button
            onClick={handleStart}
            className="w-full bg-gradient-to-r from-red-600 to-[#b71c1c] text-white font-bold py-4 rounded-xl text-lg shadow-lg shadow-red-600/30 hover:shadow-xl hover:from-red-500 hover:to-[#c62828] active:scale-[0.98] transition-all"
          >
            시뮬레이션 시작 →
          </button>
        </div>
