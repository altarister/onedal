 ...
 * 🚚 **설정 한 장** (`/`) — 기사님 확정
 *
 * 설정 화면은 배차망마다 두지 않고 한 장이다 — 두 벌이면 한쪽만 자라 문제지 탭·점검·채움 콜 수가
 * 한 배차망에서 빠진다. 문제지·주소·콜 생성이 공용이니 그 설정도 한 곳에 산다 (규칙 ③ — 같은 값이 두 곳에 살지 않는다).
 *
 * 갈라지는 것은 **배차 리스트 화면 한 장**뿐이고, 여기서는 헤더의 스위치로 고른다.
 *
 * 🔴 **시나리오 고르기 → 문제지 고르기 → 옵션 확인 → 시작까지 스크롤이 없어야 한다**
 *    (기사님). 그래서 두 가지를 지킨다:
 *      ① 설정값은 **칩 한 줄** — 슬라이더는 라벨·트랙·양끝 글자로 세 줄을 먹는다
...
/**
 * 어느 배차망 화면으로 볼 것인가 — 목록은 `nets.ts` 한 곳에 있다.
 * 배차망을 붙일 때는 `nets.ts` 만 고친다 — 이 파일은 그 목록을 읽기만 한다.
 */
 ...
 * 걸린다. 45초는 원달앱으로 끝까지(잡기 → 심사 → 결재 → 안전취소)
 * 한 바퀴 해 보는 자리다.
 */
const INTERVAL_CHOICES = [5000, 10000, 20000, 45000];
/** 🧱 채움 콜 — 집 3개(주행 40초) · 차 20개(주행 40분). 기본 0 (기사님 확정) */
...
export function SetupPage() {
  ...
  // 목록의 첫 배차망이 기본이다 — 배차망 이름은 nets.ts 만 안다
  ...
  // 🔁 기본 꺼짐 (기사님 확정) — 한 바퀴만 돌려야 무엇을 놓쳤는지 셀 수 있다
  ...
  /**
   * 📚 **고른 배차망의 문제지 책** (nets.ts `presetBook`) — 인성·화물24시는 원 단위 문제지, 픽커는 P 단위 문제지.
   ...
   *    화면에 두면 «이게 판정에 영향을 준다»는 오해만 만든다 (기사님).
   * 🚚 개별콜은 **배차망 하나만** 넘긴다 — 콜은 현황판에서 오고, 간격·채움이 없다 (기사님).
   */
  const start = () => {
  ...
  return (
    <div className="w-full h-dvh flex flex-col bg-gradient-to-b from-slate-900 via-slate-800 to-slate-900 text-slate-200 font-sans">
      ...
      {/**
        * 🔴 **넘치면 «문제지 목록»만 줄어든다** (기사님).
        *
        * 문제지 수는 늘었다 줄었다 한다. 본문 전체가 스크롤이면
      ...
      <div className="flex-1 min-h-0 overflow-hidden p-3 flex flex-col gap-2.5">
        ...
        {shownTab === 'scenario' ? (
          <>
            {/**
              * 🔴 **줄에는 제목과 문제 수만** (기사님).
              *    고른 문제지의 설명 전문을 그 자리에서 펼치면 화면이 길어져 시작 버튼이 밀린다.
          ...
          /* 🚚 **개별콜 — 설정이 없다** (기사님). 빈 리스트로 들어가 현황판에서 보낸 콜만 쌓인다 */
 ...
 * `+` 는 커밋 안 된 고침이 섞였다는 뜻 · `빌드` 는 개발 서버가 아니라 빌드한 결과물(rehearsal 등)이라는 뜻 (buildInfoPlugin.ts).
...
/**
 * 🔴 **설정값은 칩 한 줄이다** (기사님).
 * 슬라이더는 라벨·트랙·양끝 글자로 **세 줄**을 먹어 시작 버튼을 화면 밖으로 밀어낸다.
...
function PresetRow({ title, count, on, onPick, onInfo }: {
  ...
  /**
   * 🔴 **날짜를 제목에서 떼어 낸다** — 제목 끝의 날짜 괄호가 있으면 정작 무슨 문제인지가
   *    «...»로 잘린다. 날짜는 «언제 만든 문제지인가»라 덮개로 물러서도 된다.
...
// ══════════════════════════════════════════════════════════════════
//  🧪 판 점검 — «지금 이 문제지를 채점할 수 있는 상태인가» (기사님 지시)
// ══════════════════════════════════════════════════════════════════
/**
 * 🔴 **콜이 안 올라오면 둘 중 하나다:**
 *      ㉮ 우리 시스템이 옳게 걸렀다 (채점 결과)   ㉯ 시험 상태가 오염됐다 (잡음)
 *    ㉯ 는 현위치가 딴 데 · 재기동에 도착 목표가 옛 값으로 돌아감 · 취소했는데 필터가 합짐 모드에 남음 ·
 *    `tsx watch` 가 못 잡아 옛 코드가 돎 같은 것이다. 이 점검이 시작 전에 ㉯ 를 드러내야
 *    콜이 안 올라온 것을 ㉮ 로 읽을 수 있다 — 안 그러면 가려내는 데 시험마다 20~30분이 간다.
 *
 * 🔴 **화면에는 한 줄만 산다** — 다섯 줄을 늘 펴 두면 시작 버튼이 밀려난다.
...
function Preflight({ presetKey, requires, api, onOpen }: {
  ...
  /**
   * 📱 **폰 줄은 조건이 없는 문제지에도 뜬다** — 폰이 옛 필터로 돌면 어느 문제지든 채점이 아니라 잡음이다.
   *    서버 대답이 오기 전에는 조건 없는 문제지면 아무것도 안 그린다.
   */
