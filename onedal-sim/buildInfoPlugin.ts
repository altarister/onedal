/**
 * 🧾 **지금 화면이 어느 코드인가** — `virtual:build-info` 로 커밋 번호를 싣는다 (기사님)
 *
 * 기사님은 폰의 시뮬레이터 **앱으로만** 들어오신다 — 주소를 칠 수 없으니, 방금 고친 코드가 떴는지 앱 안에서 보여야 한다.
 ...
 * ⚠️ 커밋할 때 페이지를 **다시 불러오지는 않는다** — 폰에서 하던 시험을 끊지 않으려고. 새로 열 때 바뀐다.
 */
