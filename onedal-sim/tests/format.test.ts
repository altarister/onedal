...
/**
 * 🔒 **지금 동작을 그대로 잠근다** — 배차망별 표기 함수
 *
 * 이 함수들은 **한 배차망 화면만** 쓰므로 배차망 폴더(`insung/insungCall.ts` · `hwamul24/hwamul24Call.ts`)에 있다.
 * 기대값은 지금 화면 글자 그대로다 — 고칠 점이 보여도 여기서 기대값만 바꾸지 않고, 함수와 원달앱 파서를 함께 고친다.
 *
 * 원달앱 파서가 이 글자를 읽는다 — 인성은 차종 약자(오·다·라)를 기준 삼아 그 옆의 요금을 읽는다
 * (insungCall.ts 주석).
 */
