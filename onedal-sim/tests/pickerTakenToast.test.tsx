...
/**
 * 🚫 **남이 가져간 콜 — «이미 배정이 완료된 오더입니다» 토스트**
 *
 * 실물(캡처 03): 리스트에서 누른 콜을 다른 기사가 이미 가져갔으면 상세가 안 열리고 리스트 아래에 토스트가 뜬다.
 * 🔴 원달앱이 이 토스트 글자를 **카드의 출발지로 읽으면** 가짜 콜이 서버에 올라간다.
 *    시뮬레이터가 실물 글자 그대로 띄워야 원달앱이 그것을 거르는지 시험할 수 있다.
 *
...
describe('픽커 — 가져간 콜 토스트', () => {
    ...
    it('🔴 가져간 시각이 지난 카드를 누르면 — 상세를 안 열고 실물 글자 토스트 · 카드가 빠진다', () => {
        ...
        // 실물처럼 줄바꿈이 든 한 덩어리 글자 — 원달앱이 실물에서 읽는 그 모양
