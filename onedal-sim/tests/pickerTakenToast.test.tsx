...
import { PickerSimScreen, PICKER_TAKEN_AFTER_MS, PICKER_TOAST_MS } from '@altari/ui-simulators';
 ...
 *
 * 규칙(시뮬레이터 값): 리스트에 **30초 넘게** 떠 있던 카드를 누르면 «가져간 콜»로 본다 — 토스트 · 카드가 빠진다 · 상세는 안 열린다.
 * 알람 상세에서 30초 자동 복귀한 뒤 오래된 카드를 누를 때 폰 시험에서도 저절로 나온다.
 */
...
describe('픽커 — 가져간 콜 토스트', () => {
    it('규칙 값 — 30초가 지나면 가져간 콜 · 토스트는 잠깐', () => {
        expect(PICKER_TAKEN_AFTER_MS).toBe(30_000);
    ...
    it('🔴 30초 넘게 떠 있던 카드를 누르면 — 상세를 안 열고 실물 글자 토스트 · 카드가 빠진다', () => {
        const openCall = vi.fn();
        mount(<PickerSimScreen {...props({ openCall })} />);
        act(() => { buttonByText('시작하기')!.click(); });
        act(() => { vi.advanceTimersByTime(PICKER_TAKEN_AFTER_MS + 1_000); });
        act(() => { fareDiv('16,870')!.click(); });
    ...
    it('나중에 들어온 카드는 그때부터 잰다 — 30초 전 카드만 가져간 콜', () => {
        const openCall = vi.fn();
