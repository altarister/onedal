...
## 11. 설정 화면 (SettingsScreen) 특수 동작 (Task 25)

실제 오더가 없는 야간이나 시뮬레이터 환경에서 OSRM 라우팅 및 팝업 파싱을 강제로 테스트하기 위해, 설정 화면에 개발자 전용 백도어가 복구되었습니다.

- **가상 콜(Mock Order) 발생기**: "테스트 가상 콜 화면 열기" 버튼을 클릭하면, 하드코딩된 가상의 콜 데이터를 생성하여 실제 배차 상세 화면과 동일한 동작을 강제로 트리거합니다.
- 이를 통해 실기기(A24) 및 시뮬레이터 환경에서 서버와 클라이언트 간의 `/detail` 전송 및 피기백(Piggyback) 상태 전이 로직을 안전하게 검증할 수 있습니다.
