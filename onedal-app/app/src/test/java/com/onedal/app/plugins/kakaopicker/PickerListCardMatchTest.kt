...
/**
 * 👀 **누가 열었든 픽커 상세는 리스트 카드를 찾아 판정까지 간다** (폰 시험)
 *
 * 기사님: *"내가 픽업에서 리스트를 클릭했는데.. 관제엡에서는 콜로 인지 하지 않았고 평가 하지 않았어.
 * 내가 손으로 누르면 경로를 찾아서 평가해야 해"*
 *
 * 알람이 눌렀든 손으로 열었든 **상세 글자로 리스트 카드를 찾는다** — 못 찾으면 손으로 연 상세는
 * `👀 [미리보기 보류]` 로 멈추고 서버에 아무것도 안 가서 관제웹이 콜로 알지 못한다.
 *
 * 🔴 **요금만으로는 못 고른다** — 7지점 문제지에 1만 원 카드가 넷이다 (인성의 «요금이 같은 콜» 역추적이 여기서는 틀린다).
 * 🔴 **실물 픽커는 배송지를 원달앱이 읽는 글자에 안 올린다** — 그래서 픽업지로 먼저 가른다.
 */
class PickerListCardMatchTest {
    ...
    /** 폰 로그 `📄 [상세 실물]` 그대로 — 기사님이 손으로 연 시뮬레이터 상세 */
    ...
    /** 같은 때 리스트 — 7지점 문제지 카드 (`⏭️ [이미 본 콜]` 로그) */
    ...
    /**
     * 🔴 **실물 픽커 상세는 «최종 수익»을 읽는 글자에 안 올린다** (라이브 로그 «👀 미리보기 보류 — 최종 수익을 못 읽었다»).
    ...
    /** 🟢 실물 라이브 `📄 [상세 실물]` 그대로 (요금 · 배송지 · 거리가 안 읽혔다) */
    ...
    /** 🟢 같은 때 실물 리스트 카드 — «수정 위례» 카드에는 크기 글자가 없다(«퀵 승 예약 20:00») */
