...
/**
 * 🚚 **1t 차에 5t 짐을 실을 수 없다** (기사님 실측)
 *
 * 🔴 **톤 차량은 숫자가 정확히 같을 때만 통과한다.** 글자를 담고 있는지로 보면
 *
 *     "1t" -> normParsed.contains("1") || normParsed.contains("t") || normParsed.contains("톤")
 *
 *    허용에 `1t` 가 있을 때 `t` 가 든 차종이 전부 통과한다 — 5t·2.5t·11t·25t 까지.
 *    `contains("1")` 은 11t·1.4t 도 열어 준다.
 *
 * ── 왜 이게 큰 사고인가 ──
 * 잘못 잡은 5t 한 건이 **적재를 채워** 뒤에 오는 콜이 전부 «자리 없음»으로 떨어진다 — 첫짐조차 못 잡는다.
 * 한 축의 오판이 그날 운행 전체를 무너뜨리고, 잡았다 취소하면 취소 1회(배차망 10회 한도)를 쓴다.
 *
 * ⚠️ 규칙 ⑤(*"앱은 넉넉하게 올린다"*)와 어긋나지 않는다 — **넉넉한 것과 틀린 것은 다르다.**
