...
/**
 * ✅ **수락은 화면 보고보다 먼저 알린다 · 승격은 상세 글자를 덮지 않는다**
 *
 * ── 순서가 뒤집히면 ──
 * ```
 * 서버: 상세 이탈(내 오더) — 미리보기를 치운다      ← 서버 규칙대로다
 * 앱  : 수락 확인 — 기사님이 「수락하기」를 누르셨다   ← 수십 ms 늦게 닿는다
 * 앱  : POST /detail (내 오더 화면 글자)            ← 치운 콜이 다시 살아나고 주소가 짧아진다
 * ```
 *
 * 무엇을 막나
 ...
 *   전체 주소가 사라진다. 내 오더 탭에는 「초월읍」 같은 줄임 이름만 있고, 바로 앞 상세에는
 *   「경기 광주시 초월읍 경충대로 907」 같은 전체 주소가 있다
 */
