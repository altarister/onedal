...
/**
 * 🔎 **상세 대기 로그에 «누가 열었나»를 적는다** (기사님 지시 2026-09-14)
 *
 * 기사님: *"손으로 연 상세가 60초 뒤 돌아오는지 — 로그캣에 넣어서 나중에 확인할 수 있게 만들어"*
 *
 * 20:46~20:48 폰 시험은 상세 둘이 모두 알람이 연 것이라, «손으로 연 상세도 돌아온다»를 로그로 가를 수 없었다.
 * 타이머는 누가 열었든 한 곳에서 걸리므로(#124) 동작은 같다 — **기록만** 갈라 적는다.
 * 알람이 카드를 누른 뒤 상세가 뜨기까지는 실측 0.3~0.4초(20:46:27.179 → .450) — 5초 안이면 알람이 연 것이다.
 */
