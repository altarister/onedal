...
/**
 * 🗳️ **떨어진 까닭을 장부에 남긴다** (기사님 지시 — 탈락 원본이 잘 들어가는지 확인하다 드러났다).
 *
 * 탈락한 콜은 원문과 함께 장부에 잘 들어간다 (686건 중 349건이 하한 미달). 그런데
 * **어느 축에서 떨어졌는지가 안 남아** «왜 이 콜이 안 울렸나» 를 폰 로그로만 되짚을 수 있었다.
 * 폰 로그는 3일치뿐이다.
 *
 * 픽커는 «수집 전용이라 잡기 판정이 없다» 며 `withVerdict` 를 비워 뒀는데, 그 뒤 **알람 판정**이
 * 생겨 사정이 달라졌다 — 요금 · 상차거리 · 도착지 세 축으로 이미 가른다.
 *
