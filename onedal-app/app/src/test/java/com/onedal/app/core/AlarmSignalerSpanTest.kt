...
/**
 * 🔔 알람 테두리의 세로 구간 채점 — **테두리가 통과 콜 카드 전체(태그줄·요금·지역줄)를 덮는가**.
 *
 * 요금 글자 한 줄(66px)만 두르면 카드의 태그줄·지역줄이 테두리 밖에 남아, 테두리가
 * 콜 카드가 아니라 **카드와 카드 사이 빈 줄**에 그려진 것처럼 보인다.
 * 세 겹 중 JVM 에서 잡히는 것은 «높이» 다.
 *
...
class AlarmSignalerSpanTest {
    ...
    fun `동시 통과 여러 건이면 요금 최고 하나만 가리킨다 (기사님 확정 0830 - 상세 자동 진입의 대상)`() {
        // 한 스캔에 여러 건(16478·8316·14476)이 같이 통과해도 테두리·상세는 하나다
