...
/**
 * 🧩 **화물24시 카드를 한 장이라도 묶는가** (폰 실측 2026-09-21)
 *
 * 스캔앱이 화면을 24시로 알아보고 파서도 골랐는데(`🔍 Parser (Hwamul24Parser)`)
 * 카드를 한 장도 못 묶었다:
 * ```
 * 👁️ [리스트 스캔] 텍스트노드 126 · 콜그룹 0 · 통과 0 · 요금실패 0
 * ```
 *
 * 🔴 **까닭 둘** — 화면이 숫자와 단위를 **따로** 그린다.
 *   ① 한 덩어리(`30,000원`)만 요금으로 봤다
 *   ② 두 조각을 받아들여도 **«바로 다음 조각»을 보면 못 찾는다.** 요금은 큰 글자,
...
class Hwamul24CardGroupingTest {
    ...
    /**
     * 🔴 **폰에서 그대로 뜬 첫 카드** — 이 배치를 한 장도 못 묶었다.
    ...
    /**
     * 🔴 **첫 카드가 머리 줄을 삼키면 그 안의 잔액이 요금이 된다** (폰 실측 2026-09-21:
