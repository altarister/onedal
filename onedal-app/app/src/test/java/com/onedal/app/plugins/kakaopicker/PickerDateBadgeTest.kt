 ...
 *
 * 예약 **시각**(«17:30»)처럼 예약 **날짜**(«9/23(수)»)도 알아본다 — 못 알아보면 날짜가
 * 지역 이름으로 들어가 지역 칸이 한 칸씩 밀리고 출발·도착이 통째로 틀어진다.
 *
 * ── 실물 글자 (실물 카카오T픽커 · 서버 장부 `intel.rawText` 에서 그대로 옮김) ──
 *   중형 수정 예약 9/23(수) 10,857 퀵 광주 능평 수진2 12.0km
 *   → 날짜를 못 알아보면 «9/23(수) 광주 → 수정 수진2» · 진짜 콜은 «광주 능평 → 수정 수진2»
 *
...
class PickerDateBadgeTest {
     ...
     * 🔴 «준비 완료» 는 실물에서 **한 덩어리로** 온다 (장부 200건 중 44건이 붙은 채 · 쪼개져 온 것은 0건).
     */
    @Test
