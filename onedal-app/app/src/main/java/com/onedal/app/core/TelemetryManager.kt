...
class TelemetryManager(
     ...
     *
     * flush 첫머리에 로그를 남겨야, 같은 전환이 오래 걸렸을 때(0.23초 ↔ 8.1초) **늦게 깨어난 것인지
     * 깨어나서 안에서 걸린 것인지** 가를 수 있다.
     */
     ...
     * 살아 있어도 **콜을 잡을 수 없다** — 이 사실을 안 보내면 관제웹은 녹색으로 남는다.
     *
     * 🔴 `Screen Off` 이벤트로 `sendOffline()` 을 **한 번**만 보내면
     *    60초 뒤 하트비트가 `status = "ONLINE"` 으로 되돌려 버린다.
    ...
    /**
     * 🔴 **여기서 «보내기 직전에 화면을 다시 읽기»를 하지 않는다 — 해 봤고, 안 된다**.
     *
    ...
    /**
     * 🎛️ **서버한테 한 번도 못 들었으면 «잡지 않음»으로 시작한다** (실측 사고).
     *
     * 기본값을 `"AUTO"` 로 두면 **앱을 새로 깐 직후**, 서버가 «알람»이라고 답하기 전에
     * 앱이 스스로 «자동»이라 믿고 콜을 누른다:
     * ```
     * 15:35:40.0  모드: AUTO        ← 아직 서버 말을 못 들음
     * 15:35:40.1  💥 [AUTO] 꿀콜 조건 통과! 강제 터치!
     * 15:35:40.4  🔔 [알람] … 기사님이 직접 누르십니다   ← 서버 대답은 이때 왔다
     * ```
     * 화면이 안 바뀌면 첫 통신이 60초 생존신고라, 그 창이 수십 초가 된다.
    ...
    /**
     * 🚦 **«지금 무슨 일을 하는 중인가»를 보낼 때마다 물어본다** (0단계 ①).
     *
