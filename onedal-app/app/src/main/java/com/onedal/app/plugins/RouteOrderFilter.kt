...
/**
 * 🧭 경로 순서 판정 — 역주행·경로 밖 상차를 잡기 전에 거른다 (기사님 확정)
 *
 * 🔴 하차지만 보면 **상차지가 뒤에 있는 콜**이 통과한다 — 예: 파주 도착 직전의
 *    `초월읍(광주) → 금촌동(파주)` 콜은 78km 뒤로 돌아가 실어야 한다. 잡았다 취소하면 배차망 취소 횟수(10회)를 쓴다.
 *    안 잡는 것과 잡고 나서 버리는 것은 전혀 다르다.
 *
 * `orderKm` 는 서버가 경유 목록을 만들 때 동마다 함께 계산해 둔
 * **"경로 출발점에서 몇 km 지점인가"** 다 (지나온 구간 제거와 같은 값).
 * 운행 중엔 지나온 동이 서버에서 목록째 빠지므로, 키에 없다 = 경로 밖 또는 이미 지나옴.
 *
 * 규칙 (여유 0km — 뒤로 가는 콜은 버린다, 기사님):
