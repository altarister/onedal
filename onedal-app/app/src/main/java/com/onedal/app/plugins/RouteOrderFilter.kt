 ...
 *
 * `progressKm` 는 서버가 경유 목록을 만들 때 동마다 함께 계산해 둔
...
object RouteOrderFilter {
    ...
    fun check(
        pickupText: String,
        dropoffText: String,
        progressKm: Map<String, Double?>,
    ): Result {
        // 첫짐 — 경로가 없으니 순서도 없다. 기존 검사(상차 반경·도착지 매칭)만 돈다
        if (progressKm.isEmpty()) return Result(true, "첫짐 — 순서 검사 없음")

        val pickupHits = progressKm.filterKeys { pickupText.contains(it, ignoreCase = true) }
        if (pickupHits.isEmpty()) {
            return Result(false, "경로 밖 — 상차지(${pickupText.take(20)})가 경유 목록에 없음")
        }

        val dropoffHits = progressKm.filterKeys { dropoffText.contains(it, ignoreCase = true) }
