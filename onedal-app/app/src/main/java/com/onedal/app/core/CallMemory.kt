...
/**
 * 👁️ **«본 콜» 기억 — 스캔 루프의 지문 장부** (#79 · #135 로 평가 기억을 둘로 가름)
 *
 * 지문 세트 하나로는 두 가지 사실을 가르지 못한다:
 *   ① 이 콜은 **평가를 마쳤다** — 다시 평가(알람·클릭)하지 않는다
 *   ② 이 콜은 **서버에 보고했다** — 텔레메트리를 중복으로 보내지 않는다
 *
 * 🔴 한 세트면 콜을 잡는 동안(선점 잠금 · `isActive=false`) 처음 나타난 콜이 **평가 없이** ①의 도장을
 *    받는다 — `decide()` 는 첫 줄에서 돌아서는데 지문은 등재돼, 잠금이 풀려도 «이미 본 콜»로 영영 건너뛴다.
 *
 * 🔴 **#135 — ① 도 두 사실이다: «막았다»와 «눌렀다·통과했다».**
 *    상차 목록은 차가 0.5km 움직일 때마다 바뀐다. 막은 결과를 **어느 필터로 막았는지 없이** 기억하면,
 *    지난 목록으로 막힌 콜이 새 목록을 받은 뒤에도 «이미 본 콜»로 영영 건너뛰어
 *    기사님 표의 뜻 «가까워지면 올라온다»가 무너진다.
...
class CallMemory(
    ...
    /**
     * 🧹 **취소로 끝난 콜은 «눌렀다» 에서 «막았다» 로 내린다** (기사님 · 실주행 시흥동).
     *
     * `markEvaluated` 는 «또 누르지 않으려는» 지뢰 탐지기다. 그런데 그 뜻은 **그 콜이 살아 있는 동안**만
     * 맞다 — 결재가 취소로 끝났으면 그 콜은 다시 판정받을 자격이 있다. 길이 바뀌면 답도 바뀐다.
     *
     * 🔴 그대로 두면 **필터가 아무리 바뀌어도 영영 안 본다**(`onFilterVersion` 은 «막았다»만 비운다).
     *    시흥동 → 송도동(34,650원)이 그렇게 갔다 — 앱이 한 번 누른 뒤, 대전에서 인천까지 올라오는 내내
     *    필터가 93번 바뀌었는데도 다시 판정하지 않아 성남을 그대로 지나쳤다.
     * 🔴 **그렇다고 기억에서 아예 빼지는 않는다.** 빼면 다음 스캔에서 같은 필터로 또 통과해 또 누른다 —
     *    그 콜이 배차망 목록에 남아 있는 한 끝이 없다. 시뮬레이터는 취소한 콜을 목록에서 지우므로
