...
enum class ScreenContext(val value: String) {
    ...
    /**
     * 🏠 **배차망 홈 — 리스트에 들어가기 전** (기사님 실측 제보로 신설).
     *
...
data class ScrapPayload(
    ...
    /**
     * 🎛️ **«나 지금 이 모드다»라는 앱의 대답** (기사님 확정 · 0단계 ②).
     *
     * 🔴 여태 이 대답이 **없었다.** 그래서 관제웹은 모드 버튼을 누르는 순간 바뀐 것처럼
     *    그렸다(«낙관적 업데이트») — 폰이 받았는지 모르면서. 필터에는 그 대답
     *    (`filterVersion`)이 있는데 모드에만 없던 **비대칭**이었다.
