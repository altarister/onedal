...
enum class ScreenContext(val value: String) {
     ...
     *
     * 기사님: *"내가 관제앱에서 현 페이지를 확인할 수 있어야 해.. 관제에서는 폰의 상황을
     * 잘 알아야 해. 그래서 가장 위에 있는 거야."*
     *
     * 인성은 확정하면 리스트로 돌아가고 진행은 **GPS** 가 답한다. 픽커는 반대로
     * **화면이 진행을 말해 준다** — 그래서 인성 목록에는 갈 자리가 없었고 운행 중 다섯이
     * 전부 `UNKNOWN` 으로 올라가 관제웹에 «알 수 없는 화면»(빨간 깜빡임)으로 떴다.
     *
     * ⚠️ 값 이름은 shared 와 **한 글자도 다르면 안 된다** — 서버가 문자열로 받는다.
     *    이름표(관제웹 표시)는 배차망별로 `shared/screenLabels.ts` 에 있다.
     */
    RUN_TO_PICKUP("RUN_TO_PICKUP"),            // 🚚 픽업지로 이동 중
    RUN_AT_PICKUP("RUN_AT_PICKUP"),            // 🚚 픽업지 도착 — 픽업 완료 대기
    RUN_TO_DROPOFF("RUN_TO_DROPOFF"),          // 🚚 배송지로 이동 중
    RUN_AT_DROPOFF("RUN_AT_DROPOFF"),          // 🚚 배송지 도착 — 배송 완료 대기
    RUN_DONE("RUN_DONE"),                      // 🚚 배송 완료

    /**
     * 🏠 **배차망 홈 — 리스트에 들어가기 전** (기사님 실측 제보).
     *
...
data class SimplifiedOfficeOrder(
     ...
     * 단가 판정(fare ≥ deliveryDistance × ratePerKm[차종])의 입력이다.
     */
     ...
     *
     * 🔴 **현황판이 판정을 다시 계산하지 않게 하려는 것이다.** 지금은 앱 판정식을 TS 로
     *    옮겨 적은 **사본**이 화면에 있고, 그것이 이미 한 번 갈라졌다 — 앱은 요율 모델이
     *    서면 `minFare` 를 안 보는데 사본은 그것만 봐서 **가짜 «통과»** 를 냈다.
     *    값을 실어 보내면 사본이 통째로 사라진다.
     *
...
data class DispatchBasicRequest(
    ...
    val targetApp: String = "insung",
    /**
     * 🖱️ 잡은 방식(자동·알람·직접) — 6하원칙 «어떻게», 원장 기록 전용 (기사님 확정).
...
data class ScrapPayload(
    ...
    /**
     * 📦 **이 폰에 깔린 앱 버전** (기사님 확정).
     * `DeviceSession.version` 칸을 앱이 채운다.
     * 고친 것이 폰에 실제로 들어갔는지를 관제웹에서 바로 볼 수 있어야 한다.
     */
    val appVersion: String? = null,
    /**
     * 🚦 **지금 무슨 일을 하는 중인가** — 다섯 칸 중 하나 (`WorkStage`).
     * 여태 `isHolding` 불리언 하나라 «어디서 멈췄는지»를 관제웹이 몰랐다.
     * ⚠️ **이름은 안 보낸다** — 한글 낱말은 관제웹이 짓는다 (두 벌이 되지 않게).
     */
    val workStage: String? = null,
    /** 🚦 팝업이 몇 장째인가 (POPUP 일 때만) */
    val workStageStep: Int? = null,
    /** 🚦 안전취소가 몇 초 남았나 (SAFE_CANCEL 일 때만) */
    val workStageSeconds: Int? = null,
    /**
     * 🎛️ **«나 지금 이 모드다»라는 앱의 대답** (기사님 확정).
     *
     * 🔴 이 대답이 없으면 관제웹은 모드 버튼을 누르는 순간 바뀐 것처럼 그린다(«낙관적 업데이트»)
     *    — 폰이 받았는지 모르면서. 필터의 `filterVersion` 과 같은 짝이다.
     * 서버가 «내가 보낸 것»과 이 값을 대조하면 「적용중」이 저절로 파생된다 (규칙 ③).
     */
     ...
     *    접근성이 막혀 콜을 하나도 못 읽어도, 텔레메트리가 가고 화면 판별이 되면
     *    **관제웹은 파란불**이다.
     *
     ...
     * 한 번만 알리면 60초 뒤 하트비트가 `ONLINE` 으로 되돌려 관제웹이 녹색이 된다.
     * 그래서 사실을 매번 실어 보낸다.
     * ⚠️ 선택 필드 — 옛 서버는 무시하고, 갱신 안 된 APK 는 안 보낸다.
     */
    val isScreenOn: Boolean? = null,
    /**
     * 👁️ **마지막 스캔에서 축별로 몇 개가 떨어졌나** (기사님 확정).
     *
 ...
 *
 * 🔴 기본값은 서버와 같은 쪽이다 — isActive = false · minFare = 30000.
 *      isActive = true 면 서버가 죽어도 **필터 없이 계속 콜을 잡고**,
 *      minFare = 0 이면 요금 하한이 사라져 똥콜까지 잡는다.
...
data class FilterConfig(
    ...
    /**
     * 🔒 **지금 다른 콜을 심사 중인가** — 「잠김」의 까닭을 가른다 (기사님 · 실주행 오송읍).
     *
     * 🔴 `isActive=false` 는 **만석**이라는 뜻이다 (실을 차종 없음 · 하차해야 풀린다).
     *    선점 중에도 그것이 꺼지면 앱은 둘을 구별 못 해 **판정조차 안 한다** —
     *    오송읍에서 콜 넷이 목록에 보이는데 10초마다 「🔒 평가 보류」만 찍혔고, 셋을 잡는 데 3분 25초가 걸렸다.
     * 🔴 **판정은 막지 않는다.** 이 값은 «클릭을 미룰까»만 정한다 —
     *    판정을 미리 해 두어야 앞 콜이 결재되는 즉시 다음을 잡는다.
    ...
    /**
     * 🔴 **소수로 받는다** (`appFilterKeys.test.ts`) — 자동 반경이면 서버가 `4.55` 처럼 보낸다.
    ...
    /**
     * 📋 **상차 목록** — 상차지가 이 읍·면·동에 걸려야 싣는다 (하차 목록»).
     ...
     * (기사님 확정). null = 경로 위지만 순서를 모름(막지 않는다).
