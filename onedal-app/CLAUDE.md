...
- **배차망은 화면 글자로 가른다 — 앱 이름(패키지)으로 가르지 않는다** (2026-09-14 · `TargetApp.networksOnScreen`).
  시뮬레이터 앱은 세 배차망을 한 앱으로 띄워서 앱 이름으로는 늘 «인성»이 됐다.
  배차망마다 Keywords 에 `NETWORK_MARKERS`(그 배차망 화면에만 있는 글자 묶음)를 둔다.
  🔴 화면 판별 글자를 고치면 **다른 배차망 화면에 그 글자가 없는지** `NetworkByScreenTest` 가 대조한다.
  예외는 하나 — 처음 보는 픽커 화면의 글자를 로그로 모을지만 앱 이름을 본다(`isKakaoPickerApp`)

- **웹뷰(Chrome) 기반 화면은 텍스트가 압축되어 올라온다.** 새 배차망을 붙일 때
  `docs/지금/앱_스크래핑.md`(레포 루트의 `/docs`)를 먼저 읽을 것
